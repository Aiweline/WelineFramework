...
- **NFR-001 (Security)**: API Key 与敏感配置 MUST 使用 WelineFramework 内置的 SecretStore 进行本地加密存储（已明确：本地加密，无需外部KMS）
- **NFR-002 (Performance)**: 常见文本生成请求的 P95 响应时间目标：<= 3s，P99 响应时间 <= 5s（已确认）
- **NFR-003 (Reliability)**: 异步任务需保证幂等，队列使用 DLQ 与退避策略；审计日志保留期：90天，API调用日志7天，性能监控数据365天
- **NFR-004 (Scalability)**: 支持水平扩展的模型代理/队列处理，目标支持 1000+ 并发用户

### Key Entities *(include if feature involves data)*

#### 核心数据表 (23个)
- **ai**: 模型元数据（supplier, model_code, version, config, capabilities, is_copy, origin_model_id） ⚠️ 注意：表名是 `ai` 而不是 `ai_model`
