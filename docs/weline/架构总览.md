...
详细规则见 [ModuleRouter 模块文档](../../app/code/Weline/ModuleRouter/doc/README.md) 与 [开发文档.md](../开发文档.md) 路由章节。
...
模块间**查询型**协作优先 **QueryProvider / `w_query()`**；浏览器站内前端业务请求必须通过 `Weline.Api.resource()/graph()/stream()` 进入 worker，再到 `query-bin` 和 FrontendQueryGateway；**通知型**用事件。详见 [Event 模块文档](../../app/code/Weline/Event/doc/README.md) 与归属模块文档。
...
控制面常用 **NDJSON over TCP** 与 Master 通信（register、heartbeat、reload、restart 等）。更细的进程图与 IPC 说明见 [Server 模块文档](../../app/code/Weline/Server/doc/README.md) 与 [IPC 控制通道架构](../../app/code/Weline/Server/doc/IPC控制通道架构.md)。
...
| 统一 CLI | `bin/w` |
| 权限 | ACL 与菜单体系 |
| 跨模块查询 | 服务端 QueryProvider / `w_query()`；浏览器前端用 `Weline.Api.resource()` |
...
- [项目文档索引](../README.md)
- [开发文档.md](../开发文档.md)
- `AGENTS.md`（AI 阅读顺序与约束）
- [Framework 架构文档](../../app/code/Weline/Framework/doc/architecture/README.md)
