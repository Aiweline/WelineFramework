...
模块间**查询型**协作优先 **QueryProvider / `w_query()`**；浏览器站内前端业务请求必须通过 `Weline.Api.resource()/graph()/stream()` 进入 worker，再到 `query-bin` 和 FrontendQueryGateway；**通知型**用事件。详见 `dev/ai/diagrams/05-event-system.txt` 与扩展点技能。
