...
test.describe('Weline_DeveloperWorkspace backend smoke', () => {
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
    await page.setViewportSize({ width: 1440, height: 900 });
  });

  test('TC-01: developer workspace index page renders without fatal errors', async ({ page }) => {
    const errors = bindPageErrors(page);
    await gotoBackend(page, 'admin/developer-workspace', { timeout: 60000, settleMs: 1200 });
