...
- **回灌**：精确口令「回灌」= 已给框架仓时可修核→合入推送→`update:core` 回当前站→验证；未给框架仓或未说「回灌」禁止。权威：`global-constraints.md` §7；技能：`CI发布工程师-回灌验证`；Cursor：`.cursor/rules/huiguan-core-update.mdc`。已废止 QiPai/双仓 standing 合并提示规则。
