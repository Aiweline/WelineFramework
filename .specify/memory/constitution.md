...
# 运行指定测试文件（生成详细HTML报告）
php bin/w phpunit:run -b --name=BusinessInsightServiceTest

# 快速调试单个方法（不生成报告，仅开发时使用）
php bin/w phpunit:run --name=BusinessInsightServiceTest::testGetOverallStats
...
# ❌ 禁止不带 -b 参数启动服务器（会阻塞终端）
php bin/w server:start

# ❌ 禁止不带 -b 参数运行完整测试（无法生成HTML报告）
php bin/w phpunit:run --module=Weline_Ai

# ❌ 禁止不带 -b 参数运行测试文件（无法生成HTML报告）
...
2. **后台模式**：使用 `-b` 参数可以让AI在后台正确管理服务器和测试进程，生成详细的HTML测试报告，避免终端阻塞和进程管理问题。快速模式（不带 `-b`）仅用于开发调试单个测试方法时使用。
