...
> 权威：`dev/ai-command/ai/工程团队.md`；硬规则 `requirement_session_dashboard` / `pm_plan_lifecycle`。
...
|---------|------|----------|------|----------|----------|
| main | 用户需求 | 项目经理汇总 | open | UC-1 / e2e-… | 全部子项 closed + 测试过 + PM 复检 + 汇审 |
| <!-- 例：perf-hot-list | 性能检查工程师 escalate | 后端 | open | UC-perf-1 | 施工+测试+PM DoD 复检 | --> |

状态枚举：`open` | `assigned` | `in_progress` | `awaiting_test` | `pm_recheck` | `rework` | `closed`

**关项硬条件**：施工到位 + 测试席对相关 UC 验完且无问题 + PM 复检通过。禁止专席自报完成就关；禁止测试未过就关。
