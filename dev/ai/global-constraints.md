...
- **改了什么功能，就必须说清在哪个 URL 验证**：最终回复必须把「变更点 → 验证入口」写清楚，并给出用户可直接点击打开的完整 Markdown 链接（优先当前专用 WLS 测试实例上的绝对 URL，例如 `https://host:port/{backendKey}/...`）。用户应能点开后用本地内置 Browser 立刻验收，无需自己拼路径。
- 若一次改动影响多个入口，必须逐项列出，例如：列表页 URL、表单/OffCanvas 入口 URL、统一配置中心深链 URL、相关 API/文档路径；每条附一句用途（如「定位配置项」「返回引导」「添加账户」）。
- 禁止只写相对路径、模糊入口或“到某某菜单里找”；若当前没有运行中的测试实例，仍须写出完整路由/路径，并注明访问前置条件（启动 WLS、端口、后台登录等）。
