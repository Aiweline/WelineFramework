...
- PageBuilder：已迁移到 `E:\公司\远程\src\weline`；相关模块、技能、Workspace/模板/组件渲染链路验证均在目标仓库处理
...
适用：跨模块协作、复杂页面交互、涉及多个服务的工作流。PageBuilder 渲染链路已迁移到 `E:\公司\远程\src\weline`，在目标仓库执行对应验证。
...
如果本次变更还会涉及额外领域约束（例如 WLS、ACL 等），也要补充“已阅读的对应技能/约束文档”；PageBuilder 约束在目标仓库维护。
...
- 改了 PageBuilder 工作台/组件渲染：切换到 `E:\公司\远程\src\weline`，按目标仓库 PageBuilder 技能执行对应验证
