...
目标：让每次提交都具备“可验证、可追溯、可回滚”的质量闭环；并确保关键链路具备稳定的 e2e 回归能力。
...
2. 不把“局部改动”当成“全局稳定”：必须做分层回归（单元 -> 路由 -> 链路/集成 -> e2e）
3. 不靠运气发现问题：e2e 必须能稳定复现，才能在每次提交后阻断回归风险
...
- 允许先做“最小验证”：跑你修改涉及模块的 PHP 单元测试与路由验证（见下）

> 说明：静态检查的自动化脚本我们后续补齐；本初版先把“必须执行哪些命令”固化成流程。

### 3.2 PHP 单元测试（快-中）

适用：后端逻辑/模型/服务/事件/查询构建相关改动。
...
- 每个需求项的验收标准（Acceptance Criteria）：每条标准必须能被验证，并且要能追溯到对应测试用例
- 对应测试用例（Test Case）：每个需求项至少 1 个可执行的测试用例（单元/接口/链路烟测/e2e 二选一或多选）
- 需要执行的命令清单（从本体系挑）

> 如果暂时不能先写自动化测试，也要把“失败条件、验收口、原因”写进 `plan.md` / `result.md`；同时仍然要求需求项与验收标准之间要保持可追溯。

#### 4.1.1 需求 -> 测试用例 -> 验收标准（Traceability，强制）

对每个需求项，你必须填写以下三段信息，形成“端到端可追踪”：

1. `RequirementID`：需求项唯一标识（建议用 `REQ-xxx`）
2. `AcceptanceCriteria`：验收标准（建议用 `AC-xxx`，每条可被验证）
3. `TestCase`：测试用例入口（建议至少写清“用什么命令/用什么 e2e spec/验证什么断言”）
...
- RequirementID: REQ-001
  ...
  - TestCase:
    - TC-001:
      - Type: unit|http:req|route|link|e2e
      - CommandOrSpec: （例如 php bin/w phpunit:run --module=Vendor_Module / 或 tests/e2e/specs/...）
      - Assert: （命中的断言点，尽量具体）
```
...
3. 更新计划：对每个受影响模块的 `plan.md`：
   - 更新需求项与验收标准（AC）
   - 更新对应测试用例入口（TC：命令/用例 spec）
4. 更新验收口：对每个受影响模块的 `result.md`：
   - 标记哪些 AC 需要重新验证（通常重跑对应最低层 + 必要 e2e）
5. 重新执行 Gate：至少重跑“受影响需求项”的最小可验证层（单元/路由/接口/链路/e2e 视风险）

> 简化原则：需求项改了，验收标准和测试用例入口也必须一起改；只改代码不改计划，视为“缺失验收口”。
