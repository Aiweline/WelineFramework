...
## Durable disk-pressure fencing and restart safety

- DEF-073 closed the non-space atomic-write gap. A write/fsync/rename failure now leaves the previous state/config/LKG file unchanged, removes its staging file, releases the recovery reserve and creates a mode-0600 pressure marker. `storage.mutation_ready` requires free-space threshold, no marker and an intact reserve, so high reported free space cannot bypass a prior read-only/inode/atomic failure.
- DEF-074 makes the marker a restart fence. A Controller restart does not recreate the released reserve or advance interrupted publication/lease/backend maintenance. It observes the existing data plane without persistent writes and performs only bounded snapshot GC, avoiding a service-manager restart loop. Confirmed storage repair first reconstructs reserve, then clears the marker and reconciles the root-owned A/B slot plus any interrupted publication.
- DEF-075 makes every startup recovery helper honor that fence. State, security ledger and journal corruption are classified in memory without rename/repair, stale runtime artifacts are preserved, and reserve allocation failure latches pressure instead of crashing before state load. Confirmed repair reconstructs a deferred security ledger before clearing the marker.
- Red/green evidence injected temporary write/fsync and post-write reserve failures plus corrupt state/ledger/journal startup artifacts. The corrected test proves unchanged durable hashes, cleanup of incomplete reserve/staging files, an incompressible on-disk reserve, marker-latched rejection, no maintenance write, no stale-runtime cleanup, restart survival and confirmed recovery. Focused disk-pressure regression passed `1 / 59`; complete protocol security passed `43 / 1055`; Weline_Server passed `1327 / 6736 / 17 skips`; Gateway/Windows/Nginx passed `257 / 2710 / 15 skips`; real random-high-port data-plane recovery passed `1 / 1465`.
- These tests prove deterministic controller behavior and preserve existing active/LKG artifacts. Real ENOSPC, inode exhaustion, quota and read-only filesystems across Linux/macOS/Windows remain environment acceptance and are not claimed here.
