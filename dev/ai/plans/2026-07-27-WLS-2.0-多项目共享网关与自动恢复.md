...
- TASK-000～002 已按逐任务验收完成。
- TASK-003～011 的本机实现检查点已经落地，但任务级验收仍有
  `PARTIAL/PENDING/BLOCKED_ENVIRONMENT`，因此不得把代码存在等同于任务完成。
- 当前最终通用回归：Weline_Server Unit
  `1075 tests / 4904 assertions / 2 environment skips`；
  Start/Master/Orchestrator/Control/fallback `159 / 742`；Native Broker/Launcher 与
  隔离高端口真实 Nginx `92 / 1549`，后者 0 warning、0 skipped。
- 已修复一个恢复链阻断：端点白名单丢弃 Master exit timestamp，导致退出后的子进程
  保留和无子进程清理均不能提交。修复只补齐既有协议字段，专项 `2 / 15` 通过。
- 本机任务隔离高端口的真实 Broker/Controller/Nginx/双项目链路已闭合：
  `1 test / 1395 assertions`，覆盖并发注册、4 轮 heartbeat、同项目多实例排空、
  H1/H2/H3、证书/租户隔离、TLS 恢复禁用、404/421/400、Controller 无中断接管、
  交叠 Controller fencing 和 Nginx 第三探针自动恢复。
- 协议/安全/Native/async/path 聚焦回归为 `36 / 779`；Weline_Server Unit 使用
  框架 bootstrap 复跑仍为 `985 / 4165 / 1 environment skip`。
- Linux 隔离 VM 已闭合 systemd、真实 80/443、删除引导项目后的冷 reboot、
  TLS/503→ACTIVE、H3-only 降级和 90/30/301 秒 fallback/rejoin。TLS 1.3 实测发现
  per-route ticket key 不能形成租户边界，v1 已全局关闭 session cache/ticket/0-RTT。
- Linux arm64 v38 生产候选已通过自包含依赖、provenance、SBOM、组件自检与签名；
  A/B 旧槽至少保留 24 小时的门禁未被绕过。
- 用户显式要求的纯 WLS 与网关 H2/H1.1 单项目精确百万请求均为 0 错误；混合租户、
  H3 与同条件性能中位数尚未闭合。
- DEF-050～056 已在本轮阻断修复中关闭：native drain 使用持久 deadline 和幂等重试，
  deadline 到期后执行有界 finalize；所有复活入口拒绝 `desired=0`；认证网关观察原子
  更新 endpoint；Status 区分 native/fallback/gateway active；持久 native-edge 围栏
  同时约束冷启动、整组重启、期望态、复活、手工扩容和内存压力扩容；Benchmark 按
  endpoint 实际回源拓扑选择普通 Worker 或 join backend，并执行就绪和
  PID/IPC lease/generation 稳定性门禁。
- Linux 隔离 VM 的决定性 auto rejoin 证据：纯 WLS 高端口与 Gateway 443 同时返回
  200 后进入固定 300 秒 `DRAINING`，原 8 个 Worker 自然退出且未复活；到期后有界
  关闭 Dispatcher，状态成为 `GATEWAY_ACTIVE/DRAINED`，旧高端口连接失败、443 仍为
  200。最终报告跨过两个以上内存压力扩容周期，普通 Worker 仍为 0；网关实际 H2
  `1,000,000/1,000,000`、0 错误、8 后端命中、质量门禁 PASS；6182.44 QPS，
  P95 101.23ms，P99 198.54ms。
- macOS/Windows 系统服务/ACL/reboot、首次 ACME、legacy 80/443 promote 和完整
  Session 运行时亲和仍未闭合；publication 各 phase 恢复与非幂等 POST 不重放已经
  由真实数据面覆盖。当前发布判定为 `BLOCKED`，
  TASK-014 只能记录为已启动的部分验收，不能标记完成。
...
这份修订版已经关闭计划层面的已知逻辑缺口；实施只能从 TASK-000 开始，不能跳过安全基线直接扩写当前 Controller。
