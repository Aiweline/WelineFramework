...
- 当前提交候选使用正式 `app/bootstrap_phpunit.php` 完成 Weline_Server 全量回归：
  `1327 tests / 6699 assertions / 17 platform skips`，零错误、零失败；按
  `Gateway|Windows|Nginx` 过滤的跨平台门禁为
  `257 tests / 2673 assertions / 15 capability skips`，PHP lint、diff check 全绿。
...
- 本轮关闭 DEF-060～065：Start 生成受 instance generation 约束的 stateless 声明；
  ...
  `1327 / 6699 / 17`、Gateway/Windows/Nginx `257 / 2673 / 15`、原生套件
...
- 外部 CA/DNS 公网首次实签、完整 Session 运行时亲和、macOS/Windows 系统服务 ACL/reboot、
  ...
  legacy promote、双租户 H2/H3 百万以及 DEF-058～072 已不再是代码阻断项。
