...
overview: WLS 2.0 已按 WLS2-FREEZE-2026-08-11-A 冻结。产品范围仅为 REQ-SERVER-0001～0008；缺陷截止 DEF-271、测试截止 TEST-138，只允许 FRZ-BLK-001～010 和一次固定终审。Framework 数据库、macOS/Windows 真机、托管 Runner、外部 CA/DNS 分离为独立门禁；历史证据不能代替冻结源码验收，未准入工作转 2.0.1。
todos:
  - id: frz-000-scope
    content: FRZ-000：冻结需求、源码指纹、缺陷/测试截止与终态
    status: completed
  - id: frz-001-reconcile
    content: FRZ-001：只读对账 FRZ-BLK-001～010 与冻结工作树
    status: pending
  - id: frz-002-close-blockers
    content: FRZ-002：仅修 OPEN/FAILED 的冻结重大阻断
    status: pending
  - id: frz-003-final-review
    content: FRZ-003：执行并永久关闭唯一一次 FRZ-REVIEW-01
    status: pending
  - id: frz-004-fixed-validation
    content: FRZ-004：完成 TEST-001～138 当前源码证据矩阵
    status: pending
  - id: frz-005-linux-rc
    content: FRZ-005：完成数据库依赖、真实 Linux、Browser 与百万矩阵
    status: pending
  - id: frz-006-external-ga
    content: FRZ-006：取得外部门禁证据或进入 GA_BLOCKED_EXTERNAL
    status: pending
  - id: frz-007-close-task
    content: FRZ-007：同步结果、转移 2.0.1 backlog 并停止任务
    status: pending
isProject: false
---
...
本节是 `2.0.0` 的最高执行优先级；后文保留完整架构、历史缺陷和测试设计用于追踪。若后文的“继续复审”“发现新缺陷再追加”或任务状态与本节冲突，以本节为准。

### 0.1 冻结身份与权威边界

| 项 | 冻结值 |
| --- | --- |
| 冻结 ID | `WLS2-FREEZE-2026-08-11-A` |
| 产品 owner | `app/code/Weline/Server/doc/需求.md` |
| 产品范围 | `REQ-SERVER-0001`–`REQ-SERVER-0008`，含 2026-08-11 首项目安全自动建立宿主网关语义 |
| 工程分解 | 本计划 `REQ-001`–`REQ-040`，只能分解产品需求，不能扩大产品行为 |
| 缺陷截止 | `DEF-271`；禁止新增 `DEF-272+` |
| 测试截止 | `TEST-138`；禁止新增 `TEST-139+` |
| 审查 | 仅 `FRZ-REVIEW-01` 一次固定终审；关闭后不得重开开放式扫描 |
| 任务 owner | `dev/ai/codex/tasks/2026-07-27/2026-07-27-1600-wls-2-0-gateway-implementation/` |
| 证据 owner | 上述任务的 `acceptance-matrix.md` |
| 未准入工作 | `2.0.1` backlog |

冻结源码输入为 `dev@2e80939f62bedbe2b6fd7e7199e1c7c5ca250b25` 加当时 dirty worktree：WLS 非文档 tracked diff SHA-256 为 `61f386433debe78fcb1840dbee6a676a5b660f1b2a5e6b1416da22cda90a504f`，WLS 非文档 untracked manifest SHA-256 为 `5599bbff048c3f76a5c2ee40a73f2baa12c6e41580832e5c9122fe91a3465a01`，两个 WLS workflow diff SHA-256 为 `ef8475ba90875ab8f936de76958ddd51815194a92b6098dabfc96a6abbf2d452`。原任务在 turn `49fe9325-4fc5-4273-b826-e01323d43bb3` 停稳。

### 0.2 有限重大缺陷阻断表

方案 A 为主，方案 B 只吸收下表中冻结时已有复现或审查证据的 P0/P1。表外缺陷、P2/P3、优化、重构和润色不进入 `2.0.0`。

| 冻结阻断 | 等级 | 既有映射 | 冻结时事实 | 闭合条件 |
| --- | --- | --- | --- | --- |
| `FRZ-BLK-001` | P0 | `DEF-266` / `TEST-132` | `START_AUTHORIZED` 独立证明与 trust-preserved inventory 冲突，权威表仍为 IN_PROGRESS | 独立 marker 验签/事务绑定静态接受，`TEST-132` 当前源码通过 |
| `FRZ-BLK-002` | P0 | `DEF-267` / `TEST-133` | derived root authority、owner/mode/DACL 与目录持久化未被权威表接受 | root policy/identity、隔离重建、metadata 持久化闭环，`TEST-133` 通过 |
| `FRZ-BLK-003` | P1 | `DEF-268` / `TEST-134` | terminal receipt 有界、不可变排序和崩溃可重放 GC 未被权威表接受 | 16384/1024 边界及别名/断电重放闭环，`TEST-134` 通过 |
| `FRZ-BLK-004` | P0 | `DEF-269` / `TEST-135` | 停机前 byte/inode 真实预留、同卷与所有后续 rename 源/目标证明未被权威表接受 | POSIX/Windows 物理 reserve、嵌套挂载和 HELD→RELEASED 重放闭环，`TEST-135` 通过 |
| `FRZ-BLK-005` | P0 | `DEF-270` / `TEST-136` | 稳定 Launcher/CA 提交后缺独立 Guardian、5 分钟观察和单调 generation-head 的最终接受 | POSIX/Windows Guardian 包、平台入口、连续性回执和旧整代自动恢复经终审，`TEST-136` 通过 |
| `FRZ-BLK-006` | P1 | `DEF-271` / `TEST-137` | rollback observation retry 已有实现声明，但仍为 REVIEW_PENDING | 失败补偿、marker 撤销、ADMIN_STOPPED 恢复经终审，`TEST-137` 通过 |
| `FRZ-BLK-007` | P0 | `DEF-004` / `TEST-138` | 首项目 auto bootstrap 后续复审发现“非空白宿主误判”、包替换窗口与崩溃事务风险；既有 STATIC_CLOSED 不足以接受冻结源码 | 锁外/锁内空白宿主证明、签名包身份固定、事务恢复和删除项目源后的宿主独立性经终审，`TEST-138` 通过 |
| `FRZ-BLK-008` | P0 | `DEF-218`、`DEF-231` / `TEST-088`、`TEST-097` | 冻结前复审重新确认 route-serving 与 whole-project ready 可能混淆 | ACTIVE 子集可服务但 `project_ready=false`，完整 serving manifest 单写闭包经终审与两项测试通过 |
| `FRZ-BLK-009` | P0 | `DEF-225` / `TEST-091` | 并发 Start 的 generation 等待预算与严格 `+1` 在冻结前被重开验证 | 同一事实源锁、原子严格 `+1`、崩溃/重启/墙钟矩阵经终审，`TEST-091` 通过 |
| `FRZ-BLK-010` | P0/P1 | `DEF-178`、`DEF-217`、`DEF-263` / `TEST-006`、`TEST-008`、`TEST-087`、`TEST-129`、`TEST-136` | Windows Guardian/SCM 的 PRESHUTDOWN、服务 DACL、绝对 deadline、Named Pipe 残留、包内 Guardian 与恢复入口在冻结前有新增修复声明但没有最终平台接受 | 本地/CI 可验证合同全部通过且 `FRZ-REVIEW-01` 接受；真 SCM/NTFS/reboot 证据另归外部门禁 |

冻结阻断表至此封口。`FRZ-REVIEW-01` 只能将这些行判为 `PASS`、`FAILED` 或 `BLOCKED_ENVIRONMENT`，不能增加行、缺陷号、测试号或第二轮审查。若它复现表外的数据丢失、权限绕过或生产不可恢复问题，必须停止并请求产品 owner 决定；该问题不会自动进入 `2.0.0`。

### 0.3 独立依赖与外部门禁

| 门禁 | 性质 | 2.0.0 处理 |
| --- | --- | --- |
| `FRZ-DEP-DB-001` | Framework 破坏性迁移的备份、限定物理表、同事务 DDL、operation CAS 与完整恢复 | 单独任务验收；不得把 Framework 全库审查并入 WLS。未 PASS 时阻断 `LINUX_RC`，不扩大 `FRZ-BLK` |
| `FRZ-EXT-MAC-001` | root LaunchDaemon、ACL、reboot、Intel macOS ABI | 环境证据；源码闭合后缺失可进入 `GA_BLOCKED_EXTERNAL` |
| `FRZ-EXT-WIN-001` | Windows Service/SCM、NTFS、reboot、真实 MSVC | 环境证据；源码闭合后缺失可进入 `GA_BLOCKED_EXTERNAL` |
| `FRZ-EXT-CA-001` | 外部 CA/DNS 首签与真实 443 开放 | 外部凭据/域名证据；缺失可进入 `GA_BLOCKED_EXTERNAL` |
| `FRZ-EXT-RUNNER-001` | 托管 Runner 的 macOS/Windows/WLS workflow | 需要另行授权提交/推送后取证；当前不授权提交，不以本机静态结果冒充 |

### 0.4 固定执行顺序

1. 只读复核当前验证基线指纹与 `FRZ-BLK-001`–`010` 代码/测试映射；指纹漂移先停止报告。
2. 只修仍为 `OPEN/FAILED` 的冻结行，每行执行 RED → 最小修复 → 聚焦 GREEN；不得顺手重构。
3. 执行唯一一次 `FRZ-REVIEW-01`，输入仅为冻结阻断差异，结论写回验收矩阵后永久关闭。
4. 对账 `TEST-001`–`TEST-138`；先聚焦与完整 PostgreSQL/原生合同，再走真实 WLS + Browser、Linux 运行时和当前源码百万矩阵。
5. 分别处理数据库依赖与外部平台证据，输出终态并停止。

### 0.5 终态定义

| 终态 | 判定 |
| --- | --- |
| `SCOPE_FROZEN` | 本节、任务卡和验收矩阵一致；尚未声明代码完成。当前状态 |
| `BLOCKED_INTERNAL` | 任一 `FRZ-BLK` 或 `FRZ-DEP-DB-001` 失败，且有明确失败证据和恢复入口；不得靠开新审查绕过 |
| `CODE_COMPLETE` | `FRZ-BLK-001`–`010` 全部被 `FRZ-REVIEW-01` 接受，关联聚焦测试全绿，文档/矩阵一致；外部证据可仍待办 |
| `LINUX_RC` | `CODE_COMPLETE` + 数据库依赖 PASS + 当前源码完整 PostgreSQL、真实 Linux WLS、Browser 路径和百万矩阵 PASS |
| `GA_BLOCKED_EXTERNAL` | 已达到 `LINUX_RC`，仅 macOS、Windows、Runner 或外部 CA/DNS 证据缺失；不得重新进入实现循环 |
| `GA_PASS` | `LINUX_RC` 与全部外部门禁 PASS，发布材料一致 |

任何终态都必须写明证据时间、源码指纹和未覆盖风险。`GA_BLOCKED_EXTERNAL` 是合法收口，不是继续开发的理由。

### 0.6 2026-08-13 当前工作树验证基线

用户授权 `WLS2-FREEZE-2026-08-13-B` 作为当前 byte/verification baseline。该授权不改变
`WLS2-FREEZE-2026-08-11-A` 的产品范围，不新增需求、缺陷、测试或审查轮次，也不把当前
工作树行为视为已接受。

| 项 | Freeze B 值 |
| --- | --- |
| base | `dev@936f9ffc5fc4b32a5babe83eddce7c8de53fb5ad` |
| WLS non-doc tracked diff | `8cc3c25e8e24db05e89081215bb1f4de47c4d3baad045c1673dc9458e502eff0` |
| WLS non-doc untracked manifest | `b3f6f26975a8b5efb4152fb2a5e3f2f788b0f34ad3d0cf3f52522541dd99612a`（1 path） |
| WLS workflow diff | `f2e24edd2f5d7f751e54a7cb84d7d01583751650ba9a31bf8ede1fd8f7752ffb` |
| 当前状态 | `CURRENT_VALIDATION_PENDING / READY_FOR_FRZ-005` |

Freeze A 的代码门禁、Browser 与百万请求结果只作为历史证据。Freeze B 必须在三项指纹一致后
按 `FRZ-005` 顺序重新验证；`FRZ-REVIEW-01` 仍永久关闭。

## 1. 审查结论与开工门禁

| 项 | 结论 |
| --- | --- |
| 计划状态 | **SCOPE_FROZEN / ACCEPTANCE BLOCKED** |
| 源码状态 | **FROZEN DIRTY SOURCE / NOT RELEASE-READY**；只允许 §0 的 `FRZ-BLK-001`–`010`、一次固定终审和 `TEST-001`–`138`。此前测试与百万请求均不代表冻结源码 |
...
| 任务记录 | `dev/ai/codex/tasks/2026-07-27/2026-07-27-1600-wls-2-0-gateway-implementation` |
| 未决产品决策 | `0` |
| 冻结代码阻断 | `10`：见 §0.2；不得追加第 11 项 |
| 未映射源码缺陷 | `0` |
| 当前工作区保护 | 不覆盖 `app/etc/env.sample.php` 的用户改动；不触碰三个 `.orig` 文件；不操作 9501 或未知 80/443 owner |

历史 IMPLEMENTED 判定已被 2026-08-03 静态反证审查撤回。严格百万请求只证明特定旧数据面在当时条件下可承载流量。2026-08-11 起不再接受开放式“新增 P0/P1”；只按 §0 的有限阻断、固定终审、冻结测试和独立外部门禁收口。

本计划取代本文件此前所有版本；不得另建平行实施计划。`TASK-000` 至 `TASK-014` 只作为历史追踪结构，当前执行顺序以 §0.4 和任务 `plan.md` 为准。
...
| REQ-002 | 每个项目发行物必须在固定平台/架构路径携带完整、锁定且生产签名的 Gateway/Nginx 包。`auto` 优先加入可信、兼容且 ready 的宿主网关；无网关且 80/443、包、权限和平台守护前置全部安全时，首项目在宿主 bootstrap 锁内建立独立网关并注册自身；前置不满足才启动纯 WLS 高端口。首次建立不授权 upgrade/repair/rebootstrap/promote，也不得接管未知服务。 |
| REQ-003 | `gateway` 必须成功加入可信网关，或在与 `auto` 相同的 fail-closed 首次建立前置下完成安装并加入，随后达到 ACTIVE，或在首次签发时达到已鉴权的 `PENDING_CERTIFICATE/challenge-only`；其他失败干净退出。`wls` 完全不接触网关。 |
| REQ-004 | 宿主网关独立于任一项目，统一监听 80/443；项目发行包只作为签名安装源，安装后的平台定义、稳定 Launcher/Guardian 和 A/B 槽只引用宿主路径。引导项目停止、迁移或删除后，网关和其他项目继续运行。 |
...
| DEF-004 | P0（STATIC_CLOSED） | 首项目自动建立若复用普通 package lock、先创建宿主信任根再验包或安装后未验证数据面，会造成并发重复 stage、坏包副作用或假成功 | 缺少项目发行包预检、独立 root-only bootstrap lock、锁内状态复查/重复验签、同一 absolute deadline 和 trusted-ready 后置门禁 | TASK-001/003；TEST-138 |
...
| DEF-218 | P0 | 多域项目一条既有 ACTIVE 域与一条首次签发 `PENDING_CERTIFICATE` 并存时，项目级完整度信号会让健康 ACTIVE 域无法回切 gateway | 没有区分 exact publication、可服务 cert-ready subset 和全量 convergence，Agent/endpoint 把 whole-project readiness 错当成 route-serving readiness | TASK-005/007/008/009；TEST-088 |
...
| DEF-263 | P0（STATIC_CLOSED；WINDOWS_CI_PENDING） | Native Launcher 在 journal 首次 ENOENT 后扫描恢复伴随物时，把并发出现的精确小写 `rebootstrap.transaction` 当成无关文件，可能继续启动未经裁决的代 | journal-absent 扫描只识别大小写 alias 和 backup/tmp 前缀，没有把 exact target 本身列为保留恢复证据 | TASK-003/008/010/013；TEST-129 |
| DEF-264 | P1（STATIC_CLOSED；WINDOWS_POWERLOSS_PENDING） | rebootstrap candidate 进入 `PREPARED` 后仍保留 `<nonce>.install.lock`，终态/回滚回收可永久被不存在的 installer authority 阻断 | candidate install lock 没有按 journal phase 退休，也没有对删除前后崩溃、非空/链接/硬链接做 fail-closed 重放 | TASK-003/008/010/013；TEST-130 |
| DEF-265 | P0（STATIC_CLOSED；WINDOWS_POWERLOSS_PENDING） | same-CA rebootstrap 在构造 `backup/working-generation` 中途失败后，rollback 缺少 manifest-bound 清理，可能无法恢复旧代或把残留派生态带入 `ROLLED_BACK` | working copy 没有有界、no-follow、child-first、可重放的精确删除闭包与终态 absence fence | TASK-003/008/010/013；TEST-131 |
| DEF-266 | P0（IN_PROGRESS） | trust rotation 的 `START_AUTHORIZED` pre-start closure 稳定报告缺少新代 marker，使 CA 轮换无法提交 | `rebootstrap-start.authorization` 已属于 trust preserved 项、会被 raw live 枚举过滤，却仍被列为该枚举的 required entry；marker 的独立验签/transaction 绑定与派生态 inventory 混在一起 | TASK-003/008/010/013；TEST-132 |
| DEF-267 | P0（OPEN） | derived-state 叶子可恢复正确，但 category root 被替换、丢失或改 owner/mode/DACL 后仍可能启动并最终报告回滚成功 | derived manifest schema 2 只记录 `root_present`，没有认证 root authority policy/identity；目录 metadata 也没有逐级持久化闭环 | TASK-003/008/010/013；TEST-133 |
| DEF-268 | P1（OPEN） | terminal rebootstrap receipt 永不删除，累计第 4097 个 nonce 后会永久耗尽 4096 配额并禁止后续整代维护 | backup collection 只删除 generation tree，未为已 `COLLECTED` 且无任何 claim/artifact 的旧 receipt 建立摘要绑定、崩溃可重放 GC | TASK-003/008/010/013；TEST-134 |
| DEF-269 | P0（OPEN） | rebootstrap 仅按逻辑常量检查 10 GiB 保留预算，未在停网关前真实占用同文件系统 byte/inode；ENOSPC 可在 `STOP_COMMITTED/QUIESCED` 后破坏 stash/rollback | retention quota、Controller 的 8 MiB state reserve 与 whole-generation 物理恢复容量被误作同一保证，缺少同卷 allocation identity 与 HELD→RELEASED journal | TASK-003/008/010/013；TEST-135 |
| DEF-270 | P0（OPEN） | 新稳定 Launcher/CA 在短观察后写 `COMMITTED` 即撤销 journal/start authorization；若随后进入崩溃循环，平台服务本身没有不可替换执行者自动恢复保留的旧整代 | 普通 A/B 自动回滚不能覆盖稳定 Launcher/CA rebootstrap；缺少独立 Recovery Guardian、5 分钟 probation 与单调 host generation-head | TASK-003/008/010/013；TEST-136 |
| DEF-271 | P1（IMPLEMENTED；REVIEW_PENDING） | 新代观察失败转入 rollback 后，旧代 rollback observation 若失败会留下不可安全重试的启动授权/phase 组合，管理员重跑不能确定性恢复 | 缺少 `ROLLBACK_OBSERVING→ROLLING_BACK` 重试边、marker 撤销、完整停机与 exact ADMIN_STOPPED 恢复 | TASK-003/008/010/013；TEST-137 |

`STATIC_CLOSED` 仅表示当前源码结构、定向 PostgreSQL 回归或 workflow 合同已形成可审查闭环，不等于 TASK-012 的真实 WebUI、TASK-013/014 或 release gate 验收通过。`IMPLEMENTED；REVIEW_PENDING` 也不得作为闭合：必须由独立架构/缺陷/安全复审接受后再进入测试门禁。这些定向计数存在重叠，不得相加作为统一门禁总数。
...
| mode | `auto` | 加入可信 ready 网关；无网关且安全前置齐全时由首项目建立宿主独立网关并加入；其余情况纯 WLS 高端口 |
| mode | `gateway` | 必须加入可信 ready 网关或完成同一首次建立，否则失败 |
...
3. `auto`：
   - 先读取固定系统信任路径的签名 manifest；既有可信网关存在时直接加入；
   - 验证 host id、版本、协议 min/max、安全 profile、组件摘要、服务状态、控制通道和 profile 要求的 IPv4/IPv6 端口；
   - 无网关且宿主只读分类精确为 `INSTALL_REQUIRED` 时，逐级 no-follow 定位项目发行物中的 `extend/server/wls-gateway/<target-profile>`，在任何宿主 mutation 前完成 `release_ready`、签名、target/profile、组件和来源预检；
   - 预检通过后取得独立 root-only `package-bootstrap.lock`，在同一绝对 deadline 内重查可信状态；竞争胜者锁内重复验签并完成 stage → definition → activate → start → trusted/data-plane-ready，等待者发现胜者已 ready 后只加入；
   - ready 时先启动 loopback WLS，再由同一 Master 的 Agent 异步注册，并等待 bounded route ACTIVE；
   - 未知 owner、权限不足、项目包缺失/无效、平台守护建立失败、不可信、不兼容或未 ready 时，在项目有有效 active/previous-valid 证书时直接启动同 Master 纯 WLS 高端口；无有效证书则以 `TLS_CERTIFICATE_UNAVAILABLE` 失败并给出签发前置；
   - 注册超时/失败时切到同 Master fallback，不把半注册状态报告为成功。
4. `gateway`：同样允许首次 fail-closed 建立并验证，但任一失败均清理本实例并非零退出。
5. `auto` 与 `gateway` 只可调用上述“尚无网关”的初始安装事务；不得借启动路径调用 upgrade/repair/rebootstrap/promote。
...
- 发行包按 OS/architecture 由 CI 构建，包含自包含 Controller 应用及锁定 PHP CLI 运行时、锁定 Nginx、原生 Platform Broker、许可清单和 manifest；签名包与启用的 release-key inventory 必须作为同一项目发布组装输入，落到 `extend/server/wls-gateway/<target-profile>` 与 `app/code/Weline/Server/env/gateway/trusted-release-keys.json`。仅上传 CI artifact 不等于现有项目发行物已包含它；最终项目发布流水线未消费该 overlay 时，生产 auto bootstrap 必须因缺包/空信任库 fail closed。
- 生产安装只从项目固定路径读取并验签，随后复制到宿主 A/B 槽；平台定义、Guardian、稳定 Launcher 和运行中 Controller/Nginx 只引用宿主 home/slot，不从项目目录执行，也不在目标宿主临时编译 native 组件。
...
- 包先验证固定上限、外部签名和由发行系统注入项目发行物的启用 release public-key inventory，再逐项验证 manifest/hash/path/mode；包不能携带并自证自己的信任 key。禁止绝对路径、`..`、symlink/reparse entry。支持管理员提供离线签名包。
...
- Authenticated own-status 必须分离 `active_routes` 与 `desired_routes`，并分别报告 `publication_exact`、`route_serving_ready`、`project_ready`、`project_converged`。`route_serving_ready` 仅证明 exact 可服务 subset，`project_ready` 是整个项目已收敛且数据面健康的门禁，两者不得混用。所有 cert-ready route 必须在 active publication 中 ACTIVE 且具有新鲜公开证明；exact `PENDING_CERTIFICATE/challenge-only` 不阻断同项目已 ACTIVE 域，但全 pending 不得声称健康 HTTPS 或全量收敛。
...
- 动作：
  ...
  3. 让 `auto` 先 discover/join；只对精确 `INSTALL_REQUIRED` 进入项目签名发行包的首次 bootstrap coordinator，其他 prepare/install/upgrade/repair 副作用保持隔离。
...
- 验证：
  ...
  - `auto` 无网关且签名包/权限/公共端口/守护前置齐全时建立并加入宿主网关；任一前置失败才启动纯 WLS 高端口；
  - `gateway` 无网关时必须建立并加入；建立失败非零退出且无残留；
...
- 动作：
  ...
  3. CI 同步生成项目发布 overlay：签名包固定落入 `extend/server/wls-gateway/<target-profile>`，启用公钥 inventory 固定落入 Server env；最终项目发布组装必须把两者作为同一输入消费并校验 key id 绑定。仓库尚无消费方时将此边界明确保持为 fail-closed 发布阻断。
  4. 实现首项目 bootstrap coordinator：锁前逐级 no-follow resolver + exact `verifyPackage()` 预检；独立 root-only `package-bootstrap.lock` 覆盖 stage/definition/activate/start/trusted status；锁内重查和重复验签；所有等待与操作沿用一个 absolute deadline。
  5. 按 §5.4 实现固定路径、权限、UDS/Named Pipe 和三平台系统服务。
  6. 安装前验证 profile 要求的 IPv4/IPv6 80/443；未知 owner 时失败且不终止。
  7. Platform Broker 执行真实 peer identity、受限启动和 no-follow 句柄读取；Broker 不可用不 ready。
  8. Broker 取得 singleton lock/fencing token，降权启动 Controller，并生成独立 neutral default certificate。
- 验证：
  ...
  - 并发首项目只有一个 stage；等待者取锁后重查并加入；锁等待、安装和 ready 观察不重置 deadline；
  - 坏签名、target/profile 不匹配、任一级 parent symlink 在 host lock/mutation 前拒绝；install 返回 test/partial/untrusted 或数据面未 ready 不得成为 gateway decision；
  - 平台定义、Guardian、Launcher、active slot 与 status ownership 不包含首项目分发路径；移走该项目后仍以宿主槽为事实源；
...
- 文档必须写清：auto 首项目建立与后续加入边界、项目固定发行路径、宿主独立 A/B 生命周期、发行 overlay 消费门禁、信任边界、迁移时哪些携带/不携带、高端口不是透明替代、三平台权限、恢复/熔断、命令权限、H3 边界。
...
- 验收：§8 中 TEST-001 至 TEST-138 全部通过；环境不具备的项目标记 BLOCKED，不得写 PASS。
...
| TEST-068 | 宿主公共端口只读分类 | 双栈 80/443 的未知 owner 前后 PID/listener 不变并报告 `PORT_TAKEN`；PATH/系统中存在但未监听的 Nginx 不阻止首次建立；权限不足为 `PORT_PERMISSION`，安全空闲且可建立为 `INSTALL_REQUIRED`；不能建立时多个项目获得不同且重启稳定的 fallback 地址 | TASK-001/003/007/009/012 |
...
| TEST-088 | 多域证书部分收敛 | A 域 ACTIVE、B 域 exact `PENDING_CERTIFICATE/challenge-only` 时 `publication_exact=true/route_serving_ready=true/project_ready=false/project_converged=false`，A 仍可回切并服务；全 pending 时 publication exact 但 `route_serving_ready=false`，不声明健康 HTTPS；任一 cert-ready 域非 ACTIVE 或 active proof 不精确时两个 readiness 均为 false；B 签发发布后 `route_serving_ready=true/project_ready=true/project_converged=true` | TASK-005/007/008/009 |
...
| TEST-110 | 初次 auto fallback serving projection | 没有可信网关且首次建立前置明确失败时，初次 `auto` 的 live primary plain-instance lease 立即进入 manifest、Start/Status/Benchmark 并与实际 bind 一致；lease 不可跨 role/mode 复用，后续 join/recover 仍按 30/300 秒切换排空 | TASK-001/005/007/009/014 |
...
| TEST-129 | Native journal-absent 原子伴随物栅栏 | 首次 ENOENT 与目录扫描间并发发布 exact journal、大小写 alias、合法/畸形 backup/tmp 时 POSIX/Windows 全部 fail closed；Launcher 不读取/删除证据；无关文件不阻断；POSIX Release 原生自测和 Windows 真 CI 均覆盖 | TASK-003/008/010/013 |
| TEST-130 | rebootstrap candidate install-lock 生命周期 | `PREPARING` 必须保留 exact lock；`PREPARED` 及以后只在 package lock 下退休空 regular single-link lock；删除前/后崩溃幂等，非空/symlink/hardlink/大小写 alias 全部 fail closed | TASK-003/008/010/013 |
| TEST-131 | derived working-generation rollback cleanup | 仅接受 manifest category/leaf/relative record 与确定性 copy temp；完整预检后 child-first identity-revalidated 删除；首删崩溃、ENOSPC、部分树重放收敛；trust rotation/unbound tree 拒绝；`ROLLED_BACK` 前强制 absent | TASK-003/008/010/013 |
| TEST-132 | START_AUTHORIZED marker 独立证明 | trust preserved 枚举不把 `rebootstrap-start.authorization` 当 live-derived required entry；pre-start 单独 no-follow 读取、验签并绑定 host/nonce/package/profile/phase/runtime generation/launcher/CA/journal digest；缺失、篡改、旧代、alias 均拒绝 | TASK-003/008/010/013 |
| TEST-133 | derived root authority 与目录持久化 | manifest schema 3 绑定每个 category 的 present/absent、POSIX mode/uid/gid 和稳定 authority policy；preserved root identity 不得替换；可重建 root 先隔离再按平台封印；目录 metadata 深到浅 fsync/write-through，rollback pre-start 重验完整闭包 | TASK-003/008/010/013 |
| TEST-134 | terminal receipt 有界崩溃可重放 GC | raw scan 上限 16384，验证后保留最新 1024；仅删除 terminal+COLLECTED+binding cleared 且无 journal/backup/collecting/candidate/lock/atomic artifact 的 receipt；digest GC alias 在断电、批处理中断、HMAC/大小写/伪造场景安全重放 | TASK-003/008/010/013 |
| TEST-135 | rebootstrap 物理 byte/inode reserve | 在 `PREPARED`/公共停机前于所有受影响源/候选/backup 同卷建立签名 HELD reserve；Linux `posix_fallocate+st_blocks`、macOS `F_PREALLOCATE+st_blocks`、Windows `FileAllocationInfo+AllocationSize` 且拒绝 sparse/compressed/reparse；测试模式 8 MiB/128 inode；崩溃按 HELD→RELEASING→RELEASED 收敛 | TASK-003/008/010/013 |
| TEST-136 | stable Launcher/CA 提交后自动恢复 | OS service 永久指向不可由 ordinary rebootstrap 替换的 Recovery Guardian；新代经历 5 分钟 `PROBATIONARY_COMMITTED`，可信 crash/identity/data-plane 探针失败自动恢复旧整代；root/SYSTEM generation-head 单调前进并拒绝旧 receipt ABA；24h 后仅按政策回收 | TASK-003/008/010/013 |
| TEST-137 | rollback observation failure retry | `ROLLBACK_OBSERVING` 失败原子退回 `ROLLING_BACK`、撤销旧授权并恢复 exact signed ADMIN_STOPPED；15 秒观察失败与进程崩溃均可重跑，未健康不得写 `ROLLED_BACK`，Controller 数据面正常时不误停 | TASK-003/008/010/013 |
| TEST-138 | 首项目自动建立与项目生命周期解耦 | auto/gateway 在 `INSTALL_REQUIRED` 下只接受固定平台路径的 release-ready 签名包；Windows canonical 路径大小写/分隔符等价，parent symlink/坏签名/profile mismatch 在 bootstrap lock 前拒绝；两个首项目由独立 root-only lock 串行且锁内重查，winner 只 stage 一次；全链共用 absolute deadline；test/partial/untrusted/data-plane-down 不报成功；删除项目分发源后平台定义、Launcher、active slot、status ownership 只引用宿主 home/A-B 槽 | TASK-001/003/012/013 |
...
| 需求 | 实现任务 | 验收 |
| --- | --- | --- |
| REQ-001～003 | TASK-001/003/007/009 | TEST-001～004/025/068/110/138 |
| REQ-004～006 | TASK-001/003/004/005/008/010 | TEST-006/009～017/055～057/061/062/066/072～076/079/081/083/089/090/099～105/109/113～115/138 |
...
| REQ-013 | TASK-003/005/006/008/010 | TEST-027～030/032/058/060/061/078/087/090/092～095/100～103/106/107/109/113～115/129～137 |
...
### 12.7 2026-08-09 当前源码恢复反证检查点

- 产品边界固定为两种现行数据面：共享公网反向代理只使用宿主 Nginx；项目独立或降级时使用纯 WLS 高端口。两者都必须以当前源码重新产生验收证据。
- 本轮不接受“不断测试再找问题”的做法：先静态审查状态机、文件系统 authority、崩溃重放和跨平台边界，新增 DEF-263～271 / TEST-129～137；开放 P0/P1 全部完成独立复审后，才按 PostgreSQL→native/platform→当前源码百万矩阵的顺序执行验收。
- DEF-263 已经主 owner 修正并经独立复审：POSIX/Windows Launcher 在 journal 首次 ENOENT 后把 exact target、大小写 alias 和 backup/tmp 保留前缀都视为恢复证据。POSIX Release 编译、3 个 native self-test 与聚焦 PHP `4 tests / 299 assertions` 通过；Windows 真编译仍属于 TASK-013。
- DEF-264/265 的实现已落地：candidate install lock 按 phase 安全退休；same-CA `working-generation` 以 manifest-bound、no-follow、child-first 删除并在 rollback terminal 前硬栅栏。定向证据分别为 `6 tests / 333 assertions`、`12 / 1111`，组合 rebootstrap 为 `32 / 1937`；在独立复审接受前仍标记 `REVIEW_PENDING`。
- DEF-266 由组合回归反证：`rebootstrap-start.authorization` 同时属于 preserved namespace 与 live-derived required 清单，导致 CA rotation commit 必然失败。修复必须把 marker 从派生态枚举中移出，并保留独立 no-follow 验签与 journal/代绑定。
- DEF-267/268 由独立只读审查确认：derived manifest 没有 category root authority/identity，目录 metadata 缺少逐级持久化；terminal receipt 永不 GC，累计 4097 个 nonce 会永久阻断 whole-generation maintenance。
- DEF-269 为停机安全阻断：当前 10 GiB 常量仅限制逻辑 retention，Controller 的 8 MiB `recovery.reserve` 也不是整代物理容量。必须在公共停机前建立同卷、真实分配、签名且可重放的 byte/inode reserve。
- DEF-270 为恢复架构阻断：ordinary A/B 回滚不能恢复已经替换并提交的稳定 Launcher/CA。平台服务必须以不可由 ordinary rebootstrap 替换的 Recovery Guardian 为执行入口，并以 5 分钟 probation、单调 generation-head 和旧整代保留完成自动恢复闭环；24 小时 backup 在此之前只能称为认证恢复源，不能称为可执行自动回滚。
- DEF-271 的 rollback observation retry 已实现但仍待独立复审；观察失败不得写 terminal，必须撤销授权、恢复 exact ADMIN_STOPPED 并回到可重放 `ROLLING_BACK`。
- 本检查点不改变发布结论：TASK-003/008/010/012/013/014 继续 `in_progress`，当前源码统一 PostgreSQL、三平台 system service/native、WebUI、外部 CA/DNS 与 Nginx/纯 WLS 各精确 1,000,000 请求均未重跑。

### 12.8 2026-08-11 范围冻结与循环终止检查点

- §12.7 的 `in_progress` 是历史状态，已由 §0 的 `SCOPE_FROZEN / BLOCKED_INTERNAL` 取代。
- 用户批准方案 A，并只补入方案 B 中冻结时已有证据的重大缺陷；最终清单固定为 `FRZ-BLK-001`–`FRZ-BLK-010`。
- 缺陷编号封顶 `DEF-271`，测试编号封顶 `TEST-138`，原任务在 turn `49fe9325-4fc5-4273-b826-e01323d43bb3` 停稳。
- 后续只允许冻结指纹对账、有限阻断修复、一次 `FRZ-REVIEW-01` 和固定验收矩阵；表外工作转 `2.0.1`。
- Framework 数据库、macOS/Windows 真机、托管 Runner 与外部 CA/DNS 分离为独立门禁；允许以 `GA_BLOCKED_EXTERNAL` 合法收口，禁止借环境缺口重新打开无边界开发。
- FRZ-001 已在固定时间盒内完成：四项指纹匹配，`FRZ-BLK-001`–`010` 均找到 IMPLEMENTED 候选，0 文件修改、0 测试/构建、0 新编号；结论来自原任务 turn `019fef61-b328-70a0-9fa4-cb02115abbd7`。该结论只允许跳过无目标的修复轮次，不代表终审或验收通过。

### 12.9 2026-08-13 当前工作树 re-freeze 检查点

- 用户授权 Freeze B；产品范围、`DEF-271`/`TEST-138` 截止和唯一审查周期均保持 Freeze A 不变。
- 当前基线是 §0.6 记录的 `dev@936f9ffc5fc4b32a5babe83eddce7c8de53fb5ad` 与三项指纹。
- Freeze A 的所有 PASS 只保留为历史；Freeze B 当前为 `CURRENT_VALIDATION_PENDING / READY_FOR_FRZ-005`。
- 下一动作仅为当前源码 `FRZ-005` 分层验证；不得因 re-freeze 重开审查或扩大 `2.0.0`。
