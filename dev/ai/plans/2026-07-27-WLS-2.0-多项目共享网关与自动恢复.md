...
- 当前提交候选使用正式 `app/bootstrap.php` 完成 Weline_Server 全量回归：
  `1288 tests / 6441 assertions / 17 platform skips`，零错误、零失败；WLS 2.0
  扩展门禁 `174 / 2209 / 15 platform skips`，PHP lint、diff check 全绿。
- macOS 已显式开启原生集成门禁，在任务随机端口完成 Native Broker、Launcher、签名
  槽位、崩溃回滚和真实数据面恢复：`12 tests / 1995 assertions`，零跳过、零失败；
  因当前会话无法取得 sudo，launchd 系统安装、ACL 和 reboot 仍为
  `BLOCKED_ENVIRONMENT`。
- Windows 平台分支、Named Pipe/ACL/包路径与协议专项为
  `87 tests / 1810 assertions / 3 non-Windows skips`；Parallels 授权已到期且 VM
  被挂起，尚无本轮真实 Windows Service/reboot 证据，不能宣称实机通过。
...
- 先前纯 WLS 与网关 H2/H1.1 单项目百万请求同样为 0 正确性错误；两项目混合百万、
  H3 百万和同条件三轮性能中位数仍属 TASK-014 未闭合门禁。
- 首次 ACME、完整 Session 运行时亲和、macOS/Windows 系统服务 ACL/reboot、
  Windows 实机和 TEST-001～054 尚未全部闭合。当前发布判定仍为 `BLOCKED`，但真实
  legacy promote 与本轮 Linux 网关百万复测不再是阻断项。
