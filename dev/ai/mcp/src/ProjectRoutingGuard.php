            ...
            return [
                'hookSpecificOutput' => [
                    ...
                    'additionalContext' => sprintf(
                        ...
                        . "The local index for %s is current at revision %d. Before repository discovery or code/document reads, call mcp__weline_project_intelligence__get_edit_bundle once per edit batch with the current task, every remaining known path, and every affected symbol. Use only the returned regions and write through apply_compact_edit; do not list, search, or read repository files one by one. Each write plan contains 1 to 50 operations. If the task needs more than 50, automatically split it into dependency-ordered batches of at most 50, fetch a fresh bundle after each successful batch, and continue until the original task is complete; never truncate or defer work solely because of the per-call limit.\n"
