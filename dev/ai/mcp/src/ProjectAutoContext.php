...
final class ProjectAutoContext
{
    ...
    public function describe(array $hookResult): array
    {
        ...
        $context = implode("\n", [
            ...
            $refreshNote,
            $readinessNote,
            'Primary read contract: call get_edit_bundle once per edit batch with this repository, the current task, every remaining known path, and every affected symbol. It returns only exact guarded regions, symbol impact, docs, and matched skill paths; do not recursively scan or read whole files while the index is fresh.',
            'Primary write contract: emit only replacement operations. Each apply_compact_edit plan contains 1 to 50 operations. If the task needs more than 50, automatically split it into dependency-ordered batches of at most 50; after each successful batch, fetch a fresh bundle for the remaining work and continue until the original task is complete. Never truncate, omit, or defer work solely because of the per-call limit. Granular read/edit tools are compatibility-only.',
