...
| `UrlManager` 批量导入报 `SQLSTATE[23505] duplicate key ... idx_identify` | 批量写入未先做批内 `identify` 去重；且删除前模型残留 `id` 会让 `delete()` 退化成按主键删单条，`where IN` 失效 | `flushBatchRows()` 先按 `identify` 去重；删除/插入前都 `recovery()`；再按 `identify IN (...)` 批量删除旧记录后批量插入（幂等） |
