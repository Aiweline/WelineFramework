...
5. **编译期去重/累积列表** — Taglib 标签 ID 检测、临时收集器等，跨请求会累积导致误判  
...
#### 1. URL/路由解析缓存
- `Url::registerStateResets()` — parserServer, parserSites, parserCurrencies, parserLanguages, parserMatchs, parserSiteMatchs, parserUrlCache, splitUrlCache, parserCache, parsingInProgress, decode_urls
- `Request::$url_paths` — URL 路径解析缓存
- `ProcessUrlCache::$staticCache` — 请求内 URL 缓存
...
- `Taglib::$compileDepth` — 嵌套编译深度

#### 3. 请求上下文
- `RequestContext::cleanup()` — 请求 ID、区域、语言、货币等
- `SseContext::reset()` — SSE 连接、启用标志、回调等

#### 4. 用户/会话状态
- `Env::$user` — 当前请求的用户标识
- Session 实例 — BackendSession、FrontendSession、BackendApiSession、FrontendApiSession
- `CurrencyCache::$staticCache` — 请求内货币缓存

#### 5. 请求级对象实例
- `Response` 实例清理
- `Template` 单例实例清理 — _data 数组中的 title、req、env、local 以及目录路径
- `State::$is_backend` — 区域状态
...
#### 8. 维护模式缓存
- `Env::$maintenanceCached`
- `Env::$maintenanceLastCheck`
- `Env::$mergedCacheConfig`

#### 9. 主题插槽渲染缓存
- `SlotRendererService` 实例清理
- `Slot::$registeredSlots` — Taglib Slot 静态注册表

#### 10. 缓存事件防重复
- `CacheFlushedObserver::resetRequestState()`
- `CacheFactory::$dispatchingCacheEvent`
