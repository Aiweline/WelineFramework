...
For browser-visible checkout changes, use a unique instance name and an available port `>=9502`, then stop it after verification:

```powershell
php bin/w server:start ai-test-payment-{unique-id} -p {available-port}
php bin/w server:stop ai-test-payment-{unique-id}
```
