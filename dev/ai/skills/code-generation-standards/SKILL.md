...
- 问题修复必须优先根因：先定位触发路径与状态机，再改主流程或数据模型；提交前至少保留 1 个能复现/防回归的测试（单测或集成）
- 注释中的 PHP 结束符（硬性）：
...
- 用"兜底补丁"掩盖主流程缺陷（如按字符串特判、吞异常、静默降级、无监控短路）；除非用户明确要求临时止血且注明有效期与回滚条件

## 阻塞函数禁止直接使用

在 WLS（长连接服务器）环境下，以下函数会阻塞整个 Worker 进程，导致其他请求无法处理：

| 禁止直接使用 | 必须替换为 |
|-------------|-----------|
| `\sleep()` | `SchedulerSystem::sleep()` |
| `\usleep()` | `SchedulerSystem::yieldDelay()` |
| `\die()` | `throw new \RuntimeException()` + 统一错误处理 |
| `\exit()` | `throw new \RuntimeException()` + 统一错误处理 |
| `exit`（裸词） | 同上 |

### 原因

在 WLS 中：
- `\usleep(2000000)` 会**阻塞 Worker 进程 2 秒**，期间无法处理任何其他请求
- SSE 长连接等场景下，阻塞会严重影响服务器吞吐量和响应时间
...
// ✅ 正确：挂起当前 Fiber，让 Worker 处理其他请求
use Weline\Framework\Runtime\SchedulerSystem;
SchedulerSystem::yieldDelay(2000);  // 毫秒，替代 usleep(2000000)
SchedulerSystem::sleep(5);          // 秒，替代 sleep(5)
```
...
while ($sse->isAlive()) {
    $sse->sendEvent('log', $event);
    SchedulerSystem::yieldDelay(2000);  // 让出控制权
}
```

### 例外情况

以下场景可以使用原生阻塞函数（因为不在 WLS 上下文中）：
- **PHP CLI 脚本**（非服务器模式）
- **Cron 定时任务**（独立进程）
- **Composer/安装脚本**
- **单元测试**中模拟等待
