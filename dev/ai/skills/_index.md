...
| **口令「分项」**：当前核心仓提交并推送当前核心分支后，让 `E:\WelineFramework\Framework-Official` 内的 7 个子项目运行 `php bin/w core:update -b <branch>` 同步核心代码；用户只说「分项」默认 `dev`，说「分项 <分支>」则处理该分支；站点 WLS 运行中则 reload | **仅**加载 `dev/ai/skills/CI发布工程师-分项更新/SKILL.md`；脚本在 `dev/tools/fenxiang/fenxiang-update.ps1`；Codex 另见 `.codex/skills/fenxiang-update/SKILL.md`；无口令不得触发 |
