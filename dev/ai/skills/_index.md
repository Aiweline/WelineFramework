...
- Plan 模式，或用户要求详细实施计划、修改点、任务拆分、测试用例、验收、风险回滚、执行型智能体交接：先加载 `planning`；再按涉及角色加载 `技术主管-任务拆分与调度`、`QA测试主管-测试策略治理` 或专项实施技能。
...
| 场景 | 技能 |
|---|---|
| Plan 模式、工程化详细计划、执行型智能体交接 | `planning`（统一计划规范）；需要角色调度时组合 `技术主管-任务拆分与调度`，需要质量策略时组合 `QA测试主管-测试策略治理` |
