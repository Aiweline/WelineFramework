...
- 2026-03-23 08:29 Verified the authenticated BT server list and add form in-browser: the page loaded correctly, add-form defaults were fixed, and creating `BT-Test-Temp-20260323` redirected back to the list successfully.
- 2026-03-23 09:11 Traced the remaining delete failure to framework semantics: `AbstractModel->__call('delete')` only prepares the delete query, and the delete is not executed until `fetch()` is called.
- 2026-03-23 09:12 Patched `Weline_Bt_Center\Controller\Backend\BtServer::postDelete()` to call `$server->delete()->fetch()` and return a failure JSON response when no row is affected.
- 2026-03-23 09:13 Added a stable delete-button rebinding layer in the BT list template that uses `BackendConfirm`/`AdminConfirm.show(message, options).then(...)`, bypassing the older broken object-style confirm binding.
- 2026-03-23 09:18 Re-ran `php -l` on the touched controller/template, then verified via Playwright and direct PostgreSQL query that deleting `BT-Test-Temp-20260323` removed the row from both the backend list and `m_weline_bt_server`.
