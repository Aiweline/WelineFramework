...
4. 本文件：`app/code/WeShop/Analytics/doc/AI-INDEX.md`
5. 模块说明：`app/code/WeShop/Analytics/doc/README.md`
6. 只读取本次任务相关源码、配置和验证入口

## 模块身份

- 模块代码：`WeShop_Analytics`
- 目录：`app/code/WeShop/Analytics`
- Vendor：`WeShop`
- Module：`Analytics`
...
- `app/code/WeShop/Analytics/doc/README.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|WeShop_Analytics> [operation]` 或对应 `w_query` 帮助。
