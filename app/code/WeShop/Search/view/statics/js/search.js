    ...
    SearchManager.prototype = {
        ...
        setupCategoryDropdown: function () {
            ...
            const updateCategoryLabel = () => {
                ...
                if (selectedOption) {
                    // 优先使用 data-display-label 属性（不包含缩进的原始标签）
                    let labelText = selectedOption.getAttribute('data-display-label');

                    // 如果没有 data-display-label，从选项文本中提取
                    if (!labelText) {
                        labelText = selectedOption.textContent || selectedOption.innerText || '';
                        // 如果包含 lang 标签，提取其中的文本
                        const langMatch = labelText.match(/<lang>(.*?)<\/lang>/);
                        if (langMatch) {
                            labelText = langMatch[1];
                        }
                        // 移除 HTML 实体和缩进符号，只保留分类名称
                        labelText = labelText.replace(/&nbsp;/g, '').replace(/[>›]/g, '').trim();
                    }

                    // 更新 label 显示
                    this.searchCategoryLabel.textContent = labelText.trim() || '全部分类';
                }
            };
