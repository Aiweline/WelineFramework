...
`Aiweline_A2A` is the first implementation slice for the Agent-to-Agent escrow trading platform.
...
- SKU purchase also syncs the selected capability SKU into `aiweline_a2a_capability_sku`, upserts an idempotent order draft into `aiweline_a2a_order_draft`, and writes three escrow ledger rows into `aiweline_a2a_escrow_ledger`.
- Buyer escrow confirmation now converts the persisted draft into a formal trade order in `aiweline_a2a_trade_order`, migrates the escrow ledger to the official order ID, and places the provider into the scope-completion execution queue.
- Buyer request and Agent quote selection now persist into `aiweline_a2a_buyer_request` and `aiweline_a2a_agent_quote`, then feed the same order draft -> confirmation -> provider queue path as SKU purchase.
...
- Provider scope submission now writes `aiweline_a2a_provider_scope_submission`, declares execution boundaries, tool permissions, and evidence checklist, then advances the formal order into execution-ready status.
...
- Settlement branch handling now first renders a buyer refund/dispute application form. Opening the route previews case evidence and ledger impact only; submitting the form writes `aiweline_a2a_settlement_case`, locks refund/dispute evidence snapshots, updates escrow ledger rows into refund-ready or dispute-hold states, and preserves terminal case state when earlier routes are reopened.
- Agent reputation recalculation now writes `aiweline_a2a_agent_reputation`, derives tier/score/trust/risk signals from trade orders, accepted delivery evidence, refund cases, and dispute cases, and exposes a reputation evidence page.
...
- Trade actor assignments now persist formal order ownership for buyer, Agent provider, platform risk, and arbitrator roles in `aiweline_a2a_trade_actor_assignment`, then expose binding status in the role console.
...
- Arbitration ruling now writes `aiweline_a2a_arbitration_ruling`, generates `aiweline_a2a_wallet_instruction` dry-run adapter instructions, and updates escrow ledger rows without moving real funds.
...
The route is generated through the normal Weline module scanner after `etc/env.php` registers `router => a2a`. If a route-only rebuild falls back to `aiweline_a2a/frontend`, run module setup before route refresh so `app/etc/modules.php` receives the module env router value.
...
- New model: `Aiweline\A2A\Model\ProviderScopeSubmission` -> `aiweline_a2a_provider_scope_submission`.
...
- New model: `Aiweline\A2A\Model\DeliveryAcceptance` -> `aiweline_a2a_delivery_acceptance`.
...
- New model: `Aiweline\A2A\Model\SettlementCase` -> `aiweline_a2a_settlement_case`.
...
- New model: `Aiweline\A2A\Model\AgentReputation` -> `aiweline_a2a_agent_reputation`.
...
- New model: `Aiweline\A2A\Model\TradeActorAssignment` -> `aiweline_a2a_trade_actor_assignment`.
...
- New exception: `Aiweline\A2A\Exception\TradeActorAuthorizationException`.
...
- New model: `Aiweline\A2A\Model\ArbitrationRuling` -> `aiweline_a2a_arbitration_ruling`.
- New model: `Aiweline\A2A\Model\WalletInstruction` -> `aiweline_a2a_wallet_instruction`.
...
- Extended `Aiweline\A2A\Model\WalletInstruction` with idempotency key, external reference, failure reason, retry count, executed timestamp, and reconciled timestamp.
...
- Extended `AgentReputationService` to include `aiweline_a2a_arbitration_ruling` and `aiweline_a2a_wallet_instruction` rows in score, tier, trust signals, risk signals, source snapshot, and evidence rows.
...
- Added backend ACL proof controller `Aiweline\A2A\Controller\Backend\AclProof` with A2A source IDs for Agent operator, platform risk, and arbitration panel proof.
- Added `etc/backend/menu.xml` entry for `Aiweline_A2A::acl_proof` under the backend order group.
- Updated module dependencies so `Aiweline_A2A` depends on `Weline_Backend` and `Weline_Acl`.
...
- Role console now shows `ACL Source 已注册` evidence for:
  - `Aiweline_A2A::agent_operator`
  - `Aiweline_A2A::platform_risk`
  - `Aiweline_A2A::arbitration_panel`
