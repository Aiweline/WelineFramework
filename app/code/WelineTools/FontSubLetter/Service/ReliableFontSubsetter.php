...
/**
 * 可靠字体子集生成器
 * 使用php-font-lib库但添加更好的错误处理
 */
class ReliableFontSubsetter
{
    /**
     * 支持的字体格式
     */
    ...
    public function createSubset(string $inputPath, string $outputPath, array $selectedChars): array
    {
        try {
            ...
            // 使用错误抑制来避免fwrite警告
            $subsetData = $this->generateSubsetWithErrorHandling($inputPath, $normalizedChars);
     ...
     * 使用错误处理生成子集
     */
    private function generateSubsetWithErrorHandling(string $inputPath, array $normalizedChars): string
    {
        // 设置错误处理
