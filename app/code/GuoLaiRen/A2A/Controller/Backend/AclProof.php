...
namespace GuoLaiRen\A2A\Controller\Backend;

use GuoLaiRen\A2A\Service\BackendAclProofPayloadService;
use Weline\Framework\Acl\Acl;
use Weline\Framework\App\Controller\BackendController;
use Weline\Framework\Manager\ObjectManager;

#[Acl('GuoLaiRen_A2A::acl_proof', 'A2A 运行时实证', 'mdi mdi-shield-check-outline', 'A2A 交易运行时实证与权限源注册', 'Weline_Backend::order_group')]
class AclProof extends BackendController
{
    private ?BackendAclProofPayloadService $proofPayloadService = null;

    #[Acl('GuoLaiRen_A2A::acl_proof_index', '查看 A2A 实证源', 'mdi mdi-shield-search', '查看 A2A 运行时实证源状态')]
    ...
    #[Acl('GuoLaiRen_A2A::agent_operator', 'A2A Agent 运营账号', 'mdi mdi-robot-outline', 'Agent 运营账号执行交付与证据提交的后台权限')]
    public function agentOperator(): string
    {
        return $this->proofSource('provider');
    }

    #[Acl('GuoLaiRen_A2A::platform_risk', 'A2A 平台风控', 'mdi mdi-shield-alert-outline', '平台风控复核、冻结、退款复核与钱包监控权限')]
    public function platformRisk(): string
    {
        return $this->proofSource('platform');
    }

    #[Acl('GuoLaiRen_A2A::arbitration_panel', 'A2A 仲裁席位', 'mdi mdi-gavel', '仲裁证据复核与最终裁决签发权限')]
