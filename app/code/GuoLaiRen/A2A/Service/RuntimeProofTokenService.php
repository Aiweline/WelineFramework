...
class RuntimeProofTokenService
{
    ...
    private const ROLE_SOURCES = [
        'provider' => 'GuoLaiRen_A2A::agent_operator',
        'platform' => 'GuoLaiRen_A2A::platform_risk',
        'arbitrator' => 'GuoLaiRen_A2A::arbitration_panel',
    ];
    ...
    private function signingKey(): string
    {
        ...
        try {
            $env = Env::getInstance();
            $configured = \trim((string)$env->getConfig('guolairen_a2a.runtime_proof_secret', ''));
            if ($configured !== '') {
                return $configured;
            }
            ...
            return \hash('sha256', BP . '|GuoLaiRen_A2A|' . $dbPassword . '|' . $backendPrefix);
        } catch (\Throwable) {
            return \hash('sha256', BP . '|GuoLaiRen_A2A|runtime-proof');
        }
    }
