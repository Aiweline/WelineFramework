# PageBuilder 文档索引

## 核心入口

- [开发任务](./开发/task.md)
- [开发计划](./开发/plan.md)
- [表结构升级规范](./开发/表结构升级规范.md)
- [规范目录](./规范/)
- [更新日志目录](./更新日志/)

## 常用文档

### AI 建站（新架构计划，按模块拆分）

- [计划-新AI建站工作台-页面区块与智能体.md](./计划-新AI建站工作台-页面区块与智能体.md) — **仅 PageBuilder**：`blocks[]`、`ai_html`、物化、Visual、`can_publish` 消费侧、PB 冒烟与 Playwright 中的 PB 段
- [阶段2-预置工作区-双轨-IA.md](./阶段2-预置工作区-双轨-IA.md) — 阶段2 双轨切换与 `workspace_track` / `site_ready` 契约摘要
- [`Weline/Server/doc/计划-AI建站-w_query与本机hosts.md`](../../../Weline/Server/doc/计划-AI建站-w_query与本机hosts.md) — **仅 WLS**：`w_query`/hosts、跨 OS、CI、与通配证书/Worker SSL 衔接
- [`Weline/Websites/doc/计划-AI建站工作台-Websites侧.md`](../../../Weline/Websites/doc/计划-AI建站工作台-Websites侧.md) — **仅 Websites**：Agent Tools、本地供应商、订单生命周期、handoff、`site_ready`
- [智能体组件生成计划.md](./智能体组件生成计划.md) — **另一能力**：`ai-generate` / 单组件生成，与上者区分

### 使用与配置

- [可视化配置功能使用指南](./可视化配置功能使用指南.md)
- [可视化编辑器-响应式配置](./可视化编辑器-响应式配置.md)
- [响应式配置格式使用指南](./响应式配置格式使用指南.md)
- [响应式配置-快速参考](./响应式配置-快速参考.md)
- [PageBuilder-页面配置与跟踪代码](./PageBuilder-页面配置与跟踪代码.md)
- [PageBuilder-Header配置说明](./PageBuilder-Header配置说明.md)

### 国际化

- [i18n 翻译指南](./i18n-translation-guide.md)
- [Local 标签翻译指南](./local-tag-translation-guide.md)
- [Local 标签使用指南](./local-tag-usage-guide.md)
- [翻译快速参考](./translation-quick-reference.md)
- [后台字段翻译指南](./backend-field-translation-guide.md)
- [PageBuilder-多语言配置系统](./PageBuilder-多语言配置系统.md)

### 模板与 SEO

- [marketing-landing-模板说明](./marketing-landing-模板说明.md)
- [marketing-landing-快速启动](./marketing-landing-快速启动.md)
- [Sitemap配置与使用指南](./Sitemap配置与使用指南.md)
- [Sitemap自动生成与SEO集成](./Sitemap自动生成与SEO集成.md)
- [Sitemap快速参考](./Sitemap快速参考.md)

### 修复与测试

- 以 `修复-` 开头的文档用于问题修复记录。
- 以 `测试-` 开头的文档用于测试结果与验证记录。
- [故障排查-预览生成失败](./故障排查-预览生成失败.md)

## 说明

- `doc/` 作为模块文档根目录保留。
- 规范文档与变更日志已从模块根目录和 `docs/` 收拢到当前目录下。
