...
全站契约必须写在 `plan_json` root，例如 `content_locale`、`language_contract`、`locale_context`、`site_design_system`、`theme_context_snapshot` 和 `shared_prompt_context`。队列 prompt 在内存 runtime context 中读取这些全局契约，并只从 `plan_json.pages.{page_type}.{block_key}` 读取当前 block 自己的 `.block_contract`、`.image_intent` 和 `.asset_requirements`；不得在 page/block 节点上复制或覆盖全站契约，也不得从 UI 提示、历史工作表或宽泛 workspace fallback 重新推导这些契约。
...
- 全站契约只属于 root：所有动态 block 共用 `plan_json.language_contract.source_of_truth_locale`、`plan_json.locale_context.required_visible_script`、`plan_json.site_design_system` 和 `plan_json.theme_context_snapshot`。`block_goal`、`task_goal`、`story_goal`、`why_this_block`、`block_contract`、`visual_signature`、`image_intent`、`asset_requirements`、`execution_script` 只能作为意图来源，不能作为访客可见文案。
