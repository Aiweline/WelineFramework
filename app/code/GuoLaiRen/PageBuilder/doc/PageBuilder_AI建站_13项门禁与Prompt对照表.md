...
| logo_after_theme_design | `plan_json.theme.logo_generation` 必须在 `theme_design` 之后规划；logo prompt 必须引用已完成的 `theme_design.style_signature`、`art_direction`、`color_scheme`、`typography_spacing_radius`、`visual_keywords`、`tone_of_voice`、`cta_tone` 和 `forbidden_styles` | Stage1 theme prompt 必须先完成主题设计，再基于主题设计生成四个 logo option；不得只按业务 brief 或站点名生成泛化 logo |
