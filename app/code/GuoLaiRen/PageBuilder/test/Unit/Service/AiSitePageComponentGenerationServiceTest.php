...
HTML,
    ...
    public function testSharedComponentPromptAndDefaultsUseConfirmedThemeContract(): void
    {
        ...
        self::assertSame('立即开始游�?', (string)($config['cta.text'] ?? ''));
    ...
    public function testSectionDefaultConfigPrefersEnglishTaskPlanSamplesOverChineseBlueprintCopy(): void
    {
        ...
        $config = (function (string $pageType, array $section, array $blueprint, array $websiteProfile, array $scope): array {
            ...
            [
                ...
                'config' => [
                    'section_title' => '首页主视�?',
                    'section_intro' => '这是中文默认文案，不应进入英文站�?build�?',
                ],
            ],
            [
                'page_title' => '首页',
    ...
    public function testRenderedHtmlLanguageGuardRejectsMeaningfulChineseContentForEnglishLocale(): void
    {
        ...
        (function (string $html, array $renderContext): void {
            ...
            $service,
            '<section><h2>立即开始体�?/h2><p>这是中文段落，长度足够，英文站点不应通过�?/p></section>',
            ['_content_locale' => 'en_US']
        );
    }
    ...
    public function testSanitizeVisibleCopyDropsPlanningObservationCopy(): void
    {
        ...
        self::assertSame('', $sanitize->call($service, '信任感增强，并知道如何立即下�?Teen Patti Royal APK�?'));
    ...
    public function testSanitizeVisibleCopyDropsStageTaskInstructionCopy(): void
    {
        ...
        self::assertSame('印度 / 棋牌 / 下载 / APK - Welcome to Teenipiya �?India\'s Royal Card Game Hub', $sanitize->call($service, '印度 / 棋牌 / 下载 / APK - Welcome to Teenipiya �?India\'s Royal Card Game Hub'));
        ...
        self::assertSame('', $config['content.title'] ?? null);
    ...
    public function testCleanAiHtmlFragmentRejectsMalformedHtmlStructure(): void
    {
        ...
        // 错位闭合标签会先�?repairHtmlFragmentTagBalance 纠正�?
    ...
    /**
     * 强行契约：cleanAiHtmlFragment 不得�?<img src=...> 等属性中合法包含
     * "home_page" / "page:home_page:..." �?URL；只允许从可见文本中剥离这些 planning 关键字�?
    ...
    /**
     * 强行契约：泄漏校验只检"可见文本"——属性中合法�?URL / slot id 必须放行�?
     */
    ...
    {
        // 真实场景复现：stage1 留下�?slot_id=5、page_type='' �?legacy slot�?
        // �?brief 文本里包�?"Hero visual brief..." 会匹配到关键�?"hero"�?
        // �?stage2 已经�?home_page 落地了精确的 page-scoped slot
        // (slot_id=page:home_page:content-home-page-hero)�?
        // resolveSectionAssetUrl 必须优先返回精确匹配�?page-scoped slot�?
        // 不得�?legacy slot（即便其元数据偶然命中关键字）抢占�?
        $service = new AiSitePageComponentGenerationService(
            pageBlueprintService: new AiSitePageBlueprintService(),
        );
    ...
    {
        // �?stage2 没有 page-scoped slot，仅�?stage1 legacy slot 时，
        // 仍需回退�?legacy slot，但只有在前面所有更精确的查找都 miss 时才允许�?
        $service = new AiSitePageComponentGenerationService(
            pageBlueprintService: new AiSitePageBlueprintService(),
        );
        ...
        self::assertSame($legacyUrl, $resolved, '没有 page-scoped slot 时应回退�?legacy slot�?');
    }

    public function testResolveSectionAssetUrlPrefersCodeBasedSlotIdOverKeyBasedSlotId(): void
    {
        // 真实场景复现：blueprint 偶尔会给 section.key 复用通用 token（如�?about-page-story �?key 设为 highlights�?
        // 因此 expectedSlotIds 同时含有 page:about_page:content-about-page-story（基�?code�?
        // �?page:about_page:highlights（基�?key）。两者均�?REAL 时，必须优先返回 code-based 的精�?slot�?
        $service = new AiSitePageComponentGenerationService(
            pageBlueprintService: new AiSitePageBlueprintService(),
        );
        ...
        // 注意 slot 顺序：key-based 排在前面，模拟真�?manifest 的非 specificity 顺序�?
        $scope = [
            'asset_manifest' => [
                'slots' => [
        ...
        self::assertSame($codeBasedUrl, $resolved, 'code-based 精确匹配 (page:about_page:content-about-page-story) 必须优先�?key-based slot�?');
    }
    ...
    public function testResolveSectionAssetUrlPrefersRealLegacyOverPlaceholderPageScoped(): void
    {
        ...
        // �?stage1 已经留下真实 legacy slot 时，必须优先继承真实图片�?
        // 用户原话�?反而用了占位图，这个要处理�?
        $service = new AiSitePageComponentGenerationService(
            pageBlueprintService: new AiSitePageBlueprintService(),
        );
    ...
    {
        // 当同时存�?placeholder �?real �?page-scoped slot 时，
        // 必须优先返回非占位真实图片�?
        $service = new AiSitePageComponentGenerationService(
            pageBlueprintService: new AiSitePageBlueprintService(),
        );
        ...
        self::assertSame($realUrl, $resolved, '真实图片必须优先于占�?SVG 被返回�?');
    }
