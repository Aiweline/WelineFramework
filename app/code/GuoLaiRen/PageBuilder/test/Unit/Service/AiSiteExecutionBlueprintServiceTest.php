...
final class AiSiteExecutionBlueprintServiceTest extends TestCase
{
    public function testBuildPlanArtifactsProducesMarkdownAndBlueprint(): void
    {
        ...
        $pagePromptContext = $artifacts['structured']['page_plans']['home_page']['page_prompt_context'] ?? null;
        self::assertIsArray($pagePromptContext);
        self::assertSame('home_page', (string)($pagePromptContext['page_type'] ?? ''));
        self::assertSame(
            (string)($artifacts['execution_blueprint']['shared_prompt_context']['context_hash'] ?? ''),
            (string)($pagePromptContext['shared_context_hash'] ?? '')
        );
        self::assertStageOneThemeDesignSchema($pagePromptContext['theme_design'] ?? null);
        self::assertSame(
            $artifacts['plan_json']['theme_design'] ?? [],
            $pagePromptContext['theme_design'] ?? []
        );
        self::assertSame(
            $artifacts['plan_json']['theme_design']['color_scheme'] ?? [],
            $pagePromptContext['theme_hard_constraints']['color_scheme'] ?? []
        );
        self::assertSame(
            (string)($artifacts['plan_json']['theme_design']['theme_purpose'] ?? ''),
            (string)($pagePromptContext['theme_hard_constraints']['theme_purpose'] ?? '')
        );
        self::assertSame(
            (string)($artifacts['plan_json']['theme_design']['selection_reason'] ?? ''),
            (string)($pagePromptContext['theme_hard_constraints']['selection_reason'] ?? '')
        );
        self::assertSame(
            $artifacts['plan_json']['theme_design']['forbidden_styles'] ?? [],
            $pagePromptContext['theme_hard_constraints']['forbidden_styles'] ?? []
        );
