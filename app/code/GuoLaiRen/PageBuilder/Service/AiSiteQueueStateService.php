...
/**
 * AiSiteQueueStateService
 *
 * 从 AiSiteAgent.php 抽出的纯数据转换层，负责把一行 `weline_queue` 原始记录整形为
 * 前端 / 侧栏 / 事件载荷所需的公共状态（`buildQueueObserverPublicState`），
 ...
 * `AiSiteQueueStateServiceTest` 的锁定用例。
 */
class AiSiteQueueStateService
{
    private const MAX_CONTENT_JSON_DECODE_BYTES = 262144;

    /**
     * 对外暴露的 queue 公共状态字段，供前端状态栏、SSE 下发、事件回放共用。
     *
     * @param array<string, mixed> $queueRow 来自 `weline_queue` 的一行原始数据
     *
     * @return array<string, mixed>
     */
    public function buildObserverPublicState(array $queueRow): array
    {
        ...
        $rawContent = $queueRow['content'] ?? null;
        $contentRaw = $this->normalizeStateContentForJsonDecode($rawContent);
        if ($contentRaw !== '') {
            ...
            if (\is_array($decoded)) {
                ...
                if (\is_array($decoded['stage1_page_progress'] ?? null)) {
                    $stageOnePageProgress = $decoded['stage1_page_progress'];
                }
            }
        }
        if ($stageOnePageProgress === []) {
     ...
     */
    private function normalizeStateContentForJsonDecode(mixed $content): string
    {
        if (\is_array($content)) {
            try {
     ...
     * @return array<string, mixed>
     */
    private function extractStageOnePageProgressFromRawContent(mixed $content): array
    {
        if (\is_array($content)) {
            return \is_array($content['stage1_page_progress'] ?? null) ? $content['stage1_page_progress'] : [];
        }
        if (!\is_string($content) || \trim($content) === '') {
            return [];
        }

        $valueJson = $this->extractBoundedJsonValue(\trim($content), 'stage1_page_progress', 131072);
        if ($valueJson === '') {
            return [];
        }
    ...
    private function extractBoundedJsonValue(string $json, string $key, int $maxBytes): string
    {
        if ($maxBytes <= 0) {
            return '';
        }
        ...
        if ($start >= $length || ($json[$start] !== '{' && $json[$start] !== '[')) {
            return '';
        }
