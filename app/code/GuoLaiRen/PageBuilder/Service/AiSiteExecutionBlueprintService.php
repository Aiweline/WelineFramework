...
final class AiSiteExecutionBlueprintService
{
    ...
    private const STAGE_ONE_PAGE_PLAN_MAX_TOKENS = 8192;
    ...
    private function resolveStageOneResumePageTypes(
        ...
        if ($failedPageTypes !== []) {
            return $failedPageTypes;
        }

        $missing = [];
        foreach ($pageTypes as $pageType) {
            if (!$this->hasStageOnePagePlanCheckpoint($existingPagePlans, $pageType, $stageOneContract, $scope)) {
                $missing[] = $pageType;
            }
        }
    ...
    private function hasStageOnePagePlanCheckpoint(array $pagePlans, string $pageType, array $contract = [], array $scope = []): bool
    {
        ...
        foreach ($blocks as $block) {
            ...
            if ($content === '' || $this->isPromptLikeStageOneText($content, 'content', $template, $sectionName, $pageType)) {
                return false;
            }
            ...
            foreach ($fieldPlan as $row) {
                ...
                if ($this->isPromptLikeStageOneText($sample, $field, $template, $sectionName, $pageType)) {
                    return false;
                }
                if ($this->isPromptLikeStageOneText($implementationNote, $field, $template, $sectionName, $pageType)) {
                    return false;
                }
            }
            $executionScript = \is_array($block['execution_script'] ?? null) ? $block['execution_script'] : [];
            $coreCopy = \trim((string)($executionScript['core_copy'] ?? ''));
            if ($coreCopy === '' || $this->isPromptLikeStageOneText($coreCopy, 'core_copy', $template, $sectionName, $pageType)) {
                return false;
            }
        }
        foreach ($requiredBlockKeys as $seen) {
            if (!$seen) {
                return false;
            }
        }
        ...
        return !empty($validationReport['passed']);
    }
    ...
    private function generateStageOnePagePlansByAi(
        ...
        foreach ($pageTypes as $pageIndex => $pageType) {
            ...
            $tasks[$pageKey] = function (array $sessionParams) use ($scope, $websiteProfile, $planJson, $pageKey, $planLocale, $contentLocale, $instruction, $targetScope, $onProgress, &$completedPages, $totalPages, &$generationAttempts): array {
                ...
                if (!$pageCheckpointPassed) {
                    $generationAttempts['page_fanout'][$pageKey]['primary_issues'] = \array_slice(
                        \is_array($pageValidationReport['issues'] ?? null) ? $pageValidationReport['issues'] : [],
                        0,
                        12
                    );
    ...
    private function finalizeStageOneAiPagePlanForCheckpoint(
        ...
        // Stage-1 visible content and design intent must come from AI output.
        // Missing or weak page_goal/theme_alignment_summary/page_design_plan is
        // now rejected by the contract validator instead of being locally filled.

        $blocks = \is_array($pagePlan['blocks'] ?? null) ? $pagePlan['blocks'] : [];
        $requiredBlockKeys = \is_array($pageContract['required_block_keys'] ?? null) ? $pageContract['required_block_keys'] : [];
        $seenBlockKeys = [];
        foreach ($blocks as $block) {
            if (!\is_array($block)) {
                continue;
            ...
            $blockKey = \trim((string)($block['block_key'] ?? ''));
            if ($blockKey !== '') {
                $seenBlockKeys[\mb_strtolower($blockKey)] = true;
            }
        }
        foreach ($requiredBlockKeys as $requiredBlockKey) {
            $requiredBlockKey = \trim((string)$requiredBlockKey);
            if ($requiredBlockKey === '' || isset($seenBlockKeys[\mb_strtolower($requiredBlockKey)])) {
                continue;
            }
            // Missing required blocks must be fixed by the Stage-1 model against the contract.
            // Local skeleton blocks mask plan failures and can later be mistaken for real output.
        }
    ...
    private function buildAiStageOnePagePrompt(
        ...
        $archivedStageOneContract = \json_encode($planJson['stage1_contract'] ?? [], \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES) ?: '{}';
        ...
        return \implode("\n", [
            ...
            'First-pass acceptance gate: this response must pass without a recovery retry. Before returning, verify page.blocks exists, contains at least min blocks, includes every required block key exactly once, and every block has field_plan plus execution_script.core_copy. If not, rewrite the JSON before sending.',
            ...
            'Block budget: min=' . $blockBudget['min'] . ', max=' . $blockBudget['max'] . ', target=' . ($blockBudget['target'] ?? $blockBudget['min']) . ', required=' . \json_encode($blockBudget['required'], \JSON_UNESCAPED_UNICODE) . ', recommended_optional=' . \json_encode($blockBudget['optional'] ?? [], \JSON_UNESCAPED_UNICODE) . '; output the target count when possible, never fewer than min or more than max, and keep legal/support pages compact by splitting dense policy details into concise block fields.',
            ...
            'Archived Stage-1 theme contract (non-negotiable source of truth generated and stored before page fanout): ' . $archivedStageOneContract,
            ...
            '- home_page must follow the page contract: hero and final_cta are required, then use recommended optional block keys such as brand_promise, featured_offers, trust_proof, resource_preview, or experience_highlights until the target block count is reached.',
            ...
            '- Every block MUST include visual_signature with composition_pattern, spatial_rhythm, media_strategy, surface_treatment, and interaction_pattern. Adjacent blocks must not repeat the same composition_pattern + surface_treatment + media_strategy.',
            '- Every block MUST include image_intent. If needs_image=true, include image_role, image_subject, placement, and reuse_policy. If needs_image=false, include css_motif or rationale so Stage 3 renders an intentional motif instead of a placeholder image.',
            '- Do not reuse the same hero/card/media structure for multiple blocks. A later block that repeats the hero headline, CTA, or media composition is invalid even when CSS differs.',
            ...
            '- field_plan.sample and field_plan.implementation_note must not start with validator-rejected prompt words: write, rewrite, describe, use this field, do not output, 围绕, 突出, 说明, 完善, 优化. Rewrite those as actual customer-facing text or concrete rendering notes before returning JSON.',
            ...
            'Schema:',
            '{"page":{"page_goal":"string","theme_alignment_summary":"string","page_design_plan":{"page_role":"string","content_narrative":"string","visual_hierarchy":"string","visual_signature_application":"string","composition_motif":"string","color_layering":"string","section_flow":["string"],"interaction_notes":["string"],"polish_details":["string"],"anti_monotony_rule":"string"},"primary_keywords":["string"],"secondary_keywords":["string"],"blocks":[{"block_key":"string","page_flow_role":"opening|proof|details|cta|support","goal":"string","keywords":["string"],"content":"string","design_tags":{"visual":["string"],"motion":["string"],"interaction":["string"],"texture":["string"],"responsive":["string"],"color_layering":"string","implementation_note":"string"},"visual_signature":{"composition_pattern":"string","spatial_rhythm":"string","media_strategy":"string","surface_treatment":"string","interaction_pattern":"string"},"image_intent":{"needs_image":true,"image_role":"string","image_subject":"string","placement":"string","reuse_policy":"string","css_motif":"string","rationale":"string"},"field_plan":[{"field":"string","sample":"string","implementation_note":"string"}],"execution_script":{"feature_points":["string"],"core_copy":"string","typography":"string","style_tone":"string","background_direction":"string","media_assets":["string"]},"reusable":"yes|no","seo_impact":"high|medium|low"}]}}',
    ...
    private function buildAiStageOnePageRecoveryPrompt(
        ...
        return $this->buildAiStageOnePagePrompt(
            ...
            'Recovery acceptance gate: page.blocks must be a non-empty array; required block keys must appear exactly once: ' . \json_encode($blockBudget['required'], \JSON_UNESCAPED_UNICODE) . '.',
            'Every returned block must include block_key, page_flow_role, content, complete design_tags, exactly 3 field_plan rows with field/sample/implementation_note, and execution_script.core_copy.',
    ...
    private function buildAiPlanPrompt(
        ...
        $cleanPrompt = [
            ...
            'GOOD navigation_plan.header_items: [{"label":"首页","href":"/"},{"label":"关于","href":"/about"},{"label":"博客","href":"/blog"},{"label":"联系","href":"/contact"}]',
            '',
            ...
            '    "pages":{"home_page":{"page_goal":"string","theme_alignment_summary":"string","primary_keywords":["string"],"secondary_keywords":["string"],"blocks":[{"block_key":"string","goal":"string","keywords":["string"],"content":"string","design_tags":{"visual":["string"],"motion":["string"],"interaction":["string"],"texture":["string"],"responsive":["string"],"color_layering":"string","implementation_note":"string"},"visual_signature":{"composition_pattern":"string","spatial_rhythm":"string","media_strategy":"string","surface_treatment":"string","interaction_pattern":"string"},"image_intent":{"needs_image":true,"image_role":"string","image_subject":"string","placement":"string","reuse_policy":"string","css_motif":"string","rationale":"string"},"field_plan":[{"field":"string","sample":"string","implementation_note":"string"}],"execution_script":{"feature_points":["string"],"core_copy":"string","typography":"string","style_tone":"string","background_direction":"string","media_assets":["string"]},"reusable":"yes|no","seo_impact":"high|medium|low"}]}},',
            ...
            '- Every page block MUST include design_tags, visual_signature, and image_intent. Adjacent blocks must not reuse the same composition_pattern + surface_treatment + media_strategy or the same headline/core_copy.',
    ...
    private function mapAiPlanToArtifacts(
        ...
        if (!$validationPassed) {
            $deferToPartialRetry = $partialRetryMode && (
                $validationPageTypes === []
    ...
    private function isPromptLikeStageOneText(
        ...
        if (\preg_match('/^(?:write|rewrite|describe|use this field|do not output)\b/iu', $normalized) === 1) {
            return true;
        }
    ...
    private function isStageOneContractPromptLikeText(string $text): bool
    {
        ...
        return \preg_match('/^(?:write|rewrite|describe|use this field|do not output)\b/iu', $text) === 1;
    }
    ...
    private function repairAiStageOnePlanJsonBeforeValidation(
        ...
        foreach ($pageTypes as $pageType) {
            ...
            if (
                ...
                unset($page['theme_alignment_summary']);
            }
        ...
        foreach ($pageTypes as $pageType) {
            ...
            foreach ($planJson['pages'][$pageType]['blocks'] as $blockIndex => $block) {
                ...
                foreach ($fieldPlan as $fieldIndex => $row) {
                    ...
                    unset($row['implementation_note'], $row['delivery_note']);
                    $fieldPlan[$fieldIndex] = $row;
                }
    ...
    private function repairAiStageOneExecutionScriptBeforeValidation(
        ...
        if ($featurePoints !== []) {
            $executionScript['feature_points'] = \array_slice($repairedFeaturePoints, 0, 3);
        }
    ...
    private function buildStageOneFieldImplementationNoteFromSample(
        ...
        if ($isEn) {
            ...
            if (\str_contains($normalizedField, 'image') || \str_contains($normalizedField, 'media') || \str_contains($normalizedField, 'asset')) {
                return \sprintf('Use "%s" as the concrete asset brief for the "%s" block until uploaded media is available.', $sampleSnippet, $blockLabel);
            }
        ...
        if (\str_contains($normalizedField, 'image') || \str_contains($normalizedField, 'media') || \str_contains($normalizedField, 'asset')) {
            return \sprintf('将“%s”作为“%s”区块的具体素材简报，真实素材上传前按该描述实现占位视觉。', $sampleSnippet, $blockLabel);
        }
        if (\str_contains($normalizedField, 'cta') || \str_contains($normalizedField, 'button')) {
    ...
    private function repairStageOneThemeDesignBeforeValidation(array $planJson, array $scope, array $pageTypes): array
    {
        ...
        if (\trim((string)($themeDesign['art_direction'] ?? '')) === '' && !\is_array($themeDesign['art_direction'] ?? null)) {
    ...
    private function sanitizeStageOnePages(array $pages, array $fallbackPages): array
    {
        foreach ($pages as $pageType => $page) {
            ...
            if ($this->isWeakStageOnePageGoal($pageGoal, (string)$pageType)) {
                ...
                if (!$this->isWeakStageOnePageGoal($fallbackGoal, (string)$pageType)) {
                    $page['page_goal'] = $fallbackGoal;
                } else {
                    unset($page['page_goal']);
                }
            }
        ...
        return [
            ...
            'realtime_content' => $isHeader
                ? [
                    ...
                    'cta' => [['label' => $headerCopy['cta_label'], 'target' => '#contact']],
     ...
     */
    private function normalizeStageOneBlockDesignTags(array $block, array $sharedPromptContext, array $pageDesignPlan = [], bool $fillMissing = true): array
    {
        ...
        if ($fillMissing && $implementationNote === '') {
        ...
        if ($fillMissing && $pageColorLayering !== '' && !\str_contains($implementationNote, $pageColorLayering)) {
