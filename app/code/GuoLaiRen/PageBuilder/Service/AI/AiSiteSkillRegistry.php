...
final class AiSiteSkillRegistry
{
    ...
    public function buildPromptGuideLines(string $stage, array $extraCodes = [], array $skillSnapshots = []): array
    {
        ...
        $lines[] = '- Empty visible control ban: never emit empty, whitespace-only, punctuation-only, placeholder-only, or non-breaking-space-only buttons, CTA anchors, nav links, chips, badges, tags, pills, labels, metric/stat items, testimonial/source labels, or other rounded outline controls. If no concrete target-locale copy is available from the current plan_json block, omit that element or bind a localized fallback field such as cta.text; never draw decorative empty rounded rectangles.';
        $lines[] = '- Icon-only control exception: hamburger/menu/theme/icon buttons may omit visible text only when they include a non-empty target-locale aria-label or title and render a real visible icon child such as svg, img, i, or span bars. A bare empty button still fails.';
