...
final class AiSiteVisualBlockContractRenderer
{
    ...
    public function renderSectionVisualContract(
        ...
        $localeLine = $contentLocale !== ''
            ? "- content_locale: {$contentLocale} — 全部可见文案（h2 / p / 卡片 / CTA / form label / 备注）应使用该语种；不要把 plan_locale 或英文模板词直接放进最终页面。"
            : "- content_locale 未指定：使用 scope 默认语种；非默认语种字符会作为 language_consistency 诊断反馈。";
        ...
        $heroNote = $hasVerifiedHeroImage
            ...
            : "- visual.image：本区块没有验证后的图片素材时，优先用 CSS-only 视觉语言（layered background + pseudo-element + shape motif）替代，不要输出破图、占位图或 ai-site-fallback。";

        $sections = [];

        $sections[] = "AI Visual Block Design Guide — 生成前自检清单（视觉项是设计指导，非发布硬门禁）";
        $sections[] = $localeLine;
        $sections[] = $themeLine;
        $sections[] = $factsLine;
        $sections[] = $roleLine;
        ...
        $sections[] = "\n[design#visual_depth] 视觉层次建议 — 尽量在 css_extra / html_content 中命中多条视觉信号，但不要为了凑数牺牲 block 身份：";
        ...
        $sections[] = "\n[design#responsive_support] 响应式建议 — 建议包含 @media，并照顾常见移动端布局：";
        ...
        $sections[] = "  10) Typography quality: css_extra should define explicit font-family declarations for both `#componentId .pb-c-title` and one body/root selector (`#componentId .pb-c-root`, `#componentId .pb-c-copy`, or `#componentId .pb-c-text`). Prefer a named brand family before generic fallback; pure system-ui/-apple-system/Segoe UI/Roboto/Arial/sans-serif alone is only a weak design signal.";

        $sections[] = "\n[design#visual_assets_safe] 图片资源建议：";
        $sections[] = "  - 所有 <img src> 必须来自 verified_asset_src_allowlist；不要使用 example.com、unsplash、picsum、CDN 占位或空 src。";
        $sections[] = "  - 禁止破图、占位图、ai-site-fallback、Image Placeholder、broken alt（如 'image' / '...'）。alt 必须是 content_locale 的具体描述。";
        $sections[] = "  - css_extra 里 url(...) 仅允许引用 verified_asset_src_allowlist 中的 URL，建议优先用 <img> 而非背景图。";

        $sections[] = "\n[design#theme_visible] 主题可见建议：";
        ...
        $sections[] = "\n[design#stage1_content_visible] 阶段一内容承接建议：";
        $sections[] = "  - 可见文案必须从 must_include_facts / page_goal / block_goal 中提取业务名词、专有名词、数字、地名，并改写成最终访客文案；禁止直接展示蓝图句子或写成「Welcome to our brand」之类的通用句。";

        $sections[] = "\n[hard#content_quality] 内容洁净硬约束：";
        ...
        $sections[] = "\n[design#block_role_fidelity] Current block role guidance:";
        ...
        $sections[] = "\n[design#field_readability] Label/value readability guidance:";
        ...
        $sections[] = "\n[design#same_page_block_diversity] Same-page diversity guidance:";
        ...
        $sections[] = "\n[design#language_consistency] 语言一致建议：";
        $sections[] = "  - 每段可见文案（含 alt、placeholder、aria-label）优先使用 content_locale；品牌名、产品名、URL、APK、API 等必要专有词可保留原拼写。";
        $sections[] = "  - 数字、品牌名、URL 不受语种限制，但句子主干必须是 content_locale。";
        $sections[] = "  - Contact copy quality: never render malformed fragments such as support@ .com, example domains, fake phone numbers, or raw prompt/source labels. Rewrite to final visitor copy or omit the exact value.";

        $sections[] = "\n[design#render_data_quality] 渲染数据建议（结构 / 文案 / SEO）：";
        ...
        $sections[] = "\n[design#shared_blocks_ready] 共享 header/footer 协同建议：";
        $sections[] = "  - 内容区块的 .pb-c-* 类名不要与 header/footer 的 .pb-h-* / .pb-f-* 重名；不要复刻 header logo / 导航。";

        $sections[] = "\n[self-check before return] AI 必须在返回 JSON 前自查：";
        $sections[] = "  - 视觉层次与响应式是否足够支撑截图质量？若不足，请在 css_extra / css_responsive 内补齐；补齐方式须服从 visual_signature 与 block role，禁止用「标题+三列卡片」模板凑数。";
    ...
    public function renderSharedRegionVisualContract(
        ...
        return "AI Shared {$region} Design Guide — shared_blocks_ready 协同建议：\n"
            . "- content_locale: {$contentLocale}，所有可见文案使用该语种；品牌名、URL、必要专有词可保留原拼写。\n"
            . $themeLine . "\n"
            . $brandLine . "\n"
            . "- Footer/header text is optional and must be target-locale visitor copy. Do not copy the approved brief, source objective, or English brand summary verbatim; leave optional text empty if no target-locale sentence can be safely synthesized.\n"
    ...
    private function renderVisualSignatureLine(array $visualSignature): string
    {
        ...
        return '- visual_signature (primary layout direction): ' . \implode(' / ', $parts)
    ...
    private function renderThemePaletteLine(array $themePalette): string
    {
        ...
        return "- theme_palette：建议在 css_extra 中使用以下 hex token —— " . \implode(' / ', \array_slice($hexes, 0, 8));
    }
    ...
    private function renderCurrentBlockRoleLine(array $brief): string
    {
        ...
        if ($parts === []) {
            return "- current_block_role: no explicit task identifiers were supplied; use page_goal, block_goal, and must_include_facts as planning context only. Do not reuse another block's structure or copy, and never render these identifiers as visible text.";
        }

        return "- current_block_role: " . \implode(' / ', \array_slice($parts, 0, 8)) . ". Treat these as generation context, not visitor-visible copy.";
    }
    ...
    private function renderMustIncludeFactsLine(array $brief): string
    {
        ...
        if ($facts === []) {
            return "- must_include_facts：未声明强制事实；可见文案仍需源自 block_goal / page_goal，并改写为最终访客文案，禁止展示蓝图说明。";
        }

        return "- must_include_facts：以下内容是可用的规划语义，请按 content_locale 自然改写为访客文案，不要逐字硬贴或展示蓝图说明：" . \implode(' | ', \array_slice($facts, 0, 6));
    }
