...
final class AiSiteVirtualThemePlanService
{
    ...
    private function buildTaskPlanGenerationBatchPrompt(
        ...
        $lines[] = '- block_task.style_plan.color must include explicit readable contrast pairs: background-to-text, surface-to-text, CTA-to-text, and focus/hover states. Dark surfaces require light foregrounds; light surfaces require dark foregrounds.';
        $lines[] = '- block_task.style_plan.color must treat the theme palette as role tokens, not a paint bucket. Assign page base, elevated surface, muted surface, divider, accent, CTA, text, and muted text roles, then vary section depth across the page.';
    ...
    private function resolveStageTwoPageDesignPlanForPrompt(array $scope, array $structured, string $pageType): array
    {
        ...
        $candidates = [
            ...
            $confirmedPlanBook['pages'][$pageType]['page_design_plan'] ?? null,
    ...
    private function buildTaskPlanStageOneCompactContext(array $scope, string $pageType = ''): array
    {
        ...
        $summary = [
            ...
            'palette' => \is_array($planStructured['palette'] ?? null)
                ? $planStructured['palette']
                : (\is_array($scope['palette'] ?? null) ? $scope['palette'] : []),
            'theme_style' => \is_array($planStructured['theme_style'] ?? null)
                ? $planStructured['theme_style']
                : (\is_array($scope['theme_style'] ?? null) ? $scope['theme_style'] : []),
            'theme_design' => \is_array($planStructured['theme_design'] ?? null)
                ? $planStructured['theme_design']
                : (\is_array($scope['theme_design'] ?? null)
                    ? $scope['theme_design']
                    : (\is_array($themeContextSnapshot['theme_design'] ?? null) ? $themeContextSnapshot['theme_design'] : $themeContextSnapshot)),
    ...
    ): array
    {
        ...
        if ($planStructured === []) {
            $planStructured = [
                'site_strategy' => [
                    'summary' => (string)($scope['brief_description'] ?? $scope['site_tagline'] ?? ''),
                ...
                'theme_design' => \is_array($scope['theme_design'] ?? null)
                    ? $scope['theme_design']
                    : (\is_array($themeContextSnapshot['theme_design'] ?? null) ? $themeContextSnapshot['theme_design'] : $themeContextSnapshot),
                'seo_strategy' => \is_array($executionBlueprint['seo_strategy'] ?? null) ? $executionBlueprint['seo_strategy'] : [],
                'navigation_plan' => \is_array($executionBlueprint['navigation_plan'] ?? null) ? $executionBlueprint['navigation_plan'] : [],
                'footer_plan' => \is_array($executionBlueprint['footer_plan'] ?? null) ? $executionBlueprint['footer_plan'] : [],
            ];
        }
    ...
    private function buildTaskPlanPromptBase(
        ...
        $lines = [
            ...
            '- Every page block style_plan.color must name readable contrast pairs for background/text, surface/text, CTA/text, link/focus states. Dark surfaces require light foregrounds; light surfaces require dark foregrounds.',
            '- Treat the confirmed palette as role tokens, not a paint bucket: assign page base, elevated surface, muted surface, divider, accent, CTA, text, and muted text roles, then vary section depth across the page.',
