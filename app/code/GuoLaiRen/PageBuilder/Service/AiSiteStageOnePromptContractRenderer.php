...
final class AiSiteStageOnePromptContractRenderer
{
    ...
    public function renderThemeContract(array $contract): array
    {
        return [
            ...
            '- Logo generation plan rule: complete theme_design first, then write the website logo generation plan only into plan_json.theme.logo_generation with stage logo_generation, output_path plan_json.theme.logo_generation, and exactly four selectable options using asset_slot_id values plan:theme:logo_generation:option_1 through plan:theme:logo_generation:option_4. Every logo prompt_brief must derive from the completed theme_design style_signature, art_direction, color_scheme, typography_spacing_radius, visual_keywords, tone_of_voice, cta_tone, and forbidden_styles. Do not emit legacy logo generation keys in shared_components, page blocks, or scope-level fields.',
    ...
    public function renderPageContract(array $contract, string $pageType): array
    {
        ...
        return [
            ...
            '- Page image rhythm contract: non-policy pages must not end up with only the opening/banner image. Major body blocks such as benefits, game showcase, trust/proof, reviews, support/contact, FAQ/rules, and final CTA must either plan a generated image or a substantial CSS visual companion that matches its block identity. Choose generated images for the opening block plus several proof/detail/support/body blocks when the page has four or more blocks; dense legal text may choose no image with a document/rulebook CSS surface.',
            '- Large-card media contract: if a block would otherwise render as a wide empty card grid with only dots/icons/badges, plan a real generated scene/product/interface image sized for that block unless the page is policy/legal or the block is a compact FAQ/text row. Do not use CSS-only as the default escape for rich marketing sections.',
            ...
            '- Non-policy page visual asset contract: marketing, product, service, game, article, contact, and support pages must plan real generated image slots for the block roles where imagery carries the story. CSS-only is allowed for compact utility/legal rows, but not as a page-wide substitute for generated media.',
    ...
    public function renderRepairContract(array $contract, array $validationReport): array
    {
        ...
        return [
            ...
            '- If any issue code is page_missing_generated_image_intent, rebuild the page media rhythm instead of treating generated media as optional. Return the complete page object with valid image_intent on every block; for non-policy pages set needs_image=true on the opening block plus several narrative/proof/detail/support/body blocks with concrete stable generated subjects, and reserve needs_image=false for compact utility/legal rows with a complete CSS-only motif.',
