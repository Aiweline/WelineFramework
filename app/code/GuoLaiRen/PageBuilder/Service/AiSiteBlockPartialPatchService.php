...
class AiSiteBlockPartialPatchService
{
    ...
    private function buildPatchPrompt(array $read, array $scope, string $instruction): string
    {
        ...
        return "You are modifying one PageBuilder block in-place.\n"
            ...
            . "FAQ and CTA interaction contract:\n"
            . "- If the current block role, block_id, component_code, instruction, or html contains FAQ/support_faq/faq_or_rules/pb-c-faq-item, the replacement must keep complete FAQ rows. Each `.pb-c-faq-item` must contain a visible localized question title inside `.pb-c-question` (or h3.pb-c-question), and a separate `.pb-c-answer` paragraph with answer copy. A row that only shows a number, icon, or chevron before the answer is invalid.\n"
            . "- FAQ rows that display a chevron, plus/minus, accordion affordance, or collapsed answer state must also include scoped js_content or html-safe inline state attributes that make the row expandable from inside this component. Use button-like question controls with type='button', aria-expanded, and local class toggles when rewriting HTML. Do not use empty `.pb-c-question` wrappers.\n"
            . "- Every `.pb-c-cta` anchor or button must contain a visible localized label rendered in the element body. `<button ...></button>` and CTA controls with only data attributes are invalid. If no real href is allowed, use `<button type='button' class='pb-c-cta' data-pb-ai-action='primary_cta'>Finished localized CTA</button>` and preserve/provide the scoped action bridge.\n"
