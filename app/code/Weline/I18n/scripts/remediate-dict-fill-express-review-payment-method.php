...
/**
 * Upsert Express-review payment method + pending-charge copy + publishLocale.
 * Dual DB: MySQL (production Hanfu) + PostgreSQL (local clones).
 *
 * Usage:
 *   php -d memory_limit=512M app/code/Weline/I18n/scripts/remediate-dict-fill-express-review-payment-method.php --dry-run
 *   php -d memory_limit=512M app/code/Weline/I18n/scripts/remediate-dict-fill-express-review-payment-method.php --apply
 */
...
$pack = require __DIR__ . '/data/dict-fill-express-review-payment-method.v1.php';
...
$type = strtolower((string)($db['type'] ?? $db['adapter'] ?? 'pgsql'));
...
fwrite(STDOUT, "mode={$mode} db=" . ($isMysql ? 'mysql' : 'pgsql') . " words={$words} writes={$writes} missing=" . count($missing) . " locales=" . count($locales) . PHP_EOL);
if ($missing !== []) {
