...
- 动态语言切换
- 语言检测
- **标准切换标签**：`<w:i18n:switcher />`（实现类 `LanguageSwitcher`）；UI 2.0 不提供旧标签别名。
...
- `<w:i18n:switcher />` 仅输出语义化 HTML，由 Weline UI 的 `language-switcher` 懒加载组件接管，不输出内联脚本或独立全局对象。服务端 option `href` 是唯一权威导航结果；组件先写偏好 Cookie，再让浏览器原生跟随 href；同 URL 时强制刷新。
