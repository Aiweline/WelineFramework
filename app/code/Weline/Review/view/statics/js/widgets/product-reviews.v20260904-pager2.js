(function () {
  ...
  function bootRoot(root, messages) {
    ...
    var pagerRoot = root.querySelector('[data-review-pager]');
    ...
    function ensurePagerRoot() {
      if (pagerRoot && pagerRoot.isConnected) {
        return pagerRoot;
      }
      pagerRoot = root.querySelector('[data-review-pager]');
      if (pagerRoot) {
        return pagerRoot;
      }
      pagerRoot = make('nav', 'weline-review__pager');
      pagerRoot.setAttribute('data-review-pager', '');
      pagerRoot.setAttribute('aria-label', messages.pagerLabel || messages.listRegion || 'pagination');
      if (itemsRoot.parentNode) {
        itemsRoot.parentNode.insertBefore(pagerRoot, itemsRoot.nextSibling);
      }
      return pagerRoot;
    }

    function formatPageStatus(current, last) {
      var prefix = String(messages.pageLabelPrefix || '第');
      var middle = String(messages.pageLabelMiddle || '/');
      var suffix = String(messages.pageLabelSuffix || '页');
      return prefix + ' ' + String(current) + ' ' + middle + ' ' + String(last) + ' ' + suffix;
    }

    function renderPager() {
      var nav = ensurePagerRoot();
      nav.replaceChildren();
      var lastPage = Math.max(1, listState.lastPage || 1);
      var page = Math.max(1, Math.min(listState.page || 1, lastPage));
      listState.page = page;
      if (listState.total <= 0 || lastPage <= 1) {
        nav.hidden = true;
        return;
      }
      nav.hidden = false;

      var prev = make('button', 'weline-review__pager-btn', messages.prevPage || '上一页');
      prev.type = 'button';
      prev.setAttribute('data-review-page-prev', '');
      prev.disabled = page <= 1 || listState.loading;

      var status = make('span', 'weline-review__pager-status', formatPageStatus(page, lastPage));
      status.setAttribute('data-review-page-status', '');

      var next = make('button', 'weline-review__pager-btn', messages.nextPage || '下一页');
      next.type = 'button';
      next.setAttribute('data-review-page-next', '');
      next.disabled = page >= lastPage || listState.loading;
    ...
    function buildReviewArticle(review) {
    ...
    function updateListMeta(data, hasItems) {
      var totalNum = Math.max(0, Number(data.total || 0));
      listState.total = totalNum;
      listState.lastPage = Math.max(1, Math.ceil(totalNum / pageSize) || 1);
      if (hasItems) {
        setAverageText(Number(data.average_rating || 0).toFixed(1) + ' / 5');
      } else {
        setAverageText('—');
      }
      count.textContent = String(totalNum) + ' ' + String(messages.countSuffix);
    }

    async function fetchListPage(page) {
      var client = await api();
      var listData = unwrap(await client.list({
        type_code: typeCode,
    ...
    async function goToPage(page) {
      var target = Math.max(1, Number(page) || 1);
      if (listState.loading || !entityUuid) {
        return;
      }
      if (target === listState.page && itemsRoot.querySelector('.weline-review__item')) {
        return;
      }
      listState.loading = true;
      renderPager();
      try {
        var listData = await fetchListPage(target);
        renderReviews(listData, { page: target });
        if (itemsRoot.scrollIntoView) {
          itemsRoot.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        }
      } catch (error) {
    ...
    function renderPreviewShell() {
      ...
      itemsRoot.classList.remove('is-scrollable');
      itemsRoot.removeAttribute('tabindex');
      if (pagerRoot) {
        pagerRoot.hidden = true;
    ...
    function renderReviews(data, options) {
      var opts = options || {};
      var page = Math.max(1, Number(opts.page || listState.page || 1) || 1);
      ...
      updateListMeta(data, reviews.length > 0);
      listState.page = Math.min(page, listState.lastPage);
      ...
      reviews.forEach(function (review) {
        itemsRoot.appendChild(buildReviewArticle(review));
      });
      renderPager();
    }
    ...
    async function boot() {
      ...
      try {
        ...
        renderReviews(listData, { append: false, page: 1 });
      } catch (error) {
        if (!formRendered) {
