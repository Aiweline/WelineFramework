...
4. 本文件：`app/code/Weline/SystemConfig/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/SystemConfig/doc/README.md`
...
- 模块代码：`Weline_SystemConfig`
- 目录：`app/code/Weline/SystemConfig`
- Vendor：`Weline`
- Module：`SystemConfig`
...
入口/配置文件：
- `app/code/Weline/SystemConfig/etc/backend/menu.xml`
- `app/code/Weline/SystemConfig/composer.json`
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/SystemConfigQueryProvider.php；前端/跨模块读数据先查 query 帮助。
...
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_SystemConfig> [operation]` 或对应 `w_query` 帮助。
