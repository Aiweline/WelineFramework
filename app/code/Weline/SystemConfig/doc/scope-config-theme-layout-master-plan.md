...
- 产品/分类布局选择必须进入可审计、可回滚的选择版本或 SystemConfig 批次版本，不能只有当前值。
- 定时计划必须保留计划记录版本/审计，但不能用计划激活去覆盖产品/分类当前选择。
...
| 层级 | 负责模块 | 保存内容 | 生效方式 | 失败恢复 |
| --- | --- | --- | --- | --- |
| 配置批次版本 | `Weline_SystemConfig` | key/value 配置、继承删除、scope 覆盖 | 读取 fallback 链时生效 | 事务回滚整批，失败不生成 applied 版本 |
| 虚拟布局内容版本 | `Weline_Theme` | 源码、可视化 schema、AI prompt、预览快照 | 发布时切换 `published_version_id` | 草稿失败不影响发布；发布失败恢复旧发布指针和版本状态 |
| 布局选择版本 | `Weline_Theme`/业务入口 | product/category/category_product_default 选择的 layout option | 解析器按优先级读取选择 | 非法选择保存前拒绝；误保存通过选择版本或配置批次回滚 |
| 定时计划记录 | `Weline_Theme`/业务入口 | 时间窗口、priority、layout option、target、scope | 解析器按当前时间动态命中 | 计划过期自动失效；不写回旧选择，恢复靠重新解析 |

版本状态机总则：

- 保存、发布、选择变更、计划变更都必须先校验 identity、scope、ACL、base version 和 option 白名单，再改变有效状态。
- “保存失败”与“保存成功但内容错误”必须分开处理：保存失败保持旧有效状态；保存成功后通过版本回滚恢复。
- “立即回滚”只用于已成功保存或已成功发布的版本；真正的保存失败必须依赖事务、补偿和旧状态继续生效，不能要求运营再手动回滚一次。
- 后台必须显示最近版本的可回滚状态、阻断原因和回滚预检结果，不能只显示一条历史记录。
- 回滚只允许修改当前上下文对应的 module/area/scope/identity，不允许跨产品、跨分类、跨站点、跨店铺或跨 layout type 恢复。
- 回滚完成后必须清理 SystemConfig fallback 缓存、Theme runtime cache、产品/分类 payload cache 和预览缓存。

### 保存事故处理矩阵

| 场景 | 旧有效状态 | 版本记录 | 后台动作 |
| --- | --- | --- | --- |
| 配置字段校验失败 | 保持不变 | 不生成 `applied` 批次，可记录 failed 审计 | 展示字段错误，允许修正后重试 |
| 配置批次写入中途失败 | 事务回滚后保持不变 | 不生成可回滚的成功批次 | 展示失败原因和恢复结果 |
| 虚拟布局源码/可视化保存失败 | 当前 `published_version_id` 继续生效 | 不生成可发布 draft，或只记录 failed 审计 | 留在编辑页，允许重试，不影响前台 |
| AI 生成失败 | 当前 `published_version_id` 继续生效 | 不生成 draft | 返回模型/校验错误，不能自动发布空布局 |
| draft 保存成功但 publish 失败 | 旧 `published_version_id` 继续生效 | draft 保留，publish failed 记录原因 | 展示“草稿已保存、发布未生效”，允许继续编辑或重新发布 |
| 发布成功但内容不符合预期 | 新 `published_version_id` 已生效 | 生成 published 记录 | 版本列表立即回滚到上一发布版本或指定历史版本 |
| 布局选择保存成功但选错 option | 新选择已生效 | 生成选择版本或 SystemConfig 批次版本 | 选择版本立即回滚，只恢复 option 指向 |
| 定时计划配置错误 | 计划按当前规则参与解析 | 生成计划版本/审计 | 禁用计划或回滚计划配置；不修改产品/分类原始选择 |
...
| 保存失败后还需要手动回滚 | 运营误以为旧配置已被污染，恢复路径不清晰 | 保存失败必须保持旧有效状态；只有保存成功或发布成功的版本才提供立即回滚 |
| 回滚入口没有预检和可回滚状态 | 可能覆盖后续管理员修改，或者回滚到已失效字段/身份 | 保存批次和布局版本暴露 `is_rollbackable`、阻断原因、冲突 key 和预检结果，预检失败不修改有效状态 |
...
| 源码保存和发布放在同一请求时发布失败 | 草稿已保存但当前发布状态可能被半更新，运营误以为已生效 | 保存草稿和发布结果分开返回；发布失败恢复旧 `published_version_id` 和旧版本状态，草稿保留为可继续编辑版本 |
| 发布过程先归档旧版本再更新新指针 | 中途失败可能导致没有有效发布版本 | 发布前快照 asset 指针和版本状态，失败时补偿恢复并清缓存 |
| 回滚目标不属于当前 identity | 可能把其他产品、分类或 scope 的布局发布到当前对象 | 回滚前校验 asset、layout type、target type、target id、scope 和版本归属 |
| 布局选择保存非法 option | 可能把 `default`、锁定项或不存在 option 写成对象专属布局 | 保存前通过 Theme option 白名单和身份规则校验，失败时不改变当前有效选择 |
| 布局选择历史和布局源码历史混在一起 | 回滚时不知道恢复的是“选哪个布局”还是“布局内容” | 单独定义布局选择版本；选择回滚只恢复 layout option 指向，源码回滚只恢复 Theme published version |
...
- 明确误保存恢复策略：回滚只作用于同一 module、area、scope、locale 的版本批次，冲突 key 默认阻止覆盖后续修改。
- 增加配置版本回滚预检：返回 `is_rollbackable`、冲突 key、阻断原因、预计恢复结果和缓存失效范围。
...
- 支持发布补偿恢复：发布前快照 asset 指针和版本状态；发布失败恢复旧发布状态并返回可解释错误。
- 支持布局版本回滚预检：校验 asset、layout type、target type、target id、scope、版本可发布状态和缓存刷新结果。
...
- 产品/分类 layout option 选择变更生成选择版本或进入 SystemConfig 批次版本；选择回滚只恢复 option 指向，不修改布局源码版本。
...
## Extends Registry 刷新约束

SystemConfig 配置模板不是 `Weline_SystemConfig` 自己独占的文件，而是由各业务模块通过 `extends/module/Weline_SystemConfig/Config/{area}/{code}.phtml` 贡献给 `Weline_SystemConfig`。因此涉及配置中心模板、Theme 配置入口、支付配置入口或 WeShop 配置入口时，不能只刷新目标模块自身的路由/注册信息。

- 路由可以用 `RouteUpdateService::updateRoutes(['Weline_SystemConfig'])` 生成 `weline_systemconfig/backend/config` 入口。
- 配置模板索引必须用全量 `php bin/w extends:rebuild`，或等价的“收集所有 extenders 并重建目标模块 extended_by”的流程。
- 如果只刷新 `Weline_SystemConfig` 自身，`generated/extends.php` 可能只保留 SystemConfig 自己的 QueryProvider，导致 `Weline_Payment`、`WeShop_Payment`、Theme 等模块贡献的配置模板从后台配置中心消失。
- 发布门禁要验证 `w_query('system_config', 'getTemplates', ['area' => 'backend', 'force_reload' => true])` 仍能返回业务模块贡献的模板。
