...
- 立即回滚只针对成功批次；保存失败必须保证当前有效配置不变，不能要求管理员用回滚修复半保存状态。
...
版本记录不能只是审计流水，还要能直接支撑后台“立即回滚”和“回滚预检”。

每个保存批次需要能回答：

| 字段/状态 | 说明 |
| --- | --- |
| `is_rollbackable` | 当前批次是否还能自动回滚 |
| `rollback_blockers_json` | 不能回滚的原因，例如后续版本冲突、字段定义已删除、scope 不匹配 |
| `base_versions_json` | 保存时提交的行版本，用于识别并发冲突 |
| `affected_cache_keys_json` | 保存和回滚后需要失效的缓存范围 |
| `source_summary_json` | 保存前有效来源摘要，便于回滚继承状态 |

后台点击回滚前必须先执行预检：

- 批次状态必须是 `applied`，且 module、area、scope、locale 与当前上下文一致。
- 批次涉及的 key 仍在当前模板白名单中，或按历史白名单进入兼容回滚模式。
- 当前 row version 没有被后续保存改变；如果改变，返回冲突列表。
- 敏感字段只按密文、secret 引用或删除状态恢复，不展示明文历史。
- 预检通过后再执行回滚；预检失败不能修改当前配置。

失败分类需要明确返回给后台：

| 失败类型 | 当前有效配置 | 版本记录 | 后台表现 |
| --- | --- | --- | --- |
| 字段校验失败 | 不变 | 不生成 `applied` 批次，可记录 failed 审计 | 显示字段错误 |
| ACL/env-lock 失败 | 不变 | 不生成 `applied` 批次 | 显示权限或锁定原因 |
| base version 冲突 | 不变 | 不生成 `applied` 批次 | 提示刷新并展示冲突 key |
| 数据库写入失败 | 事务回滚后不变 | 可记录 failed 批次和恢复状态 | 显示保存失败和恢复结果 |
| 缓存失效失败 | 配置已保存但需要标记风险 | 生成 `applied_with_cache_warning` 或等价状态 | 提示重新清缓存/重试失效 |

后台版本 UI 必须显示三类状态：

- `applied`：保存已生效，允许执行回滚预检。
- `rollbackable`：预检通过，可以立即回滚。
- `blocked`：不能自动回滚，必须展示冲突 key、后续版本、字段定义缺失、scope 不匹配或 ACL 阻断原因。

保存成功后的“立即回滚”必须恢复到保存前来源状态：如果保存前来自继承值，回滚应删除当前 scope 覆盖行并恢复继承；如果保存前是当前 scope 覆盖值，回滚应恢复旧值和旧 row version。
...
注意：配置模板是“其他模块扩展 `Weline_SystemConfig`”的贡献项。只对 `Weline_SystemConfig` 做目标模块级路由/注册刷新，可能会清掉其他模块贡献给 `Weline_SystemConfig` 的模板索引。发布或开发刷新配置中心模板时，必须执行全量 `php bin/w extends:rebuild`，或使用等价的“按所有 extenders 重建目标模块扩展索引”的流程，确保 `generated/extends.php` 仍包含 `Weline_Payment`、`WeShop_Payment` 等贡献模块的配置模板。
...
SystemConfig 只负责 key/value 配置的版本批次，不保存 Theme 虚拟布局源码、可视化 schema 或 AI 草稿正文。

- 如果产品、分类或分类下产品默认布局的“选择哪个 layout option”以配置字段表达，则保存必须进入 SystemConfig 批次版本。
- 如果布局选择保留在 Theme/业务表，也必须提供等价的选择版本或审计记录，并通过 SystemConfig adapter 暴露管理入口。
- Theme 虚拟布局内容版本由 Theme 自己保存；SystemConfig 只负责普通配置、scope 切换、配置树入口和 adapter 展示。
- SystemConfig 保存失败必须回滚整批配置；Theme publish 失败必须由 Theme 恢复旧 `published_version_id`。
- 两边的回滚都必须清理相关配置/布局解析缓存，否则后台看到已恢复，前台仍可能命中旧 payload。
...
- 保存失败不会留下半批次配置。
- 保存失败后当前有效配置保持原值，不需要也不能依赖管理员手动回滚。
- 最近保存批次可以立即回滚。
- 回滚能恢复保存前的继承状态或覆盖值。
- 回滚前能预检并展示是否可回滚、冲突 key、阻断原因和预计恢复结果。
- 回滚遇到后续修改能识别冲突并阻止误覆盖。
