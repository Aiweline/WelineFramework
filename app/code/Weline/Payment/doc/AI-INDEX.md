...
- `Api`：公开接口契约。跨模块调用优先找已发布 Interface 或 QueryProvider，不要直接依赖对方内部 Service/Model。 文件数：38
...
- `Service`：模块内业务编排层。跨模块读取数据优先发布/使用 w_query。 文件数：27
...
- P4D-002：`AssetPaymentService` / `PaymentAllocation` 负责
  reserve-before-cash、zero-cash、terminal effect 与累计资产退款；
  CustomerAsset 通过 Composer suggest 与 `Api\CustomerAssetConflictInterface`
  保持可选/公共边界；详见 `doc/facade-v2.md`。
...
- 识别到 QueryProvider 相关 PHP 文件：Test/Unit/Query/PaymentQueryProviderAuthorizationTest.php、extends/module/Weline_Framework/Query/PaymentQueryProvider.php；前端/跨模块读数据先查 query 帮助。
