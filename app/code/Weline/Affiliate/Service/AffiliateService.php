...
use Weline\Affiliate\Model\AffiliateAttribution;
...
use Weline\Affiliate\Model\AffiliateWithdrawal;
...
class AffiliateService
{
    ...
    private const VISITOR_COOKIE = 'weline_affiliate_visitor';
    private const ATTRIBUTION_COOKIE = 'weline_affiliate_share';
    private const ATTRIBUTION_SESSION_KEY = 'weline_affiliate_attribution';
    ...
    public function __construct(
        ...
        private readonly ?AffiliateScopeResolver $scopeResolver = null,
        private readonly ?AffiliateScopeFormDataService $scopeFormDataService = null,
        private readonly ?AffiliateAnalyticsService $analyticsService = null,
    ) {
    }
    ...
    public function createAffiliate(
        ...
        $existing = $this->getAffiliateAccountByScope(
            $customerId,
            (int) $scope['website_id'],
            (string) $scope['store_code'],
            (string) $scope['channel_code']
        );
        ...
        $affiliate->clearData()->save([
            ...
            Affiliate::schema_fields_WEBSITE_ID => (int) $scope['website_id'],
            Affiliate::schema_fields_STORE_CODE => (string) $scope['store_code'],
            Affiliate::schema_fields_CHANNEL_CODE => (string) $scope['channel_code'],
    ...
    public function calculateCommission(string $referralCode, float $orderTotal): float
    {
        ...
        $affiliateRow = $affiliate->getData();
        if (!is_array($affiliateRow) || !$this->isAffiliateEffectiveInScope($affiliateRow)) {
            return 0.0;
        }
    ...
    public function recordShareClick(string $shareCode, int $customerId = 0): array
    {
        ...
        $affiliateRow = $this->getAffiliateRowById($affiliateId);
        $scopeMismatch = !is_array($affiliateRow) || !$this->isAffiliateEffectiveInScope($affiliateRow);
        ...
        return [
            'share_code' => $shareCode,
            'target_url' => $targetUrl,
            'attributed' => !$isSelfClick && !$scopeMismatch,
    ...
    public function handleCheckoutOrderCreated(array $payload): void
    {
        ...
        if (!$this->isAffiliateEffectiveInScope($affiliate)) {
            return;
        }
    ...
    public function handleCustomerRegistered(array $payload): void
    {
        ...
        if (!is_array($attribution) && $referralCode !== '') {
            ...
            if (is_array($affiliate) && !$this->isAffiliateEffectiveInScope($affiliate)) {
                $affiliate = null;
            }
            if (is_array($affiliate)) {
    ...
    public function getAffiliateSummary(int $customerId): array
    {
        ...
        $requestScope = $this->resolveRequestScope();
        $scopeLabel = $this->scopeForm()->buildScopeLabel(
            (int) ($account[Affiliate::schema_fields_WEBSITE_ID] ?? 0),
            (string) ($account[Affiliate::schema_fields_STORE_CODE] ?? ''),
            (string) ($account[Affiliate::schema_fields_CHANNEL_CODE] ?? '')
        );
        $analytics = $this->analytics()->buildAffiliateAnalytics($affiliateId, [
            'website_id' => (int) ($account[Affiliate::schema_fields_WEBSITE_ID] ?? 0),
            'store_code' => (string) ($account[Affiliate::schema_fields_STORE_CODE] ?? ''),
            'channel_code' => (string) ($account[Affiliate::schema_fields_CHANNEL_CODE] ?? ''),
        ]);
        ...
        return [
            ...
            'current_scope' => [
                'website_id' => (int) ($requestScope['website_id'] ?? 0),
                'store_code' => (string) ($requestScope['store_code'] ?? ''),
                'channel_code' => (string) ($requestScope['channel_code'] ?? ''),
                'scope_label' => $this->scopeForm()->buildScopeLabel(
                    (int) ($requestScope['website_id'] ?? 0),
                    (string) ($requestScope['store_code'] ?? ''),
                    (string) ($requestScope['channel_code'] ?? '')
                ),
            ],
            ...
            'platform_funnel' => $analytics['platform_funnel'] ?? [],
        ] + $metrics + $workspace;
    }
    ...
    public function getAffiliateAccount(int $customerId): ?array
    {
        ...
        $this->applyNotDeletedScope($affiliate);
        $rows = $affiliate->limit(1)->select()->fetchArray();

        foreach ($rows as $row) {
            if (is_array($row) && !$this->isSoftDeletedRow($row)) {
                return $row;
            }
        }
    ...
    {
        $scope = $this->resolveRequestScope();
        $existing = $this->resolveAffiliateAccountForScope($customerId, $scope);
        ...
        return [
            ...
            Affiliate::schema_fields_WEBSITE_ID => (int) ($affiliate->getData(Affiliate::schema_fields_WEBSITE_ID) ?? 0),
            Affiliate::schema_fields_STORE_CODE => (string) ($affiliate->getData(Affiliate::schema_fields_STORE_CODE) ?? ''),
            Affiliate::schema_fields_CHANNEL_CODE => (string) ($affiliate->getData(Affiliate::schema_fields_CHANNEL_CODE) ?? ''),
     ...
     * @return array<string, mixed>|null
     */
    public function resolveAffiliateAccountForScope(int $customerId, array $scope): ?array
    {
        if ($customerId <= 0) {
            return null;
        }
        ...
        $affiliate->clear()
            ->where(Affiliate::schema_fields_CUSTOMER_ID, $customerId);
        $this->applyNotDeletedScope($affiliate);
        $rows = $affiliate->select()->fetchArray();
        ...
        foreach (is_array($rows) ? $rows : [] as $row) {
            ...
            if (!AffiliateScopeMatcher::matches($row, $scope)) {
                continue;
            }
            $score = AffiliateScopeMatcher::specificityScore($row);
            if ($score > $bestScore) {
                $bestScore = $score;
                $best = $row;
            }
    ...
    /** @return array{website_id:int,store_code:string,channel_code:string} */
    public function resolveRequestScope(): array
    {
        $resolved = $this->scopeResolver()->resolve();

        return [
            'website_id' => (int) ($resolved['website_id'] ?? 0),
    ...
    public function isAffiliateEffectiveInScope(array $affiliate, ?array $scope = null): bool
    {
        $scope ??= $this->resolveRequestScope();

        return AffiliateScopeMatcher::matches($affiliate, $scope);
    }
    ...
    public function getAffiliateRecord(int $affiliateId): ?Affiliate
    {
        ...
        if (!$affiliate->getId() || $this->isSoftDeleted($affiliate)) {
            return null;
        }
    ...
    public function softDeleteAffiliate(int $affiliateId): void
    {
        $affiliate = $this->getAffiliateRecord($affiliateId);
        if (!$affiliate instanceof Affiliate) {
            throw new \InvalidArgumentException((string) \__('Affiliate not found.'));
        }

        $now = date('Y-m-d H:i:s');
        $affiliate->setData([
            Affiliate::schema_fields_STATUS => self::STATUS_DISABLED,
            Affiliate::schema_fields_DELETED_AT => $now,
            Affiliate::schema_fields_UPDATED_AT => $now,
        ]);
        $affiliate->save();
    }
    ...
    public function getAffiliateList(int $page = 1, int $pageSize = 20, array $filters = []): array
    {
        ...
        if (array_key_exists('website_id', $filters) && (int) $filters['website_id'] > 0) {
            $affiliate->where(Affiliate::schema_fields_WEBSITE_ID, (int) $filters['website_id']);
        }

        if (!empty($filters['store_code'])) {
            $affiliate->where(Affiliate::schema_fields_STORE_CODE, (string) $filters['store_code']);
        }

        if (!empty($filters['channel_code'])) {
            $affiliate->where(Affiliate::schema_fields_CHANNEL_CODE, (string) $filters['channel_code']);
        }
    ...
    public function saveAffiliate(array $data): Affiliate
    {
        ...
        if ($affiliateId) {
            ...
            if ((int) $scope['website_id'] <= 0) {
                throw new \InvalidArgumentException((string) \__('Website scope is required before creating an affiliate account.'));
            }
            $existing = $this->getAffiliateAccountByScope(
                $customerId,
                (int) $scope['website_id'],
        ...
        if (!$affiliateId) {
            $payload[Affiliate::schema_fields_WEBSITE_ID] = (int) $scope['website_id'];
            $payload[Affiliate::schema_fields_STORE_CODE] = (string) $scope['store_code'];
            $payload[Affiliate::schema_fields_CHANNEL_CODE] = (string) $scope['channel_code'];
        }

        $affiliate->setData($payload);
    ...
    public function getAffiliateAccountByScope(
        int $customerId,
        int $websiteId = 0,
        string $storeCode = '',
        string $channelCode = ''
        ...
        $affiliate->clear();
        $this->applyNotDeletedScope($affiliate);
        $affiliate->where(Affiliate::schema_fields_CUSTOMER_ID, $customerId)
            ->where(Affiliate::schema_fields_WEBSITE_ID, (int) $scope['website_id'])
            ->where(Affiliate::schema_fields_STORE_CODE, (string) $scope['store_code'])
            ->where(Affiliate::schema_fields_CHANNEL_CODE, (string) $scope['channel_code']);

        $row = $affiliate->find()->fetchArray();
    ...
    {
        $this->eventsManager()->dispatch('Weline_Affiliate::' . $eventName, $payload);
    }
    ...
    private function customerSession(): AuthenticatedSessionInterface
    {
        $factory = $this->sessionFactory ?? ObjectManager::getInstance(SessionFactory::class);

        return $factory->createFrontendSession();
    }

    private function scopeResolver(): AffiliateScopeResolver
    {
        return $this->scopeResolver ?? ObjectManager::getInstance(AffiliateScopeResolver::class);
    }

    private function scopeForm(): AffiliateScopeFormDataService
    {
        return $this->scopeFormDataService ?? ObjectManager::getInstance(AffiliateScopeFormDataService::class);
    }

    private function analytics(): AffiliateAnalyticsService
    {
        return $this->analyticsService ?? ObjectManager::getInstance(AffiliateAnalyticsService::class);
    }

    public function buildAdminAnalytics(int $affiliateId): array
    {
        $affiliate = $this->getAffiliateRowById($affiliateId);

        return $this->analytics()->buildAffiliateAnalytics($affiliateId, is_array($affiliate) ? $affiliate : []);
    }

    private function applyNotDeletedScope(Affiliate $affiliate): void
    {
        $affiliate->where(Affiliate::schema_fields_DELETED_AT, null, 'is null');
    }

    private function isSoftDeleted(Affiliate $affiliate): bool
    {
        return trim((string) ($affiliate->getData(Affiliate::schema_fields_DELETED_AT) ?? '')) !== '';
    }

    /**
     * @param array<string, mixed> $row
     */
    private function isSoftDeletedRow(array $row): bool
    {
        return trim((string) ($row[Affiliate::schema_fields_DELETED_AT] ?? $row['deleted_at'] ?? '')) !== '';
    }
}
