...
/**
 * Async PHPUnit / integration test runner for framework test management.
 */
final class PhpUnitTestRunTask implements TaskConsumerInterface
{
    public function name(): string
    {
        return (string)__('框架单元测试运行');
    }
    ...
    {
        return (string)__('按模块异步执行 PHPUnit，并回写测试运行进度。');
    }
    ...
    public function execute(TaskContextInterface $task): string
    {
        ...
        return 'QUEUE_DONE: framework_test_unit_exit_' . $exitCode;
    }
    ...
    private function decode(TaskContextInterface $task): array
    {
        ...
        if (!is_array($decoded)) {
            throw new \InvalidArgumentException('framework_test_unit_content_invalid');
        }
