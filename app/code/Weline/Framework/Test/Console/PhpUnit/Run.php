...
namespace Weline\Framework\Test\Console\PhpUnit;
...
use Weline\Framework\Test\Pest\Pest as PestTest;
...
';
    ...
    private function generateTestReport(array $output, int $returnCode, int $expectedTotalTests = 0, array $testFiles = []): void
    {
        ...
        # 仅在 PHPUnit 没给出统计时使用源码方法数兜底。
        # data provider 会让实际 case 数大于方法数，禁止覆盖真实统计并产生 >100% 通过率。
        if ($totalTests === 0 && $expectedTotalTests > 0) {
