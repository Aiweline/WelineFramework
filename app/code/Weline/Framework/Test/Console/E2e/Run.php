...
class Run extends CommandAbstract
{
    ...
    private function buildPlaywrightArgs(array $args, array $control): array
    {
        ...
        // Cli::parseArgs may normalize the string "1" to boolean true while
        // retaining the original numeric argv entry. Preserve the explicit
        // inline value so Playwright never receives an invalid bare option
        // such as `--workers` for the documented `--workers=1` form.
        foreach ($args as $key => $value) {
            if (!is_int($key) || !is_string($value)
                || preg_match('/^--([A-Za-z0-9_-]+)=(.*)$/D', $value, $matches) !== 1
            ) {
                continue;
            }
            $rawKey = $matches[1];
            $rawKey = $aliasMap[$rawKey] ?? $rawKey;
            $rawInlineValues[$rawKey] = $matches[2];
        }
        ...
        foreach ($args as $key => $value) {
            ...
            if (($value === true || $value === 'true') && array_key_exists($normalizedKey, $rawInlineValues)) {
                $value = $rawInlineValues[$normalizedKey];
            }
            if (($value === true || $value === 'true') && $normalizedKey === 'workers') {
                // The framework command dispatcher coerces the scalar value
                // "1" to true before execute(); for this value-bearing option
                // the only valid boolean-normalized meaning is one worker.
                $value = '1';
            }
    ...
    private function resolveDisplayMode(array $playwrightArgs, array $control): array
    {
        ...
        // --headless 优先；否则无显式 headed/ui 时默认无头（避免 Agent/本机弹窗）。
