...
所有测试都继承自 `Weline\Framework\Test\TestCore`，该类提供了：
...
- `app/code/Weline/Framework/Test/Pest/Pest.php` - Pest 集成类
- `app/code/Weline/Framework/Test/Pest/Boot.php` - Pest 启动类
- `app/code/Weline/Framework/Test/Console/Pest/Run.php` - Pest 测试运行命令
- `app/code/Weline/Framework/Test/Console/PhpUnit/Run.php` - PHPUnit 测试运行命令（默认支持 Pest）
...
见 `AUTOMATED_TESTING.md` 中「后台测试管理」一节。命名空间已统一为 `Weline\Framework\Test`（含 `TestCore`、CLI、`TestCollectionService`）。
