...
1. **URL 字符串回写**：事件数据为 URL **字符串**（引用传递）；观察者通过 `$event->setData('data', $newUrl)` 回写
2. **执行顺序**：观察者按照 `sort` 属性排序执行，数值越小越先执行
3. **性能考虑**：该事件在每次生成URL且 `seo=on` 时触发；应避免耗时操作
4. **派发门禁**：仅 `Env::seo` 为真时派发。业务身份 query（如画布 `theme_id`）**禁止**挂本事件——改挂始终派发的 `Weline_Framework_Url::url_generate_params`
5. **URL格式**：确保修改后的URL格式正确

## 更新日志

- **2026-09-20**：恢复仅 seo=on 派发；画布身份参迁至 `url_generate_params`
