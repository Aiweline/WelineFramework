...
本文档详细说明了 Weline Framework 模块提供的 `Weline_Framework_Router::route_before` 事件及其使用方法。该事件在路由处理之前触发，允许其他模块在路由前执行操作。

## 事件列表

### 1. Weline_Framework_Router::route_before - 路由处理前事件

#### 基本信息

- **事件名称**：`Weline_Framework_Router::route_before`
- **事件类型**：路由处理事件
- **触发时机**：在路由处理之前
...
`Weline_Framework_Router::route_before` 事件在路由处理之前触发，此时：
- 路由对象已创建
- 控制器尚未执行
- 可以修改路由信息
- 可以执行路由前检查
...
<config xmlns:xs="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <event name="Weline_Framework_Router::route_before">
...
`Weline_Framework_Router::route_before` 事件传递的数据：
...
- **2024-12-19**：初始版本，添加 `Weline_Framework_Router::route_before` 事件文档
