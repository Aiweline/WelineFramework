# Weline Framework 模块 - 事件文档

## 概述

本文档详细说明了 Weline Framework 模块提供的 `Weline_Framework_Router::before_start` 事件及其使用方法。该事件在路由开始处理之前触发，允许其他模块在路由开始前执行操作。

## 事件列表

### 1. Weline_Framework_Router::before_start - 路由开始前事件

#### 基本信息

- **事件名称**：`Weline_Framework_Router::before_start`
- **事件类型**：路由处理事件
- **触发时机**：在路由开始处理之前（在 `start()` 方法开始时）
- **触发位置**：`app/code/Weline/Framework/Router/Core.php` 第 126 行
- **配置文件**：`app/code/Weline/Framework/etc/event.xml`

#### 功能说明

`Weline_Framework_Router::before_start` 事件在路由开始处理之前触发，此时：
- 路由对象已创建
- URL 尚未处理
- 路由缓存尚未检查
- 可以执行全局路由前检查（如维护模式、访问限制等）

该事件主要用于：
- 维护模式检查
- 全局访问控制
- 路由前安全检查
- 路由前日志记录

#### 触发时机

```php
// app/code/Weline/Framework/Router/Core.php
public function start()
{
    # ----------事件：路由开始前 开始------------
    /**@var EventsManager $eventManager */
    $eventManager = ObjectManager::getInstance(EventsManager::class);
    $eventData = ['router' => $this];
    $eventManager->dispatch('Weline_Framework_Router::before_start', $eventData);
    # ----------事件：路由开始前 结束------------
    
    # 获取URL
    $this->url = $url = $this->processUrl();
    // ... 后续路由处理逻辑
}
```

#### 使用场景

- **维护模式检查**：检查系统是否处于维护模式，如果是则阻止访问或显示维护页面
- **全局访问控制**：执行全局的访问权限检查
- **路由前安全检查**：执行路由前的安全检查逻辑
- **路由前日志记录**：记录路由访问日志

#### 使用方法

##### 基本用法

在模块的 `etc/event.xml` 文件中注册观察者：

```xml
<?xml version="1.0"?>
<config xmlns:xs="http://www.w3.org/2001/XMLSchema-instance"
        xs:noNamespaceSchemaLocation="urn:Weline_Framework::Event/etc/xsd/event.xsd"
        xmlns="urn:Weline_Framework::Event/etc/xsd/event.xsd">
    <event name="Weline_Framework_Router::before_start">
        <observer name="Your_Module::before_start_check"
                  instance="Your\Module\Observer\BeforeStartCheckObserver"
                  disabled="false"
                  shared="true"
                  sort="0"/>
    </event>
</config>
```

创建观察者类：

```php
<?php

namespace Your\Module\Observer;

use Weline\Framework\Event\Event;
use Weline\Framework\Event\ObserverInterface;

class BeforeStartCheckObserver implements ObserverInterface
{
    /**
     * 执行观察者逻辑
     *
     * @param Event $event
     * @return void
     */
    public function execute(Event &$event): void
    {
        $router = $event->getData('router');
        
        // 执行路由前检查逻辑
        if (!$this->canAccess($router)) {
            throw new \Exception('访问被拒绝');
        }
    }
    
    private function canAccess($router): bool
    {
        // 访问检查逻辑
        return true;
    }
}
```

#### 事件数据

`Weline_Framework_Router::before_start` 事件传递的数据：

| 字段 | 类型 | 说明 |
|------|------|------|
| router | Core | 路由对象实例 |

**获取数据**：

```php
$router = $event->getData('router');
```

#### 使用示例

##### 示例 1：维护模式检查

```php
<?php

namespace Weline\Maintenance\Observer;

use Weline\Framework\App\Env;
use Weline\Framework\App\Exception;
use Weline\Framework\DataObject\DataObject;
use Weline\Framework\Event\Event;
use Weline\Framework\Event\EventsManager;
use Weline\Framework\Event\ObserverInterface;
use Weline\Framework\Http\Request;
use Weline\Framework\Manager\ObjectManager;

class BeforeStart implements ObserverInterface
{
    public function execute(Event &$event): void
    {
        if (Env::getInstance()->getConfig('maintenance', false)) {
            /**@var Request $req */
            $req = ObjectManager::getInstance(Request::class);
            /**@var EventsManager $eventManager */
            $eventManager = ObjectManager::getInstance(EventsManager::class);
            $data = new DataObject(['white_urls' => []]);
            $eventManager->dispatch('Weline_Maintenance::maintenance', $data);
            $white_urls = $data->getData('white_urls');
            $white = false;
            foreach ($white_urls as $white_url_string) {
                if (str_contains($req->getUri(), $white_url_string)) {
                    $white = true;
                    break;
                }
            }
            if (!$white) {
                throw new Exception(__('程序维护中...'));
            }
        }
    }
}
```

##### 示例 2：全局访问日志记录

```php
<?php

namespace Your\Module\Observer;

use Weline\Framework\Event\Event;
use Weline\Framework\Event\ObserverInterface;
use Weline\Framework\Http\Request;
use Weline\Framework\Manager\ObjectManager;

class LogRouteAccessObserver implements ObserverInterface
{
    public function execute(Event &$event): void
    {
        $router = $event->getData('router');
        $request = ObjectManager::getInstance(Request::class);
        
        $logData = [
            'time' => date('Y-m-d H:i:s'),
            'uri' => $request->getUri(),
            'method' => $request->getMethod(),
            'ip' => $_SERVER['REMOTE_ADDR'] ?? '',
        ];
        
        // 记录日志
        error_log(json_encode($logData, JSON_UNESCAPED_UNICODE));
    }
}
```

#### 注意事项

1. **执行顺序**：观察者按照 `sort` 属性排序执行，数值越小越先执行
2. **异常处理**：如果观察者抛出异常，会阻止路由处理继续执行
3. **性能考虑**：该事件在每次请求时都会触发，应避免执行耗时操作
4. **路由对象**：路由对象是只读的，不应直接修改
5. **早期检查**：该事件在路由处理的早期阶段触发，适合执行全局性的检查

#### 系统行为说明

1. **默认观察者**：
   - `Weline\Maintenance\Observer\BeforeStart`：维护模式检查（sort=0）

2. **执行顺序**：
   - 观察者按照 `sort` 属性从小到大排序执行
   - 建议将全局检查（如维护模式）放在前面（较小的 sort 值）

#### 相关文件

- **事件配置**：`app/code/Weline/Framework/etc/event.xml`
- **事件定义**：`app/code/Weline/Framework/event.php`
- **触发位置**：`app/code/Weline/Framework/Router/Core.php`（第 126 行）
- **框架开发文档**：`docs/dev/开发文档.md`

## 扩展开发

如果需要扩展路由开始前的功能，可以：

1. **创建新的观察者**：实现 `ObserverInterface` 接口
2. **注册观察者**：在模块的 `etc/event.xml` 中注册
3. **设置执行顺序**：通过 `sort` 属性控制执行顺序
4. **处理异常**：确保观察者中的异常被正确处理

## 更新日志

- **2024-12-19**：初始版本，添加 `Weline_Framework_Router::before_start` 事件文档

## 相关资源

- [Weline Framework 事件系统文档](../../doc/开发/服务器事件系统.md)
- [事件调试功能使用指南](../../../../../docs/事件调试功能使用指南.md)
- [观察者模式最佳实践](../../../../../docs/dev/开发文档.md)
