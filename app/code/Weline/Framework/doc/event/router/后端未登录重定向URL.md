...
本文档详细说明了 Weline Framework 模块提供的 `Framework_Router::backend_no_login_redirect_url` 事件及其使用方法。该事件在后端控制器初始化时触发，允许其他模块添加未登录时的重定向URL。

## 事件列表

### 1. Framework_Router::backend_no_login_redirect_url - 后端未登录重定向URL事件

#### 基本信息

- **事件名称**：`Framework_Router::backend_no_login_redirect_url`
- **事件类型**：路由配置事件
- **触发时机**：在后端控制器初始化时
...
`Framework_Router::backend_no_login_redirect_url` 事件在后端控制器初始化时触发，用于配置未登录用户访问后端时的重定向URL。
...
`Framework_Router::backend_no_login_redirect_url` 事件传递的数据：
...
- **2024-12-19**：初始版本，添加 `Framework_Router::backend_no_login_redirect_url` 事件文档
