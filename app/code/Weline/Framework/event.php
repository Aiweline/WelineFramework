...
return [
    ...
    'Weline_Framework::App::storefront_scope_ready_gate' => [
        'name' => __('店面范围就绪门禁'),
        'description' => __('Website、Store、SalesChannel 范围冻结后、任何 FPC 查询前执行店面强制门禁。'),
        'doc' => 'app/店面范围就绪门禁.md',
        'version' => '1.0.0',
        'type' => 'runtime',
        'data_contract' => [
    ...
    'Weline_Framework_Http::response_ready' => [
        'name' => __('最终 HTTP 响应就绪'),
        'description' => __('FPM 在最终 Response 发出前触发；观察者可通过 response 字段替换或装饰响应。持久运行时使用 App::run_after，避免重复处理。'),
        'doc' => 'http/最终响应就绪.md',
        'version' => '1.0.0',
        'type' => 'runtime',
        'data_contract' => [
            'response' => ['type' => 'Weline\\Framework\\Http\\Response', 'required' => true, 'description' => '可替换的最终 HTTP Response'],
        ],
    ],
    'Weline_Framework_Fpc::cache_hit_response' => [
        'name' => __('FPC 命中响应就绪'),
        'description' => __('FPC 早期命中已经构造 Response、但尚未返回客户端时触发；观察者可按请求上下文安全装饰 response。'),
        'doc' => 'fpc/缓存命中响应.md',
        'version' => '1.0.0',
        'type' => 'runtime',
        'data_contract' => [
            'response' => ['type' => 'Weline\\Framework\\Http\\Response', 'required' => true, 'description' => 'FPC 命中后可替换的 HTTP Response'],
        ],
    ],
    ...
    'Weline_Framework_Phrase::dictionary_collect' => [
        'name' => __('Phrase 请求词典收集'),
        'description' => __('当前请求第一次执行 __()、Parser::parse() 或 Parser::parseKey() 时触发，同一请求作用域只触发一次。观察者只能向 layers 追加词典层（owner/name/scope_key/mode/priority/hash/words/keyed_words/error）；存在 exclusive 独占层时不加载模块 CSV、全局语言包或数据库全局词典；两个不同 owner 同时声明独占视为作用域冲突。'),
        'doc' => 'phrase/请求词典收集.md',
        'version' => '1.0.0',
        'type' => 'integration',
        'data_contract' => [
            'request_id' => ['type' => 'string', 'required' => true, 'description' => '当前请求 ID'],
            'locale' => ['type' => 'string', 'required' => true, 'description' => '当前请求语言'],
            'modules' => ['type' => 'array', 'required' => true, 'description' => '当前请求关联模块'],
            'layers' => ['type' => 'array', 'required' => true, 'description' => '词典层列表，观察者只能追加，不得移除他人层'],
        ],
    ],
    'Weline_Framework_Phrase::dictionary_ready_after' => [
        'name' => __('Phrase 请求词典组装完成'),
        'description' => __('请求事件词典组装完成后派发，只用于诊断、性能记录和前端词典快照准备。观察者不得修改词典、不得重新查询、调用 AI 或写入发布词典；观察者异常不能影响页面输出。'),
        'doc' => 'phrase/请求词典组装完成.md',
        'version' => '1.0.0',
        'type' => 'integration',
        'data_contract' => [
            'request_id' => ['type' => 'string', 'required' => true, 'description' => '当前请求 ID'],
            'locale' => ['type' => 'string', 'required' => true, 'description' => '当前请求语言'],
            'scope_key' => ['type' => 'string', 'required' => true, 'description' => '最高优先级词典层的作用域标识'],
            'mode' => ['type' => 'string', 'required' => true, 'description' => 'overlay 或 exclusive'],
            'owners' => ['type' => 'array', 'required' => true, 'description' => '贡献词典层的 owner 列表'],
            'layer_hashes' => ['type' => 'array', 'required' => true, 'description' => '各词典层 name => hash'],
            'word_count' => ['type' => 'int', 'required' => true, 'description' => '按原文取词的词条数'],
            'keyed_word_count' => ['type' => 'int', 'required' => true, 'description' => '按 entry_key 取词的词条数'],
        ],
    ],
    'Weline_Framework_Phrase::word_missing' => [
        'name' => __('Phrase 请求词典缺词'),
        'description' => __('只在存在事件词典层的请求中触发；同一请求内相同缺词只通知一次；不传占位符实际值。最终返回默认语言原文并继续处理占位符；观察者异常不能影响页面输出。'),
        'doc' => 'phrase/请求词典缺词.md',
        'version' => '1.0.0',
        'type' => 'integration',
        'data_contract' => [
            'request_id' => ['type' => 'string', 'required' => true, 'description' => '当前请求 ID'],
            'locale' => ['type' => 'string', 'required' => true, 'description' => '当前请求语言'],
            'scope_key' => ['type' => 'string', 'required' => true, 'description' => '请求词典作用域标识'],
            'source_text' => ['type' => 'string', 'required' => true, 'description' => '缺词原文（不含占位符实际值）'],
            'entry_key' => ['type' => 'string|null', 'required' => true, 'description' => 'parseKey 缺词时的 entry_key，__() 缺词为 null'],
    ...
    'Weline_Framework_Setup::before_schema_diff_commit' => [
        'name' => __('SchemaDiff 提交前'),
        'description' => __('在 SchemaDiff 执行 DDL 之前触发，允许 owning 模块做数据自愈（如唯一键去重）后再建索引。'),
        'doc' => 'setup/SchemaDiff提交前.md',
    ],
    'Weline_Framework_Setup::upgrade_migrations' => [
        'name' => __('执行模块数据库迁移'),
        'description' => __('在 SchemaDiff 成功后、ModuleSetup 提交前触发，允许数据库模块执行文件迁移并对齐已完成的模块版本游标。'),
        'doc' => 'setup/系统升级后.md',
    ],
    'Weline_Framework_Setup::upgrade_after' => [
        'name' => __('系统升级后'),
        'description' => __('模块与数据库升级阶段完成后、优化和清理之前触发，允许其他模块执行升级后维护。'),
        'doc' => 'setup/系统升级后.md',
    ],
    ...
    'Weline_Framework::resource_changed' => [
        'name' => __('资源已变更'),
        'description' => __('在 Framework 默认主库事务内发布版本化不可变资源变更，支持 critical sync 与可靠 async Observer。'),
        'doc' => 'framework/resource_changed.md',
        'version' => '1.0.0',
        'type' => 'integration',
        'schema_version' => 1,
        'async_mapper' => \Weline\Framework\Event\ResourceChange\ResourceChangePayloadMapper::class,
        'async_data_contract' => 'resource_change.v1',
        'data_contract' => [
            'schema_version' => ['type' => 'int', 'required' => true, 'description' => 'ResourceChange schema，v1 固定为 1'],
            'event_id' => ['type' => 'string', 'required' => true, 'description' => '32 位小写十六进制幂等 ID'],
            'resource' => ['type' => 'array', 'required' => true, 'description' => 'type/id/action/revision'],
            'website' => ['type' => 'array', 'required' => true, 'description' => '网站范围，0/default 是有效默认站'],
            'impact' => ['type' => 'array', 'required' => true, 'description' => '当前/旧 namespace 与 URL'],
            'before' => ['type' => 'array', 'required' => true, 'description' => '变更前允许字段快照'],
            'after' => ['type' => 'array|null', 'required' => true, 'description' => '变更后快照，delete 为 null'],
            'context' => ['type' => 'array', 'required' => true, 'description' => 'Worker 可恢复白名单上下文'],
        ],
    ],
