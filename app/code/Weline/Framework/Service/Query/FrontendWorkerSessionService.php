...
final class FrontendWorkerSessionService
{
    ...
    public function createSession(string $deployVersion, string $workerBuildId): array
    {
        ...
        $sessions = $this->getArray(self::SESSION_KEY);
        ...
        $this->setArray(self::SESSION_KEY, $sessions);
    ...
    public function consumeNonce(string $token, string $nonce): void
    {
        ...
        $allNonces = $this->getArray(self::NONCE_KEY);
        ...
        if (isset($nonces[$nonce])) {
            ...
            $this->setArray(self::NONCE_KEY, $allNonces);
    ...
    public function createStreamTicket(string $channel, array $params = []): array
    {
        ...
        $tickets = $this->getArray(self::STREAM_TICKET_KEY);
        ...
        $this->setArray(self::STREAM_TICKET_KEY, $tickets);
    ...
    {
        $value = SessionFactory::session()->get($key);
        return \is_array($value) ? $value : [];
    }

    private function setArray(string $key, array $value): void
    {
        SessionFactory::session()->set($key, $value);
    }
