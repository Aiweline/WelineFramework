...
class StateManager
{
    ...
    public static function registerFrameworkResets(): void
    {
        ...
        // ========== 20. ObjectManager 缓存污染 ==========

        // ObjectManager 的某些缓存是进程级的（reflections, methodParamsMetadata 等），
        // 但 $instances 和 $origin_instances 必须在每个请求后清理特定的请求级实例
        // 这个已经通过各个 removeInstance 回调处理了，不需要额外重置

        // ========== 21. SSE 上下文状态污染 ==========

        // SseContext 的所有静态变量都是请求级状态，已在 registerFrameworkResets 中注册
        // 这里确保 reset() 方法被调用
        // （已在第 456-460 行注册）

        // ========== 22. Url 解析递归保护标志 ==========

        // Url::$parsingInProgress - 递归保护标志，异常中断可能残留 true
        ...
        // 在开发模式下自动检测未注册的静态变量
        self::registerResetCallback('scan_static_pollution', function () {
