...
abstract class Query implements QueryInterface
{
    ...
    public function insert(array $data, array|string $exist_update_fields = []): QueryInterface
    {
        ...
        # sqlite 不支持 批量插入时更新 所以稍后要检测字段，逐条检测更新还是插入，性能和一般插入一样慢
        if ($exist_update_fields) {
            if (is_string($exist_update_fields)) {
                $this->exist_update_sql = $exist_update_fields;
            } else {
                $this->exist_update_sql = implode(',', $exist_update_fields);
            }
        }
        ...
        if (count($this->insert)) {
            ...
            foreach ($first_insert as $field => $value) {
                if ($field == $this->identity_field && empty($value)) {
                    continue;
                }
                $fields .= "`$field`,";
            }

            # FIXME 检测插入数据更新条件不包含主键时，主键为空时，插入数据异常，第一条数据主键为空，此时后续数据将不得拥有主键字段数据!
            if ($this->exist_update_sql and !in_array($this->identity_field, explode(',', $this->exist_update_sql))) {
                $primary_key_has_empty = false;
                foreach ($this->insert as $key => $insert) {
                    foreach ($insert as $field => $value) {
                        if ($field == $this->identity_field) {
                            if (empty($value)) {
                                $primary_key_has_empty = true;
                            }
                            if (!empty($value) and $primary_key_has_empty) {
                                throw new DbException(__('插入数据异常，第一条数据主键为空，此时后续数据将不得拥有主键字段数据!【Sqlite暂不支持多条数据插入时,主键字段数据时而为空，时而有数据的情况】'));
                            }
                            unset($insert[$field]);
                        }
                    }
                    $this->insert[$key] = $insert;
                }
            }
        }
        $fields           = rtrim($fields, ',') . ')';
     ...
     */
    public function getSql(string $sql): string|array
    {
