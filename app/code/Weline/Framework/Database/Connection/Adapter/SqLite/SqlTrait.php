...
trait SqlTrait
{
    ...
    private function prepareSql($action): void
    {
        ...
        # sqlite 不支持创建表时设置约束条件，处理下$this->additional_sql
        $this->additional_sql = '';
        if ($this->table == '') {
                ...
                if ($this->exist_update_sql) {
                    ...
                    foreach ($this->insert as $insert_key => $insert) {
                        ...
                        foreach ($exist_update_fields as $update_field) {
                            if(!isset($insert[$update_field])) {
                                throw new DbException('数据中缺少更新字段：'.$update_field);
                            }
                        ...
                        foreach ($exists as $exist) {
                            ...
                            foreach ($this->insert as $insert_key => $insert) {
                                ...
                                if ($insert_data_value_key == $exist_update_value_key) {
                                    ...
                                    foreach ($exist_update_fields as $update_field) {
                                        if(is_int($insert[$update_field])){
                                            $exist_update_where .= $update_field . ' = ' . $insert[$update_field] . ' AND ';
                                        }else{
                                            $exist_update_where .= $update_field . ' = "' . addcslashes((string)($insert[$update_field]), '"') . '" AND ';
                                        }
                                    ...
                                    if (isset($insert[$this->identity_field])) {
                                        $main_key_exist_update_where = 'where ' . $this->identity_field . ' = "' . addcslashes((string)($insert[$this->identity_field]), '"') . '"';
                                        foreach ($exist_update_fields as $exist_update_field) {
                                            if (!in_array($exist_update_field, $added_insert_update_fields)) {
                                                $main_key_exist_update_where .= $exist_update_field . ' = "' .addcslashes((string)($insert[$exist_update_field]), '"') . '" AND ';
                                            }
                                        }
                                        ...
                                        foreach ($this->_unit_primary_keys as $unit_primary_key) {
                                            ...
                                            $unit_primary_key_insert_update_where .= $unit_primary_key . ' = "' .addcslashes((string)($insert[$unit_primary_key]), '"') . '" AND ';
                                        }
                                        foreach ($exist_update_fields as $exist_update_field) {
                                            if (!in_array($exist_update_field, $added_insert_update_fields)) {
                                                $unit_primary_key_insert_update_where .= $exist_update_field . ' = "' .addcslashes((string)($insert[$exist_update_field]), '"') . '" AND ';
                                            }
                                        }
                            ...
                            if (count($insert_updates) > 0) {
                                ...
                                foreach ($insert_updates as $insert_update_where => $insert_update) {
                                    ...
                                    foreach ($insert_update as $insert_update_field => $insert_update_value) {
                                        $sql .= $insert_update_field . ' = "' . addcslashes((string)$insert_update_value, '"') . '", ';
                                    }
