 ...
 * Windows 脚本执行器
 * 
 * @DESC 在 Windows 系统上执行安装脚本
 */
class WindowsScriptExecutor implements InstallScriptExecutorInterface
{
    /** @var array 支持的脚本扩展名 */
    private const SUPPORTED_EXTENSIONS = ['php', 'ps1', 'bat'];
    ...
    {
        // 优先使用 script_windows
        if (isset($item['script_windows']) && !empty($item['script_windows'])) {
            $path = $envDir . $item['script_windows'];
    ...
    private function isPathSafe(string $path, string $baseDir): bool
    {
        ...
        // Windows 路径不区分大小写
        return stripos($realPath, $realBaseDir) === 0;
    }
}
