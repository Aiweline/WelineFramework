/**
 * Weline Framework — 核心国际化（i18n）JS 模组
 *
 * 归属：Weline_Framework（与 Phrase 短语解析同属框架 i18n 能力面）。
 * 外置 Weline_I18n 仅增强（语言切换器 UI / 国旗 / AI 翻译等），不承载本核心。
 *
 * 功能：
 * 1. 字典 / translate / setDictionary / loadDictionary
 * 2. 语言切换链接与当前语言显示
 * 3. switchLang 与空白页恢复
 * 4. 安装 window.Weline.i18n / window.WelineI18n
 *
 * 加载：Theme declare / data-weline-load="i18n"（见 Framework weline.modules.js）
 */
(function (window, document) {
    ...
    function writeCookieValue(key, value, expiry = 365, options = {}) {
        ...
        // SameSite default matches server Set-Cookie for language preference.
        if (!Object.prototype.hasOwnProperty.call(normalizedOptions, 'SameSite')
            && !Object.prototype.hasOwnProperty.call(normalizedOptions, 'samesite')) {
    ...
    function detectCookieNameSuffix() {
        const match = document.cookie.match(
            /(?:^|; )WELINE_(?:USER_LANG|USER_CURRENCY|WEBSITE_ID|WEBSITE_CODE)(_w\d+)=/i
        );
    ...
    function resolvePreferenceCookieNames(baseKey) {
        ...
        const suffix = detectCookieNameSuffix();
        if (suffix) {
            push(baseKey + suffix);
        }
        ...
        try {
            ...
            for (let i = 0; i < parts.length; i++) {
                const name = String(parts[i].split('=')[0] || '').trim();
                if (name === baseKey || name.indexOf(baseKey + '_w') === 0) {
                    push(name);
                }
            }
        } catch (error) {
    ...
    function getHeaderLanguageCompactLabel(option, langCode) {
        ...
        const compact = String(option.getAttribute('data-compact-label') || '').trim();
        if (compact) {
            return compact;
        }

        const switcher = option.closest('[data-i18n-switcher]');
        const switcherCompact = switcher
            ? String(switcher.getAttribute('data-current-compact') || '').trim()
            : '';
        if (switcherCompact && sameLang(option.getAttribute('data-lang') || option.dataset.lang, langCode)) {
            return switcherCompact;
        }

        const native = String(option.getAttribute('data-native') || '').trim();
        if (native && native.length <= 8 && !/[（(].+[，,]/.test(native)) {
            return native;
        }
     ...
     */
    function installWelineI18nFacade() {
        const facade = {
            get currentLang() {
                return window.WelineI18n ? window.WelineI18n.currentLang : i18nObj.currentLang;
            },
            set currentLang(value) {
                i18nObj.currentLang = value;
                if (window.WelineI18n) {
                    window.WelineI18n.currentLang = value;
                }
            },
            get dictionary() {
                return window.WelineI18n ? window.WelineI18n.dictionary : i18nObj.dictionary;
            },
            set dictionary(value) {
                i18nObj.dictionary = value || {};
                if (window.WelineI18n) {
                    window.WelineI18n.dictionary = i18nObj.dictionary;
                }
            },
            get apiUrl() {
                return window.WelineI18n ? window.WelineI18n.apiUrl : i18nObj.apiUrl;
            },
            setDictionary: (dict) => i18nObj.setDictionary(dict),
            translate: (key, params) => i18nObj.translate(key, params),
        ...
        if (window.Weline && typeof window.Weline === 'object') {
            window.Weline.i18n = facade;
        }
        return facade;
    }
