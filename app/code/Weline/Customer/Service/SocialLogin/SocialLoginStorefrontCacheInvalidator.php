...
/**
 * Local storefront/FPC cleanup after social-login SystemConfig changes.
 * CDN purge remains with Weline_Framework::resource_changed receivers.
 */
final class SocialLoginStorefrontCacheInvalidator
{
    public function clearForSocialLoginConfig(string $reason): void
    {
        $this->clearStaticHookCaches($reason);
    ...
    private function clearStaticHookCaches(string $reason): void
    {
        try {
            ...
            $this->logFailure('template_static_hook', $reason, $e);
        }
    }
    ...
    private function logFailure(string $step, string $reason, \Throwable $e): void
    {
        ...
        w_log_warning(
            (string) __('社媒登录配置变更后清理前台/FPC 缓存失败：%{1}', [$e->getMessage()]),
