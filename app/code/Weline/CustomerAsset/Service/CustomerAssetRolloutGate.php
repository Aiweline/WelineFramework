...
/**
 * Durable Website-scoped rollout gate shared by CustomerAsset and MIG-P4D.
 */
final class CustomerAssetRolloutGate implements CommerceRolloutGateInterface
{
    public const CAPABILITY = CustomerAssetService::CAPABILITY;
    public const CONFIG_ROOT = 'commerce.rollout.customer_asset';
    public const CONFIG_MODE = self::CONFIG_ROOT . '.mode';
    public const CONFIG_ALLOWLIST = self::CONFIG_ROOT . '.allowlist';

    private const CONFIG_MODULE = 'Weline_CustomerAsset';
    ...
    public function setMode(
        ...
        if ($capability !== self::CAPABILITY) {
            throw new \InvalidArgumentException('customer_asset_rollout_capability_invalid');
        }
        ...
        if ($this->envConfiguration() !== null) {
            throw new \RuntimeException('customer_asset_rollout_env_locked');
        }

        // Disable first so a partial configuration write stays fail closed.
        ...
        if ($readback['mode'] !== $expected['mode']
            ...
            throw new \RuntimeException('customer_asset_rollout_readback_mismatch');
        }
    }
    ...
    public function configuration(): array
    {
        ...
        try {
            ...
            throw new \RuntimeException(
                'customer_asset_rollout_config_unavailable',
                0,
                $exception,
            );
        }
    ...
    private function envConfiguration(): ?array
    {
        ...
        if (!is_array($value) || array_is_list($value)) {
            throw new \RuntimeException('customer_asset_rollout_config_invalid:env_root');
        }
    ...
    private function normalizeConfiguration(array $configuration): array
    {
        ...
        if ($keys !== ['allowlist', 'mode']) {
            throw new \RuntimeException('customer_asset_rollout_config_invalid:fields');
        }
        ...
        if (!is_string($mode)
            ...
            throw new \RuntimeException('customer_asset_rollout_config_invalid:mode');
        }

        $rawRows = $configuration['allowlist'];
        if (!is_array($rawRows) || !array_is_list($rawRows)) {
            throw new \RuntimeException('customer_asset_rollout_config_invalid:allowlist');
        }
        $allowlist = [];
        $rows = [];
        foreach ($rawRows as $row) {
            if (!is_array($row) || array_keys($row) !== ['website_id']) {
                throw new \RuntimeException(
                    'customer_asset_rollout_config_invalid:allowlist_entry',
                );
            }
            $websiteId = $row['website_id'];
            if (!is_int($websiteId) || $websiteId < 0) {
                throw new \RuntimeException(
                    'customer_asset_rollout_config_invalid:allowlist_values',
                );
            }
            $key = self::scopeKey($websiteId);
            if (isset($allowlist[$key])) {
                throw new \RuntimeException(
                    'customer_asset_rollout_config_invalid:allowlist_duplicate',
                );
            }
            $allowlist[$key] = true;
        ...
        foreach ($subjects as $subject) {
            ...
            if (preg_match('/^website:(0|[1-9][0-9]*)$/D', $subject, $match) !== 1) {
                throw new \InvalidArgumentException(
                    'customer_asset_rollout_subject_invalid:' . $subject,
                );
            }
            $key = self::scopeKey((int) $match[1]);
            if (isset($rows[$key])) {
                throw new \InvalidArgumentException(
                    'customer_asset_rollout_subject_duplicate:' . $subject,
                );
            }
    ...
    {
        if ($websiteId < 0) {
            throw new \InvalidArgumentException('customer_asset_rollout_website_invalid');
        }
    ...
    private function write(string $key, mixed $value): void
    {
        if (!$this->configStore()->setScopedConfig(
            ...
            throw new \RuntimeException(
                'customer_asset_rollout_config_write_failed:' . $key,
            );
        }
    }
