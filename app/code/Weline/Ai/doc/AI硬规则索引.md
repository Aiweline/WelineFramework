...
| **工程团队、大型团队、子智能体、停工汇报、技术方案会、对齐冻结会、用例冻结、contracts、deps、流水线、问题上报、监工、测试账号、admin/admin、框架专席、合规复审、组件协商、验收签收** | [工程团队.md](../../../../../dev/ai-command/ai/工程团队.md)；[AI工程交付流程.md](./AI工程交付流程.md) §3b/§5；MCP `engineering_team_for_new_requirements` / `local_dev_test_accounts_self_serve` / skill `engineering_team`（`weline-engineering-team`） | 复杂需求不打 `Team:架构师:`；简单需求打 `Team:`；内容运营打这两种前缀；重大架构矛盾不停工；**开发完才补主路径用例**；无 contracts/deps 就开施工；**向用户要本机账号密码**；触发专席跳过合规复审；涉 UI 无 UI/原型实质签收就交出；未协商就扩展组件 | 简单→`监工:`；复杂由父会话自己选 team，每句 `Team:{席位}:`；框架优先+全专席双轨+流水线流动；对齐冻结会钉 UC+`contracts.md`+`deps.md`；唤醒式并发；`surfaces.md`/`components.md`；验收 `acceptance-ui`+`acceptance-prototype`；纪要 `doc/开发/team/{slug}/`；本机后台默认 **admin/admin**，前台自建 |
...
| 主题、layout、widget、partial、**编辑器预览/theme-preview 与店面同构**、**三态身份权威** | [Theme/doc/AI-INDEX.md](../Theme/doc/AI-INDEX.md)、[Theme开发总指南.md](../Theme/doc/开发/Theme开发总指南.md)、[**preview-and-runtime-modes.md**](../Theme/doc/preview-and-runtime-modes.md)；MCP `preview_storefront_delivery_parity`；技能 `weline-theme-development` | 改 `generated/`、`view/tpl`；layout 内嵌非 `Weline_Theme` 部件；**仅为 preview/editor_mode/visual_editor 提前 return / 跳过 / 抽空真实店面逻辑**；预览与发布走两套实现；**混淆三态权威**（用 path 猜画布主题、用 URL 覆盖 Token、正式态读 draft/s*） | `php bin/w frontend:check-theme-layout-widgets`；**预览与真实同一套业务逻辑**；身份：可视化=**参数**、版本真实预览=**Token 反解析**、正式=**RequestContext**；禁止为安静画布丢逻辑 |
