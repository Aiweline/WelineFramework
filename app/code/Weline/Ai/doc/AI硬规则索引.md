...
> Weline **开发（编码/工程）**任务的**第一层路由**。本文只做两件事：**(1) 判定这一回合走不走 MCP**；**(2) 按任务关键词给出「必须先读 / 禁止 / 验证」**。
> 规则**正文**按主题拆到 `doc/规则/` 下的六个分册；**动作顺序**见 [AI工程交付流程.md](./AI工程交付流程.md)；**权威与冲突裁决**见 [AI开发治理.md](./AI开发治理.md)。本文与分册冲突时以分册正文为准。

## 文档族（谁写什么）

| 文档 | 职责 | 不负责 |
|------|------|--------|
| **本文 `AI硬规则索引.md`** | 调用范围判定 + 任务路由表 + 写模板决策流 + 会话纠正短表 | 规则正文（在 `规则/` 分册） |
| [`规则/00-MCP调用与会话.md`](./规则/00-MCP调用与会话.md) | MCP 调用范围、技能来源、会话引导、宿主编辑器规则、Codex 委派、学习冲突门禁、模块 doc 资料边界 | 具体领域规范 |
| [`规则/10-工程流程与验收.md`](./规则/10-工程流程与验收.md) | 需求澄清→计划→团队→实现→复审→测试→收口、e2e、审图、汇审、交付地址 | 领域编码规范 |
| [`规则/20-前端与主题.md`](./规则/20-前端与主题.md) | Theme/布局/部件/UI 浮层/版心/JS 加载/审图/响应式、前端 i18n | 后端与数据规范 |
| [`规则/30-后端与数据.md`](./规则/30-后端与数据.md) | Controller/路由/ORM 升级、Taglib/Hook/Event、配置与范围、Query/REST/BinQuery、缓存性能、注释与代码风格 | 前端视觉规范 |
| [`规则/40-集成与运维.md`](./规则/40-集成与运维.md) | 支付/货源壳+Provider、图片与媒体、生图、Visitor 像素数据分析、Cursor 调试工具链、运行状态查询 | 编码流程门禁 |
| [`规则/50-内容运营.md`](./规则/50-内容运营.md) | 产品优化/详情优化/翻译优化/主图优化/新建文章/审查文章/规格修复、默认站全语种翻译 | 框架编码规则 |
| [`AI工程交付流程.md`](./AI工程交付流程.md) | 阶段 0→7 的强制动作序列 | 规则条文 |
| [`AI开发治理.md`](./AI开发治理.md) | 权威顺序、模块三文档契约、范围与安全、实现与验收契约 | 任务路由 |
| [`文档索引.md`](./文档索引.md) | 全仓轻量索引树与 MCP 检索入口 | 规则 |

**MCP 编译落点**：`hard-constraints.v1` 由本文与各分册编译；**权威常量** `HardConstraintsCatalog::AUTHORITATIVE_DOC`（本文）与 `AUTHORITATIVE_WORKFLOW_DOC`（交付流程）。改本文、分册或交付流程后，必须跑 `php app/code/Weline/Ai/Mcp/tests/guidance-workflow-contract.php` 与 `mcp-skills-catalog.php`。
...
| **内容运营技能**（`content_ops_skills_skip_mcp`） | 产品优化 / 详情优化 / 翻译优化 / 主图优化 / 新建文章 / 审查文章 / 规格修复 | **跳过**：宿主 Read `dev/ai-command/**` + 模块 `doc/ai/skills/**`；**禁止** prepare_project / resolve_skill / get_skill / 拉索引。细则见 [规则/50-内容运营.md](./规则/50-内容运营.md) |
| 打招呼 `hi`/`你好`/`hello`（无编码任务）或指令「提取技能」 | 可 `prepare_project` / `resolve_skill(list_all)` **仅列** MCP 技能+指令；不开始写码 |  |
| 编码/工程 | 改代码或模块文档、诊断/评审、部署规划、功能验收收口 | **强制** ensure（若需）→ `prepare_project` → 读并遵守 `hard_constraints` → 宿主原生编辑；按需 `resolve_task_context` / `search_project_knowledge` / `get_skill`。MCP 挂不上则宿主 Read 本文 + 对应分册，不得编造规则。**上下文丢失**（长对话压缩后看不到 `hard_constraints`）须**重新** `prepare_project` |
...
> 关键词命中即按行执行。「必须先读」不得跳过；完成后跑「验证」列的命令（如有）。写代码前另需完成 [扩展点选型](../../Framework/doc/3-开发/扩展点选型.md)。

### 工程流程与验收（细则 → [规则/10](./规则/10-工程流程与验收.md)）
...
| 审图、截图、发图审查、**用户附图/粘贴图**、截图不说、线稿/抽取线图、原型调整、**有图=视觉UI+原型必上** | [dev/ai-command/theme/审图.md](../../../../../dev/ai-command/theme/审图.md)；**MCP** `user_image_attachment_triggers_shentu` / `ui_skill_surface_signal_gate` / `image_attachment_shentu_bundle`；**任意附图即审**（含后台/CMS/错误页）；**web_ui 附图强制 participate + 原型上场**；**非报错 `ui_shot` 默认=改 UI**（禁止只点评）；**同回合联合**：线稿抽取 → `prototype` 调整 → `frontend-design` → `weline-theme-development`；`error_shot` 优先修异常且错误页可读 | 非本仓前端图仍套店面修复；`ui_shot` 只点评不改；跳过线稿/原型直接糊 CSS；把产品 UI 截图当闲聊插图；有图却 `ui_skill_decision=skip` 或不派原型；缺技能仍写 E/F pass | 分流（含 error/ui）+ 线稿 + 原型调整 + 技能门禁 + 检查清单；Browser 验收 |
...
| 注释、文件头、`<?=` / `<?php` 开标签 | [开发标准与验收.md](../../Framework/doc/3-开发/开发标准与验收.md)；MCP `no_php_tags_in_comments` | **注释**（`//` `#` `/* */` `/** */` `<!-- -->`）内出现 `<?=` / `<?php` / 短开标签；文件头残留生成器短回显；用 `<?php /* ?>…<?=…*/` 包死代码 | 检索注释内开标签；**非**禁止 `// $x = 1;` 这类普通注释掉语句 |
| 注释语言、代码风格、可读性 | [开发标准与验收.md](../../Framework/doc/3-开发/开发标准与验收.md)；MCP `chinese_comments_friendly_style` | 新增说明性注释/PHPDoc 却用英文堆砌；过度巧妙/过度抽象/工业式套话导致难读；无视周围既有风格 | Diff 抽查注释语言与命名/控制流可读性 |
...
| **生图 / GenerateImage / AI 画图 / 出图落盘 / Banner·Hero·主图导出** | [weline-image-pipeline.md](../../Product/doc/ai/skills/ecommerce-detail-suite/companions/weline-image-pipeline.md)；MCP `generated_images_prefer_webp`；MediaManager `AiDrawService` / Ai `output_format` | 默认交付大 PNG/JPG；生图工具返回 PNG 却不转码就当终态；`pub/media` 长期留 jpg/png 店面图 | **默认 WebP**；`output_format=webp`；PNG 仅用户明示 / 确需非 WebP 透明 / favicon·ico；落盘后路径与库引用一致 |
...
| **定时翻译进度、队列/cron 是否在跑、翻译记录有没有新增、运行状态查询** | MCP `runtime_status_query_local_first`；见 [规则/00](./规则/00-MCP调用与会话.md) | **未明示就查生产/SSH weline**；把 SSH 默认 profile 当默认查线上；「翻译特例必须查生产」 | 默认查本机 DB/进程；仅用户说线上/生产/ssh weline/aiweline.com 才查远端 |

### 内容运营（**跳过 MCP**；细则 → [规则/50](./规则/50-内容运营.md)）
...
> 只列**最高频**纠正点；完整条文见对应分册与上表「禁止」列。命中即回头读分册。

| 纠正点 | 权威 |
|--------|------|
| Hook 只有 phtml 无 `hook.php` / type 段非 `partials`\|`layouts` | [规则/30](./规则/30-后端与数据.md)；[Hook创建规范.md](../../Hook/doc/Hook创建规范.md) |
| Event 名未文档化就 dispatch | [规则/30](./规则/30-后端与数据.md)；[事件命名与注册规范.md](../../Framework/doc/3-开发/事件命名与注册规范.md) |
| 手写 select / 手写国家省市区 / 浮层手写 left·top / 自写版心容器 | [规则/20](./规则/20-前端与主题.md)、[规则/30](./规则/30-后端与数据.md) |
| 硬编码视觉字面量 / 自造色板间距 / 基础组件私写颜色 / 后台裸表单 | [规则/20](./规则/20-前端与主题.md) |
| 前端英文源串 / 文案不写中英 CSV / `@lang{含,逗号}` ParseError | [规则/20](./规则/20-前端与主题.md) |
| 部件直接 `@static` / `<script src>` 拉 JS 模块 / `weline.js` 写死业务名 | [规则/20](./规则/20-前端与主题.md) |
| 主题布局内嵌他模块 widget / 默认注入未固化 / design 覆盖 theme.css·js | [规则/20](./规则/20-前端与主题.md) |
| 业务「配置」另造私有表 / embed `fields` ≠ 声明 `key` / 「范围」写成路径通配 | [规则/30](./规则/30-后端与数据.md) |
| raw fetch/ajax / alert·confirm / 裸 MutationObserver | [规则/30](./规则/30-后端与数据.md) |
| 改 Model 不 bump version / URL 写成 `/getEdit` `/postSave` / routes.xml 或 generated 直改 | [规则/30](./规则/30-后端与数据.md) |
| 注释内写 `<?=` `<?php` / 新增注释英文堆砌 / 代码过度巧妙难读 | [规则/30](./规则/30-后端与数据.md) |
| Taglib callback 裸 `@static(...)` 导致 404 | [规则/30](./规则/30-后端与数据.md)、[规则/20](./规则/20-前端与主题.md) |
| 业务类自做进程内缓存 / 清理与驱动 key 不一致 | [规则/30](./规则/30-后端与数据.md) |
| 图片缺 HTML 宽高 / 用 file:image 当选图器 / 手拼 identity_path | [规则/40](./规则/40-集成与运维.md) |
| 生图默认交 PNG·JPG 不转 WebP | [规则/40](./规则/40-集成与运维.md) |
| 壳里重写供应商业务 / 支付域不上场支付开发工程师 | [规则/40](./规则/40-集成与运维.md) |
| 未自行验证就宣称完成 / 缺审图 / 缺汇审 / unit 无真实跑测 evidence | [规则/10](./规则/10-工程流程与验收.md) |
| 交付 URL 格式错误或省略 / 验收 Browser 未禁缓存 / WB-OP 抢焦点 / 写完不关 Browser | [规则/10](./规则/10-工程流程与验收.md) |
| 多 todo 计划未逐项举证就说「已完成」/ 计划未按七维合规审核 | [规则/10](./规则/10-工程流程与验收.md) |
| 让用户手写 MCP Settings / 手写 `.cursor/rules` 当规则源 | [规则/00](./规则/00-MCP调用与会话.md) |
| 未提 Codex 却因 CLI 存在自动委派 / 静默委派不提示 | [规则/00](./规则/00-MCP调用与会话.md) |
| 查翻译/cron/队列状态默认去生产 | [规则/00](./规则/00-MCP调用与会话.md)、[规则/40](./规则/40-集成与运维.md) |
| 把宿主 SKILL.md 当工程技能权威 | [规则/00](./规则/00-MCP调用与会话.md) |
...
**脏改区 dirty-load（硬）**：任何 Write / StrReplace / ApplyPatch 之前，必须先宿主 Read **当前磁盘**工作区文件；所有修改必须在该**已存在的脏改内容**上继续编辑。禁止拿 HEAD、索引、对话历史里的旧缓冲、其它会话快照、agent-transcript 摘录或任何更旧基线来当「原文件」再写回。多 Agent / 多聊天并行时，不得用本会话持有的旧版本覆盖其它会话正在推进的脏文件。Hash 漂移只能 fail-closed 保留磁盘现场，禁止「先还原旧版再覆盖」。

MCP 子进程只允许只读 Git 检查，禁止上述全部 Git 写操作及 force/discard 变体。分支切换只能由工作区所有者显式执行。完整条款见 [规则/00-MCP调用与会话.md](./规则/00-MCP调用与会话.md) 与 [AI开发治理.md](./AI开发治理.md)。
...
会话完整硬约束由 `prepare_project.agent_guidance.hard_constraints` 提供。`guidance-bundle.v1.workflow_contract` 只返回任务匹配的规范和权威文档入口。`rules` 可携带验证后的学习规则；`fragments` 自带路径、行号与来源 hash；`token_usage.estimated` 为估算值。
