...
## 模块 doc 禁放一次性工作资料（`module_doc_forbids_ephemeral_work_artifacts`，强制）
...
权威：`HardConstraintsCatalog::mcpOperationalRules()` → `module_doc_forbids_ephemeral_work_artifacts`；工程团队路径见 [工程团队.md](../../../../../dev/ai-command/ai/工程团队.md)。

## Codex CLI 宿主委派（`host_delegate_explore_plan_review_to_codex_cli`，**用户显式 opt-in**）
...
| **用户可见状态（仅 opt-in 且启动委派时强制）**：启动 `codex` 前聊天明示「Codex 正在工作：{阶段}…」；完成写「Codex 已完成」；回退写「Codex 不可用，已回退宿主：{原因}」（见 `user_visible_status`） | 静默跑 `codex`，界面上看起来仍是宿主自己在干活 |
...
权威：`HardConstraintsCatalog::hostCodexDelegation()` → `prepare_project.agent_guidance.host_codex_delegation`（含 `opt_in`）；规则正文 `host_delegate_explore_plan_review_to_codex_cli`。
...
| **计划、架构方案、Plan Mode、SwitchMode、计划模式、简单跳过计划、计划正文聚焦、Codex CLI 委派、codex exec、codex review** | [AI工程交付流程.md](./AI工程交付流程.md) §1c/§3；MCP `host_plan_mode_for_planning` / `plan_content_focus_only` / `host_delegate_explore_plan_review_to_codex_cli` / `requirement_acceptance_always` / `requirement_fe_be_scope_analysis`；`agent_guidance.host_codex_delegation` | 非简单却不切 Plan Mode；**未提 Codex 却自动委派**；**已 opt-in 且 CLI 可用时 Cursor 自写笼统计划**；**Codex 宿主嵌套调自身**；命令写死 `-m`；**跳过计划却不做验收**；有 Web 却无本机 Browser WB-OP；布局/吐槽/审图跳过原型+UI；**计划写无关散文/流程复述把主题带偏** | 默认 `SwitchMode`→`plan`（宿主自写三节计划）；**仅用户提及 Codex 且 CLI 可用**→`codex exec` 出三节计划，Cursor 只编码；编码后 `codex review --uncommitted`；simple+理由可 skip；计划正文只写**背景+方案+细节**；Web→WB-OP 视觉+逻辑；审图强制原型+UI 调整 |
...
| **未提 Codex 却因 CLI 存在自动委派** / 用户已 opt-in 且 CLI 可用时自行写笼统计划 / Codex 宿主递归调用自身 / 命令写死 `-m` / **静默委派不提示「Codex 正在工作」** | 本文 `host_delegate_explore_plan_review_to_codex_cli`；`agent_guidance.host_codex_delegation.opt_in` + `user_visible_status` |
