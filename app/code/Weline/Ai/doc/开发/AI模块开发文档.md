...
文生图请求的 `params.identity_transparent_raster_required=true`、`params.transparent_raster_required=true`、`params.identity_transparent_png_required=true` 或 `params.transparent_png_required=true` 会被识别为品牌身份资产生成请求，适用于网站 header、favicon、品牌识别位的 logo/icon。AI 服务层会追加 logo/icon 专用提示词约束，要求主体居中、无文字、无场景背景，且硬性输出只能是带真实 alpha 的 PNG 或 WebP。
...
> 硬性透明身份资产规则：`identity_transparent_raster_required=true`、`transparent_raster_required=true`、`identity_transparent_png_required=true` 或 `transparent_png_required=true` 表示必须由模型/供应商原生返回带 alpha 的透明 PNG/WebP。JPEG、SVG、无 alpha 的 PNG/WebP、可抠图背景、本地抠图 fallback、占位图或伪成功都不允许通过。旧的可抠图背景兼容行为只允许用于非硬性 `requested_transparent_background=true` 调用。

当模型明确支持原生透明背景时，适配层会传供应商对应的透明背景参数（例如 `background=transparent`）并要求 `output_format` 为 `png` 或 `webp`。对于上述硬性请求，如果供应商或模型不支持、能力未知，或原生透明参数返回不支持错误，适配层必须失败并保留真实错误，不得继续生成可抠图背景。只有普通 `requested_transparent_background=true` 兼容调用可以移除透明背景参数并生成干净、主体明确、背景简单的 logo/icon 图像，让调用方自行决定后处理。
