...
### 🧪 测试文档
- [单元测试指南](./开发/单元测试指南.md) - PHPUnit 测试入口与约束
- [Playwright测试指南](./开发/Playwright测试指南.md) - 浏览器自动化与真实路径验收

### 📊 项目管理
模块当前需求与版本进度：
- [需求](./需求.md) - 当前确认的业务逻辑、范围与验收标准
- [开发日志](./开发日志.md) - 按目标版本记录实现、复审、测试与验收状态
- [AI开发治理](./AI开发治理.md) - 全局知识归属与开发门禁
- [AI工程交付流程](./AI工程交付流程.md) - 强制工作流（定位→选型→计划→实现→验收）
- [文档索引](./文档索引.md) - 轻量索引树与 MCP 检索入口
- [统一知识库与MCP协议](./统一知识库与MCP协议.md) - readiness 与动态检索协议

### 🔌 API参考
- [API文档](./API/API.md) - API接口完整说明
...
### 核心功能
1. **AI 管理** - 模型 | 适配器 | 智能体 | 技能 | 风格 | 供应商账户 聚合管理
2. **默认模型** - 在后台 AI 中心配置各服务类型的全局默认模型
3. **场景配置** - 场景适配器管理
4. **智能体** - 已注册智能体、描述覆盖与工具启停
5. **模型管理** - 多供应商、多模型统一管理
6. **REST API** - Chat、Model、API Key 等接口

### 后台入口

登录后台后进入“应用与扩展 → AI 中心 → 默认模型”，即可配置文本、图像、翻译、代码等服务类型的全局默认模型。
进入“AI 中心 → AI 智能体”（或 AI 管理 Tab「智能体」）可查看已注册智能体与工具，覆盖描述并启停工具。
...
- [框架文档](../../Framework/doc/README.md)
- [开发文档总览](../../../../../docs/dev/开发文档.md)
- [统一知识库与 MCP 协议](./统一知识库与MCP协议.md)

## 📅 更新记录

- **2026-08-27**: MCP 改为单一共享 STDIO 注册；工具公开进程/源码代次并拒绝旧运行时；引导脚本仅在插件产物和宿主代次一致时返回 ready，否则执行一次性可核验重启。
