...
- `Setup`：安装/升级装配。不要手改 generated，也不要在 Setup/Upgrade.php 做字段 CRUD。 文件数：6
- `Taglib`：模板标签扩展。改前读 Weline_Taglib 与 Theme 文档。 文件数：1
- `etc`：模块配置。禁止 routes.xml；路由由控制器和 setup:upgrade --route 生成。 文件数：23
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/AiProviderAccountQueryProvider.php、extends/module/Weline_Framework/Query/AiQueryProvider.php；前端/跨模块读数据先查 query 帮助。
