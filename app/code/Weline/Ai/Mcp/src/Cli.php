...
final class Cli
{
    ...
    public static function runCtl(array $argv): int
    {
        ...
        try {
            return match ($command) {
                ...
                'archive' => self::archive($arguments),
                'maintenance' => self::maintenance($arguments),
                'index-gc' => self::indexGc($arguments),
    ...
    public static function runDaemon(array $argv): int
    {
        ...
        try {
            ...
            try {
                ...
                $worker = new Worker($store, $analyzer, $config);
                $result = match ($command) {
                    'once' => $worker->runOnce(),
                    'drain' => $worker->drain((int) ($options['max-jobs'] ?? 100)),
                    'maintenance' => [
                        'session_lifecycle' => (new SessionLifecycleService($store, $analyzer, $config))->sweep(),
                        'index_gc' => (new IndexGarbageCollector($store, $config))->sweep(null, true),
                    ],
                    'run' => $worker->run(),
    ...
    private static function hook(array $arguments): int
    {
        ...
        try {
            ...
            if ($injectProjectContext || self::bool($options['inject-project-rules'] ?? false)) {
                $additionalContext[] = 'Step 0 (engineering MANDATORY): run `php app/code/Weline/Ai/Mcp/scripts/ensure-project-guidance.php` and auto-repair host guidance when status is not ready. Then git switch dev if needed, MUST call prepare_project status=ready, obey hard_constraints before edits, and carry a session-bound readiness_id. Resolve task guidance through the MCP; cold-start .mdc is MCP-generated by ensure.';
    ...
    private static function archive(array $arguments): int
    {
        ...
        if ($target !== 'session') {
            throw new RuntimeException('Usage: learningctl archive session --id SESSION_ID');
        }
        [$positionals, $options] = self::options($arguments, ['config', 'data-dir', 'id']);
        self::noPositionals($positionals);
        $sessionId = self::requiredOption($options, 'id');
        [$config, $store] = self::open($options);
        try {
            self::writeJson((new SessionLifecycleService(
                $store,
                new Analyzer($store, $config),
                $config,
            ))->archive($sessionId, 'explicit'));
        } finally {
            $store->close();
        }
    ...
    /** @param list<string> $arguments */
    private static function maintenance(array $arguments): int
    {
        [$positionals, $options] = self::options($arguments, ['config', 'data-dir', 'owner']);
        self::noPositionals($positionals);
        [$config, $store] = self::open($options);
        try {
            $owner = trim((string) ($options['owner'] ?? '')) ?: null;
            $analyzer = new Analyzer($store, $config);
            self::writeJson([
                'session_lifecycle' => (new SessionLifecycleService(
                    $store,
                    $analyzer,
                    $config,
                ))->sweep($owner),
    ...
    /** @param list<string> $arguments */
    private static function indexGc(array $arguments): int
    {
        $subcommand = array_shift($arguments) ?? 'dry-run';
        if (!in_array($subcommand, ['dry-run', 'run'], true)) {
            throw new RuntimeException('index-gc subcommand must be dry-run or run');
        }
        [$positionals, $options] = self::options($arguments, ['config', 'data-dir', 'owner']);
        self::noPositionals($positionals);
        [$config, $store] = self::open($options);
        try {
            self::writeJson((new IndexGarbageCollector(
                $store,
                $config,
            ))->sweep(
                trim((string) ($options['owner'] ?? '')) ?: null,
                true,
                $subcommand === 'run',
            ));
        } finally {
    ...
    private static function intelligence(array $arguments): int
    {
        ...
        [$config, $store] = self::open($options);
        try {
            self::writeJson((new ToolService(
                $store,
                $config,
                new Analyzer($store, $config),
            ))->call($tool, $payload));
...
Commands:
  ...
  learningctl archive session --id SESSION_ID
  learningctl maintenance [--owner NAME]
  learningctl index-gc dry-run|run [--owner NAME]
  ...
  learningd once|drain|maintenance|run [--config PATH] [--data-dir PATH]
