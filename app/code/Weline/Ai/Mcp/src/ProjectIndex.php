...
final class ProjectIndex
{
    ...
    public const OWNER_MANIFEST = '.weline-index-owner.json';
    public const LEASE_FILE = '.weline-index-lease.lock';
    public const OWNER_SCHEMA = 'weline-index-owner.v1';
    public const OWNER_NAME = 'weline-project-intelligence';

    private string $root;
    private string $projectId;
    private string $projectDirectory;
    private string $databasePath;
    private SessionIdentity $ownershipIdentity;
    private ?PDO $database = null;
    private mixed $leaseHandle = null;
    ...
    public function __construct(private readonly Config $config, array $resolved)
    {
        ...
        $this->acquireLease();
        $this->assertExistingOwnershipManifest();
    ...
    public function close(): void
    {
        ...
        $this->database = null;
        if (is_resource($this->leaseHandle)) {
            try {
                $this->writeOwnershipManifest();
            } finally {
                flock($this->leaseHandle, LOCK_UN);
                fclose($this->leaseHandle);
            }
        }
    ...
    private function acquireLease(): void
    {
        $path = $this->projectDirectory . DIRECTORY_SEPARATOR . self::LEASE_FILE;
        $handle = fopen($path, 'c+');
        if (!is_resource($handle) || !flock($handle, LOCK_SH)) {
            if (is_resource($handle)) {
                fclose($handle);
            }
            throw new RuntimeException('Unable to acquire project index generation lease');
        }
        @chmod($path, 0600);
        $this->leaseHandle = $handle;
    }

    private function assertExistingOwnershipManifest(): void
    {
        $path = $this->projectDirectory . DIRECTORY_SEPARATOR . self::OWNER_MANIFEST;
        if (!file_exists($path)) {
            return;
        }
        if (!is_file($path) || is_link($path)) {
            throw new RuntimeException('Project index ownership manifest is unsafe');
        }
        try {
            $manifest = Json::decode((string) file_get_contents($path), []);
        } catch (Throwable $exception) {
            throw new RuntimeException('Project index ownership manifest is unreadable', 0, $exception);
        }
        if (!is_array($manifest) || !$this->validOwnershipManifest($manifest)) {
            throw new RuntimeException('Project index ownership manifest is invalid');
        }
    }

    private function writeOwnershipManifest(): void
    {
        if (!is_resource($this->leaseHandle)) {
            return;
        }
        ...
        if (is_file($path) && !is_link($path)) {
            ...
            if (is_string($existing)) {
                try {
                    $manifest = Json::decode($existing, []);
                } catch (Throwable) {
                    $manifest = [];
                }
                if (is_array($manifest)) {
                    if (!$this->validOwnershipManifest($manifest)) {
                        throw new RuntimeException('Project index ownership manifest is invalid');
                    }
                    $createdAt = (string) ($manifest['created_at'] ?? $createdAt);
                }
            }
        }
        $manifest = [
            'schema_version' => self::OWNER_SCHEMA,
            'owner' => self::OWNER_NAME,
            'generation' => basename($this->projectDirectory),
            'project_id_hash' => hash('sha256', $this->projectId),
            'repository_hash' => hash('sha256', $this->root),
        ...
        if (file_put_contents($temporary, $encoded, LOCK_EX) !== strlen($encoded)) {
            @unlink($temporary);
            throw new RuntimeException('Unable to write project index ownership manifest');
        }
        @chmod($temporary, 0600);
        if (!rename($temporary, $path)) {
            @unlink($temporary);
            throw new RuntimeException('Unable to publish project index ownership manifest');
        }
        @chmod($path, 0600);
    }

    /** @param array<string, mixed> $manifest */
    private function validOwnershipManifest(array $manifest): bool
    {
        $signature = trim((string) ($manifest['signature'] ?? ''));

        return ($manifest['schema_version'] ?? '') === self::OWNER_SCHEMA
            && ($manifest['owner'] ?? '') === self::OWNER_NAME
            && ($manifest['generation'] ?? '') === basename($this->projectDirectory)
    ...
    private function enforcePrivateFiles(): void
    {
        foreach ([
            ...
            $this->projectDirectory . DIRECTORY_SEPARATOR . self::OWNER_MANIFEST,
            $this->projectDirectory . DIRECTORY_SEPARATOR . self::LEASE_FILE,
        ] as $file) {
