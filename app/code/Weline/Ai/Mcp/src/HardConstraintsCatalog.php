...
final class HardConstraintsCatalog
{
    ...
    public static function mcpInstructions(): string
    {
        return 'MCP ROLE (knowledge plane + mandatory hard-rule gate): indexes skills/code maps/hard rules. '
            ...
            . 'ecommerce_advisor_for_commerce: 电商相关须 Team:电商顾问:（运营策划禁写码；领域决策「要开发什么」后 escalate 通知 PM；合规面含政策页/顶栏/FAQ Hub·实体；改可见串 suggested_seats 须含翻译工程师；先解析已支持国家再分国联网查政策；日常联网研究运营；讨论必查；合并原合规）. '
    ...
    public static function rules(): array
    {
        return [
            ...
            [
                ...
                'summary' => 'MANDATORY PM charter for engineering-team (and 监工 equivalent): 项目经理 is sole SESSION bookkeeper and plan-lifecycle owner—NOT code author, NOT multi-seat roleplay, NOT a substitute for architecture/specialty合规/code review. Lifecycle: review plan gaps → staff seats → monitor → on every seat closed|escalate|waiting_peer require notify_pm=true + @项目经理：本席已交付/上报，请检查并更新 SESSION → PM same-turn DoD check (deliverable paths exist; related_web_urls/evidence when required; frozen UC intent untouched; owner matches contracts; FORBID closing plan_id before test) → update SESSION → wake deps (deps-satisfied seats MAY run in parallel; SESSION bookkeeping MUST NOT serialize the whole team) → wait Tester pass notify → PM recheck acceptance process → close plan_id only then. Findings (e.g. performance) MUST open SESSION child plan_id and stay open until build+test+PM recheck. Rework: record on SESSION then restaff—loop until clean. Report completion to user ONLY when all plan_ids closed + triggered specialty reviews pass + gaps 「无」 + 汇审 pass. Complements requirement_session_dashboard, findings_wake_pm, ui_prototype_gate_before_test.',
                'doc' => 'dev/ai-command/ai/工程团队.md',
            ],
            [
