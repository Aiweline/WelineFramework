...
SH;
        ...
        return [
            ...
            'applies_to' => [
                ...
                'cursor_and_non_codex_hosts_when_user_opts_in_and_cli_available',
            ],
            'exemptions' => [
                'content_ops_skills_skip_mcp',
                'pure_chat_identity_concept_qa',
            ],
            // Default OFF: CLI presence alone MUST NOT auto-delegate. User must opt in this turn.
            'enabled_when' => 'user_explicitly_mentions_codex_and_codex_cli_available_and_host_is_not_codex',
            'opt_in' => [
                'required' => true,
                'gate' => 'user_message_mentions_codex',
                'forbid_auto_delegate_on_cli_presence_alone' => true,
                'default_when_not_mentioned' => 'host_self_explore_plan_review_via_host_plan_mode_for_planning',
                'trigger_tokens' => [
                    'Codex',
                    'codex',
                    'Codex CLI',
                ...
                'rationale' => 'Codex CLI delegation is opt-in only. If the user did not mention Codex this turn, the host MUST explore/plan/review itself—never launch `codex` just because the binary exists.',
            ],
            ...
            'plan_mode_relationship' => [
                'when_not_opted_in' => 'host_authors_plan_via_host_plan_mode_for_planning',
                'when_opted_in' => [
    ...
    public static function mcpInstructions(): string
    {
        return 'MCP ROLE (knowledge plane + mandatory hard-rule gate): indexes skills/code maps/hard rules. '
            ...
            . 'module_doc_forbids_ephemeral_work_artifacts: one-off SESSION/team/scratch/ops notes MUST go under repo-root dev/ (dev/session, dev/team, dev/tmp)—FORBID writing them into app/code/*/doc/. '
            ...
            . 'host_codex_delegation: OPT-IN only—forbid `codex` unless user mentions Codex this turn; '
            . 'when opted-in + CLI→Codex explore/plan/review (no -m; ≠knowledge.codex.enabled); '
            . 'MUST announce Codex 正在工作 before/while CLI runs (forbid silent); '
            . 'Cursor codes only after Codex plan; Codex host no nested codex; see agent_guidance.host_codex_delegation. '
    ...
    public static function rules(): array
    {
        return [
            ...
            [
                ...
                'summary' => 'MANDATORY for every executable coding/engineering requirement (监工 and team): at kickoff create/maintain repository-root dev/session/{feature-slug}.md (same slug as owning-module doc/开发/spec/{slug}.md; template dev/ai-command/ai/templates/requirement-session.md). SESSION is the single dashboard for wave/phase, plan_id lifecycle table, incomplete gaps, review index, delivery-notify log, and related_web_urls—dev/team/{slug}/ meetings remain detail files. FORBID placing SESSION under any module doc/ (module_doc_forbids_ephemeral_work_artifacts). ONLY 项目经理 (or 监工 parent) may edit SESSION; specialty seats FORBID editing SESSION or closing plan_ids. No SESSION → FORBID construction code edits. SESSION 未完成清单 non-empty (not 「无」) → FORBID claiming done / user completion report. Complements pm_plan_lifecycle, engineering_team_for_new_requirements, closeout_requires_huishen.',
                'doc' => 'dev/ai-command/ai/工程团队.md',
            ],
            [
    ...
    public static function mcpOperationalRules(): array
    {
        return [
            ...
            [
                'id' => 'module_doc_forbids_ephemeral_work_artifacts',
                'summary' => 'MANDATORY: Module `app/code/*/doc/` is for durable module knowledge ONLY—三文档契约 (`需求.md`/`功能现状.md`/`开发日志.md`), README, lasting guides/API/architecture docs, `doc/ai/skills/**`, durable feature specs at owning-module `doc/开发/spec/{slug}.md`, and lasting charters already published as product knowledge. FORBIDDEN: writing one-off / temporary operational work materials into any module `doc/` (including `doc/开发/session/**`, `doc/开发/team/**` wave folders, scratch notes, one-off ops briefs, investigation dumps, temporary acceptance/channel logs, disposable meeting dumps). Those MUST go under repository-root `dev/`: SESSION=`dev/session/{slug}.md`; team minutes=`dev/team/{slug}/` (roster.md, channel/, meetings/, surfaces/contracts/deps, stop-work.md); throwaways=`dev/tmp/`. Historical files already under module `doc/开发/team|session` may remain read-only for continuity—NEW writes MUST use `dev/`. Complements requirement_session_dashboard / engineering_team_for_new_requirements path contracts; do not “archive” temp ops into module doc to make them look durable.',
            ],
        ];
    }
}
