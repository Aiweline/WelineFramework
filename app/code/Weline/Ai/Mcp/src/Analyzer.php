...
final class Analyzer
{
    ...
    public function processJob(array $job): array
    {
        return match ($job['job_type']) {
            'analyze_session' => $this->analyzeGenerationBoundSession($job),
            'review_feedback' => [
     ...
     *  @return array<string, mixed>
     */
    private function analyzeGenerationBoundSession(array $job): array
    {
        $sessionId = self::required($job, 'session_id');
        $session = $this->store->getSession($sessionId);
        $generation = (int) ($job['session_generation'] ?? 0);
        if ($session['lifecycle_state'] !== 'active'
            || $generation !== (int) $session['lifecycle_generation']) {
            throw new ToolException(
                'SESSION_GENERATION_MISMATCH',
                'Analysis job targets a stale or archived Session generation',
                false,
            );
        }
