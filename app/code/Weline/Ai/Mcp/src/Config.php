...
final class Config
{
    ...
    private function __construct(
        ...
        private readonly array $runtimeMigrations = [],
    ) {
    }
    ...
    public static function load(?string $path = null, ?string $dataDirOverride = null): self
    {
        ...
        if ((bool) ($values['knowledge']['auto_generate_skills'] ?? false)
            ...
            || (bool) ($values['knowledge']['learning_skills']['enabled'] ?? false)
            || (bool) ($values['knowledge']['learning_skills']['inject_on_prompt'] ?? false)
            || (is_string($environmentSkillOutputDirectory) && trim($environmentSkillOutputDirectory) !== '')
        ) {
            $runtimeMigrations[] = 'knowledge.repository-projections:retired-in-0.13.0';
        }
        $values['knowledge']['auto_generate_skills'] = false;
        $values['knowledge']['auto_doc_sync'] = false;
        $values['knowledge']['learning_skills']['enabled'] = false;
        $values['knowledge']['learning_skills']['inject_on_prompt'] = false;
        $values['knowledge']['learning_skills']['output_directory'] = '';
        $values['knowledge']['learning_skills']['max_module_skill_projections'] = 0;
        ...
        return new self($values, $sourcePath, $runtimeMigrations);
    }
    ...
    /** @return list<string> */
    public function runtimeMigrations(): array
    {
        return $this->runtimeMigrations;
    }
    ...
    private static function validate(array $values): void
    {
        ...
        foreach ([
            ...
            'privacy.sweep_batch_size' => [1, 1_000],
