...
final class GuidanceWorkflowCatalog
{
        ...
        return [
            ...
            'norms' => [
                ...
                [
                    ...
                    'summary' => '同模块：仅本模块可 <w:widget> 内嵌且禁同部件 JSON 双路径；跨模块：外国部件只能空槽+拥有模块 JSON 默认注入；Theme 仅可内嵌 Weline_Theme；部件施工派部件开发工程师',
                    'detail_doc' => 'app/code/Weline/Theme/doc/开发/Theme开发总指南.md',
    ...
    public static function engineeringTeamSurface(): array
    {
        return [
            ...
            'verification_commands' => [
                ...
                'rg -n "one_seat_one_agent|peer_talk_via_channel|dual_track_all|acceptance-ui|ui_prototype_gate_before_test|component-negotiate|framework_first|seat_skill_mirrors|findings_wake_pm|requirement_session_dashboard|pm_plan_lifecycle|notify_pm|API|部件开发工程师|主题开发工程师|支付开发工程师|电商顾问|数据分析|性能检查工程师|提示词优化工程师" dev/ai-command/ai/工程团队.md',
    ...
    public static function themeDevelopmentSurface(): array
    {
        return [
            ...
            'label' => '主题开发专席（薄）',
            'description' => '工程团队「主题开发工程师」专席薄表面：仅专席双轨、路径归属、work_mode 门禁与前后端/部件边界；权威技能正文复用 frontend_development / weline-theme-development，禁止复制 frontend 全文。',
            'triggers' => [
            ...
            'verification_commands' => [
                ...
                'rg -n "theme_binding|theme:disk:compile|weline-code|semantic" dev/ai-command/ai/主题开发.md',
                'php bin/w resource:compile welineModules',
            ],
            'template_surface_rules' => [
                'forbidden' => [
    ...
    public static function widgetDevelopmentSurface(): array
    {
        return [
            ...
            'label' => '部件开发规范',
            'description' => '工程团队「部件开发工程师」专席：Widget 注册、模板、@widget/@param、空槽、default_injections、placement；硬规则本模块才可布局标签内嵌、跨模块只走 JSON 注入、禁止 JSON+布局双路径重复渲染。',
            'triggers' => [
            ...
            'norms' => [
                ...
                ['id' => 'widget_js_weline_modules', 'summary' => '部件禁止带 <?= 的内联 script；JS 走 weline.modules.js + data-weline-load；改登记后 resource:compile welineModules'],
            ...
            'template_surface_rules' => [
                'forbidden' => [
                    ...
                    'Widget templates with inline <script> containing <?=',
                ],
                'required' => [
    ...
    public static function performanceCheckSurface(): array
    {
        return [
            ...
            'description' => '工程团队「性能检查工程师」专席：必须检查性能。须深懂 Weline 框架结构与业务特性；审查 HotCache/CachePolicy 缓存设计是否合规；与架构师共同讨论并定制优化方向；开发后用真实证据复审。HARD：未弄清框架+业务特性禁止开药方；建议必须落在框架机制内；默认可写只读探针，业务返工交归属席；热路径/缓存相关复杂 team 必须上场。',
            'triggers' => [
