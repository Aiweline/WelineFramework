...
final class GuidanceWorkflowCatalog
{
        ...
        return [
            ...
            'phases' => [
                ...
                ['id' => 'engineering_team', 'label' => '工程团队编制与停工门禁', 'tools' => ['get_skill'], 'docs' => [
                    ...
                    'Parent is 项目经理 (switchboard). Framework first + dual_track_all: each triggered specialty seat has 施工 + 合规复审 (事件/扩展点/Taglib/UI/i18n…). Fail → rework before acceptance.',
                    ...
                    'UI in_scope: staff 原型+前端+主题+UI; components.md; insufficient → 原型∥UI real peer talk → component-negotiate.md. Acceptance: UI+原型 substantive signoff (acceptance-ui/acceptance-prototype)—e2e green does not waive 交出.',
                    ...
                    'Minutes: owning-module doc/开发/team/{slug}/ (roster.md, channel/, surfaces.md, components.md, contracts.md, deps.md, align-freeze, {seat}-review). Subagent closed is not delivery. get_skill(engineering_team|weline-engineering-team).',
                ]],
        ...
        return [
            ...
            'norms' => [
                ...
                [
                    ...
                    'summary' => '同模块：布局已 <w:widget> 内嵌则禁同部件 JSON default_injections；跨模块：禁止布局互调部件，外国部件只能空槽+拥有模块 JSON 默认注入；Theme 布局仅可内嵌 Weline_Theme',
                    'detail_doc' => 'app/code/Weline/Theme/doc/开发/Theme开发总指南.md',
    ...
    public static function engineeringTeamSurface(): array
    {
        return [
            ...
            'label' => '工程团队',
            'description' => '父会话自己选模式：简单用监工（监工:）；复杂进入 team——父会话仅 Team:项目经理:，每席必须真实子智能体（one_seat_one_agent），席间经 channel/{thread}.md + resume 互聊（peer_talk_via_channel）。禁止父会话扮演多席。框架优先、全专席双轨、组件协商、验收UI+原型签收。内容运营两种前缀都不用。',
            'triggers' => [
            ...
            'norms' => [
                ...
                ['id' => 'component_reuse_or_negotiate', 'summary' => 'UI优先复用组件；不足须原型∥UI（±主题）真实互聊写入 component-negotiate.md'],
                ...
                ['id' => 'persist_team_minutes', 'summary' => '纪要落盘 owning-module doc/开发/team/{slug}/（含 roster+channel）；子智能体 closed 不是交付'],
            ],
            'verification_commands' => [
                ...
                'rg -n "one_seat_one_agent|peer_talk_via_channel|dual_track_all|acceptance-ui|component-negotiate|framework_first" dev/ai-command/ai/工程团队.md',
            ],
            'template_surface_rules' => [
                'forbidden' => [
                ...
                'required' => [
                    ...
                    'Parent session acts as 项目经理 only; launch one real subagent per staffed seat; paste prompt skeleton; register agent_id in roster.md',
