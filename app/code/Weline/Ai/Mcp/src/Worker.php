...
final class Worker
{
    ...
    {
        $maintenance = $scanIdleSessions
            ? (new SessionLifecycleService($this->store, $this->analyzer, $this->config))->sweep(null, false)
            : ['maintenance_due' => false, 'sessions_purged' => 0];
        ...
        try {
            ...
            if (!$this->store->completeJob((string) $job['id'], $result)) {
                return [
                    'processed' => false,
                    'cancelled' => true,
                    'job_id' => $job['id'],
            ...
            if (!$this->store->failJob($job, $exception, $retryable, $maximumAttempts)) {
                return [
                    'processed' => false,
                    'cancelled' => true,
                    'job_id' => $job['id'],
