...
HTML;
    ...
    private static function generateFormHtml(string $formId, string $model, string $scope, string $mode, string $title): string
    {
        ...
        $formHtml .= '</div>';
        $formHtml .= '<form class="w-form w-form-vertical" id="' . $formId . '" action="/datatable/api/form" method="POST" data-model="' . htmlspecialchars($model, ENT_QUOTES, 'UTF-8') . '" data-scope="' . htmlspecialchars($scope, ENT_QUOTES, 'UTF-8') . '" data-mode="' . htmlspecialchars($mode, ENT_QUOTES, 'UTF-8') . '" data-record-id="">';
