        ...
        return function ($tag_key, $config, $tag_data, $attributes) {
            ...
            if ($enableForm) {
                ...
                if ($hasNestedForm && empty($form)) {
                    ...
                    $formId = 'form-' . $id;
                    $formHtml = self::generateFormHtml($formId, $model, $scope, $formMode, $formTitle, $apiUrl, $fieldApiUrl);
                    $formJs = "'{$formId}'";
                }
            } else {
        ...
        var checkManager = setInterval(function() {
            if (typeof window.DataTableManager !== "undefined" && window.DataTableManager.initTable) {
                ...
                window.DataTableManager.initTable('#' + tableId, {
                    ...
                    apiUrl: resolveApiUrl('{$apiUrlJs}'),
                    fieldApiUrl: resolveApiUrl('{$fieldApiUrlJs}'),
...
HTML;
    ...
    // 生成表单HTML（简化版）
    private static function generateFormHtml(string $formId, string $model, string $scope, string $mode, string $title, string $apiUrl = 'datatable/rest/v1/data-table', string $fieldApiUrl = 'datatable/rest/v1/form/fields'): string
    {
                ...
                if (typeof DataTableFormManager !== "undefined" && DataTableFormManager._instance) {
                    ...
                    if (document.readyState === "loading") {
                        document.addEventListener("DOMContentLoaded", function() {
                            DataTableFormManager.initForm("' . $formId . '", {model: "' . $modelJs . '",scope: "' . $scope . '",mode: "' . $mode . '",recordId: "",autoFields: true,excludeFields: [],includeFields: [],apiUrl: "' . $apiUrlJs . '",fieldApiUrl: "' . $fieldApiUrlJs . '"});
                        });
                    } else {
                        DataTableFormManager.initForm("' . $formId . '", {model: "' . $modelJs . '",scope: "' . $scope . '",mode: "' . $mode . '",recordId: "",autoFields: true,excludeFields: [],includeFields: [],apiUrl: "' . $apiUrlJs . '",fieldApiUrl: "' . $fieldApiUrlJs . '"});
                    }
