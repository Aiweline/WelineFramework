...
final class Table implements TaglibInterface
{
    ...
    public static function callback(): callable
    {
        return static function ($tagKey, $config, $tagData, $attributes): string {
            ...
            try {
                ...
                $rootStyle = self::customPropertyStyle([
                    '--w-datatable-height' => self::cssLength((string)($attributes['height'] ?? '')),
                    '--w-datatable-width' => self::cssLength((string)($attributes['width'] ?? '')),
                ]);
                $uiConfig = [
                    'id' => $id,
                    'model' => $model,
    ...
    private static function renderTable(
        ...
        $styleHtml = $rootStyle === '' ? '' : ' style="' . self::escape($rootStyle) . '"';
        ...
        return <<<HTML
<section id="{$rootId}" class="w-datatable" data-w-component="data-table" data-w-config="{$configHtml}"{$styleHtml}>
    ...
    <div class="w-table-wrap w-datatable__viewport"><table class="{$classHtml}" id="{$idHtml}">{$content}</table></div>
</section>
...
HTML;
    ...
    {
        $result = [];
        foreach (array_filter(array_map('trim', preg_split('/[;,]+/', $joinConfig) ?: [])) as $condition) {
            if (preg_match('/^([A-Za-z][\w]*)\.([A-Za-z][\w]*)\s*=\s*([A-Za-z][\w]*)\.([A-Za-z][\w]*)$/', $condition, $match)) {
                $result[] = [
                    'left_alias' => $match[1],
                    'left_field' => $match[2],
                    'right_alias' => $match[3],
    ...
    private static function customPropertyStyle(array $properties): string
    {
        $parts = [];
