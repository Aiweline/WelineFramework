...
4. 本文件：`app/code/Weline/DataTable/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/DataTable/doc/README.md`
...
- 模块代码：`Weline_DataTable`
- 目录：`app/code/Weline/DataTable`
- Vendor：`Weline`
- Module：`DataTable`
...
入口/配置文件：
- `app/code/Weline/DataTable/etc/backend/menu.xml`
- `app/code/Weline/DataTable/composer.json`

- `Api`：公开接口契约。跨模块调用优先找已发布 Interface 或 QueryProvider，不要直接依赖对方内部 Service/Model。 文件数：6
- `Controller`：HTTP/后台/前台控制器入口。新增控制器后运行 setup:upgrade --route，同步路由。 文件数：5
- `Helper`：模块内辅助能力。跨模块不要直接调用未发布 Helper。 文件数：10
...
- `Taglib`：模板标签扩展。改前读 Weline_Taglib 与 Theme 文档。 文件数：7
...
- `view/statics`：静态资源源文件。浏览器业务请求必须走 Weline.Api.*。 文件数：5
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/DataTableQueryProvider.php；前端/跨模块读数据先查 query 帮助。
...
- `app/code/Weline/DataTable/doc/测试用例文档.md`
- `app/code/Weline/DataTable/doc/重要说明-路由配置.md`
- `app/code/Weline/DataTable/doc/需求文档.md`
