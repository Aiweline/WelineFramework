...
4. 本文件：`app/code/Weline/Cdn/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Cdn/doc/README.md`
...
入口/配置文件：
- `app/code/Weline/Cdn/etc/backend/menu.xml`
- `app/code/Weline/Cdn/.module_config.json`
- `app/code/Weline/Cdn/composer.json`

- `Api`：公开接口契约。跨模块调用优先找已发布 Interface 或 QueryProvider，不要直接依赖对方内部 Service/Model。 文件数：4
...
- 识别到 QueryProvider 相关 PHP 文件：Service/CdnAdminQueryService.php、Service/CdnRuleCollector.php、Test/Unit/Query/CdnQueryProviderDescriptorTest.php、extends/module/Weline_Framework/Query/CdnQueryProvider.php；前端/跨模块读数据先查 query 帮助。
...
- `app/code/Weline/Cdn/doc/event/CDN缓存清理.md`
- `app/code/Weline/Cdn/doc/event/CDN账户列表.md`
- `app/code/Weline/Cdn/doc/event/CDN预热URL投递.md`
- `app/code/Weline/Cdn/doc/hook/backend/partials/domain/toolbar-after.md`
- `app/code/Weline/Cdn/doc/功能现状.md`
- `app/code/Weline/Cdn/doc/开发日志.md`
- `app/code/Weline/Cdn/doc/控制器CDN注释规则需求文档.md`
- `app/code/Weline/Cdn/doc/需求.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Cdn> [operation]` 或对应 `w_query` 帮助。
