...
PHP;
            $html[] = '<style>';
            $html[] = '.weline-website-select{position:relative;width:100%;min-width:0}';
            $html[] = '.weline-website-trigger{display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;min-height:42px;padding:8px 12px;background:var(--backend-color-card-bg,#fff);border:1px solid var(--backend-color-border-default,#dbe3ef);border-radius:6px;color:var(--backend-color-text-primary,#162033);text-align:left;cursor:pointer}';
            $html[] = '.weline-website-trigger:hover,.weline-website-select.is-open .weline-website-trigger{border-color:var(--backend-color-primary,#556ee6);box-shadow:0 0 0 3px rgba(85,110,230,.18);outline:0}';
            ...
            $html[] = '.weline-website-empty{color:var(--backend-color-text-secondary,#64748b);font-size:13px}';
            $html[] = '.weline-website-tag{display:inline-flex;align-items:center;gap:6px;max-width:100%;padding:2px 8px;border-radius:999px;background:rgba(85,110,230,.1);color:#556ee6;font-size:12px;line-height:1.4;white-space:nowrap}';
            $html[] = '.weline-website-tag-id,.weline-website-item-id{flex:0 0 auto;color:#64748b;font-size:11px;font-weight:700;font-variant-numeric:tabular-nums;line-height:1.2;padding:1px 6px;border:1px solid #dbe3ef;border-radius:999px;background:#f8fafc}';
            $html[] = '.weline-website-tag-label{min-width:0;overflow:hidden;text-overflow:ellipsis}';
            $html[] = '.weline-website-tag-meta{color:#64748b;font-size:11px;padding:1px 6px;border:1px solid #dbe3ef;border-radius:999px;background:#f8fafc}';
            $html[] = '.weline-website-tag-remove{margin-left:2px;cursor:pointer;opacity:.75;font-size:14px;line-height:1}';
            $html[] = '.weline-website-tag-remove:hover{opacity:1;color:#dc3545}';
            $html[] = '.weline-website-actions{display:inline-flex;align-items:center;gap:4px;flex:0 0 auto}';
            $html[] = '.weline-website-clear{border:0;background:transparent;color:#94a3b8;cursor:pointer;font-size:16px;line-height:1;padding:0}';
            $html[] = '.weline-website-clear:hover{color:#ef4444}';
            $html[] = '.weline-website-chevron{color:#64748b;font-size:18px;line-height:1;flex:0 0 auto}';
            $html[] = '.weline-website-dropdown{display:none;padding:8px;background:var(--backend-color-card-bg,#fff);border:1px solid #dbe3ef;border-radius:10px;box-shadow:0 16px 36px rgba(15,23,42,.16);box-sizing:border-box;overflow:hidden}';
            $html[] = '.weline-website-search{display:block;width:100%;min-height:36px;padding:6px 10px;margin:0 0 7px;border:1px solid #dbe3ef;border-radius:6px;background:#f8fafc;color:#162033;box-sizing:border-box;flex:0 0 auto}';
            $html[] = '.weline-website-list{display:block;flex:1 1 auto;min-height:0;height:280px;max-height:280px;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;touch-action:pan-y}';
            $html[] = '.weline-website-item{display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;padding:8px 10px;border:0;border-radius:7px;background:transparent;color:#162033;text-align:left;cursor:pointer}';
            $html[] = '.weline-website-item:hover,.weline-website-item.is-selected{background:#f1f5f9;outline:0}';
            ...
            $html[] = '.weline-website-item-meta{color:#64748b;font-size:12px;white-space:nowrap}';
            $html[] = '.weline-website-check{color:#556ee6;font-weight:700;opacity:0}';
            $html[] = '.weline-website-item.is-selected .weline-website-check{opacity:1}';
            $html[] = '.weline-website-empty-state{padding:7px 10px;border-radius:7px;text-align:center;color:#64748b}';
            $html[] = '</style>';
            ...
            $html[] = '        panel.hidden = false; panel.style.display = "block"; panel.style.position = "fixed"; panel.style.zIndex = String(cfg.zIndex || 4000); panel.style.left = "0px"; panel.style.top = "0px";';
            $html[] = '        var anchorW = Math.round(anchor.getBoundingClientRect().width); var minW = Math.max(cfg.minWidth || 0, anchorW); panel.style.minWidth = minW + "px"; panel.style.maxWidth = cfg.maxWidth ? cfg.maxWidth + "px" : "calc(100vw - 16px)";';
            ...
            $html[] = '    ? value.map(function(v){ return normalizeSelectedValue(v); }).filter(Boolean)';
            $html[] = '    : String(value || "").split(",").map(function(v){ return normalizeSelectedValue(v); }).filter(Boolean);';
            ...
            $html[] = '  window.WelineSmartDropdown.mount(trigger, dropdown, { minWidth: 280, maxWidth: 420, preferredHeight: 320, zIndex: 4200, gap: 6 });';
            ...
            $html[] = '  });';
            $html[] = '}';
            $html[] = 'search.addEventListener("input", function(){ renderList(search.value || ""); ensureListScrollable(); });';
