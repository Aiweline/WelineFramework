...
class RegistrarSelect implements TaglibInterface
{
    ...
    public static function callback(): callable
    {
        return function ($tag_key, $config, $tag_data, $attributes) {
            ...
            $html[] = '.weline-registrar-dropdown{position:absolute;left:0;right:0;top:calc(100% + 4px);z-index:1080;padding:.75rem;background:var(--backend-color-card-bg,#fff);border:1px solid var(--backend-color-border-default,#dee2e6);border-radius:var(--backend-border-radius-md,.375rem);box-shadow:var(--backend-shadow-lg,0 .5rem 1rem rgba(0,0,0,.15))}';
            ...
            $html[] = '"use strict";';
            $html[] = 'var id = <?= json_encode($attributes["id"], JSON_THROW_ON_ERROR) ?>;';
            ...
            $html[] = '      } else { selected = [value]; dropdown.style.display = "none"; }';
            ...
            $html[] = 'trigger.addEventListener("click", function(e){ e.stopPropagation(); dropdown.style.display = dropdown.style.display === "none" ? "block" : "none"; if (dropdown.style.display !== "none") { search.focus(); renderList(search.value || ""); } });';
            $html[] = 'search.addEventListener("input", function(){ renderList(search.value || ""); });';
            $html[] = 'document.addEventListener("click", function(e){ if (!dropdown.contains(e.target) && !trigger.contains(e.target)) { dropdown.style.display = "none"; } });';
