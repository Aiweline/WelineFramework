...
class WebsiteForm implements TaglibInterface
{
    ...
    public static function callback(): callable
    {
        return function ($tag_key, $config, $tag_data, $attributes) {
            ...
            $selectedPoolIds = $attributes['selected_pool_ids'] ?? '[]';
            ...
            $html[] = '            id="<?= htmlspecialchars($Taglib__id) ?>_domain_select"';
            $html[] = '            name="pool_ids"';
            $html[] = '            value="<?= htmlspecialchars($selectedPoolIds) ?>"';
            $html[] = '            display="\'' . addslashes(__('点击选择域名（可多选）')) . '\'"';
            $html[] = '            class="w-100"';
            $html[] = '            multiple="true"';
