...
| **ACME `addAcmeTxtRecord` 门闸** | — | **`DomainResolveService::validateAcmeDns01HostingViaAdapters`** | 写 TXT 前：仅用 **注册商 + DNS 托管适配器 API** 判断是否可写。**写 TXT 之后** 的「等广播」在 **`SslCertificateService::performDns01Challenge`**：默认循环 **`dns_get_record`(TXT)**（时长见 **`websites.acme_dns.txt_poll_*`**，可选 `txt_visible_use_public_doh`）。**`DnsSwitchService`** 仍用 **`validateAuthoritativeDnsMatchesProvider`**。 |
