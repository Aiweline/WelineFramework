...
final class ProvisioningQueryHandler
{
    ...
    public function execute(string $operation, array $params = []): mixed
    {
        return match ($operation) {
            ...
            default => throw new \InvalidArgumentException(
                (string)__('Websites 配置查询不支持的操作：%{1}', $operation)
            ),
        };
    }

    /** @return list<array<string, mixed>> */
    public function getDescriptorOperations(): array
    {
        return [
            [
            ...
            [
                ...
                'params' => [['name' => 'order_id', 'type' => 'int', 'required' => true, 'description' => __('订单 ID')]],
            ],
            [
                'name' => 'getOrderByDomain',
                'description' => __('根据域名获取配置订单'),
                'params' => [['name' => 'domain', 'type' => 'string', 'required' => true, 'description' => __('域名')]],
            ],
            [
                'name' => 'getDomainLifecycleStatus',
                'description' => __('按根域名获取生命周期状态详情'),
                'params' => [['name' => 'domain', 'type' => 'string', 'required' => true, 'description' => __('根域名')]],
            ],
            [
                'name' => 'getOrders',
    ...
    private function startProvisioning(array $params): array
    {
        ...
        $options = [
            ...
            'skip_purchase' => !empty($params['skip_purchase']) || !empty($params['domain_owned']),
        ];
