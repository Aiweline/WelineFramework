...
const CANDIDATE_ROUTES = ["code","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Code 后台流程', () => {
  moduleCase(
    test,
    { module: MODULE, id: 'CODE-SMOKE-001' },
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'CODE-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
