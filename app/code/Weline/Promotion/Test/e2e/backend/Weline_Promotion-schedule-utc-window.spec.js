/**
 * Promotion theme form shows website-TZ hint and starts_at/ends_at fields.
 * Uses UI login against the live WLS host (avoids broken FPM session bootstrap).
 *
 * @weline-e2e-spec { module: Weline_Promotion, type: flow, layer: backend }
 */
...
test.describe('Promotion schedule UTC window @backend', () => {
  test('theme form exposes timezone-aware starts_at/ends_at', async ({ page }) => {
    ...
    await page.goto(`${ORIGIN}/${PREFIX}/promotion/backend/theme/form`, {
    ...
    const start = page.locator('[data-testid="promotion-theme-starts-at"]');
    const end = page.locator('[data-testid="promotion-theme-ends-at"]');
