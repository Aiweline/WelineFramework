 ...
 * 主题配置控制?- 卡片式界?
 * 
 * 不使?meta-manager 标签，直接根据元数据渲染卡片式配置界?
 */
class ThemeConfig extends BackendController
{
    private const DEFAULT_SCOPE = 'default';

    private function getPreviewThemeScopeService(): PreviewThemeScopeService
    {
        return ObjectManager::getInstance(PreviewThemeScopeService::class);
    }
    ...
    public function getOptions()
    {
        ...
        if (!$theme->getId()) {
            return $this->fetchJson(['code' => 404, 'msg' => __('Theme not found')]);
        }
    ...
    public function getFileParams()
    {
        ...
        $category = $this->request->getParam('category'); // default, account, header ?
        $value = $this->request->getParam('value'); // 文件名（不含扩展名）
        
        if (!$themeId || !$type || !$category) {
            return $this->fetchJson(['code' => 400, 'msg' => __('Missing required parameters')]);
        }
    ...
    public function postSaveFileParams()
    {
        ...
        try {
            ...
            if ($action === 'reset') {
                ...
                if (!$paramKey) {
                    return $this->fetchJson(['code' => 400, 'msg' => __('Missing parameter key for reset action')]);
                }
            ...
            foreach ($params as $paramKey => $paramValue) {
                // 如果参数包含 translatable 定义，需要处?i18n
                // 这里 setParamValues 内部会自动判?translatable
            ...
            return $this->fetchJson(['code' => 500, 'msg' => __('Save failed: ') . $e->getMessage()]);
        }
    }
    ...
    public function postSaveSelection()
    {
        ...
        $category = $this->request->getPost('category'); // default, account, header ?
        ...
        try {
            ...
            // 对于 layouts ?partials，category 是必需?
            if (in_array($typeForStorage, ['layouts', 'partials'])) {
                ...
                // colors ?variables 不需?category
                $configKey = "{$typeForStorage}.value";
                $baseIdentify = "theme.{$area}.{$typeForStorage}";
            }
            
            // 直接使用 ThemeData::set() 方法保存，它会自动处理所有逻辑
            // ThemeData::set() 内部会使?MetaData::set()，最终调?MetaConfig 保存
            // identify 格式：{type}.{category}.value ?{type}.value
            $identify = $configKey; // 例如：partials.header.value ?colors.value
            
            $success = ThemeData::set($identify, (string)$value, $scope);
            
            if (!$success) {
                // 如果 ThemeData::set() 失败，尝试直接使?MetaConfig::setConfig()
                try {
                    // 尝试查找对应?Meta 记录以获?meta_id
                    $metaId = null;
                    $metaIdentify = null;
                    
                    try {
                        // 构建 Meta 表中?meta_identify（不包含 theme.{area} 前缀?
                        if (in_array($typeForStorage, ['layouts', 'partials'])) {
                        ...
                        $meta = $this->metadataRepository()->search(new MetadataSearch(
                            namespace: 'theme',
                            type: $typeForStorage,
                            identify: $metaIdentifyForQuery,
                            area: $area,
                        ))[0] ?? null;

                        if ($meta !== null) {
                            $metaId = $meta->id;
                            $metaIdentify = $meta->identify;
                        } else {
    ...
    private function formatLayoutsWithMeta(WelineTheme $theme, string $area, array $layouts): array
    {
        ...
        foreach ($layouts as $layoutType => $options) {
            ...
            foreach ($options as $option) {
                ...
                // 如果 meta 中没?name，尝试从 ThemeData 获取
                if (empty($meta['name'])) {
    ...
    public function postPublishPreviewScope()
    {
        $themeId = (int)$this->request->getPost('theme_id', 0);
        $area = (string)$this->request->getPost('area', 'frontend');
        $scope = (string)$this->request->getPost('scope', self::DEFAULT_SCOPE);

        if ($themeId <= 0) {
            return $this->fetchJson(['code' => 400, 'msg' => __('Please select a theme')]);
        }
        ...
        try {
            $result = $this->getPreviewThemeScopeService()->publishPreviewScope($themeId, $area, $scope);
            ThemeData::clearCache();

            return $this->fetchJson([
                'code' => 200,
                'msg' => __('Preview config published to formal scope'),
                'data' => $result,
            ]);
        } catch (\Throwable $throwable) {
            return $this->fetchJson([
                'code' => 500,
                'msg' => __('Publish failed: ') . $throwable->getMessage(),
            ]);
        }
    }

    public function postDiscardPreviewScope()
    {
        $themeId = (int)$this->request->getPost('theme_id', 0);
        $area = (string)$this->request->getPost('area', 'frontend');
        $scope = (string)$this->request->getPost('scope', self::DEFAULT_SCOPE);
        ...
        try {
            $result = $this->getPreviewThemeScopeService()->discardPreviewScope($themeId, $area, $scope);
            ThemeData::clearCache();

            return $this->fetchJson([
                'code' => 200,
                'msg' => __('Preview config discarded'),
                'data' => $result,
            ]);
        } catch (\Throwable $throwable) {
            return $this->fetchJson([
                'code' => 500,
                'msg' => __('Discard failed: ') . $throwable->getMessage(),
            ]);
        }
    }
}
