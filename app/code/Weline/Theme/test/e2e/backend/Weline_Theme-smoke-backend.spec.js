/**
 * Weline_Theme：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
...
const CANDIDATE_ROUTES = ["themeEditor","themeeditor","theme_editor","index","config/themeConfig","config/themeconfig","config/theme_config","config/variables","config/layout","config/partials","widget/paramRender/form","widget/paramrender/form","widget/param_render/form","config/meta/configData","config/component/saveParams","builder/schema","virtualTheme/manifest","virtualtheme/manifest","virtual_theme/manifest","ai/agents","theme","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Theme 后台流程', () => {
  moduleCase(
    test,
    { module: MODULE, id: 'THEME-SMOKE-001' },
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'THEME-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
