...
async function callEditorRequest(page, url, method = 'GET', body = null) {
  return page.evaluate(async (rawInput) => {
    // The editor dispatches same-origin editor URLs directly, so a raw path that
    // misses the randomized backend prefix would 404 instead of reaching the controller.
