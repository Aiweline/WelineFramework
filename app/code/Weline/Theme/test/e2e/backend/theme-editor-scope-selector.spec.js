...
async function expectAuthorizedScopedPreview(page) {
  ...
  await expect.poll(
    ...
    { timeout: 60000, message: 'frontend editor canvas must load storefront path with editor_mode (path=layout)' },
  ).toMatch(/editor_mode=1/);
  await expect.poll(
    () => preview.getAttribute('src'),
    { timeout: 60000, message: 'editor canvas must not use theme-preview/content as storefront path' },
  ).not.toContain('theme/frontend/theme-preview/content');
  await expect.poll(
    () => preview.getAttribute('src'),
  ...
  await page.waitForFunction(() => {
    ...
    if (!frame.src.includes('editor_mode=1')) return false;
  ...
  expect(report.src).not.toContain('theme/frontend/theme-preview/content');
