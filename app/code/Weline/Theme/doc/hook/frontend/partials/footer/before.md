...
本文档详细说明了 Weline Theme 模块提供的 `Weline_Theme::frontend::partials::footer::before` Hook 及其使用方法。该 Hook 允许其他模块在页脚组件渲染之前注入内容。

## Hook 信息

### 基本信息

- **Hook 名称**：`Weline_Theme::frontend::partials::footer::before`
- **显示名称**：页脚之前
- **Hook 类型**：标准格式 Hook
- **定义模块**：Weline_Theme
- **区域（Area）**：frontend
...
在渲染页脚组件之前触发，允许其他模块在页脚开始处注入内容。

### 触发时机

该 Hook 在以下时机触发：
- **触发位置**：主题模板文件中，具体位置取决于布局和组件
- **触发条件**：当主题渲染页脚组件之前自动触发
...
例如，对于 Hook `Weline_Theme::frontend::partials::footer::before`，文件名应为：
```
view/hooks/Weline_Theme--frontend--partials--footer--before.phtml
```
...
```phtml
<!-- view/hooks/Weline_Theme--frontend--partials--footer--before.phtml -->
<div class="custom-footer-before-content">
...
- **初始版本**：添加 `Weline_Theme::frontend::partials::footer::before` Hook 文档
