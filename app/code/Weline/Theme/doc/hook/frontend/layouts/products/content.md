...
- **Hook 名称**：`Weline_Theme::frontend::layouts::products::content`
- **显示名称**：产品列表布局内容
- **功能说明**：覆盖产品列表布局的主内容区域，允许商品模块在保留默认页头、页脚和布局资源的同时提供完整商品列表内容。

## 使用方法

在模块的 `view/hooks/` 目录下创建文件：`view/hooks/Weline_Theme/frontend/layouts/products/content.phtml`

## 边界

- 该 Hook 位于 `products/default.phtml` 的默认内容 slot 外层。
