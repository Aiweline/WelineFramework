...
404 布局须包含 `Weline_Theme::frontend::layouts::base::body-end`，以便注入 `Weline_Cart` 的 `product-purchase-actions.js` 与 `Weline_Theme` 的 `storefront-shopper-toast.js`；推荐卡片「加入购物车」须使用 Cart 契约（`weline-cart-product-card-add-to-cart` + `data-action="add"` + `data-global-offer-uuid`）。
