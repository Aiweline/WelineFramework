...
Theme 对外提供 `w_query('theme', 'copyTargetLayoutData', ...)`，用于 CMS 等业务模块在复制业务对象时同步 Theme-owned 数据。调用方传入源/目标 `target_type + target_id`、`layout_type`、`layout_option` 和 `scope`；Theme 会复制布局选择、可视化布局行和严格属于源 target 的虚拟布局版本。跨模块调用方不得直接写 Theme 布局表。
