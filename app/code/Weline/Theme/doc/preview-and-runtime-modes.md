...
|------|------|--------------------------------------------------------------|----------|
| 1 | **可视化编辑预览** | **请求参数为主**（query + typed `editor_context`） | 编辑器 iframe → `theme-preview/content` |
...
- URL query：`theme_id` / `frontend_theme_id` / `editor_area` / `preview_area` / `page_type` / `layout_option` / `status` / `version_id` / `scope` / `interaction_mode` / `editor_mode` 等
- typed **`editor_context`**（JSON）：与 query 对齐的编辑会话身份；保存/删部件/预览 content 必须带同一套上下文
- 版本面板在画布内切换历史版：仍走参数（如 `status` + `version_id`），**不是**店面 Token 路径

### 正确用法

- iframe / `#btnPreview` → **只**打 `theme-preview/content`（后台会话 + `editor_context`）
- **禁止**在可视化预览里调用 `start-preview` 种 Cookie（会误进「版本真实预览」态）
- 改预览身份：改参数 / 重建 `editor_context`，不要假设 RequestContext 里已有店面 Scope
- 调试时以 Network 里 content URL 的 query + `editor_context` 为准，不要用访客路径反推
...
- **首页预览 URL path 必须是 `/`，禁止把空串交给 `getFrontendUrl('')`**：空串会复用当前 `REQUEST_URI`；经 BinQuery 时会变成 `/framework/query-bin?weline_preview_token=…`。用 `ThemePageTypeResolver::getFrontendUrlPathForPreview()`；并拒绝 `framework/query-bin` 伪 `theme_public_route`
...
- 实现锚点（店面侧）：有效 Token 时安装 `LayoutIdentity`（与 `theme-preview/content` 对齐），并走 draft `processSlots`，禁止误入正式实体硬切

### 与「画布里看版本」的区别

| | 画布内版本 | 版本真实预览 |
|--|-----------|--------------|
| 载体 | `theme-preview/content` + 参数 | 店面 URL + Token |
...
无有效预览 Token、非编辑器 content 请求时，**一律**走本态；不得偷偷读 draft 工作区。
...
否则在 theme-preview/content 或带 editor_context？ → 信参数（态 1）
...
| 画布 content | `theme-preview` / Content 模板与控制器 |
