...
- 请求期：仅在**无可用固化模板**时动态固化激活主题；不得把「每请求 overlay」当成替代固化的长期主路径。
- XOR：登录页等保持 `placement=injection` + 空槽（禁止旁路 fetch）。

## UC

- UC-1：游客打开含 required JSON 注入槽的页，固化模板（或首次动态固化后的模板）含对应 `data-widget-code`。
- UC-2：无 `user_deleted@*` 时，任意主题下对应布局固化产物均含该默认注入（插件安装后全主题重固化）。
