...
2026-09-25 边界收口：以下“必装”指 `required=true`；未标 required/false 的推荐项仍需显式安装，与 `REQ-THEME-0014` 一致。版本化决定、继承与删除旧链路按[主题固化物实施方案](./layout-entity-per-version-isolation.md)（待实施）：用户意图复制成目标版本自己的决定，运行只查目标 V/R；不从源版本借省略权限。
...
5. **已有固化模板**：结构变更、源模板/注入声明变更时重固化受影响结构；纯配置只更新 sidecar/binding，不改关系 PHTML。
