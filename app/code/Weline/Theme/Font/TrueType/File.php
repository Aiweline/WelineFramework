...
use Weline\Theme\Font\AdobeFontMetrics;
use Weline\Theme\Font\Font;
use Weline\Theme\Font\BinaryStream;
use Weline\Theme\Font\Table\Table;
use Weline\Theme\Font\Table\DirectoryEntry;
use Weline\Theme\Font\Table\Type\glyf;
use Weline\Theme\Font\Table\Type\name;
use Weline\Theme\Font\Table\Type\nameRecord;
...
class File extends BinaryStream {
  ...
  function getFontType(){
    // Weline\Theme\Font\{TrueType|WOFF|OpenType|EOT}\File → penultimate segment
    $class_parts = explode("\\", get_class($this));
    $count = count($class_parts);

    return $count >= 2 ? $class_parts[$count - 2] : $class_parts[0];
  }
  ...
  function parseTableEntries() {
    ...
    $class = 'Weline\\Theme\\Font\\' . $type . '\\TableDirectoryEntry';
  ...
  protected function readTable($tag) {
    ...
    if (!self::$raw) {
      ...
      $class = 'Weline\\Theme\\Font\\Table\\Type\\' . $name_canon;
