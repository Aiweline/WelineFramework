...
{
    public const table = 'w_delivery_addresses';
    
    public const fields_ID = 'delivery_address_id';
    ...
    public function install(ModelSetup $setup, Context $context): void
    {
        if (!$setup->tableExist()) {
            $setup->createTable('运送地址表')
                ...
                ->addIndex(
                    TableInterface::index_type_KEY,
                    'idx_customer_default',
                    [self::fields_CUSTOMER_ID, self::fields_IS_DEFAULT],
                    '客户默认地址复合索引'
                )
                ->create();
        }
    ...
    {
        $customerId = $this->getData(self::fields_CUSTOMER_ID);
