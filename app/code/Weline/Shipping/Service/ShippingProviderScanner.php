...
class ShippingProviderScanner
{
    ...
    public function scanProviderDefinitions(bool $forceReload = false): array
    {
        ...
        if (!$forceReload && $this->cachedDefinitions !== null && $this->cachedExtendsMtime === $currentMtime) {
            return $this->cachedDefinitions;
        }

        $definitions = [];
        $extendedBy = ExtendsData::getExtendedBy('Weline_Shipping', $forceReload);
        ...
        foreach ($extendedBy as $sourceModule => $extensions) {
            ...
            foreach ((array)$extensions as $extension) {
                ...
                if (!str_starts_with($relativePath, 'extends/module/Weline_Shipping/ShippingProvider/')) {
                    continue;
                }
                ...
                try {
                    ...
                    if (!$reflection->implementsInterface(ShippingProviderInterface::class)) {
                        continue;
                    }
                    if (!$reflection->isSubclassOf(AbstractShippingProvider::class)) {
                        w_log_error('Shipping provider must extend AbstractShippingProvider: ' . $className);
                        continue;
                    }
                    ...
                    w_log_error('检查配送提供商接口失败: ' . $className . ', 错误: ' . $throwable->getMessage());
                    continue;
                }
    ...
    public function getProviderInstances(bool $forceReload = false): array
    {
        ...
        foreach ($this->scanProviderDefinitions($forceReload) as $definition) {
            ...
            try {
                ...
                w_log_error('实例化配送提供商失败: ' . $className . ', 错误: ' . $throwable->getMessage());
            }
        }
