/**
 * Weline_Shipping：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
...
const CANDIDATE_ROUTES = ["shippingAddress","shippingaddress","shipping_address","shippingService","shippingservice","shipping_service","region","carrier","rateTemplate","ratetemplate","rate_template","tracking","freeShippingRule","freeshippingrule","free_shipping_rule","manager","zone","deliveryAddress","deliveryaddress","delivery_address","shipping","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Shipping 后台流程', () => {
  moduleCase(
    test,
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'SHIPPING-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
