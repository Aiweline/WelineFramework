(function () {
    ...
    async function validateShippingFields(snapshot) {
    ...
    var changeBtn = root.querySelector('[data-change-address]');
    if (changeBtn) {
    ...
    function onPostalFieldEvent(event) {
    ...
    root.addEventListener('input', onPostalFieldEvent);
    root.addEventListener('change', onPostalFieldEvent);
    var postalLookupTimer = null;
    var postalJobSeq = 0;
    var postalJobQueue = Promise.resolve();
    var POSTAL_LOOKUP_DEBOUNCE_MS = 500;
    function setAddressCascadeLoading(busy) {
        if (!window.WelineThemeAddress || typeof window.WelineThemeAddress.setCascadeLoading !== 'function') {
            return;
        }
        window.WelineThemeAddress.setCascadeLoading(ADDRESS_CODE, !!busy);
    }
    function finishPostalJobLoading(jobId) {
        if (jobId === postalJobSeq) {
            setAddressCascadeLoading(false);
        }
    }
    function schedulePostalLookup() {
        window.clearTimeout(postalLookupTimer);
        var field = root.querySelector('[data-shipping-field][name="postal_code"]');
        var postal = text(field && field.value).trim();
        if (postal.length < 3) {
            setAddressCascadeLoading(false);
            if (window.WelineThemeAddress && typeof window.WelineThemeAddress.clearPostalCountryPrompt === 'function') {
                window.WelineThemeAddress.clearPostalCountryPrompt(ADDRESS_CODE);
            }
            return;
        }
        // 防抖等待期间即显示 loading，避免用户以为不会自动回填
        setAddressCascadeLoading(true);
        postalLookupTimer = window.setTimeout(function () {
            enqueuePostalLookup();
        }, POSTAL_LOOKUP_DEBOUNCE_MS);
    }
    function clearPostalCascade(countryCode, options) {
        options = options || {};
        var keepCountry = options.keepCountry !== false;
        var formEl = root.closest('form') || form;
        var scope = editor || root;
        var names = [
            'province', 'city', 'district', 'street',
            'province_code', 'province_region_id',
            'city_code', 'city_region_id',
            'district_code', 'district_region_id',
            'street_id', 'street_code'
        ];
        if (!keepCountry) {
            names = names.concat(['country', 'country_code']);
        }
        names.forEach(function (name) {
            var input = scope.querySelector('[name="' + name + '"]')
                || (formEl ? formEl.querySelector('[name="' + name + '"]') : null);
            if (input) {
                input.value = '';
            }
        });
    ...
    function preservePostalValue(postal) {
        var field = root.querySelector('[data-shipping-field][name="postal_code"]');
        if (field && text(field.value).trim() === postal) {
            field.value = postal;
        }
    }
    function applyPostalHit(countryCode, postal, jobId) {
        if (typeof window.WelineThemeAddress.applyPostalCandidate !== 'function') {
            return Promise.resolve(null);
        }
        return window.WelineThemeAddress.postalLookup(countryCode, postal, 8).then(function (rows) {
            if (jobId !== postalJobSeq) {
                return null;
            }
            var latestField = root.querySelector('[data-shipping-field][name="postal_code"]');
            var latest = text(latestField && latestField.value).trim();
            if (latest !== postal) {
                return null;
            }
            if (!Array.isArray(rows) || !rows.length) {
                return clearPostalCascade(countryCode).then(function () {
                    preservePostalValue(postal);
                    return null;
                });
            }
            return window.WelineThemeAddress.applyPostalCandidate(ADDRESS_CODE, Object.assign({
                ...
                var latestField2 = root.querySelector('[data-shipping-field][name="postal_code"]');
                var latest2 = text(latestField2 && latestField2.value).trim();
                if (latest2 && latest2 !== postal) {
                    enqueuePostalLookup();
                    return null;
                }
                preservePostalValue(postal);
    ...
    function enqueuePostalLookup() {
        ...
        postalJobQueue = postalJobQueue.then(function () {
            ...
            if (postal.length < 3) {
                ...
                finishPostalJobLoading(jobId);
                return null;
            }
            var snap = readShippingSnapshot();
            var countryCode = snap.country_code || 'CN';
            var countriesPromise = typeof window.WelineThemeAddress.postalCountries === 'function'
                ? window.WelineThemeAddress.postalCountries(postal)
                : Promise.resolve([]);
            return countriesPromise.then(function (countries) {
                ...
                if (!Array.isArray(countries) || !countries.length) {
                    return applyPostalHit(countryCode, postal, jobId);
                }
                if (countries.length === 1) {
                    var onlyRow = countries[0] || {};
                    var only = text(onlyRow.country_code || onlyRow.code || '').toUpperCase() || countryCode;
                    if (typeof window.WelineThemeAddress.setPostalCountryPin === 'function') {
                        window.WelineThemeAddress.setPostalCountryPin(ADDRESS_CODE, countries, postal);
                    }
                    // 唯一命中：直接回填（目录已含 NZ 后，1010 会走下方多国分支）
                    return applyPostalHit(only, postal, jobId);
                }
                // 多国歧义（如 1010→AU/NZ）：清空并展开国家待选
                return clearPostalCascade(countryCode, {keepCountry: false}).then(function () {
                    preservePostalValue(postal);
                    if (typeof window.WelineThemeAddress.promptPostalCountries !== 'function') {
                        return null;
                    }
                    // 多国待选：结束 loading，展开国家菜单供用户选择
                    finishPostalJobLoading(jobId);
                    return window.WelineThemeAddress.promptPostalCountries(ADDRESS_CODE, countries, postal);
                });
    ...
    root.setAttribute('data-shipping-js-rev', '20260904-addr-pick1');
    window.WelineShippingCheckoutAddress = {
        rev: '20260904-addr-pick1',
        root: root,
