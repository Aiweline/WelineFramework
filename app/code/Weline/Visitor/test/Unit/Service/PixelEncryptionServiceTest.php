...
class PixelEncryptionServiceTest extends TestCore
{
    ...
    /**
     * 确保当前环境有可用加密 token（空库 / 仅已删除行时自生成，避免依赖环境脏数据）。
     */
    private function ensureUsableCurrentToken(): PixelEncryptionToken
    {
        $token = $this->service->getCurrentVersionToken();
        if ($token instanceof PixelEncryptionToken && $token->getEncryptionToken() !== '') {
            return $token;
        }

        $version = 'phpunit-current-' . bin2hex(random_bytes(4));
    ...
    public function testEncryptAndDecryptWithVersion()
    {
        ...
        try {
            ...
            if (!$token) {
                // 如果没有token，生成一个
                $this->createdTokens[] = $this->service->generateTokenForVersion($version);
            }
        } catch (\Exception $e) {
            // 如果生成失败，跳过此测试
    ...
    public function testGetCurrentVersionToken()
    {
        ...
        // token可能不存在（空库 / 仅已删除），空结果必须是 null，不能是空模型对象
        if ($token === null) {
        ...
        $this->assertGreaterThan(0, $token->getTokenId());
