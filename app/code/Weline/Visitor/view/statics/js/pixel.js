(function() {
    ...
    var PIXEL_SCRIPT_VERSION = '2026.09.16-sandbox-early-buffer';
    ...
    function __resolveConversionBusinessKey(meta) {
        ...
        try {
            var root = document.querySelector('[data-order-uuid],[data-checkout-group-uuid],[data-transaction-no]');
            if (root) {
                bags.push({
                    order_uuid: root.getAttribute('data-order-uuid') || '',
                    checkout_group_uuid: root.getAttribute('data-checkout-group-uuid') || '',
                    transaction_id: root.getAttribute('data-transaction-no') || '',
                    transaction_no: root.getAttribute('data-transaction-no') || ''
                });
            }
        } catch (eDom) {
        }
    ...
    function __conversionDedupeStorageKey(eventName, businessKey) {
        var scope = 'default.default.default';
        try {
            scope = __resolvePixelStorageScope() || scope;
        } catch (eScope) {
        }
        return 'weline_pixel_dedupe:' + scope + ':' + String(eventName || '') + ':' + String(businessKey || '');
    }
    ...
    function __isConversionDedupeDuplicate(eventName, meta) {
        ...
        var key = __conversionDedupeStorageKey(eventName, businessKey);
        var now = Date.now();
        try {
    ...
    /**
     * 去重丢弃（仅沙盒厂商）：黄标记入监视流，不代替主 track 事件。
     */
    ...
    window.WelinePixel = {
        ...
        track: function (eventName, meta, options) {
            ...
            // 转化去重不拦截主 track：事件流/监视/后端上报照常；仅沙盒厂商 fanout 丢弃（见 __fanoutPixelVendors）。
            ...
            // 主 track 成功后再记键：本轮沙盒 fanout 仍放行，刷新后的沙盒发送才丢弃
            try {
                __markConversionDedupeSeen(normalizedEventName, mergedMeta);
            } catch (eMark) {
            }
            return payload;
        },
    ...
    function __fanoutPixelVendors(event) {
        ...
        vendors.forEach(function (vendor) {
            ...
            if (mode === 'sandbox') {
                // 仅沙盒厂商去重：主事件流已由 publish 透传；重复则黄标并跳过沙盒 iframe
