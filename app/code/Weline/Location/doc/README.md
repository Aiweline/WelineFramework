...
window.__WelineThemeConfig = {
    geo: {
        ipApiUrl: '/location/rest/v1/frontend/location/ip'  // 自定义IP定位API地址
    }
};
```
...
#### `WelineLocation.getCurrentPosition(options)`
...
#### `WelineLocation.watchPosition(callback, options)`
...
**示例：**
```javascript
const watchId = WelineLocation.watchPosition((position) => {
...
#### `WelineLocation.getLocationByIP()`
...
#### `WelineLocation.getLocation(options)`
...
#### `WelineLocation.setCacheDuration(duration)`
...
#### `WelineLocation.isGeolocationSupported()`
