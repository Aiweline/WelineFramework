...
4. 本文件：`app/code/Weline/Eav/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Eav/doc/README.md`
...
入口/配置文件：
- `app/code/Weline/Eav/etc/backend/menu.xml`
- `app/code/Weline/Eav/composer.json`
...
- `Model`：ORM 数据模型与字段 schema。字段结构用 #[Col]/#[Index] 后执行 setup:upgrade。 文件数：14
...
- `Ui`：后台/编辑器 UI 参数、schema 或渲染支撑。 文件数：2
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/EavAdminQueryProvider.php；前端/跨模块读数据先查 query 帮助。

## doc 目录

- `app/code/Weline/Eav/doc/README.md`
- `app/code/Weline/Eav/doc/eav-entity-and-attribute-conventions.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Eav> [operation]` 或对应 `w_query` 帮助。
