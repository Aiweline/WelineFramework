/**
 * Weline_Currency：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
...
const CANDIDATE_ROUTES = ["config","currency","index","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Currency 后台流程', () => {
  moduleCase(
    test,
    { module: MODULE, id: 'CURRENCY-SMOKE-001' },
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'CURRENCY-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
