...
# Weline_TranslationService AI 开发入口
...
4. 本文件：`app/code/Weline/TranslationService/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/TranslationService/doc/README.md`
...
- 目录：`app/code/Weline/TranslationService`
- Vendor：`Weline`
...
入口/配置文件：
- `app/code/Weline/TranslationService/etc/module.xml`
- `app/code/Weline/TranslationService/etc/backend/menu.xml`
- `app/code/Weline/TranslationService/composer.json`
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/TranslationQueryProvider.php；前端/跨模块读数据先查 query 帮助。

## doc 目录

- `app/code/Weline/TranslationService/doc/AI翻译标签使用指南.md`
- `app/code/Weline/TranslationService/doc/README.md`
- `app/code/Weline/TranslationService/doc/event/生命周期事件.md`
- `app/code/Weline/TranslationService/doc/event/翻译事件.md`
- `app/code/Weline/TranslationService/doc/使用指南.md`
- `app/code/Weline/TranslationService/doc/功能现状.md`
- `app/code/Weline/TranslationService/doc/开发日志.md`
- `app/code/Weline/TranslationService/doc/需求.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_TranslationService> [operation]` 或对应 `w_query` 帮助。
