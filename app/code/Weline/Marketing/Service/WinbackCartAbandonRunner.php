...
/**
 * 购物车遗弃挽回：读启用活动 → 时间窗 → cart_signals → 再验 → 活跃结账去重 → 发信 → 写日志。
 *
 * SendLog.order_uuid 存 `cart:{cart_key}`。
 * 依赖均可注入，便于纯 PHP 单测。
 */
final class WinbackCartAbandonRunner
{
    /** @var callable():int */
    private $nowTs;

    /** @var callable(array):array */
    private $listStaleCarts;

    /** @var callable(string):?array */
    private $getStaleCart;

    /** @var callable(array):bool */
    private $hasQuotedSession;
     ...
     * @param callable(array):array|null $listStaleCarts
     * @param callable(string):?array|null $getStaleCart
     * @param callable(array):bool|null $hasQuotedSession
    ...
    public function __construct(
        ?callable $nowTs = null,
        ?callable $listStaleCarts = null,
        ?callable $getStaleCart = null,
        ?callable $hasQuotedSession = null,
        ...
        $this->listStaleCarts = $listStaleCarts ?? [$this, 'defaultListStaleCarts'];
        $this->getStaleCart = $getStaleCart ?? [$this, 'defaultGetStaleCart'];
        $this->hasQuotedSession = $hasQuotedSession ?? [$this, 'defaultHasQuotedSession'];
    ...
    public function run(): array
    {
        ...
        foreach ($campaigns as $campaign) {
            ...
            $listed = ($this->listStaleCarts)($params);
            ...
            foreach ($items as $item) {
                ...
                $stats['processed']++;
                $cartKey = \trim((string)($item['cart_key'] ?? ''));
                if ($cartKey === '') {
                    $stats['skipped']++;
                    ($this->writeLog)([
                        ...
                        'reason' => 'missing_cart_key',
                        'sent_at' => \gmdate('Y-m-d H:i:s', $now),
                    ]);
                    continue;
                }

                $logUuid = 'cart:' . $cartKey;
                $anchorAt = \trim((string)($item['updated_at'] ?? ''));
                if ($anchorAt === '') {
                    $anchorAt = \trim((string)($item['created_at'] ?? ''));
                }
                $anchorTs = $this->parseUtcTs($anchorAt);
                if ($anchorTs === null || $now < ($anchorTs + $abandonHours * 3600)) {
                    $stats['skipped']++;
                    // Not yet abandon-eligible: do not pollute send log.
                    continue;
                }
                ...
                if ($cooldownHours > 0) {
                    $last = ($this->lastSentAt)($campaignId, $logUuid, $step);
                ...
                $fresh = ($this->getStaleCart)($cartKey);
                if ($fresh === null) {
                    $stats['skipped']++;
                    ($this->writeLog)([
                        'campaign_id' => $campaignId,
                ...
                $hasEmail = \array_key_exists('has_email', $fresh)
                    ? !empty($fresh['has_email'])
                    : (\trim((string)($fresh['email'] ?? $fresh['customer_email'] ?? '')) !== '');
                $reachable = !empty($fresh['reachable']);
                $continueUrl = \trim((string)($fresh['continue_cart_url'] ?? ''));
                if (!$hasEmail || !$reachable || $continueUrl === '') {
                    $stats['skipped']++;
                    ($this->writeLog)([
                        'campaign_id' => $campaignId,
                        'order_uuid' => $logUuid,
                ...
                $dedupParams = ['cart_key' => $cartKey];
                $customerId = (int)($fresh['customer_id'] ?? $item['customer_id'] ?? 0);
                if ($customerId > 0) {
                    $dedupParams['customer_id'] = $customerId;
                }
                if (($this->hasQuotedSession)($dedupParams)) {
                    $stats['skipped']++;
                    ($this->writeLog)([
                        'campaign_id' => $campaignId,
                        'order_uuid' => $logUuid,
    ...
    private function defaultListStaleCarts(array $params): array
    {
        if (!\function_exists('w_query')) {
            return ['items' => [], 'meta' => []];
        }
        $result = w_query('cart_signals', 'list_stale_carts', $params);

        return \is_array($result) ? $result : ['items' => [], 'meta' => []];
    }

    private function defaultGetStaleCart(string $cartKey): ?array
    {
        if (!\function_exists('w_query')) {
            return null;
        }
        $result = w_query('cart_signals', 'get_stale_cart', ['cart_key' => $cartKey]);
     ...
     * @param array<string, mixed> $params
     */
    private function defaultHasQuotedSession(array $params): bool
    {
        if (!\function_exists('w_query')) {
            return false;
        }
        ...
        try {
            $result = w_query('checkout_signals', 'has_quoted_session', $params);
            if (\is_array($result) && \array_key_exists('has_quoted', $result)) {
                return !empty($result['has_quoted']);
            }
        } catch (\Throwable) {
            // Operation missing or provider error → fallback below.
        }

        $customerId = (int)($params['customer_id'] ?? 0);
        if ($customerId <= 0) {
            $cartKey = \trim((string)($params['cart_key'] ?? ''));
            if (\preg_match('/^customer:(\d+)$/i', $cartKey, $m) === 1) {
                $customerId = (int)$m[1];
            }
        }
        if ($customerId <= 0) {
        ...
        try {
            $listed = w_query('checkout_signals', 'list_stale_quotes', [
                'lookback_hours' => 168,
                'limit' => 200,
            ]);
        } catch (\Throwable) {
        ...
        $items = \is_array($listed) && \is_array($listed['items'] ?? null) ? $listed['items'] : [];
        foreach ($items as $quote) {
            if (!\is_array($quote)) {
                continue;
            }
            if ((int)($quote['customer_id'] ?? 0) === $customerId) {
                return true;
            }
        }
    ...
    /** @param array<string, mixed> $cartDto */
    private function defaultSendMail(array $cartDto): array
    {
        /** @var WinbackMailNotifier $notifier */
        $notifier = ObjectManager::getInstance(WinbackMailNotifier::class);

        return $notifier->notifyCartAbandon($cartDto);
    }
    ...
    private function defaultLoadCampaigns(): array
    {
        ...
        $model->clear()
            ...
            ->where(WinbackCampaign::schema_fields_TYPE, WinbackCampaign::TYPE_CART_ABANDON_REMINDER)
            ->select()
            ->fetch();
        $out = [];
