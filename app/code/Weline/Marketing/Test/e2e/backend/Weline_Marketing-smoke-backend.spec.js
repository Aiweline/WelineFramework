/**
 * Weline_Marketing：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
...
const CANDIDATE_ROUTES = ["winback","winbackdashboard","lifecycle","segment","campaign","rule","coupon","marketing","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Marketing 后台流程', () => {
  moduleCase(
    test,
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'MARKETING-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
