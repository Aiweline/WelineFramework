...
其他模块需要引用 Checkout 订单字段名或状态码时，应依赖公开契约，而不是直接依赖 `Model\Order` 或 `Model\OrderItem`：

- `Weline\Checkout\Api\OrderSchemaInterface`：订单表名、字段名、订单状态、支付状态和配送状态。
- `Weline\Checkout\Api\OrderItemSchemaInterface`：订单项表名和字段名。

这些契约只表达 Checkout 的通用数据语义，不包含 WeShop 或其他业务模块的专属逻辑。业务模块需要读取跨模块数据时，仍应优先通过 `w_query` 暴露的查询入口或 Checkout 服务提供的稳定能力接入。
...
- `getData`：聚合购物车、配送方式、支付方式与结账身份（默认 `guest_allowed=true`）
- `placeOrder` / `createOrder`：从购物车下单并尝试启动支付；未登录走匿名结账，需有效邮箱
