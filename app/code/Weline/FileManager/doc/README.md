...
- The first source-code queue slice is available only for recoverable single
  file trash. `SOURCE_QUEUE_TRASH` requires the source policy to be enabled for
  `project`, `local_project`, or `app_code`, validates one existing allowlisted
  source file under 128 KB before queue creation, writes a
  ...
  moving the file into same-root `.wls-trash`. Broader source queues remain
  disabled and cannot reuse ordinary `QUEUE_TRASH` or `QUEUE_COMPRESS`.
...
- Broader source-tree write policy remains layered; the current source-code
  ...
  enabled source roots, with `SOURCE_QUEUE_TRASH` as the only queue-backed
  source action. Broader source queue operations remain future slices. The
