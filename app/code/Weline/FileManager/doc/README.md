...
- Dedicated source-code queue slices are available only for explicitly named,
  bounded intents. `SOURCE_QUEUE_TRASH` requires the source policy to be enabled
  for `project`, `local_project`, or `app_code`, validates one existing
  allowlisted source file under 128 KB before queue creation, writes a
  `source_trash_entry` payload, and the worker re-checks root key, root path,
  `source_path`/`source_relative_path` consistency, extension allowlist,
  protected path segments, `max_entries=1`, and `max_bytes<=128 KB` before
  moving the file into same-root `.wls-trash`. `SOURCE_QUEUE_ARCHIVE` uses the
  same source-file policy, but writes a `source_archive_file` payload and
  creates a read-only ZIP snapshot under
  `var/wls-panel/file-manager/source-archives/`. `SOURCE_QUEUE_ARCHIVE_TREE`
  is the first bounded read-only source directory queue: it accepts one
  existing child directory under the current enabled source-policy path, writes
  a `source_archive_tree` payload, caps the worker at 200 entries and 10 MB,
  rejects symlinks/path escapes/protected segments, re-checks root key and
  source path consistency, and writes the ZIP only under the same panel-owned
  source archive directory. `SOURCE_QUEUE_ARCHIVE_SELECTION` extends that
  read-only path to several explicitly named entries in the current
  source-policy directory: it accepts up to 20 direct child files or
  directories, writes a `source_archive_selection` payload, caps worker
  traversal at 200 entries and 10 MB, and revalidates root key, parent path,
  entry paths, source extensions, protected paths, symlinks, and the archive
  root before creating the ZIP. Source archive targets are generated
  server-side, must remain inside that panel-owned var directory, and never
  write beside the source file or directory. Broader multi-directory,
  multi-root, upload,
  delete, purge, overwrite, and ordinary `QUEUE_TRASH`/`QUEUE_COMPRESS` source
...
- Broader source-tree write policy remains layered; the current source-code
  ...
  enabled source roots, with `SOURCE_QUEUE_TRASH`, `SOURCE_QUEUE_ARCHIVE`, and
  `SOURCE_QUEUE_ARCHIVE_TREE`, and `SOURCE_QUEUE_ARCHIVE_SELECTION` as the only
  queue-backed source actions. Broader
  source queue operations remain future slices. The
  queue-backed destructive path for ordinary writable roots is recoverable by
  default, has a dedicated restore-history view, and now includes explicit
  permanent purge for queue-created trash entries only.
