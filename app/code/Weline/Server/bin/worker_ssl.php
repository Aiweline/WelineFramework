...
foreach ($argv as $arg) {
    if (\str_starts_with($arg, '--name=')) {
        ...
        $deferSsl = true;  // 延迟 SSL 模式：先监听 TCP，在 accept 后手动启用 SSL
...
$wlsLog = function (string $message, string $level = 'INFO') use (&$logBuffer, &$logBufferSize, $shouldLog, $workerId, $instanceName, $flushLog) {
    ...
    $logMessage = "{$color}[{$timestamp}] [WLS-SSL] Worker #{$workerId} ({$instanceName}) [{$level}] {$message}\033[0m\n";
...
try {
    $wlsLog("Worker 启动，监听 ssl://{$host}:{$port}", 'INFO');
    ...
    \error_log('[WLS Worker SSL] Bootstrap error: ' . $e->getMessage());
}
...
if ($freeMemory > 0 && $freeMemory < $requiredMemory) {
    ...
    $wlsLog("内存不足警告：系统可用内存 {$freeMB}MB，WLS 需要 {$requiredMB}MB（缓存 {$cacheMB}MB + 50MB 预留）", 'WARN');
    
    // 如果严重不足（低于需求的 50%），报错退出
    if ($freeMemory < $requiredMemory * 0.5) {
        $wlsLog("内存严重不足，无法启动。请增加系统内存或减少 env.php 中的 server.cache.static_file_max_total 配置", 'ERROR');
        exit(1);
    }
    ...
    $wlsLog("自动缩减静态文件缓存至 {$newCacheMB}MB", 'WARN');
    $WLS_STATIC_CACHE_MAX_TOTAL = $newCacheSize;
}

$wlsLog("内存缓存配置：静态文件缓存上限 " . \round($WLS_STATIC_CACHE_MAX_TOTAL / 1024 / 1024, 1) . "MB，单文件上限 " . \round($WLS_STATIC_CACHE_MAX_SIZE / 1024, 1) . "KB，淘汰阈值 " . \round($WLS_CACHE_EVICTION_THRESHOLD / 1024 / 1024, 1) . "MB", 'INFO');
// ========== 内存缓存配置结束 ==========
...
\register_shutdown_function(function() use ($wlsLog, $flushLog, $workerId, $port, $processLogFile, $instanceName, $fatalErrorTypes) {
    ...
    $line = '[' . \date('Y-m-d H:i:s') . '] [instance:' . $instanceName . '] [Worker:' . $workerId . '] [Port:' . $port . '] [SSL] ' . $error['message'] . ' in ' . $error['file'] . ':' . $error['line'] . "\n";
    $written = false;
    if (\defined('BP') && BP !== '') {
        $crashLogDir = BP . 'var' . DS . 'log';
        if (!\is_dir($crashLogDir)) {
...
// 方案1b：使用 socket 扩展创建支持 SO_REUSEPORT 的 socket（直接 SSL，无同端口重定向）
    ...
    if (!$socket) {
        $wlsLog("Socket 创建失败 (defer-ssl): {$errstr} (errno: {$errno})", 'ERROR');
        \error_log("[WLS Worker SSL] Failed to create socket (defer-ssl): {$errstr}");
        exit(1);
    }
    
    $wlsLog("延迟 SSL 模式: 监听 tcp://{$host}:{$port}，accept 后手动启用 SSL", 'INFO');
    ...
    if (!$socket) {
        ...
        \error_log("[WLS Worker SSL] Failed to create socket: {$errstr}");
        exit(1);
    }
}
...
if ($controlPort > 0) {
    ...
    if ($ipcClient->connect('127.0.0.1', $controlPort)) {
        ...
        $ipcClient->onMessage(function (array $msg, \Weline\Server\IPC\ControlClient $client) use (&$shouldExit, &$ipcDraining, &$ipcReceivedShutdown, &$socket, &$drainStartTime, $wlsLog, $workerId) {
            ...
            switch ($type) {
                ...
                case \Weline\Server\IPC\ControlMessage::TYPE_CACHE_CLEAR:
                    ...
                    $wlsLog("收到 cache_clear 命令，已清理缓存（opcache + ObjectManager + 静态文件缓存）", 'INFO');
                    break;
        ...
        $ipcClient->onDisconnect(function (bool $receivedShutdown, \Weline\Server\IPC\ControlClient $client) use (&$ipcReceivedShutdown, $wlsLog, $instanceName, $controlPort, $workerId) {
            ...
            // 触发 Master 复活（仅 Worker #1 参与，优先级 3，延迟 6 秒）
            if ($workerId === 1) {
...
$gracefulExit = function (string $reason = '') use ($socket, &$connections, &$requestBuffers, &$connectionLastActivity, $processName, $flushLog) {
    ...
    // 记录退出原因
    if ($reason) {
        \error_log("[WLS-SSL Worker] 退出原因: {$reason}");
    }
...
while (true) {
    ...
    if ($masterPid > 0 && !$ipcReceivedShutdown && ($now - $lastMasterCheck) >= $masterCheckInterval) {
        ...
        if ($ipcClient && $ipcClient->isConnected()) {
            ...
            if ($masterAlive) {
                ...
                $masterDeadCount++;
                $wlsLog("Master PID {$masterPid} 不可达且 IPC 断开 ({$masterDeadCount}/{$masterDeadThreshold})", 'WARN');
                if ($masterDeadCount >= $masterDeadThreshold) {
                    $wlsLog("Master PID {$masterPid} 已死亡，Worker 自行退出（孤儿保护）", 'WARN');
            ...
            if ($idleTime >= $keepAliveTimeout) {
                ...
                if ($hasBufferedData) {
                    ...
                    if ($idleTime >= $keepAliveTimeout * 3) {
                        ...
                        $wlsLog("连接超时且缓冲区有数据，强制关闭 (connId: {$connId}, 剩余: " . \strlen($writeBuffers[$connId]) . " 字节)", 'WARN');
                        @\fclose($conn);
    ...
    $readSockets = \array_merge($readSockets, $validConnections, $pendingConns);
    ...
    if ($changed === false) {
        ...
        $error = \error_get_last();
        $wlsLog("stream_select 失败: " . ($error['message'] ?? 'unknown'), 'WARN');
        continue;
    }
        ...
        if ($conn) {
            ...
            if ($deferSsl) {
                ...
                if ($cryptoResult === true) {
                    // 握手立即完成
                    if ($isDev) {
                        $wlsLog("SSL 握手成功: {$peerName} (connId: {$connId})", 'INFO');
                    }
                    ...
                    continue; // 跳过后续处理，等待下一轮事件循环
                } else {
                    // 握手失败
                    $error = \error_get_last();
                    $errorMsg = $error['message'] ?? 'unknown';
                    $wlsLog("SSL 握手失败: {$peerName} (connId: {$connId}) - {$errorMsg}", 'WARN');
                    @\fclose($conn);
                    continue;
                }
            } else {
    ...
    foreach ($pendingHandshakes as $connId => $handshakeInfo) {
        ...
        if ($elapsed > $handshakeTimeout) {
            ...
            $wlsLog("SSL 握手超时: {$peerName} (connId: {$connId})", 'WARN');
            continue;
        }
    ...
    foreach ($read as $conn) {
        ...
        if (!$isSseMode) {
            ...
            if ($responseLen > 100000) { // 大于 100KB 的响应
                $wlsLog("大响应入缓冲区: connId={$connId}, 总长度={$responseLen}, 已写={$totalWritten}, 剩余={$remainingBytes}, URI=" . ($requestUri ?? 'unknown'), 'DEBUG');
            }
            ...
            while ($totalWritten < $responseLen && $retries < $maxRetries) {
                ...
                if ($elapsed > $writeTimeout) {
                    $wlsLog("响应写入超时 (connId: {$connId})，已写入 {$totalWritten}/{$responseLen} 字节", 'WARN');
                    @\fclose($conn);
                    unset($connections[$connId]);
                    unset($requestBuffers[$connId]);
                    unset($connectionLastActivity[$connId]);
                ...
                if ($selectResult === false) {
                    ...
                    $wlsLog("stream_select 失败 (connId: {$connId})", 'WARN');
                    @\fclose($conn);
                    unset($connections[$connId]);
                    unset($requestBuffers[$connId]);
                    unset($connectionLastActivity[$connId]);
                ...
                if ($written === false) {
                    $wlsLog("响应写入失败 (connId: {$connId})", 'WARN');
                    @\fclose($conn);
                    unset($connections[$connId]);
                    unset($requestBuffers[$connId]);
                    unset($connectionLastActivity[$connId]);
            ...
            if ($retries >= $maxRetries) {
                $wlsLog("响应写入重试次数超限 (connId: {$connId})，已写入 {$totalWritten}/{$responseLen} 字节", 'WARN');
                @\fclose($conn);
                unset($connections[$connId]);
                unset($requestBuffers[$connId]);
                unset($connectionLastActivity[$connId]);
    ...
    foreach ($write as $conn) {
        ...
        while (isset($writeBuffers[$connId]) && $writeBuffers[$connId] !== '' && $writeAttempts < $maxWriteAttempts) {
            ...
            if ($written === false) {
                // 写入失败，关闭连接并清理
                $wlsLog("缓冲区写入失败 (connId: {$connId}, 剩余: {$bufferLen} 字节)", 'WARN');
                @\fclose($conn);
                unset($connections[$connId]);
                unset($requestBuffers[$connId]);
                unset($connectionLastActivity[$connId]);
        ...
        if ($initialBufferLen > 100000 && $totalWrittenThisLoop > 0) {
            ...
            $wlsLog("大缓冲区批量发送: connId={$connId}, 初始={$initialBufferLen}, 本轮写入={$totalWrittenThisLoop}, 剩余={$remainingLen}, 尝试次数={$writeAttempts}", 'DEBUG');
        }
    }
    ...
    $consecutiveErrors = 0;
        ...
        // 记录错误日志
        \error_log("[WLS-SSL Worker #{$workerId}] 事件循环异常 ({$consecutiveErrors}/{$maxConsecutiveErrors}): {$errorMessage} in {$errorFile}:{$errorLine}");
        ...
        if ($consecutiveErrors >= $maxConsecutiveErrors) {
            \error_log("[WLS-SSL Worker #{$workerId}] 连续错误过多，优雅退出");
...
function handleRequest(
    ...
    try {
        ...
        \error_log('[WLS Worker SSL] Request error: ' . $e->getMessage());
