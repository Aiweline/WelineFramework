 ...
 *
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │  WLS 仅监听 loopback 明文 HTTP/1.1，不能直接暴露公网。                         │
 * │  公网 TLS 1.3、H2/H1 与可用时 H3 只由项目托管 Nginx 提供。                     │
 * │  Nginx 回源示例：proxy_pass http://127.0.0.1:9090;                            │
 * │  worker.php 只接受由认证 Master 启动的内部监听。                              │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
...
if (!\in_array($wlsListenerMode, ['single', 'reuseport', 'shared_fd', 'worker_ports'], true)) {
...
try {
    ...
    if (\defined('STDOUT') && \is_resource(STDOUT)) {
        $tips = [
            "\033[38;5;208m⚠ WLS 仅监听 loopback 明文 HTTP/1.1，不能直接暴露公网\033[0m",
            "\033[38;5;208m公网 TLS 与 HTTP 协商只由项目托管 Nginx 提供\033[0m",
            "\033[38;5;208mNginx 回源示例：proxy_pass http://127.0.0.1:9090;\033[0m",
        ];
        foreach ($tips as $tip) {
            \fwrite(STDOUT, $tip . "\n");
        }
    ...
    $sessionHost = \trim((string) ($sessionRuntime['host'] ?? '127.0.0.1'));
    if ($sessionHost === '') {
    ...
    $memoryHost = \trim((string) ($memoryRuntime['host'] ?? '127.0.0.1'));
    if ($memoryHost === '') {
...
// POSIX direct 使用 Master 预绑定的单个共享 accept queue，不使用 Dispatcher 或字节透传。
    ...
    $expectedSharedEndpoint = \str_contains($host, ':')
        ? '[' . \trim($host, '[]') . ']:' . $port
        : $host . ':' . $port;
    $actualSharedEndpoint = (string)@\stream_socket_get_name($socket, false);
    if ($actualSharedEndpoint !== $expectedSharedEndpoint) {
        @\fclose($socket);
        $socket = null;
        WlsLogger::error_(
            "Inherited direct listener FD {$listenFd} endpoint mismatch: expected "
...
if ($controlPort > 0 || $supervisorEnabled) {
    ...
    $handler = new \Weline\Server\IPC\ChildControl\Handler\WorkerControlHandler(
            ...
            switch ($type) {
                ...
                case \Weline\Server\IPC\ControlMessage::TYPE_READY_ACK:
                    ...
                    if (!$accepted) {
                        ...
                        $shouldExit = true;
                        $ipcDraining = true;
                        $drainStartTime = (\hrtime(true) / 1_000_000_000) - 10.0;
                ...
                case \Weline\Server\IPC\ControlMessage::TYPE_DRAIN:
                    ...
                    // Close only this Worker's listener. With shared_fd the
                    // Master and healthy generation keep the common accept queue.
                    if ($socket && \is_resource($socket)) {
                        @\fclose($socket);
                        $socket = null;
                        \Weline\Server\Service\Runtime\WorkerReadinessState::markListenerClosed();
                    }
                    WlsLogger::info_("收到 drain 命令，开始排水...");
                    break;
        ...
        static function () use (&$ipcReconnectDueTime, &$ipcReconnectAttempts, &$ipcReconnectMaxAttempts, &$ipcReceivedShutdown, &$shouldExit, $isGatewayJoinBackend): void {
            ...
            $ipcReconnectDueTime = wlsWorkerMonotonicNow() + 1.0;
        }
    );
    ...
    if ($readyReported) {
        ...
        $ipcReconnectAttempts = 0;
        $ipcReconnectMaxAttempts = $isGatewayJoinBackend ? 3 : 30;
        $ipcReconnectDueTime = wlsWorkerMonotonicNow() + 5.0;
    }
}
// ========== IPC 控制通道结束 ==========
...
$writableConnections = [];
/** @var array<int, array{connection: resource, started_at: float, retry_at: float, attempts: int}> */
$writeZeroProgress = [];
...
if (isset($memoryGuardConfig['worker_memory_warning_threshold']) && \is_numeric($memoryGuardConfig['worker_memory_warning_threshold'])) {
    $memoryWarningThreshold = \max(0.50, \min(0.99, (float)$memoryGuardConfig['worker_memory_warning_threshold']));
}
if (isset($memoryGuardConfig['worker_memory_drain_threshold']) && \is_numeric($memoryGuardConfig['worker_memory_drain_threshold'])) {
    $memoryDrainThreshold = \max($memoryWarningThreshold, \min(0.99, (float)$memoryGuardConfig['worker_memory_drain_threshold']));
} else {
    // D22: unify with SSL default 0.92 when key absent.
    $memoryDrainThreshold = 0.92;
}
...
$lastLongLivedSaturationReport = 0.0; // 上次饱和上报的 monotonic 秒（节流）
...
$gracefulExit = function (string $reason = '', bool $waitForRequests = true) use ($socket, &$connections, &$requestBuffers, &$connectionLastActivity, &$activeRequests, $processName, $gracefulShutdownTimeout, $stopShutdownTimeout, &$ipcClient, $workerId, $port, $ipcRole, &$plannedExitReason, $sendExitReasonToMaster) {
    ...
    if ($waitForRequests && !empty($connections)) {
        $waitStartMonotonicNs = \Weline\Server\Runtime\WorkerFiberContextTracker::monotonicNowNs();
        $waitDeadlineMonotonicNs = \Weline\Server\Runtime\WorkerFiberContextTracker::deadlineAfterSeconds(
            (float)$waitTimeout,
            $waitStartMonotonicNs,
        );
        WlsLogger::info_("等待 " . \count($connections) . " 个活跃连接完成...");
        
        while (!empty($connections)
            && !\Weline\Server\Runtime\WorkerFiberContextTracker::deadlineReached($waitDeadlineMonotonicNs)
        ) {
            // 继续处理现有连接的数据
            $read = $connections;
            $write = [];
            ...
            if (!empty($connections)) {
                ...
                $elapsed = (int)\floor(
                    \Weline\Server\Runtime\WorkerFiberContextTracker::monotonicElapsedSeconds(
                        $waitStartMonotonicNs,
                    )
                );
...
while (true) {
    ...
    if ($shouldExit) {
        if ($ipcDraining) {
            // DRAIN 先关闭 listener，但已 accept 连接（包括 SSE/WebSocket）
            // 必须留在事件循环中，直到自然结束或 hard deadline。
            ...
            if ($drainElapsed >= $drainDeadlines['soft']) {
                foreach ($longLivedConnections as $drainConnectionId => &$drainConnectionState) {
                    if (!wlsHttpIsActiveWebSocketConnection($drainConnectionState)
            ...
            $bufferedRequestConnections = 0;
            foreach ($requestBuffers as $drainRequestBuffer) {
                if (\is_string($drainRequestBuffer) && $drainRequestBuffer !== '') {
                    ++$bufferedRequestConnections;
                    ++$pendingApplicationWork;
                }
            }
            $drainObserved = [
            ...
            if ($drainAction === \Weline\Server\IPC\ControlMessage::DRAIN_ACTION_CLOSE_IDLE) {
                $remaining = \count($connections);
                foreach ($connections as $cconn) {
                    if (\is_resource($cconn)) {
                        @\fclose($cconn);
                    }
                }
                $connections = [];
                ...
                if ($ipcClient && $ipcClient->isConnected()) {
                    $ipcClient->send(\Weline\Server\IPC\ControlMessage::drainingComplete(
                        ...
                        reason: $plannedExitReason,
                        drainReport: \Weline\Server\IPC\ControlMessage::drainCompletionReport(
                            outcome: \Weline\Server\IPC\ControlMessage::DRAIN_OUTCOME_IDLE_CLEANUP,
                            elapsedSeconds: $drainElapsed,
                            softDeadlineSeconds: $drainDeadlines['soft'],
                            hardDeadlineSeconds: $drainDeadlines['hard'],
                            observed: $drainObserved,
                            terminated: ['connections' => $remaining],
                        ),
                    ));
                    $ipcClient->flushPendingWrites(0.2);
                }
            ...
            if ($drainAction === \Weline\Server\IPC\ControlMessage::DRAIN_ACTION_FORCE) {
                $terminatedLongLived = [];
                foreach ($longLivedConnections as $longLivedConnectionId => $longLivedState) {
                    if ((isset($connections[$longLivedConnectionId])
                        ...
                        || \array_key_exists($longLivedConnectionId, $activeFibers)
                    ) {
                ...
                $terminatedRequests = 0;
                foreach (\array_keys($activeFibers) as $fiberConnectionId) {
                    if (wlsCancelActiveFiberForHttpConnection(
                        $activeFibers,
                        (int)$fiberConnectionId,
                        $fiberScheduler,
                        $activeRequests,
                    )) {
                        ++$terminatedRequests;
                ...
                if ($ipcClient && $ipcClient->isConnected()) {
                    $ipcClient->send(\Weline\Server\IPC\ControlMessage::drainingComplete(
                        ...
                        reason: $plannedExitReason,
                        drainReport: \Weline\Server\IPC\ControlMessage::drainCompletionReport(
                            outcome: \Weline\Server\IPC\ControlMessage::DRAIN_OUTCOME_FORCED,
                            elapsedSeconds: $drainElapsed,
                            softDeadlineSeconds: $drainDeadlines['soft'],
                            hardDeadlineSeconds: $drainDeadlines['hard'],
                            observed: $drainObserved,
                ...
                WlsLogger::warning_(
                    '[WorkerDrain] phase=hard_deadline_force_close'
                    . ', elapsed_sec=' . $drainElapsed
                    . ', connections=' . $terminatedConnections
                    . ', active_requests=' . $terminatedRequests
                    . ', long_lived=' . $terminatedDrainCounters['long_lived_connections']
                );
                $gracefulExit($ipcReceivedShutdown ? 'shutdown命令（硬期限）' : '热重载（硬期限）');
            }

            static $lastDrainWaitLogAt = 0.0;
            if ($drainElapsed >= $drainDeadlines['soft'] && ($drainNow - $lastDrainWaitLogAt) >= 1.0) {
                WlsLogger::warning_(
                    '[WorkerDrain] phase=application_wait'
                    . ', elapsed_sec=' . $drainElapsed
                    . ', hard_deadline_sec=' . $drainDeadlines['hard']
                    . ', active=' . $activeRequests
                    . ', fibers=' . \count($activeFibers)
                    . ', request_buffers_nonempty=' . $bufferedRequestConnections
                    . ', long_lived=' . $drainCounters['long_lived_connections']
                );
                $lastDrainWaitLogAt = $drainNow;
            }
    ...
    if ($now - $lastTimeoutCheck >= $connectionTimeoutCheckInterval) {
        ...
        foreach ($connections as $connId => $conn) {
            ...
            if ($idleTime >= $keepAliveTimeout) {
                ...
                wlsCancelActiveFiberForHttpConnection(
                    $activeFibers,
                    $connId,
                    $fiberScheduler,
                    $activeRequests
    ...
    // SSE 仅参与写事件；已完成 101 的 WebSocket 必须持续参与读事件，
    // 否则握手后的 frame 会永久留在内核缓冲区。
    $readSockets = [];
    ...
    foreach ($connections as $connId => $conn) {
        $longLivedState = $longLivedConnections[$connId] ?? null;
        if (($applicationAdmissionOpen && $longLivedState === null)
            || wlsHttpIsActiveWebSocketConnection($longLivedState)
        ) {
            $readSockets[] = $conn;
        }
    }
    ...
    foreach ($writableConnections as $connId => $wc) {
        if (\is_resource($wc)) {
            ...
            unset($writableConnections[$connId], $writeBuffers[$connId], $writeZeroProgress[$connId]);
        }
    }
    ...
    // I/O readiness 会立即唤醒事件循环，FiberScheduler 也会按最近定时器缩短该上限。
    // 空闲的 Nginx 回源 Keep-Alive 连接不能把 Worker 退化成 1ms 轮询。
    $loopWaitUsec = 100000;
    ...
    $fiberScheduler->tick(
        ...
        static function (\Fiber $fiber, \Throwable $failure): void {
            ...
            WlsLogger::error_(
                'Request Fiber resume/capture failed; Worker quarantine requested: '
                . $failure->getMessage()
    ...
    wlsHttpDrainAfterResponseIfRequested(
        $socket,
        $shouldExit,
        $ipcDraining,
        $drainStartTime,
    ...
    foreach ($requestBuffers as $bufferedConnId => $bufferedRequest) {
        ...
        if (wlsHttpIsActiveWebSocketConnection($longLivedConnections[$bufferedConnId] ?? null)) {
    ...
    foreach ($longLivedConnections as $webSocketConnId => $webSocketState) {
        if (!wlsHttpIsActiveWebSocketConnection($webSocketState)
    ...
    foreach ($read as $conn) {
        $connId = \get_resource_id($conn);
        $activeWebSocket = wlsHttpIsActiveWebSocketConnection($longLivedConnections[$connId] ?? null);

        if (!$applicationAdmissionOpen && !$activeWebSocket) {
            continue;
        }
        ...
        if (wlsHttpDrainAfterResponseIfRequested(
            $socket,
            $shouldExit,
            $ipcDraining,
            $drainStartTime,
 ...
 */
/**
 * Consume a Runtime quarantine request after its current response has reached
 * the Worker boundary, stop admission, and let the normal drain loop retire
 * the contaminated process.
 */
function wlsHttpDrainAfterResponseIfRequested(
    mixed &$socket,
    bool &$shouldExit,
    bool &$ipcDraining,
...
function wlsFinalizeTerminatedFiberResponseStep(
    ...
    $response = '';
    $boundaryResult = $fiberResults[$connId] ?? null;
    unset($fiberResults[$connId]);
    try {
        $response = $fiber->getReturn() ?? '';
    } catch (\Throwable $returnFailure) {
        if (($boundaryResult['request_exit'] ?? false) === true) {
            $response = "HTTP/1.1 204 No Content\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
        } else {
            $failure = $boundaryResult['failure'] ?? $returnFailure;
            \Weline\Server\Service\WorkerResponseMemoryGuard::requestDrainAfterResponse(
                'request_fiber_boundary_failure'
            );
            WlsLogger::error_(
                'Request Fiber boundary failed; Worker quarantine requested: ' . $failure->getMessage()
            );
            $body = (string)__('请求边界清理失败，请稍后重试。');
    ...
    $handleDuration = \round((wlsWorkerMonotonicNow() - $handleStartTime) * 1000, 2);
    ...
    foreach ($activeFibers as $afConnId => $afData) {
        ...
        if ($af->isTerminated()) {
            ...
            wlsFinalizeTerminatedFiberResponseStep(
                ...
                (float) ($afData['handleStartTime'] ?? wlsWorkerMonotonicNow()),
                $connections,
                $requestBuffers,
                $connectionLastActivity,
                $requestLogged,
...
function wlsReleaseIdleFibersStep(
    ...
    int|float &$lastFiberIdleCheck,
    array &$writeBuffers,
    array &$writableConnections,
    array &$pendingClose
): void {
    $nowMonotonicNs = \Weline\Server\Runtime\WorkerFiberContextTracker::monotonicNowNs();
    $idleCheckInterval = 5;
    $doReleaseIdle = $fiberReleaseIdleRequested
        || ($fiberIdleTtlSec > 0
            && ($nowMonotonicNs - $lastFiberIdleCheck) >= ($idleCheckInterval * 1_000_000_000));
    if (!$doReleaseIdle || $activeFibers === []) {
        return;
    }

    $lastFiberIdleCheck = $nowMonotonicNs;
    ...
    foreach ($activeFibers as $afConnId => $afData) {
        $idleDecision = \Weline\Server\Runtime\WorkerFiberContextTracker::idleReleaseDecision(
            $afData,
            $nowMonotonicNs,
            $fiberHeartbeatTimeout,
            $releaseThreshold,
        );
        if (($idleDecision['release'] ?? false) !== true) {
            continue;
        }
        if (($idleDecision['reason'] ?? '') === 'heartbeat_timeout') {
            $inactiveTime = (int)\floor((float)($idleDecision['inactive_seconds'] ?? 0.0));
    ...
    foreach ($toRelease as $afConnId => $afData) {
        $releasedFiber = $afData['fiber'];
        $fiberScheduler->cancelTimersForFiber($releasedFiber);
        wlsUnwindRequestFiberForCancellation(
            $releasedFiber,
            $afData['context'] ?? null,
            'idle_or_heartbeat_timeout',
        );
        \Weline\Framework\Manager\ObjectManager::clearRequestScopeForFiber($releasedFiber);
 ...
 * @param array<int, array{connection: resource, started_at: float, retry_at: float, attempts: int}> $writeZeroProgress
 */
        ...
        while (isset($writeBuffers[$connId]) && $writeBuffers[$connId] !== '' && $writeAttempts < $maxWriteAttempts) {
            ...
            if ($written === 0) {
                ...
                if ($streamTimedOut || $zeroElapsed >= $zeroProgressTimeoutSeconds) {
                    ...
                    WlsLogger::warning_(
                        "HTTP 写队列无进展，关闭连接 (connId: {$connId}, reason: {$reason}, "
...
/**
 * 取消普通 HTTP 连接上的请求 Fiber，并确保 activeRequests 只递减一次。
 *
 * @param array<int,array<string,mixed>> $activeFibers
 */
function wlsCancelActiveFiberForHttpConnection(
    array &$activeFibers,
    int $connectionId,
    \Weline\Server\Scheduler\FiberScheduler $fiberScheduler,
    int &$activeRequests
): bool {
    if (!\array_key_exists($connectionId, $activeFibers)) {
        return false;
    }

    $state = $activeFibers[$connectionId];
    unset($activeFibers[$connectionId]);
...
/**
 * A WebSocket becomes a frame connection only after the application response
 * passed the complete RFC 6455 upgrade validation.
 */
function wlsHttpIsActiveWebSocketConnection(mixed $state): bool
{
...
/**
 * Advance one non-blocking WebSocket read turn on the production HTTP worker.
 *
 * @param array<int,resource> $connections
 * @param array<int,string> $requestBuffers
 * @param array<int,array<string,mixed>> $longLivedConnections
 */
function wlsHttpWebSocketReadStep(
    mixed $conn,
    int $connId,
    array &$connections,
    array &$requestBuffers,
    ...
    if (!\is_resource($conn)
        ...
        || $connections[$connId] !== $conn
        || !wlsHttpIsActiveWebSocketConnection($longLivedConnections[$connId] ?? null)
    ) {
        unset(
            $connections[$connId],
            $requestBuffers[$connId],
    ...
    if (($result['error_code'] ?? null) !== null) {
        WlsLogger::warning_(
            'WebSocket 协议错误，关闭连接 (connId: ' . $connId
...
function wlsDispatchRequestFiberStep(
    ...
    float &$lastLongLivedSaturationReport,
    ...
    $staticFastPathStartedAt = wlsWorkerMonotonicNow();
    ...
    try {
        $suspendedFiberContext = \Weline\Framework\Runtime\WlsFiberContext::captureForFiber($requestFiber);
        ...
        $fiberScheduler->cancelTimersForFiber($requestFiber);
        wlsUnwindRequestFiberForCancellation(
            $requestFiber,
            null,
    ...
    $fiberActivityMonotonicNs = \Weline\Server\Runtime\WorkerFiberContextTracker::monotonicNowNs();
    $activeFibers[$connId] = [
        'fiber' => $requestFiber,
        'conn' => $conn,
        'rawRequest' => $rawRequest,
        'handleStartTime' => $handleStartTime,
        'context' => $suspendedFiberContext,
    ...
    // A normal suspended request did not mutate the long-lived registry and
    // must not turn Fiber dispatch into an O(n) connection-table scan. The
    // only non-long-lived exception is a previously reported saturation that
    // still needs its one-shot cleared transition.
...
function sendResponseAndCleanup(
    ...
    if ($responseStatus === 101) {
        ...
        // Matcher 只表示请求意图；应用没有返回有效 101 时仍是普通 HTTP。
        unset($longLivedConnections[$connId]);
    }
    ...
    if ($drainRequestedBeforeResponse && !$isSseMode && !$isWebSocketMode) {
...
function handleRequest(
    ...
    try {
        ...
        try {
            ...
            $handleEndTime = wlsWorkerMonotonicNow();
