...
WLS 是常驻内存的多进程 HTTP/HTTPS 运行时。`auto` 在 Linux/macOS 选择 direct，Windows 只选择 Dispatcher。HTTPS 默认同时启用协议边缘：客户端在公开端口通过 QUIC/TLS 自动协商 HTTP/3、HTTP/2 或 HTTP/1.1，协议边缘再复用私有 HTTP/1.1 长连接进入 Worker。这里的 direct 精确定义为“没有 WLS Dispatcher”；协议边缘只承担 TLS/QUIC、ALPN、会话票据和连接多路复用，不执行安全规则、FPC、静态缓存或业务路由。所有拓扑共用 Master、IPC、READY、RuntimePolicyBundle 和 WorkerPolicyKernel，因此 direct 不会丢失策略。
...
- 协议边缘 active config digest：仅证明 Caddy 已原子接受当前 READY upstream 集合；不能代替 Master Registry 或策略 digest。
...
HTTPS 默认协议边缘启用时，公开 TCP/UDP 端口由 Caddy accept；POSIX direct 不启动 WLS Dispatcher，Caddy 只把已完成协议解复用的请求发送到 READY Worker 的 loopback 端口。Worker 私有端口要求 loopback peer 与实例级 256-bit token 同时成立，绕过边缘直打 Worker固定返回 403。关闭协议边缘且只启用 HTTP/1.1 的兼容配置，才使用 Linux SO_REUSEPORT 或 macOS Master 共享 listener FD 让 Worker 直接 accept。
...
协议边缘启用时，上述 `accept/TLS` 被拆成“Edge 公开 accept/TLS/QUIC”和“Worker 私有 HTTP/1.1 accept”，但从 `WorkerPolicyKernel` 开始的阶段完全相同。Caddy 会覆盖而不是透传客户端自带的内部 token、客户端协议和 `X-Forwarded-*`；Worker 只有在 token 校验及 loopback 校验都通过后才采用这些 transport metadata，并在进入模块代码前删除内部认证 Header。
...
15. 仅在 Worker 自行终结 TLS 的 legacy transport 中，OpenSSL 首请求用户态缓冲才需要 200ms 有界首读泵送；协议边缘模式由 Caddy 终结 TLS，Worker 私有 keep-alive 路径不执行该补偿。
...
### 3.8.8 2026-07-13 h3/h2/h1 协议边缘与 Dispatcher FPC 修复
