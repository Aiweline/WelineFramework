...
WLS 是常驻内存的多进程 HTTP/HTTPS 运行时。`auto` 在 Linux/macOS 选择 direct，Windows 只选择 Dispatcher。HTTPS 默认启用 **WLS Native Protocol Engine**：客户端在公开端口通过 QUIC/TLS 自动协商 HTTP/3、HTTP/2 或 HTTP/1.1，原生协议引擎再复用私有 HTTP/1.1 长连接进入 Worker。这里的 direct 精确定义为“没有 WLS Dispatcher”；原生协议引擎只承担 TLS/QUIC、ALPN、会话票据和连接多路复用，不执行安全规则、FPC、静态缓存或业务路由。所有拓扑共用 Master、IPC、READY、RuntimePolicyBundle 和 WorkerPolicyKernel，因此 direct 不会丢失策略。Caddy 只保留为运维显式选择的迁移兼容适配器，不属于默认依赖、推荐链路或 WLS 产品架构。
...
- `HttpProtocolSelection`：公开协议顺序、协议边缘、Alt-Svc 与 TLS 会话复用的唯一选择结果；`server:status` 必须展示实际值。
- RuntimePolicyBundle active digest：当前进程应执行规则的唯一版本。
- WLS Native Protocol Engine active config digest：仅证明原生协议引擎已原子接受当前 READY upstream 集合；不能代替 Master Registry 或策略 digest。
...
flowchart LR
  ...
  M --> E["WLS Native Protocol Engine\nQUIC/TLS + ALPN + session tickets"]
  M -->|"Windows 默认 / POSIX 显式"| D["Dispatcher\n实例级 L4 guard + READY 透传"]
  E -->|"POSIX direct：私有长连接池"| W["READY Workers\nWorkerPolicyKernel"]
  E -->|"Windows / 显式 dispatcher"| D
  D --> W
  CL["Linux/macOS Client"] -->|"h3/h2/h1 自动协商"| E
  CW["Windows Client"] -->|"h3/h2/h1 自动协商"| E
  P --> D
  P --> W
...
HTTPS 默认启用 WLS Native Protocol Engine，由它直接 accept 公开 TCP/UDP 端口；POSIX direct 不启动 WLS Dispatcher，原生协议引擎只把已完成协议解复用的请求发送到 READY Worker 的 loopback 端口。Windows 则由原生协议引擎进入 WLS 内部 Dispatcher。Worker 私有端口要求 loopback peer 与实例级 256-bit token 同时成立，绕过原生入口直打 Worker 固定返回 403。关闭协议引擎且只启用 HTTP/1.1 的兼容配置，才使用 Linux SO_REUSEPORT 或 macOS Master 共享 listener FD 让 Worker 直接 accept。
...
WLS Native Protocol Engine 启用时，上述 `accept/TLS` 被拆成“原生入口公开 accept/TLS/QUIC”和“Worker 私有 HTTP/1.1 accept”，但从 `WorkerPolicyKernel` 开始的阶段完全相同。原生入口会覆盖而不是透传客户端自带的内部 token、客户端协议和 `X-Forwarded-*`；Worker 只有在 token 校验及 loopback 校验都通过后才采用这些 transport metadata，并在进入模块代码前删除内部认证 Header。

### 3.0.1 HTTP 自动协商与连接复用

| 客户端能力 | 公开协议 | 发现/协商 | Worker 内部协议 |
|---|---|---|---|
| HTTP/3 | h3 / QUIC UDP | 客户端直接发起 QUIC，或由 `Alt-Svc`/HTTPS SVCB 发现 | HTTP/1.1 keep-alive |
| HTTP/2 | h2 / TLS TCP | ALPN `h2` | HTTP/1.1 keep-alive |
| HTTP/1.1 | h1 / TLS TCP | ALPN 回退 | HTTP/1.1 keep-alive |

- 默认启用 `h3,h2,h1`；HTTP 实例固定 h1。HTTP/3 与 HTTP/2 不满足能力时不得伪装为已启用。
- TLS 1.2/1.3 session ticket 默认开启；客户端可复用 TLS 会话，不需要每个连接都做完整握手。HTTP/2/3 在单连接内复用多个并发 stream。WLS Native Protocol Engine 使用实例隔离的持久 ticket key ring：密钥只写入 `var/server/protocol-edge/{instance}/session-ticket-keys.json`，12 小时轮换、最多保留 4 把；配置 reload 或原生协议进程重启后仍可解密已签发票据，不与其它 WLS 实例共享密钥。
- 公网 TLS 与 legacy Worker TLS 使用同一实例级 `wls.ssl` 事实：原生协议引擎按允许集合生成精确 TLS min/max，`performance` profile 使用 `x25519/secp256r1`；不能出现 Worker 是 TLS 1.3-only、公开 h3/h2 入口却仍接受 TLS 1.2 的分叉。
- `server:start` 在任何 Master/Worker 创建前验证 WLS Native Protocol Engine 的当前平台二进制、ALPN、QUIC、session ticket、upstream keep-alive 与原子 reload 能力。缺失时只下载固定摘要的 Go 工具链并从仓库内固定模块构建自包含二进制，构建后重新执行真实 probe；失败即停止启动，不以静默退回 h1 或临时内存票据掩盖能力缺失。
- WLS Native upstream 连接池只连接当前 READY 集合。Direct rolling reload 先发布新集合并等待 active digest ACK，再 drain 旧 Worker；公开 TLS/QUIC listener 与既有连接在代码 reload 时保持存活，持久 ticket key ring 同时保证新连接的 session resumption 不因配置装载而失效。
...
1. 正常启动或重载时，Dispatcher 路由快照、协议边缘 upstream 集合和 legacy direct 公开 accept 都不得接纳非 READY Worker。
...
5. Dispatcher 一个批次只发布两次业务快照；协议边缘 direct 使用代际 fence、不可变配置 digest 与 active ACK 保证新旧代安全切换；legacy direct 使用公开 bind ACK。
...
7. Dispatcher 只负责 L4 准入、路由、字节透传、背压和故障转移，不负责 HTTP 规则、FPC、静态缓存或首页渲染；生命周期变化只由 Master 决策。协议边缘启用时，私有复用连接的 AcceptGate 仅执行实例总连接/总速率/超时并将 edge loopback 视为 transport whitelist；真实客户端 IP 规则始终由 Worker 按认证请求 envelope 执行，不能用连接级 PROXY v2 代替。
...
13. 协议边缘 direct 的公开主端口由边缘独占，Worker loopback 端口按槽位独占；legacy direct 的公开主端口才是多 Worker 共享资源。单 Worker 自主回收只能按冻结的 PID/lease 清理，禁止按公开端口猜测并 kill。
14. 仅在协议边缘关闭的 macOS legacy direct 中，`php://fd/3` listener 才由 Worker 继承；它只能用于事件就绪，TLS Worker 必须原生 `socket_accept()` 后 `socket_export_stream()`。
15. 仅在 Worker 自行终结 TLS 的 legacy transport 中，OpenSSL 首请求用户态缓冲才需要 200ms 有界首读泵送；默认由 WLS Native Protocol Engine 终结公开 TLS，Worker 私有 keep-alive 路径不执行该补偿。
...
22. Worker 默认不按累计请求数自动回收；`worker_max_requests` 只有显式配置为正数时生效。阈值到达后必须先关闭 listener 并进入 drain，再等待已接纳请求有界完成；禁止等待长期 keep-alive 连接表自然清空，也禁止多个超限 Worker 在压力结束时同时退出。
...
|---|---|---|---|---|
| Windows | 固定 K 路 PowerShell launcher | Dispatcher + 当前 ABI 可验证的最优 Worker | 2/4/8/16 Worker 实机；核对 raw PID/REGISTER PID、helper TTL/临时文件、Defender 下 p95 | PHP 8.4 只安装官方 PECL event 3.1.4 精确 ABI 包，固定摘要并由新 PHP 实载验证；无可信包时 stream/select 是平台稳定基线 |
...
本节保留替换前的性能与缺陷证据，便于回归对照；它不是当前默认架构。当前默认协议入口已经由 WLS Native Protocol Engine 接管。
...
跨平台启动探测不再依赖版本字符串猜测。Linux ext-event 使用排在 sockets 之后的 `30-event.ini`，安装完成后由新的同一 PHP 二进制重新验证；SO_REUSEPORT 通过真实双 socket bind/listen 与 512 次分流采样确认。本节记录当时 Caddy 兼容边缘的真实探测基线，现仅用于对照；默认 WLS Native Protocol Engine 使用自身 capability probe。
...
### 3.8.11 2026-07-14 WLS Native Protocol Engine 接管公开协议

默认 `auto/native` 已不再启动或探测 Caddy。仓库内 Go 模块构建自包含的 WLS 原生协议二进制，直接拥有公开 TCP/UDP listener、TLS 1.2/1.3、ALPN h2、QUIC h3、Alt-Svc、持久 session ticket key ring、0-RTT 安全方法门禁、READY upstream keep-alive 池和原子配置 reload。Caddy 只在用户显式配置 `wls.http.protocol_edge=caddy` 时进入兼容链路。
...
### 3.8.12 2026-07-15 Native TLS profile、Windows Dispatcher 启动闭环与最终门禁

WLS Native Protocol Edge 现在与 PHP Worker 使用同一个 `wls.ssl.key_exchange_profile` 事实源。`performance` 在 Go TLS 入口固定为 `X25519,P-256`，`system` 才交还 Go 运行时默认组；实例、Endpoint、Master IPC 和 benchmark 报告都发布有效 profile，避免公开 Native 入口与 Worker 入口使用不同密钥交换策略。协议入口仍不固定 TLS 1.3 cipher suite，由客户端和 TLS 实现协商。
...
| 平台进行中 | Linux/Windows 原生矩阵 | 独立 runner / CI | Linux Native 长稳已通过；Windows ARM64 单 Worker Dispatcher、三协议、TLS 复用已通过，空闲机 4/16 Worker 与长稳仍需复验 |
