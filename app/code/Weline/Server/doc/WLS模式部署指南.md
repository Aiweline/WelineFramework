...
| 模式 | 说明 |
|------|------|
| **WLS 直接对外（推荐）** | WLS 原生监听 **80（HTTP）/ 443（HTTPS）**；Linux 默认 SO_REUSEPORT direct，macOS 默认共享 listener FD direct，Windows 默认 Dispatcher。 |
| **WLS 自定义端口** | 使用 `-p 9981` 等监听高端口，适合开发或内网入口。 |
| **外部反向代理（可选）** | 只有已有统一入口、跨机 LB 或独立证书终结需求时，才在 WLS 单一公开端口前放置 Nginx/Caddy；WLS 本身不依赖它们。 |
...
**Master 不做数据面流量分发**：Windows 单端口多 Worker 分流由 Dispatcher 完成；Linux 由 SO_REUSEPORT 分流；macOS 虽由 Master bind 一个公开 listener，但 Master 只持有/传递 FD，不 accept 连接，真实 accept 由 Worker 竞争同一队列。Direct 时客户端直接连 Worker，不存在 Dispatcher 或第二条后端连接。
...
| 单端口 443 多 Worker 负载均衡 | ✅ 已实现 | Windows 使用 Dispatcher；Linux 使用 SO_REUSEPORT；macOS 使用共享 listener accept queue |
...
- **direct**：Linux/macOS 默认；所有 Worker 共享公开端口。POSIX direct 固定使用 stream SSL + event loop，`event_buffer + direct` 会在启动预检时拒绝。
...
- Linux direct：4 个 Worker 使用 SO_REUSEPORT 共享同一公开端口。
- macOS direct：Master 只 bind 一次并传递 listener FD 3，4 个 Worker 在同一 accept queue 上接收连接。
- Windows/Dispatcher：公开端口只由 Dispatcher 监听，Worker 后端端口是实例内部实现，不对客户端公开。

Nginx/Caddy 不是 WLS 依赖；仅在选择外部代理时，将它代理到 WLS 的一个公开端口。如需跨机高可用，应由外部 LB 在多个 WLS 实例之间均衡，而不是直连某个 Worker 的内部端口。
...
WLS 的生产 TCP 默认是 HTTP/2，客户端或运行时不满足时自动回退 HTTP/1.1。HTTP/3 由 WLS 自带的 ngtcp2/nghttp3/OpenSSL 原生 QUIC Adapter 提供，不进入 TCP ALPN 默认顺序；只有 UDP listener、Adapter 自检、策略、Worker 和预热全部 READY，且请求通过 QUIC 到达或浏览器依据 WLS 发布的 `Alt-Svc` 升级时才使用。WLS 不依赖 Caddy、Nginx 或 Go sidecar。HTTP/2 Adapter 现在宣告 `SETTINGS_MAX_CONCURRENT_STREAMS=64`，每个 Stream 使用独立 Fiber；Worker 用默认 32（可配范围 16–32）的解析准入预算、512KiB 待写高水位、pending-response gate 和连接轮转做有界背压。只有“一个 TLS 连接上多个同时在途 Stream 全部完成”的运行门禁才算多路复用证据，单纯打开客户端 multiplex 选项或只看连接复用率都不算。

HTTP/3 原生组件只能由运维显式执行 `php bin/w server:http3:build` 准备，且当前 PHP 必须已经预装并启用 FFI；该命令不会安装 FFI 或修改 `ffi.enable`。Linux 路径可下载固定摘要的 OpenSSL 3.5.7、ngtcp2 1.23.0、nghttp3 1.16.0 与 curl 8.21.0 源码，并把依赖封装进 WLS 私有 PIC-static transport。macOS 路径编译 WLS 传输桥接库后，把全部非系统 dylib 稳定快照到 owner-only、内容寻址目录，改写 install name/edge 为 `@loader_path` 私有闭包，删除所有 `LC_RPATH`，逐个 ad-hoc codesign，再核对哈希、owner、权限、签名、依赖图和 dyld 实际 loaded-image 绝对路径。两条路径都先发布不可用于生产的 immutable candidate；只有新 PHP 子进程精确固定 fingerprint/SHA 并通过真实 QUIC/TLS、Ticket 轮换与独立 TLS-context Session 恢复自检后，才在锁内原子更新 active。普通 `server:start` 只读选择身份和当前运行证据仍匹配的 manifest，不调用编译器或自检；组件缺失、证据失效或 Linux eBPF 路由权限不足时不发布 `Alt-Svc`，Direct TCP 继续使用 HTTP/2 并自动回退 HTTP/1.1。`linux_pic_static_dependency_bundle` 只有在当前进程确为匹配平台/架构的 Linux、manifest ready、`dependency_linkage=pic-static` 且当前版本化运行证据全部成立时才为 true；通用库可加载或 macOS 证据不能推导该 Linux readiness。
