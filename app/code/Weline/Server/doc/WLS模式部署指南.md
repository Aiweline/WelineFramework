...
> 性能提示：`server:benchmark --instance <name>` 自动压测该实例绑定的公网端点。Nginx 模式要求 owner/config generation 一致且已经 live 验证；纯 WLS 模式要求 endpoint 的 edge/origin/protocol policy 一致。只有显式内部 host/port 才归因为 `wls_endpoint`。托管 Nginx **默认最佳性能配置**：GET 边缘微缓存（`edge_cache=true`，TTL 60s）、可用时 gzip（comp_level=2）、upstream keepalive=256、worker_connections=32768、access_log off。**HTML/API** 在有 Cookie/Authorization/Upgrade 时跳过边缘缓存；**静态扩展名**（css/js/图片字体等）使用独立 location，浏览器同域自动带 Cookie 时仍可 HIT。健康检查 `/_wls/` 不走边缘缓存。修改 `proxy_cache_path` 的 zone 大小后需 `server:nginx:stop` 再 `start`（reload 无法重建共享内存区）。
>
