...
/**
 * Weline Server Linux 直连策略
 * 
 * 使用 SO_REUSEPORT 特性，多个 Worker 进程直接监听同一端口。
 * 内核自动进行负载均衡，无需 Dispatcher 中间层。
 * 
 * 优势：
 * - 最优性能：无中间层，直接处理请求
 * - 多核并行：SSL 握手分散到各 Worker
 * - 简单架构：无 Dispatcher 进程
 * 
 * 适用平台：Linux 3.9+、macOS
 * 
 * @author Aiweline
 ...
 */
class LinuxDirectStrategy implements ServerStrategyInterface
{
    /**
    ...
    {
        return __('Linux/Mac SO_REUSEPORT 直连模式');
    }
    ...
    public function start(ServerConfig $config): bool
    {
        ...
        // 启动 HTTP 重定向 Worker（如果启用）
        if ($config->httpRedirectEnabled && $config->sslEnabled) {
            $this->startHttpRedirectWorker($config);
        }
     ...
     * 启动 Worker 进程
     * 
     * 所有 Worker 监听同一端口，使用 SO_REUSEPORT 特性
     * 
     ...
     */
    private function saveInstanceInfo(ServerConfig $config, array $workerPids): void
    {
        ...
        $data = [
            ...
            'master_mode' => 'linux-direct',  // 运行模式标识，供 Master 进程使用
            'main_port' => $config->port,     // 主端口（所有 Worker 共用此端口）
            'host' => $config->host,
            'port' => $config->port,
    ...
    public function getStatus(string $instanceName): array
    {
        ...
        return [
            'running' => !empty(\array_filter($workers, fn($w) => $w['running'])),
            'mode' => $this->getIdentifier(),
            'workers' => $workers,
            'dispatcher' => null, // 直连模式无 Dispatcher
    ...
    {
        return __('Worker 直接监听端口（SO_REUSEPORT），内核自动负载均衡，无需 Dispatcher 中间层');
    }
}
