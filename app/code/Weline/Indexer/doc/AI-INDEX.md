...
4. 本文件：`app/code/Weline/Indexer/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Indexer/doc/README.md`
6. 只读取本次任务相关源码、配置和验证入口

## 模块身份

- 模块代码：`Weline_Indexer`
- 目录：`app/code/Weline/Indexer`
- Vendor：`Weline`
- Module：`Indexer`
...
入口/配置文件：
- `app/code/Weline/Indexer/composer.json`
...
- `app/code/Weline/Indexer/doc/README.md`
- `app/code/Weline/Indexer/doc/功能现状.md`
- `app/code/Weline/Indexer/doc/开发日志.md`
- `app/code/Weline/Indexer/doc/需求.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Indexer> [operation]` 或对应 `w_query` 帮助。
