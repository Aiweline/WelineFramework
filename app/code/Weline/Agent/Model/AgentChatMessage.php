...
{
    public const schema_table = 'weline_agent_chat_message';
    ...
    private ?AgentChatSession $session = null;
    ...
    public function getSession(): ?AgentChatSession
    {
        if ($this->session === null && $this->getData(self::schema_fields_SESSION_ID)) {
            $this->session = (new AgentChatSession())->load($this->getData(self::schema_fields_SESSION_ID));
        }
        return $this->session;
    }
