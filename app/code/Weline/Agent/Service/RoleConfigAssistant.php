...
class RoleConfigAssistant
{
    ...
    public function getTemplates(array $availableSkillCodes): array
    {
        $templates = [
            ...
            [
                'code' => 'coding_agent',
                'name' => '编码助手',
                'description' => '代码阅读、调试、Shell 诊断与补丁规划。',
                'scenario_adapter_code' => 'agent',
                ...
                'best_for' => ['调试', '自动化', '实现支持'],
            ],
            [
                'code' => 'it_ops_assistant',
                'name' => 'IT 运维助手',
                'description' => '监控、事件分流与稳定性优先排障。',
                'scenario_adapter_code' => 'agent_it_ops',
                ...
                'best_for' => ['健康检查', '日志分析', '事件 SOP'],
            ],
            [
                'code' => 'seo_assistant',
                'name' => 'SEO 助手',
                'description' => '关键词策略、优化规划与内容结构指导。',
                'scenario_adapter_code' => 'agent_seo',
    ...
    public function buildSuggestion(array $input, array $models, array $adapters, array $skills): array
    {
        ...
        if ($aiDraft !== null) {
            ...
            $warnings[] = (string) __('AI 建议不可用，已应用模板草稿。');
        } else {
            $warnings[] = (string) __('未找到可用模型，已应用模板草稿。');
        }
    ...
    private function buildTemplateDraft(
        ...
        $profile = $this->normalizeProfile((string) ($input['agent_profile'] ?? 'multi_project'));
        $roleName = trim((string) ($input['role_name'] ?? ''));

        if ($roleName === '') {
            $roleName = (string) ($template['name'] ?? 'Agent Assistant');
        ...
        $adapterCode = (string) ($template['scenario_adapter_code'] ?? 'agent');
        ...
        return [
            'code' => $this->slugify((string) ($input['role_code'] ?? (string) ($template['code'] ?? 'agent_role'))),
            'name' => $roleName,
    ...
    private function buildAiDraft(array $input, array $template, array $models, array $adapters, array $skills): ?array
    {
        ...
        $prompt = [
            'You configure Weline_Agent roles.',
    ...
    private function sanitizeDraft(array $draft, array $template, array $models, array $adapters, array $skillCodes): array
    {
        ...
        if ($name === '') {
            $name = (string) ($template['name'] ?? 'Agent Assistant');
        }

        $code = $this->slugify((string) ($draft['code'] ?? ''));
        if ($code === '') {
            $code = $this->slugify((string) ($template['code'] ?? 'agent_role'));
        }
    ...
    private function getProfileMeta(string $profile): array
    {
        return match ($profile) {
            'single_project' => [
                'name' => '单项目',
                'description' => '面向单项目的轻量协作助手。',
                'description_suffix' => '针对单项目聚焦优化。',
            ...
            'enterprise' => [
                'name' => '企业治理',
                'description' => '以治理为先、可审计可预期的执行助手。',
                'description_suffix' => '针对治理与合规优化。',
            ...
            default => [
                'name' => '多项目',
                'description' => '面向多项目并行与优先级分发的协调助手。',
                'description_suffix' => '针对多项目路由优化。',
