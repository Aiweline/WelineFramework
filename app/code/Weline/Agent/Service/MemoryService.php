...
class MemoryService
{
    ...
    public function __construct(
        private readonly AgentMemoryNode $memoryNode,
        private readonly AgentMemoryEdge $memoryEdge,
     ...
     * 从对话结果中提取并保存记忆
     */
    public function extractAndSave(AgentResult $result, AgentChatSession $session): void
    {
    ...
    public function getRelevantMemories(string $contextId, int $limit = 10): array
    {
        ...
        $memories = $this->memoryNode->reset()
            ->where(AgentMemoryNode::schema_fields_STATUS, AgentMemoryNode::STATUS_ACTIVE)
            ->order(AgentMemoryNode::schema_fields_IMPORTANCE, 'DESC')
            ->order(AgentMemoryNode::schema_fields_LAST_ACCESSED, 'DESC')
    ...
    public function search(string $query, int $limit = 10): array
    {
        ...
        $memories = $this->memoryNode->reset()
            ->where(AgentMemoryNode::schema_fields_STATUS, AgentMemoryNode::STATUS_ACTIVE)
            ->whereLike(AgentMemoryNode::schema_fields_NODE_VALUE, "%{$query}%")
            ->order(AgentMemoryNode::schema_fields_IMPORTANCE, 'DESC')
            ->limit($limit)
            ->select()
            ->fetch();
    ...
    public function saveMemoryNode(
        ...
        $existing = $this->memoryNode->reset()
            ->where(AgentMemoryNode::schema_fields_NODE_KEY, $key)
            ->find()
            ->fetch();

        if ($existing->getId()) {
            // 更新现有记忆
            $existing->setData(AgentMemoryNode::schema_fields_NODE_VALUE, $value);
            // 更新重要性（取最大值）
            $currentImportance = (float) $existing->getData(AgentMemoryNode::schema_fields_IMPORTANCE);
            $existing->setData(AgentMemoryNode::schema_fields_IMPORTANCE, max($currentImportance, $importance));
        ...
        $node->setData(AgentMemoryNode::schema_fields_NODE_TYPE, $type);
        $node->setData(AgentMemoryNode::schema_fields_NODE_KEY, $key);
        $node->setData(AgentMemoryNode::schema_fields_NODE_VALUE, $value);
        $node->setData(AgentMemoryNode::schema_fields_IMPORTANCE, $importance);
        $node->setData(AgentMemoryNode::schema_fields_STATUS, AgentMemoryNode::STATUS_ACTIVE);

        if ($session) {
            $node->setData(AgentMemoryNode::schema_fields_SESSION_ID, $session->getId());
        }
    ...
    public function createEdge(
        ...
        $edge->setData(AgentMemoryEdge::schema_fields_SOURCE_NODE_ID, $sourceNodeId);
        $edge->setData(AgentMemoryEdge::schema_fields_TARGET_NODE_ID, $targetNodeId);
        $edge->setData(AgentMemoryEdge::schema_fields_RELATION_TYPE, $relationType);
        $edge->setData(AgentMemoryEdge::schema_fields_WEIGHT, $weight);
        $edge->save();
    ...
    public function cleanup(int $batchSize = 100): int
    {
        ...
        foreach ($memories->getItems() as $memory) {
            if ($memory->shouldForget()) {
                $memory->setData(AgentMemoryNode::schema_fields_STATUS, AgentMemoryNode::STATUS_FORGETTING);
                $memory->save();
                $cleaned++;
            }
        }
    ...
    public function getUserPreferences(string $contextId): array
    {
        $preferences = $this->memoryNode->reset()
            ->where(AgentMemoryNode::schema_fields_NODE_TYPE, self::TYPE_PREFERENCE)
            ->where(AgentMemoryNode::schema_fields_STATUS, AgentMemoryNode::STATUS_ACTIVE)
            ->order(AgentMemoryNode::schema_fields_IMPORTANCE, 'DESC')
            ->limit(20)
            ->select()
        ...
        foreach ($preferences->getItems() as $pref) {
            $key = $pref->getData(AgentMemoryNode::schema_fields_NODE_KEY);
            $value = $pref->getData(AgentMemoryNode::schema_fields_NODE_VALUE);
            $result[$key] = $value;
        }
