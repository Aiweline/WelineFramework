...
class GlobFileSearchTool implements ToolInterface
{
    ...
    {
        return __('按 glob 模式查找文件（例如 **/*.php、app/code/**/*.phtml）。');
    }
    ...
    public function getParameters(): array
    {
        return [
            ...
            'properties' => [
                'glob_pattern' => [
                    'type' => 'string',
                    'description' => __('Glob 模式，例如 **/*.php 或 app/code/**/*.php'),
                ],
                'path' => [
                    'type' => 'string',
                    'description' => __('搜索基路径，默认 app/code'),
                ],
                'max_results' => [
                    'type' => 'integer',
                    'default' => 50,
                    'description' => __('最大返回文件数'),
                ],
            ],
