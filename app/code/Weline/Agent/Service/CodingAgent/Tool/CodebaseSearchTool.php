 ...
 * 否则回退到 grep 文本搜索。Agent 不强制依赖 AiKnowledge。
 */
class CodebaseSearchTool implements ToolInterface
{
    ...
    {
        return __('对代码库做语义搜索：按含义查找相关代码、类与 API。用于探索结构、找相似实现或理解工作原理。');
    }
    ...
    public function getParameters(): array
    {
        return [
            ...
            'properties' => [
                'query' => [
                    'type' => 'string',
                    'description' => __('自然语言或关键词（例如「会话存在哪里」「处理 ACL 的类」）'),
                ],
                'type' => [
                    'type' => 'string',
                    'enum' => ['all', 'docs', 'api', 'code', 'config'],
                    'description' => __('过滤：all|docs|api|code|config，默认 all'),
                ],
                'module' => [
                    'type' => 'string',
                    'description' => __('按模块过滤，例如 Weline_Framework、Weline_Ai'),
                ],
                'limit' => [
                    'type' => 'integer',
                    'default' => 15,
                    'description' => __('最大结果数（1-30）'),
                ],
            ],
            'required' => ['query'],
        ];
