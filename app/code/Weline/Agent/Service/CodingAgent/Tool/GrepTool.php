...
class GrepTool implements ToolInterface
{
    ...
    {
        return __('在路径下按模式搜索文件，返回匹配行及其文件路径与行号。');
    }
    ...
    public function getParameters(): array
    {
        return [
            ...
            'properties' => [
                'pattern' => [
                    'type' => 'string',
                    'description' => __('搜索模式（支持正则）'),
                ],
                'path' => [
                    'type' => 'string',
                    'description' => __('搜索目录（相对项目根），默认 app/code'),
                ],
                'glob' => [
                    'type' => 'string',
                    'description' => __('文件 glob 过滤，例如 *.php'),
                ],
                'max_results' => [
                    'type' => 'integer',
                    'default' => 50,
                    'description' => __('最大返回结果数'),
                ],
            ],
            'required' => ['pattern'],
        ];
    ...
    public function execute(array $args): mixed
    {
        ...
        if (empty($pattern)) {
            return ['error' => __('pattern 必填')];
        }

        $fullPath = $this->resolvePath($path);
        if ($fullPath === null || !is_dir($fullPath)) {
            return ['error' => __('无效或不可访问的路径：%{1}', [$path])];
        }
