...
- **Hook 名称**：`Weline_Agent::chat::message::tools`
- **显示名称**：消息工具栏扩展
- **功能说明**：在聊天消息的工具栏区域触发，允许其他模块为消息添加自定义操作按钮，如复制、重新生成、收藏等。

## 使用方法

在模块的 `view/hooks/` 目录下创建文件：`view/hooks/Weline_Agent--chat--message--tools.phtml`
...
```html
<!-- 在模块的 view/hooks/Weline_Agent--chat--message--tools.phtml 文件中 -->
