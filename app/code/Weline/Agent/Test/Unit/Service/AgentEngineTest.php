...
namespace Weline\Agent\Test\Unit\Service;
...
use Weline\Agent\Interface\SkillInterface;
...
use Weline\Agent\Service\AgentEngine;
use Weline\Agent\Service\ChatSessionManager;
use Weline\Agent\Service\ContextBuilder;
use Weline\Agent\Service\MemoryService;
use Weline\Agent\Service\PermissionValidator;
use Weline\Agent\Service\SkillContext;
use Weline\Agent\Service\SkillPackageManager;
use Weline\Agent\Service\SkillResult;
...
final class AgentEngineTest extends TestCase
{
    ...
    /** @var AgentToolCall&MockObject */
    private AgentToolCall $toolCallModel;
    ...
    protected function setUp(): void
    {
        ...
        $this->toolCallModel = $this->getMockBuilder(AgentToolCall::class)
            ->disableOriginalConstructor()
    ...
    public function testExecuteReturnsAssistantContentWithoutToolCalls(): void
    {
        ...
        $messageRecord = $this->createStub(AgentChatMessage::class);
        $messages = [
        ...
        $this->sessionManager
            ...
            ->willReturnCallback(function (
                AgentChatSession $passedSession,
                string $messageRole,
                string $content,
                array $toolCalls = [],
                string $toolName = '',
        ...
        $this->assertSame(AgentChatMessage::ROLE_USER, $capturedWrites[0]['role']);
        $this->assertSame('Original prompt', $capturedWrites[0]['content']);
        $this->assertSame(AgentChatMessage::ROLE_ASSISTANT, $capturedWrites[1]['role']);
    ...
    public function testExecuteProcessesToolLoopAndPersistsAssistantToolRequest(): void
    {
        ...
        $session = $this->createSessionDouble();
        $messageRecord = $this->createStub(AgentChatMessage::class);
        $skillModel = $this->createMock(AgentSkill::class);
        $tools = [
            [
                'name' => 'demo.skill',
        ...
        $skillModel
            ...
            ->willReturnCallback(static function (string $field): mixed {
                if ($field === AgentSkill::schema_fields_CLASS_NAME) {
        ...
        $this->aiService
            ...
            ->willReturnCallback(function (
                ...
                $this->assertSame('agent', $scenarioCode);
                $this->assertSame($tools, $params['tools']);
        ...
        $this->assertSame(AgentChatMessage::ROLE_TOOL, $capturedWrites[2]['role']);
        ...
        $this->assertSame(AgentChatMessage::ROLE_ASSISTANT, $capturedWrites[3]['role']);
    ...
    public function testExecuteHumanizesInternalContractErrors(): void
    {
        ...
        $this->sessionManager
            ->expects($this->once())
            ->method('addMessage')
            ->with($session, AgentChatMessage::ROLE_USER, '提示词', [], '', '')
        ...
        $result = $this->engine->execute('提示词', $role, $session);
    ...
    private function createRoleDouble(): AgentRole
    {
        $role = $this->createMock(AgentRole::class);
        ...
        $role->method('getData')->willReturnCallback(static function (string $field): mixed {
            return match ($field) {
                AgentRole::schema_fields_CODE => 'role-code',
                AgentRole::schema_fields_NAME => '角色名称',
                AgentRole::schema_fields_SCENARIO_ADAPTER_CODE => '',
                AgentRole::schema_fields_MODEL_ID => 0,
                default => null,
            };
        });
    ...
    private function createSessionDouble(): AgentChatSession
    {
        $session = $this->createMock(AgentChatSession::class);
        $session->method('getId')->willReturn(11);
        $session->method('getData')->willReturnCallback(static function (string $field): mixed {
            return match ($field) {
                AgentChatSession::schema_fields_CONTEXT_ID => 'ctx-1',
                AgentChatSession::schema_fields_CHANNEL => AgentChatSession::CHANNEL_WEB,
                default => null,
            };
        });
