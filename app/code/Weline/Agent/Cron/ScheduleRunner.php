...
namespace Weline\Agent\Cron;

use Weline\Cron\CronTaskInterface;
use Weline\Agent\Model\AgentSchedule;
use Weline\Agent\Model\AgentScheduleLog;
...
class ScheduleRunner implements CronTaskInterface
{
    public function __construct(
        ...
        private readonly AgentScheduleLog $logModel,
    ...
    {
        return 'agent_schedule_runner';
    }

    public function tip(): string
    {
        return '检查并执行到期的 智能体调度任务';
    }
    ...
    public function execute(): string
    {
        ...
        foreach ($schedules->getItems() as $schedule) {
            ...
            try {
                // 创建执行日志
                $log = $this->logModel;
                $log->setData(AgentScheduleLog::schema_fields_SCHEDULE_ID, $schedule->getId());
                $log->setData(AgentScheduleLog::schema_fields_STATUS, AgentScheduleLog::STATUS_RUNNING);
                $log->setData(AgentScheduleLog::schema_fields_STARTED_AT, time());
                $log->setContextSnapshot($schedule->getContext());
                $log->save();
                $logId = $log->getId();

                // 获取角色
                $role = $this->roleModel->load($schedule->getData(AgentSchedule::schema_fields_ROLE_ID));
                if (!$role->getId()) {
                ...
                $result = $this->agentEngine->execute(
                    $schedule->getData(AgentSchedule::schema_fields_PROMPT),
                    $role
                );
