...
final class AcceptedOfferDetailEnricher
{
    ...
    public function __construct(
        ...
        private readonly mixed $structuredFetcher = null,
    ) {
        if ($cooldownMilliseconds < 0
            || $maximumAttempts < 1
            || $maximumAttempts > 5
            || ($structuredFetcher !== null && !is_callable($structuredFetcher))
        ) {
    ...
    public function enrich(array $collected, HanfuProductClassifier $classifier): array
    {
        ...
        foreach ($offers as $index => $offer) {
            ...
            try {
                $structuredFetcher = $this->structuredFetcher;
                if ($structuredFetcher === null) {
                    static $structuredClient = null;
                    $structuredClient ??= new PublicHttpClient();
                    $payload = $structuredClient->offerDetail($offerId);
                } else {
                    $payload = $structuredFetcher($offerId);
                }
                if (!is_array($payload)) {
                    throw new \RuntimeException('hanfu_1688_offer_mtop_response_invalid');
                }
                $detail = $this->parser->parseMtop($payload, $url);
                $detail['image_urls'] = $this->mergeImages(
                    is_array($offer['image_urls'] ?? null) ? $offer['image_urls'] : [],
                    is_array($detail['image_urls'] ?? null) ? $detail['image_urls'] : [],
                );
