...
final class PublicHttpClient
{
    ...
    public function __construct(
        ...
        if ($minimumDelayMilliseconds < 0 || $maximumResponseBytes < 1024) {
    ...
    /** @return array<string,mixed> */
    private function mtopRequest(string $data, ?string $timestamp, ?string $sign): array
    {
        $query = [
            'jsv' => '2.7.0',
            'appKey' => self::APP_KEY,
            'api' => self::FACTORY_API,
            'v' => self::FACTORY_API_VERSION,
            'type' => 'json',
            'dataType' => 'json',
            'data' => $data,
        ];
        ...
        $url = self::FACTORY_ENDPOINT . '?' . http_build_query($query, '', '&', PHP_QUERY_RFC3986);
