...
$done = '/tmp/p-img-opt-wave22-20260918/done.tsv';
$batch = '/tmp/p-img-opt-wave22-20260918/batch.tsv';
...
foreach ($jobs as $j) {
    ...
    $ar = $h > 0 ? $w / $h : 0.0;
    $short = min($w, $h);
    if ($short < 1024 || abs($ar - (float)$j['expect_ar']) > 0.06 || $bpp < 1.2 || $bpp <= (float)$j['src_bpp']) {
        fwrite(STDERR, "gate fail {$path}: {$w}x{$h} bpp={$bpp} ar={$ar}\n");
        exit(1);
    }
    ...
    fwrite($fh, "{$j['product_id']}\t{$j['expect_id']}\t{$j['object_key']}\t{$j['why']}\t{$j['before']}\t{$w}x{$h}\t否\t否\n");
