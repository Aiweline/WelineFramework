...
- commit claim 在写入前持久化；目录 DML、Store overlay、Inventory 命令与
  成功 receipt 处于同一目标事务边界
- 失败后目录与库存回滚，claim 复位为 `draft`；返回稳定错误码，不透传异常详情
- attrs/price 复制显式 Website 行及来源 Store 行；跨站先写目标 Website
  默认值，再把来源 Store overlay 写到目标 Store，`cleared` 保持终止语义

## 测试入口

```bash
php vendor/bin/phpunit --bootstrap app/code/Weline/Product/Test/Unit/bootstrap.php \
  app/code/Weline/Product/Test/Unit/Service/ProductCopyServiceTest.php
php vendor/bin/phpunit --bootstrap app/code/Weline/Product/Test/Unit/bootstrap.php \
