...
写入走 `SystemConfig::setScopedConfig`，与配置中心 website/store/channel 范围一致。

## 支付示例

见根目录 `deploy.env-map.php.example`：把 `payment/method/paypal/environment` 在 `dev` 写成 `sandbox`、`prod` 写成 `live`。密钥不要写进映射文件。
