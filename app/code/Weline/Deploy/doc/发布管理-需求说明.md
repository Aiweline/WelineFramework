...
- 点击发布后：
  ...
  5. **失败自动回滚（A，仅正式站）**：一旦 Git 更新已开始且工作树相对发布前 commit 有变化（或已写入版本戳），**仅当** `isProductionDeploy()` 为真时，`release()` 捕获异常后须本地恢复到发布前 commit（有分支则 `checkout -B`，避免 detached），还原 `current.json`（若曾覆盖），并写入一条 `trigger_type=auto_rollback` 历史；原失败记录的 `error_message` 追加「已自动回滚…」或「自动回滚失败…」。**开发 / local / pre / staging / test 环境不自动回滚**（错误信息追加「开发环境跳过发布失败自动回滚」），便于保留失败现场排查。备份失败、Git 未动前的失败不触发自动回滚。
...
- **保护说明**（静态文案）：明确列出不会覆盖的文件：
  ...
  - 以及 `app/code/Aiweline`、`app/code/GuoLaiRen`、`app/code/WeShop` 等业务模块目录（与 `Update\Core` 常量一致）。

#### 4.3.2 执行动作

- 按钮：**「开始核心更新」** → 确认弹窗 → POST 异步或同步执行 `Update\Core` 等价逻辑。
- 执行范围：复用 CLI 的 `CORE_UPDATE_PATHS`（`app/code/Weline` 与必要核心路径；不含业务模块）增量同步；**不**跑项目 `deploy:release` 全量 Git 拉取。冲突检测对比「上次同步到本地的核心 commit」；仅本地私改核心文件时拦截（或 `-f` 强制覆盖）。
- 正式站执行前同样触发 §4.4 整站备份；核心更新自身仍受 `CORE_UPDATE_PROTECTED_PATHS` 保护，不覆盖 env/部署配置。
