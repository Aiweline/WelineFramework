...
- 点击发布后：
  ...
  5. **失败自动回滚（A）**：一旦 Git 更新已开始且工作树相对发布前 commit 有变化（或已写入版本戳），`release()` 捕获异常后须本地恢复到发布前 commit（有分支则 `checkout -B`，避免 detached），还原 `current.json`（若曾覆盖），并写入一条 `trigger_type=auto_rollback` 历史；原失败记录的 `error_message` 追加「已自动回滚…」或「自动回滚失败…」。备份失败、Git 未动前的失败不触发自动回滚。
