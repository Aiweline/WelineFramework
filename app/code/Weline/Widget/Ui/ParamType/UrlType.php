...
class UrlType extends AbstractParamType
{
    ...
    public function getHtml(string $key, array $param, mixed $value, int|string $layoutId = '', array $attrs = []): string
    {
        ...
        $inputHtml .= '<input type="url" class="w-input" id="' . htmlspecialchars($fieldId) . '" name="' . htmlspecialchars($key) . '" value="' . htmlspecialchars($currentValue) . '" placeholder="' . htmlspecialchars($placeholder) . '"' . ($required ? ' required' : '') . '>';
        if (!empty($currentValue)) {
            $inputHtml .= '<a href="' . htmlspecialchars($currentValue) . '" target="_blank" rel="noopener" class="w-button" data-tone="neutral" data-variant="outline">' . __('测试') . '</a>';
        }
        $inputHtml .= '</div>';
        if ($openInNewTab !== null) {
            $targetFieldId = $fieldId . '_target';
            $targetValue = is_array($value) && isset($value['target']) ? $value['target'] === '_blank' : $openInNewTab;
            $inputHtml .= '<div class="w-check w-param-form-check w-param-form-check--spaced">';
        ...
        if (!empty($suggestions)) {
            ...
            foreach ($suggestions as $label => $suggestUrl) {
                $inputHtml .= '<button type="button" class="w-button w-param-url-suggestion" data-tone="quiet" data-size="sm" data-url="' . htmlspecialchars($suggestUrl) . '" data-target="' . htmlspecialchars($fieldId) . '">' . htmlspecialchars($label) . '</button> ';
            }
            $inputHtml .= '</div>';
        }
