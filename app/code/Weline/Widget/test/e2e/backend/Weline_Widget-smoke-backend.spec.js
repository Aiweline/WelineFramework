...
const CANDIDATE_ROUTES = ["editor","widget","preview/widget","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Widget 后台流程', () => {
  moduleCase(
    test,
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'WIDGET-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
