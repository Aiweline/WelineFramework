# 开发者工作空间文档

本文档库包含开发者工作空间模块维护的源文档。模块 `doc/` 文件跟随 Git 分发；运行时索引、搜索缓存、翻译结果和任务状态写入用户当前配置的主库。

## 跨模块边界

- `Weline_Backend` 是文档管理后台、作者目录和后台 ACL 的必需依赖。
- 语言列表与本地化名称只通过 `Weline_I18n\Api\Localization\LocaleCatalogInterface` 和 Framework 的 `LocaleNameProviderInterface` 读取，不直接访问 I18n Model。
- 文档翻译调用通过 `Weline_Ai\Api\AiRuntimeInterface` 进入 AI 运行时；模型、场景绑定、适配器扫描、账号可用性和 usage 聚合只通过 `Weline\Ai\Api\Configuration\ScenarioConfigurationInterface` 读取或写入。
- WLS 性能链路通过 `w_query('server', 'wlsPerformanceRequests|wlsPerformanceRequestDetail', ...)` 读取；短期 payload 只依赖 Framework `SharedCacheStateFactoryInterface`，未安装 Server 或非持久运行时自动回退本地文件。
- API 文档扫描只依赖 `Weline\Api\Api\Documentation\ApiDocumentationProviderInterface`；当前网站货币只依赖 `Weline\Websites\Api\Localization\WebsiteCurrencyCatalogInterface`。
- `Weline_Api`、`Weline_Server`、`Weline_Visitor`、`Weline_Websites`、`Weline_Seo` 是按能力启用的可选模块，必须在 `etc/module.php` 与 Composer `suggest` 同步声明。
- 新代码禁止引用其他模块的 `Model/Service/Helper/Controller`。跨模块返回值必须是标量、数组或 `Api` 下的不可变数据对象。

## 浏览器文档页 API 契约

- `/dev/tool/docs` 与 `/dev/tool/docs/api` 是前台独立工具页：控制器 `layoutType=blank.full`，模板必须是内容片段（禁止自绘完整 HTML），由前端主题空白全屏布局提供 document shell、Theme head/body hooks，以及 Frontend Header Base（Theme head → `Weline_Frontend::templates/public/head.phtml`）；页面只追加 docs/api 专用 CSS/JS，并在需要时调用 `Weline.load('api')`。
- 公开文档目录和搜索只调用 `developer_workspace.docsRequest`；该操作仅允许 `GET /dev/tool/docs/{tree|documents|document|search}`，并保留原 JSON 形状。
- API 调用测试工作台属于框架开发基础设施：Frontend Worker 继续走 `Weline.Api.resource(provider)[operation](params)`；本轮明确要求文档 REST 调试与登录经原生 `sendHttp` 保留 HTTP 状态、Header 和原始 JSON 响应，不再使用 `Weline.Api.request`。普通业务请求契约保持不变。
- 调试客户端的功能基线包括 REST/Worker 请求编辑、示例导入、Token、沙盒/正式环境、语言货币、状态/耗时/大小/Header/Body 响应诊断、SDK 下载与指南；迁移 UI 时不得删减这些产品能力。
- API 页面只使用原生 DOM 与 Weline UI 的 `w-*` 外壳；jQuery、jsTree、Bootstrap、Prism 和字体图标不属于运行时依赖。桌面分栏可调整，窄屏自动单列。
- 通用 `adminRequest` 继续要求后台身份，禁止为了公开文档读取或任意 REST 代理而放宽鉴权。
- 开发面板的 shell、session、document、routes、trace、DB explain 与 SEO crawl 必须经 `developer_workspace.panelRequest` 的固定路由/方法白名单；该 Query 操作仍由 `PanelAccessService` 执行 DEV 或生产 token-cookie 二次鉴权。前台禁止对上述路径使用 `adminRequest`（否则无后台 Session 会 toast「Operation authorization requirements are not satisfied.」）。
- 登录弹窗继承官方 `w-dialog` 与 `w-dialog__surface` 的统一尺寸和视口边界；页面不单独覆盖外框宽度。保留官方 toast 与浮层规则，避免外框小于表单导致登录按钮裁切。

## 统计信息

- 总文章数: 929
- 成功导出: 929
- 跳过文章: 0
- 创建目录: 499 个
- 创建缺失分类: 491 个
- 导出时间: 2025-09-17 09:34:49

## 分类统计

- **快速开始**: 2 篇文章
- **快速开始/Model**: 2 篇文章
- **快速开始/Controller**: 2 篇文章
- **框架规范**: 1 篇文章
- **快速开始/Event**: 2 篇文章
- **分类_50**: 1 篇文章
- **分类_51**: 1 篇文章
- **分类_52**: 1 篇文章
- **分类_53**: 1 篇文章
- **分类_54**: 1 篇文章
- **分类_55**: 1 篇文章
- **分类_56**: 1 篇文章
- **分类_58**: 1 篇文章
- **分类_59**: 10 篇文章
- **分类_60**: 2 篇文章
- **分类_61**: 3 篇文章
- **分类_62**: 2 篇文章
- **分类_66**: 1 篇文章
- **分类_69**: 1 篇文章
- **分类_70**: 1 篇文章
- **分类_71**: 1 篇文章
- **分类_72**: 1 篇文章
- **分类_73**: 1 篇文章
- **分类_74**: 1 篇文章
- **分类_75**: 1 篇文章
- **分类_76**: 1 篇文章
- **分类_78**: 1 篇文章
- **分类_79**: 10 篇文章
- **分类_80**: 2 篇文章
- **分类_81**: 3 篇文章
- **分类_82**: 2 篇文章
- **分类_86**: 1 篇文章
- **分类_89**: 1 篇文章
- **分类_90**: 1 篇文章
- **分类_91**: 1 篇文章
- **分类_92**: 1 篇文章
- **分类_93**: 1 篇文章
- **分类_94**: 1 篇文章
- **分类_95**: 1 篇文章
- **分类_96**: 1 篇文章
- **分类_98**: 1 篇文章
- **分类_99**: 10 篇文章
- **分类_100**: 2 篇文章
- **分类_101**: 3 篇文章
- **分类_102**: 2 篇文章
- **分类_106**: 1 篇文章
- **分类_109**: 1 篇文章
- **分类_111**: 2 篇文章
- **分类_112**: 2 篇文章
- **分类_114**: 3 篇文章
- **分类_115**: 1 篇文章
- **分类_116**: 1 篇文章
- **分类_117**: 1 篇文章
- **分类_118**: 1 篇文章
- **分类_119**: 1 篇文章
- **分类_120**: 1 篇文章
- **分类_121**: 1 篇文章
- **分类_122**: 1 篇文章
- **分类_123**: 1 篇文章
- **分类_126**: 1 篇文章
- **分类_129**: 10 篇文章
- **分类_131**: 2 篇文章
- **分类_132**: 2 篇文章
- **分类_134**: 3 篇文章
- **分类_135**: 1 篇文章
- **分类_136**: 1 篇文章
- **分类_137**: 1 篇文章
- **分类_138**: 1 篇文章
- **分类_139**: 1 篇文章
- **分类_140**: 1 篇文章
- **分类_141**: 1 篇文章
- **分类_142**: 1 篇文章
- **分类_143**: 1 篇文章
- **分类_146**: 1 篇文章
- **分类_148**: 1 篇文章
- **分类_150**: 10 篇文章
- **分类_152**: 2 篇文章
- **分类_153**: 2 篇文章
- **分类_155**: 3 篇文章
- **分类_156**: 1 篇文章
- **分类_157**: 1 篇文章
- **分类_158**: 1 篇文章
- **分类_159**: 1 篇文章
- **分类_160**: 1 篇文章
- **分类_161**: 1 篇文章
- **分类_162**: 1 篇文章
- **分类_163**: 1 篇文章
- **分类_164**: 1 篇文章
- **分类_167**: 1 篇文章
- **分类_169**: 1 篇文章
- **分类_171**: 10 篇文章
- **分类_173**: 2 篇文章
- **分类_174**: 2 篇文章
- **分类_176**: 3 篇文章
- **分类_177**: 1 篇文章
- **分类_178**: 1 篇文章
- **分类_179**: 1 篇文章
- **分类_180**: 1 篇文章
- **分类_181**: 1 篇文章
- **分类_182**: 1 篇文章
- **分类_183**: 1 篇文章
- **分类_184**: 1 篇文章
- **分类_185**: 1 篇文章
- **分类_188**: 1 篇文章
- **分类_190**: 1 篇文章
- **分类_192**: 10 篇文章
- **分类_194**: 2 篇文章
- **分类_195**: 2 篇文章
- **分类_197**: 3 篇文章
- **分类_198**: 1 篇文章
- **分类_199**: 1 篇文章
- **分类_200**: 1 篇文章
- **分类_201**: 1 篇文章
- **分类_202**: 1 篇文章
- **分类_203**: 1 篇文章
- **分类_204**: 1 篇文章
- **分类_205**: 1 篇文章
- **分类_206**: 1 篇文章
- **分类_209**: 1 篇文章
- **分类_211**: 1 篇文章
- **分类_213**: 10 篇文章
- **分类_215**: 2 篇文章
- **分类_216**: 2 篇文章
- **分类_218**: 3 篇文章
- **分类_219**: 1 篇文章
- **分类_220**: 1 篇文章
- **分类_221**: 1 篇文章
- **分类_222**: 1 篇文章
- **分类_223**: 1 篇文章
- **分类_224**: 1 篇文章
- **分类_225**: 1 篇文章
- **分类_226**: 1 篇文章
- **分类_227**: 1 篇文章
- **分类_230**: 1 篇文章
- **分类_232**: 1 篇文章
- **分类_234**: 10 篇文章
- **分类_236**: 2 篇文章
- **分类_237**: 2 篇文章
- **分类_239**: 3 篇文章
- **分类_240**: 1 篇文章
- **分类_241**: 1 篇文章
- **分类_242**: 1 篇文章
- **分类_243**: 1 篇文章
- **分类_244**: 1 篇文章
- **分类_245**: 1 篇文章
- **分类_246**: 1 篇文章
- **分类_247**: 1 篇文章
- **分类_248**: 1 篇文章
- **分类_251**: 1 篇文章
- **分类_253**: 1 篇文章
- **分类_255**: 10 篇文章
- **分类_257**: 2 篇文章
- **分类_258**: 2 篇文章
- **分类_260**: 3 篇文章
- **分类_261**: 1 篇文章
- **分类_262**: 1 篇文章
- **分类_263**: 1 篇文章
- **分类_264**: 1 篇文章
- **分类_265**: 1 篇文章
- **分类_266**: 1 篇文章
- **分类_267**: 1 篇文章
- **分类_268**: 1 篇文章
- **分类_269**: 1 篇文章
- **分类_272**: 1 篇文章
- **分类_274**: 1 篇文章
- **分类_276**: 10 篇文章
- **分类_278**: 2 篇文章
- **分类_279**: 2 篇文章
- **分类_281**: 3 篇文章
- **分类_282**: 1 篇文章
- **分类_283**: 1 篇文章
- **分类_284**: 1 篇文章
- **分类_285**: 1 篇文章
- **分类_286**: 1 篇文章
- **分类_287**: 1 篇文章
- **分类_288**: 1 篇文章
- **分类_289**: 1 篇文章
- **分类_290**: 1 篇文章
- **分类_293**: 1 篇文章
- **分类_295**: 1 篇文章
- **分类_297**: 10 篇文章
- **分类_299**: 2 篇文章
- **分类_300**: 2 篇文章
- **分类_302**: 3 篇文章
- **分类_303**: 1 篇文章
- **分类_304**: 1 篇文章
- **分类_305**: 1 篇文章
- **分类_306**: 1 篇文章
- **分类_307**: 1 篇文章
- **分类_308**: 1 篇文章
- **分类_309**: 1 篇文章
- **分类_310**: 1 篇文章
- **分类_311**: 1 篇文章
- **分类_314**: 1 篇文章
- **分类_316**: 1 篇文章
- **分类_318**: 10 篇文章
- **分类_320**: 2 篇文章
- **分类_321**: 2 篇文章
- **分类_323**: 3 篇文章
- **分类_324**: 1 篇文章
- **分类_325**: 1 篇文章
- **分类_326**: 1 篇文章
- **分类_327**: 1 篇文章
- **分类_328**: 1 篇文章
- **分类_329**: 1 篇文章
- **分类_330**: 1 篇文章
- **分类_331**: 1 篇文章
- **分类_332**: 1 篇文章
- **分类_335**: 1 篇文章
- **分类_337**: 1 篇文章
- **分类_339**: 10 篇文章
- **分类_341**: 2 篇文章
- **分类_342**: 2 篇文章
- **分类_344**: 3 篇文章
- **分类_345**: 1 篇文章
- **分类_346**: 1 篇文章
- **分类_347**: 1 篇文章
- **分类_348**: 1 篇文章
- **分类_349**: 1 篇文章
- **分类_350**: 1 篇文章
- **分类_351**: 1 篇文章
- **分类_352**: 1 篇文章
- **分类_353**: 1 篇文章
- **分类_356**: 1 篇文章
- **分类_358**: 1 篇文章
- **分类_360**: 10 篇文章
- **分类_362**: 2 篇文章
- **分类_363**: 2 篇文章
- **分类_365**: 3 篇文章
- **分类_366**: 1 篇文章
- **分类_367**: 1 篇文章
- **分类_368**: 1 篇文章
- **分类_369**: 1 篇文章
- **分类_370**: 1 篇文章
- **分类_371**: 1 篇文章
- **分类_372**: 1 篇文章
- **分类_373**: 1 篇文章
- **分类_374**: 1 篇文章
- **分类_377**: 1 篇文章
- **分类_379**: 1 篇文章
- **分类_381**: 10 篇文章
- **分类_383**: 2 篇文章
- **分类_384**: 2 篇文章
- **分类_386**: 3 篇文章
- **分类_387**: 1 篇文章
- **分类_388**: 1 篇文章
- **分类_389**: 1 篇文章
- **分类_390**: 1 篇文章
- **分类_391**: 1 篇文章
- **分类_392**: 1 篇文章
- **分类_393**: 1 篇文章
- **分类_394**: 1 篇文章
- **分类_395**: 1 篇文章
- **分类_398**: 1 篇文章
- **分类_400**: 1 篇文章
- **分类_402**: 10 篇文章
- **分类_404**: 2 篇文章
- **分类_405**: 2 篇文章
- **分类_407**: 3 篇文章
- **分类_408**: 1 篇文章
- **分类_409**: 1 篇文章
- **分类_410**: 1 篇文章
- **分类_411**: 1 篇文章
- **分类_412**: 1 篇文章
- **分类_413**: 1 篇文章
- **分类_414**: 1 篇文章
- **分类_415**: 1 篇文章
- **分类_416**: 1 篇文章
- **分类_419**: 1 篇文章
- **分类_421**: 1 篇文章
- **分类_423**: 10 篇文章
- **分类_425**: 2 篇文章
- **分类_426**: 2 篇文章
- **分类_428**: 3 篇文章
- **分类_429**: 1 篇文章
- **分类_430**: 1 篇文章
- **分类_431**: 1 篇文章
- **分类_432**: 1 篇文章
- **分类_433**: 1 篇文章
- **分类_434**: 1 篇文章
- **分类_435**: 1 篇文章
- **分类_436**: 1 篇文章
- **分类_437**: 1 篇文章
- **分类_440**: 1 篇文章
- **分类_442**: 1 篇文章
- **分类_444**: 10 篇文章
- **分类_446**: 2 篇文章
- **分类_447**: 2 篇文章
- **分类_449**: 3 篇文章
- **分类_450**: 1 篇文章
- **分类_451**: 1 篇文章
- **分类_452**: 1 篇文章
- **分类_453**: 1 篇文章
- **分类_454**: 1 篇文章
- **分类_455**: 1 篇文章
- **分类_456**: 1 篇文章
- **分类_457**: 1 篇文章
- **分类_458**: 1 篇文章
- **分类_461**: 1 篇文章
- **分类_463**: 1 篇文章
- **分类_465**: 10 篇文章
- **分类_467**: 2 篇文章
- **分类_468**: 2 篇文章
- **分类_470**: 3 篇文章
- **分类_471**: 1 篇文章
- **分类_472**: 1 篇文章
- **分类_473**: 1 篇文章
- **分类_474**: 1 篇文章
- **分类_475**: 1 篇文章
- **分类_476**: 1 篇文章
- **分类_477**: 1 篇文章
- **分类_478**: 1 篇文章
- **分类_479**: 1 篇文章
- **分类_482**: 1 篇文章
- **分类_484**: 1 篇文章
- **分类_486**: 10 篇文章
- **分类_488**: 2 篇文章
- **分类_489**: 2 篇文章
- **分类_491**: 3 篇文章
- **分类_492**: 1 篇文章
- **分类_493**: 1 篇文章
- **分类_494**: 1 篇文章
- **分类_495**: 1 篇文章
- **分类_496**: 1 篇文章
- **分类_497**: 1 篇文章
- **分类_498**: 1 篇文章
- **分类_499**: 1 篇文章
- **分类_500**: 1 篇文章
- **分类_503**: 1 篇文章
- **分类_505**: 1 篇文章
- **分类_507**: 10 篇文章
- **分类_509**: 2 篇文章
- **分类_510**: 2 篇文章
- **分类_512**: 3 篇文章
- **分类_513**: 1 篇文章
- **分类_514**: 1 篇文章
- **分类_515**: 1 篇文章
- **分类_516**: 1 篇文章
- **分类_517**: 1 篇文章
- **分类_518**: 1 篇文章
- **分类_519**: 1 篇文章
- **分类_520**: 1 篇文章
- **分类_521**: 1 篇文章
- **分类_524**: 1 篇文章
- **分类_526**: 1 篇文章
- **分类_528**: 10 篇文章
- **分类_530**: 2 篇文章
- **分类_531**: 2 篇文章
- **分类_533**: 3 篇文章
- **分类_534**: 1 篇文章
- **分类_535**: 1 篇文章
- **分类_536**: 1 篇文章
- **分类_537**: 1 篇文章
- **分类_538**: 1 篇文章
- **分类_539**: 1 篇文章
- **分类_540**: 1 篇文章
- **分类_541**: 1 篇文章
- **分类_542**: 1 篇文章
- **分类_545**: 1 篇文章
- **分类_547**: 1 篇文章
- **分类_549**: 10 篇文章
- **分类_551**: 2 篇文章
- **分类_552**: 2 篇文章
- **分类_554**: 3 篇文章
- **分类_555**: 1 篇文章
- **分类_556**: 1 篇文章
- **分类_557**: 1 篇文章
- **分类_558**: 1 篇文章
- **分类_559**: 1 篇文章
- **分类_560**: 1 篇文章
- **分类_561**: 1 篇文章
- **分类_562**: 1 篇文章
- **分类_563**: 1 篇文章
- **分类_566**: 1 篇文章
- **分类_568**: 1 篇文章
- **分类_570**: 10 篇文章
- **分类_572**: 2 篇文章
- **分类_573**: 2 篇文章
- **分类_575**: 3 篇文章
- **分类_576**: 1 篇文章
- **分类_577**: 1 篇文章
- **分类_578**: 1 篇文章
- **分类_579**: 1 篇文章
- **分类_580**: 1 篇文章
- **分类_581**: 1 篇文章
- **分类_582**: 1 篇文章
- **分类_583**: 1 篇文章
- **分类_584**: 1 篇文章
- **分类_587**: 1 篇文章
- **分类_589**: 1 篇文章
- **分类_591**: 10 篇文章
- **分类_593**: 2 篇文章
- **分类_594**: 2 篇文章
- **分类_596**: 3 篇文章
- **分类_597**: 1 篇文章
- **分类_598**: 1 篇文章
- **分类_599**: 1 篇文章
- **分类_600**: 1 篇文章
- **分类_601**: 1 篇文章
- **分类_602**: 1 篇文章
- **分类_603**: 1 篇文章
- **分类_604**: 1 篇文章
- **分类_605**: 1 篇文章
- **分类_608**: 1 篇文章
- **分类_610**: 1 篇文章
- **分类_612**: 10 篇文章
- **分类_614**: 2 篇文章
- **分类_615**: 2 篇文章
- **分类_617**: 3 篇文章
- **分类_618**: 1 篇文章
- **分类_619**: 1 篇文章
- **分类_620**: 1 篇文章
- **分类_621**: 1 篇文章
- **分类_622**: 1 篇文章
- **分类_623**: 1 篇文章
- **分类_624**: 1 篇文章
- **分类_625**: 1 篇文章
- **分类_626**: 1 篇文章
- **分类_629**: 1 篇文章
- **分类_631**: 1 篇文章
- **分类_633**: 10 篇文章
- **分类_635**: 2 篇文章
- **分类_636**: 2 篇文章
- **分类_638**: 3 篇文章
- **分类_639**: 1 篇文章
- **分类_640**: 1 篇文章
- **分类_641**: 1 篇文章
- **分类_642**: 1 篇文章
- **分类_643**: 1 篇文章
- **分类_644**: 1 篇文章
- **分类_645**: 1 篇文章
- **分类_646**: 1 篇文章
- **分类_647**: 1 篇文章
- **分类_650**: 1 篇文章
- **分类_652**: 1 篇文章
- **分类_654**: 10 篇文章
- **分类_656**: 2 篇文章
- **分类_657**: 2 篇文章
- **分类_659**: 3 篇文章
- **分类_660**: 1 篇文章
- **分类_661**: 1 篇文章
- **分类_662**: 1 篇文章
- **分类_663**: 1 篇文章
- **分类_664**: 1 篇文章
- **分类_665**: 1 篇文章
- **分类_666**: 1 篇文章
- **分类_667**: 1 篇文章
- **分类_668**: 1 篇文章
- **分类_671**: 1 篇文章
- **分类_673**: 1 篇文章
- **分类_675**: 10 篇文章
- **分类_677**: 2 篇文章
- **分类_678**: 2 篇文章
- **分类_680**: 3 篇文章
- **分类_681**: 1 篇文章
- **分类_682**: 1 篇文章
- **分类_683**: 1 篇文章
- **分类_684**: 1 篇文章
- **分类_685**: 1 篇文章
- **分类_686**: 1 篇文章
- **分类_687**: 1 篇文章
- **分类_688**: 1 篇文章
- **分类_689**: 1 篇文章
- **分类_692**: 1 篇文章
- **分类_694**: 1 篇文章
- **分类_696**: 10 篇文章
- **分类_698**: 2 篇文章
- **分类_699**: 2 篇文章
- **分类_701**: 3 篇文章
- **分类_702**: 1 篇文章
- **分类_703**: 1 篇文章
- **分类_704**: 1 篇文章
- **分类_705**: 1 篇文章
- **分类_706**: 1 篇文章
- **分类_707**: 1 篇文章
- **分类_708**: 1 篇文章
- **分类_709**: 1 篇文章
- **分类_710**: 1 篇文章
- **分类_713**: 1 篇文章
- **分类_715**: 1 篇文章
- **分类_717**: 10 篇文章
- **分类_719**: 2 篇文章
- **分类_720**: 2 篇文章
- **分类_722**: 3 篇文章
- **分类_723**: 1 篇文章
- **分类_724**: 1 篇文章
- **分类_725**: 1 篇文章
- **分类_726**: 1 篇文章
- **分类_727**: 1 篇文章
- **分类_728**: 1 篇文章
- **分类_729**: 1 篇文章
- **分类_730**: 1 篇文章
- **分类_731**: 1 篇文章
- **分类_734**: 1 篇文章
- **分类_736**: 1 篇文章
- **分类_738**: 10 篇文章

## 目录结构

```
├── 分类_100
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_101
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_102
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_106
│   └── README.md
├── 分类_109
│   └── README.md
├── 分类_111
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_112
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_114
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_115
│   └── 如何自定义Tag.md
├── 分类_116
│   └── README.md
├── 分类_117
│   └── README.md
├── 分类_118
│   └── README.md
├── 分类_119
│   └── 服务器部署.md
├── 分类_120
│   └── 服务器事件系统.md
├── 分类_121
│   └── README.md
├── 分类_122
│   └── README.md
├── 分类_123
│   └── README.md
├── 分类_126
│   └── README.md
├── 分类_129
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_131
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_132
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_134
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_135
│   └── 如何自定义Tag.md
├── 分类_136
│   └── README.md
├── 分类_137
│   └── README.md
├── 分类_138
│   └── README.md
├── 分类_139
│   └── 服务器部署.md
├── 分类_140
│   └── 服务器事件系统.md
├── 分类_141
│   └── README.md
├── 分类_142
│   └── README.md
├── 分类_143
│   └── README.md
├── 分类_146
│   └── README.md
├── 分类_148
│   └── w-delete.md
├── 分类_150
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_152
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_153
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_155
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_156
│   └── 如何自定义Tag.md
├── 分类_157
│   └── README.md
├── 分类_158
│   └── README.md
├── 分类_159
│   └── README.md
├── 分类_160
│   └── 服务器部署.md
├── 分类_161
│   └── 服务器事件系统.md
├── 分类_162
│   └── README.md
├── 分类_163
│   └── README.md
├── 分类_164
│   └── README.md
├── 分类_167
│   └── README.md
├── 分类_169
│   └── w-delete.md
├── 分类_171
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_173
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_174
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_176
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_177
│   └── 如何自定义Tag.md
├── 分类_178
│   └── README.md
├── 分类_179
│   └── README.md
├── 分类_180
│   └── README.md
├── 分类_181
│   └── 服务器部署.md
├── 分类_182
│   └── 服务器事件系统.md
├── 分类_183
│   └── README.md
├── 分类_184
│   └── README.md
├── 分类_185
│   └── README.md
├── 分类_188
│   └── README.md
├── 分类_190
│   └── w-delete.md
├── 分类_192
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_194
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_195
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_197
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_198
│   └── 如何自定义Tag.md
├── 分类_199
│   └── README.md
├── 分类_200
│   └── README.md
├── 分类_201
│   └── README.md
├── 分类_202
│   └── 服务器部署.md
├── 分类_203
│   └── 服务器事件系统.md
├── 分类_204
│   └── README.md
├── 分类_205
│   └── README.md
├── 分类_206
│   └── README.md
├── 分类_209
│   └── README.md
├── 分类_211
│   └── w-delete.md
├── 分类_213
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_215
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_216
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_218
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_219
│   └── 如何自定义Tag.md
├── 分类_220
│   └── README.md
├── 分类_221
│   └── README.md
├── 分类_222
│   └── README.md
├── 分类_223
│   └── 服务器部署.md
├── 分类_224
│   └── 服务器事件系统.md
├── 分类_225
│   └── README.md
├── 分类_226
│   └── README.md
├── 分类_227
│   └── README.md
├── 分类_230
│   └── README.md
├── 分类_232
│   └── w-delete.md
├── 分类_234
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_236
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_237
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_239
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_240
│   └── 如何自定义Tag.md
├── 分类_241
│   └── README.md
├── 分类_242
│   └── README.md
├── 分类_243
│   └── README.md
├── 分类_244
│   └── 服务器部署.md
├── 分类_245
│   └── 服务器事件系统.md
├── 分类_246
│   └── README.md
├── 分类_247
│   └── README.md
├── 分类_248
│   └── README.md
├── 分类_251
│   └── README.md
├── 分类_253
│   └── w-delete.md
├── 分类_255
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_257
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_258
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_260
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_261
│   └── 如何自定义Tag.md
├── 分类_262
│   └── README.md
├── 分类_263
│   └── README.md
├── 分类_264
│   └── README.md
├── 分类_265
│   └── 服务器部署.md
├── 分类_266
│   └── 服务器事件系统.md
├── 分类_267
│   └── README.md
├── 分类_268
│   └── README.md
├── 分类_269
│   └── README.md
├── 分类_272
│   └── README.md
├── 分类_274
│   └── w-delete.md
├── 分类_276
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_278
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_279
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_281
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_282
│   └── 如何自定义Tag.md
├── 分类_283
│   └── README.md
├── 分类_284
│   └── README.md
├── 分类_285
│   └── README.md
├── 分类_286
│   └── 服务器部署.md
├── 分类_287
│   └── 服务器事件系统.md
├── 分类_288
│   └── README.md
├── 分类_289
│   └── README.md
├── 分类_290
│   └── README.md
├── 分类_293
│   └── README.md
├── 分类_295
│   └── w-delete.md
├── 分类_297
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_299
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_300
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_302
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_303
│   └── 如何自定义Tag.md
├── 分类_304
│   └── README.md
├── 分类_305
│   └── README.md
├── 分类_306
│   └── README.md
├── 分类_307
│   └── 服务器部署.md
├── 分类_308
│   └── 服务器事件系统.md
├── 分类_309
│   └── README.md
├── 分类_310
│   └── README.md
├── 分类_311
│   └── README.md
├── 分类_314
│   └── README.md
├── 分类_316
│   └── w-delete.md
├── 分类_318
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_320
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_321
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_323
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_324
│   └── 如何自定义Tag.md
├── 分类_325
│   └── README.md
├── 分类_326
│   └── README.md
├── 分类_327
│   └── README.md
├── 分类_328
│   └── 服务器部署.md
├── 分类_329
│   └── 服务器事件系统.md
├── 分类_330
│   └── README.md
├── 分类_331
│   └── README.md
├── 分类_332
│   └── README.md
├── 分类_335
│   └── README.md
├── 分类_337
│   └── w-delete.md
├── 分类_339
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_341
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_342
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_344
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_345
│   └── 如何自定义Tag.md
├── 分类_346
│   └── README.md
├── 分类_347
│   └── README.md
├── 分类_348
│   └── README.md
├── 分类_349
│   └── 服务器部署.md
├── 分类_350
│   └── 服务器事件系统.md
├── 分类_351
│   └── README.md
├── 分类_352
│   └── README.md
├── 分类_353
│   └── README.md
├── 分类_356
│   └── README.md
├── 分类_358
│   └── w-delete.md
├── 分类_360
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_362
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_363
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_365
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_366
│   └── 如何自定义Tag.md
├── 分类_367
│   └── README.md
├── 分类_368
│   └── README.md
├── 分类_369
│   └── README.md
├── 分类_370
│   └── 服务器部署.md
├── 分类_371
│   └── 服务器事件系统.md
├── 分类_372
│   └── README.md
├── 分类_373
│   └── README.md
├── 分类_374
│   └── README.md
├── 分类_377
│   └── README.md
├── 分类_379
│   └── w-delete.md
├── 分类_381
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_383
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_384
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_386
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_387
│   └── 如何自定义Tag.md
├── 分类_388
│   └── README.md
├── 分类_389
│   └── README.md
├── 分类_390
│   └── README.md
├── 分类_391
│   └── 服务器部署.md
├── 分类_392
│   └── 服务器事件系统.md
├── 分类_393
│   └── README.md
├── 分类_394
│   └── README.md
├── 分类_395
│   └── README.md
├── 分类_398
│   └── README.md
├── 分类_400
│   └── w-delete.md
├── 分类_402
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_404
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_405
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_407
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_408
│   └── 如何自定义Tag.md
├── 分类_409
│   └── README.md
├── 分类_410
│   └── README.md
├── 分类_411
│   └── README.md
├── 分类_412
│   └── 服务器部署.md
├── 分类_413
│   └── 服务器事件系统.md
├── 分类_414
│   └── README.md
├── 分类_415
│   └── README.md
├── 分类_416
│   └── README.md
├── 分类_419
│   └── README.md
├── 分类_421
│   └── w-delete.md
├── 分类_423
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_425
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_426
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_428
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_429
│   └── 如何自定义Tag.md
├── 分类_430
│   └── README.md
├── 分类_431
│   └── README.md
├── 分类_432
│   └── README.md
├── 分类_433
│   └── 服务器部署.md
├── 分类_434
│   └── 服务器事件系统.md
├── 分类_435
│   └── README.md
├── 分类_436
│   └── README.md
├── 分类_437
│   └── README.md
├── 分类_440
│   └── README.md
├── 分类_442
│   └── w-delete.md
├── 分类_444
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_446
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_447
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_449
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_450
│   └── 如何自定义Tag.md
├── 分类_451
│   └── README.md
├── 分类_452
│   └── README.md
├── 分类_453
│   └── README.md
├── 分类_454
│   └── 服务器部署.md
├── 分类_455
│   └── 服务器事件系统.md
├── 分类_456
│   └── README.md
├── 分类_457
│   └── README.md
├── 分类_458
│   └── README.md
├── 分类_461
│   └── README.md
├── 分类_463
│   └── w-delete.md
├── 分类_465
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_467
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_468
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_470
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_471
│   └── 如何自定义Tag.md
├── 分类_472
│   └── README.md
├── 分类_473
│   └── README.md
├── 分类_474
│   └── README.md
├── 分类_475
│   └── 服务器部署.md
├── 分类_476
│   └── 服务器事件系统.md
├── 分类_477
│   └── README.md
├── 分类_478
│   └── README.md
├── 分类_479
│   └── README.md
├── 分类_482
│   └── README.md
├── 分类_484
│   └── w-delete.md
├── 分类_486
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_488
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_489
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_491
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_492
│   └── 如何自定义Tag.md
├── 分类_493
│   └── README.md
├── 分类_494
│   └── README.md
├── 分类_495
│   └── README.md
├── 分类_496
│   └── 服务器部署.md
├── 分类_497
│   └── 服务器事件系统.md
├── 分类_498
│   └── README.md
├── 分类_499
│   └── README.md
├── 分类_50
│   └── README.md
├── 分类_500
│   └── README.md
├── 分类_503
│   └── README.md
├── 分类_505
│   └── w-delete.md
├── 分类_507
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_509
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_51
│   └── 服务器事件系统.md
├── 分类_510
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_512
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_513
│   └── 如何自定义Tag.md
├── 分类_514
│   └── README.md
├── 分类_515
│   └── README.md
├── 分类_516
│   └── README.md
├── 分类_517
│   └── 服务器部署.md
├── 分类_518
│   └── 服务器事件系统.md
├── 分类_519
│   └── README.md
├── 分类_52
│   └── 服务器部署.md
├── 分类_520
│   └── README.md
├── 分类_521
│   └── README.md
├── 分类_524
│   └── README.md
├── 分类_526
│   └── w-delete.md
├── 分类_528
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_53
│   └── README.md
├── 分类_530
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_531
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_533
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_534
│   └── 如何自定义Tag.md
├── 分类_535
│   └── README.md
├── 分类_536
│   └── README.md
├── 分类_537
│   └── README.md
├── 分类_538
│   └── 服务器部署.md
├── 分类_539
│   └── 服务器事件系统.md
├── 分类_54
│   └── README.md
├── 分类_540
│   └── README.md
├── 分类_541
│   └── README.md
├── 分类_542
│   └── README.md
├── 分类_545
│   └── README.md
├── 分类_547
│   └── w-delete.md
├── 分类_549
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_55
│   └── README.md
├── 分类_551
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_552
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_554
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_555
│   └── 如何自定义Tag.md
├── 分类_556
│   └── README.md
├── 分类_557
│   └── README.md
├── 分类_558
│   └── README.md
├── 分类_559
│   └── 服务器部署.md
├── 分类_56
│   └── README.md
├── 分类_560
│   └── 服务器事件系统.md
├── 分类_561
│   └── README.md
├── 分类_562
│   └── README.md
├── 分类_563
│   └── README.md
├── 分类_566
│   └── README.md
├── 分类_568
│   └── w-delete.md
├── 分类_570
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_572
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_573
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_575
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_576
│   └── 如何自定义Tag.md
├── 分类_577
│   └── README.md
├── 分类_578
│   └── README.md
├── 分类_579
│   └── README.md
├── 分类_58
│   └── 如何自定义Tag.md
├── 分类_580
│   └── 服务器部署.md
├── 分类_581
│   └── 服务器事件系统.md
├── 分类_582
│   └── README.md
├── 分类_583
│   └── README.md
├── 分类_584
│   └── README.md
├── 分类_587
│   └── README.md
├── 分类_589
│   └── w-delete.md
├── 分类_59
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_591
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_593
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_594
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_596
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_597
│   └── 如何自定义Tag.md
├── 分类_598
│   └── README.md
├── 分类_599
│   └── README.md
├── 分类_60
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_600
│   └── README.md
├── 分类_601
│   └── 服务器部署.md
├── 分类_602
│   └── 服务器事件系统.md
├── 分类_603
│   └── README.md
├── 分类_604
│   └── README.md
├── 分类_605
│   └── README.md
├── 分类_608
│   └── README.md
├── 分类_61
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_610
│   └── w-delete.md
├── 分类_612
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_614
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_615
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_617
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_618
│   └── 如何自定义Tag.md
├── 分类_619
│   └── README.md
├── 分类_62
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_620
│   └── README.md
├── 分类_621
│   └── README.md
├── 分类_622
│   └── 服务器部署.md
├── 分类_623
│   └── 服务器事件系统.md
├── 分类_624
│   └── README.md
├── 分类_625
│   └── README.md
├── 分类_626
│   └── README.md
├── 分类_629
│   └── README.md
├── 分类_631
│   └── w-delete.md
├── 分类_633
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_635
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_636
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_638
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_639
│   └── 如何自定义Tag.md
├── 分类_640
│   └── README.md
├── 分类_641
│   └── README.md
├── 分类_642
│   └── README.md
├── 分类_643
│   └── 服务器部署.md
├── 分类_644
│   └── 服务器事件系统.md
├── 分类_645
│   └── README.md
├── 分类_646
│   └── README.md
├── 分类_647
│   └── README.md
├── 分类_650
│   └── README.md
├── 分类_652
│   └── w-delete.md
├── 分类_654
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_656
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_657
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_659
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_66
│   └── README.md
├── 分类_660
│   └── 如何自定义Tag.md
├── 分类_661
│   └── README.md
├── 分类_662
│   └── README.md
├── 分类_663
│   └── README.md
├── 分类_664
│   └── 服务器部署.md
├── 分类_665
│   └── 服务器事件系统.md
├── 分类_666
│   └── README.md
├── 分类_667
│   └── README.md
├── 分类_668
│   └── README.md
├── 分类_671
│   └── README.md
├── 分类_673
│   └── w-delete.md
├── 分类_675
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_677
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_678
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_680
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_681
│   └── 如何自定义Tag.md
├── 分类_682
│   └── README.md
├── 分类_683
│   └── README.md
├── 分类_684
│   └── README.md
├── 分类_685
│   └── 服务器部署.md
├── 分类_686
│   └── 服务器事件系统.md
├── 分类_687
│   └── README.md
├── 分类_688
│   └── README.md
├── 分类_689
│   └── README.md
├── 分类_69
│   └── README.md
├── 分类_692
│   └── README.md
├── 分类_694
│   └── w-delete.md
├── 分类_696
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_698
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_699
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_70
│   └── README.md
├── 分类_701
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_702
│   └── 如何自定义Tag.md
├── 分类_703
│   └── README.md
├── 分类_704
│   └── README.md
├── 分类_705
│   └── README.md
├── 分类_706
│   └── 服务器部署.md
├── 分类_707
│   └── 服务器事件系统.md
├── 分类_708
│   └── README.md
├── 分类_709
│   └── README.md
├── 分类_71
│   └── 服务器事件系统.md
├── 分类_710
│   └── README.md
├── 分类_713
│   └── README.md
├── 分类_715
│   └── w-delete.md
├── 分类_717
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_719
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_72
│   └── 服务器部署.md
├── 分类_720
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_722
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_723
│   └── 如何自定义Tag.md
├── 分类_724
│   └── README.md
├── 分类_725
│   └── README.md
├── 分类_726
│   ├── README.md
│   └── 开发
│       └── .gitkeep
├── 分类_727
│   └── 服务器部署.md
├── 分类_728
│   └── 服务器事件系统.md
├── 分类_729
│   └── README.md
├── 分类_73
│   └── README.md
├── 分类_730
│   └── README.md
├── 分类_731
│   └── README.md
├── 分类_734
│   └── README.md
├── 分类_736
│   └── w-delete.md
├── 分类_738
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_74
│   └── README.md
├── 分类_75
│   └── README.md
├── 分类_76
│   └── README.md
├── 分类_78
│   └── 如何自定义Tag.md
├── 分类_79
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 分类_80
│   ├── 1 框架目录.md
│   └── 99 开发服务器.md
├── 分类_81
│   ├── 1 说明.md
│   ├── 2 快速创建模块.md
│   └── 3 创建控制器.md
├── 分类_82
│   ├── 如何自定义Tag.md
│   └── 概述.md
├── 分类_86
│   └── README.md
├── 分类_89
│   └── README.md
├── 分类_90
│   └── README.md
├── 分类_91
│   └── 服务器事件系统.md
├── 分类_92
│   └── 服务器部署.md
├── 分类_93
│   └── README.md
├── 分类_94
│   └── README.md
├── 分类_95
│   └── README.md
├── 分类_96
│   └── README.md
├── 分类_98
│   └── 如何自定义Tag.md
├── 分类_99
│   ├── API_REFERENCE.md
│   ├── API文档.md
│   ├── README.md
│   ├── README_FIELD_VALIDATION.md
│   ├── README_INHERITANCE.md
│   ├── README_STRUCTURE.md
│   ├── RESET_BUTTON_IMPROVEMENT.md
│   ├── 使用指南.md
│   ├── 功能完善总结.md
│   └── 示例代码.md
├── 前言
│   └── .gitkeep
├── 安装
│   └── .gitkeep
├── 快速开始
│   ├── Controller
│   │   ├── 创建控制器.md
│   │   └── 控制器类规则.md
│   ├── Event
│   │   ├── 事件说明及其作用.md
│   │   └── 声明一个事件观察者，创造一个事件.md
│   ├── Model
│   │   ├── 使用.md
│   │   └── 创建.md
│   ├── Plugin
│   │   └── .gitkeep
│   ├── 模组安装，升级，卸载时处理数据.md
│   └── 缓存.md
└── 框架规范
    └── 类.md
```

## 说明

- 空目录使用 `.gitkeep` 文件保持目录结构
- 所有文档按分类层级组织
- 文档包含完整的元数据信息
- 缺失的分类自动创建为 `分类_ID` 格式
- 无分类的文章放入 `未分类` 目录

## API 文档登录地址（2026-09-08）

前端登录从已收集的 `Weline\Api\Api\Rest\V1\Auth::postLogin` 文档记录取得 `route.path`，经现有 `buildRestUrl` 与原生 `sendHttp` 提交，不另写一套前端登录地址。`ApiDocCollector` 继续委托公开 `ApiDocumentationProviderInterface`，保留当前 `apiArea` 配置、其他 REST 路由和后台登录行为。

当前环境的双 `api` 登录路径实测 404 HTML；文档记录提供的 `/api/api/api/rest/v1/auth/login` 正常账号 HTTP 对照为 200 / code 200。页面登录尚待浏览器重验。回归命令：`node app/code/Weline/DeveloperWorkspace/Test/Unit/ApiDocsIdentityDemoRegression.cjs`，包含真实路径、可配置区域与登录响应保存；凭据只使用脚本内占位值。
