...
import { createLocalChatModel, createLocalLLMAdapter } from '@src/background/llm';
...
export function createChatModel(providerConfig: ProviderConfig, modelConfig: ModelConfig): BaseChatModel {
  ...
  switch (modelConfig.provider) {
    // 本地 WASM 推理（无需 API Key，完全离线）
    case ProviderTypeEnum.Local: {
      const localModel = createLocalChatModel({
        modelId: modelConfig.modelName,
        temperature,
        topP,
        maxTokens,
      });
      // 返回适配 LangChain BaseChatModel 接口的包装器
      return createLocalLLMAdapter(localModel);
    }
    ...
    case ProviderTypeEnum.OpenRouter: {
      ...
      return createOpenAIChatModel(providerConfig, modelConfig, {
        headers: {
          'HTTP-Referer': 'https://weline.io',
          'X-Title': 'AutoLeadAgent',
        },
      });
    }
