...
export function getDefaultProviderConfig(providerId: string): ProviderConfig {
  switch (providerId) {
    ...
    case ProviderTypeEnum.Local:
      return {
        apiKey: 'local', // 本地模型不需要 API Key，但需要一个占位符
        name: getDefaultDisplayNameFromProviderId(ProviderTypeEnum.Local),
        type: ProviderTypeEnum.Local,
        modelNames: [...(llmProviderModelNames[ProviderTypeEnum.Local] || [])],
        createdAt: Date.now(),
      };
...
export const llmProviderStore: LLMProviderStorage = {
  ...
  async setProvider(providerId: string, config: ProviderConfig) {
    ...
    if (providerType === ProviderTypeEnum.AzureOpenAI) {
      ...
      providerType !== ProviderTypeEnum.Local  // 本地模型不需要 API Key
    ) {
      if (!config.apiKey?.trim()) {
