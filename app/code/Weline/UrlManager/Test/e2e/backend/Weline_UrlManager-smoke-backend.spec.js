...
test.describe('Weline_UrlManager backend smoke', () => {
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
    await page.setViewportSize({ width: 1440, height: 900 });
  });

  test('TC-01: url manager index page renders without fatal errors', async ({ page }) => {
    const errors = bindPageErrors(page);
    await gotoBackend(page, 'admin/url-manager', { timeout: 60000, settleMs: 1200 });
