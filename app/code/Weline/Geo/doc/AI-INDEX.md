...
4. 本文件：`app/code/Weline/Geo/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Geo/doc/README.md`
...
入口/配置文件：
- `app/code/Weline/Geo/etc/module.xml`
- `app/code/Weline/Geo/etc/backend/menu.xml`
- `app/code/Weline/Geo/composer.json`

- `Console`：php bin/w 命令入口。新增/变更命令后用真实 CLI 验证。 文件数：2
- `Controller`：HTTP/后台/前台控制器入口。新增控制器后优先跑完整 `setup:upgrade`；仅需重建路由图时可用 `setup:upgrade --route`（选填）。 文件数：7
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/GeoAdminQueryProvider.php；前端/跨模块读数据先查 query 帮助。

## doc 目录

- `app/code/Weline/Geo/doc/API参考文档.md`
- `app/code/Weline/Geo/doc/README.md`
- `app/code/Weline/Geo/doc/event/feed_item_add.md`
- `app/code/Weline/Geo/doc/event/feed_item_delete.md`
- `app/code/Weline/Geo/doc/event/feed_item_update.md`
- `app/code/Weline/Geo/doc/事件系统使用指南.md`
- `app/code/Weline/Geo/doc/使用指南.md`
- `app/code/Weline/Geo/doc/快速入门指南.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Geo> [operation]` 或对应 `w_query` 帮助。
