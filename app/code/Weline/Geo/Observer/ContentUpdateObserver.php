...
class ContentUpdateObserver
{
    ...
    protected function triggerAutoPush(Feed $feed): void
    {
        try {
            ...
            // 获取所有启用的平台
            /** @var \Weline\Geo\Model\Platform $platformModel */
            $platformModel = ObjectManager::getInstance(\Weline\Geo\Model\Platform::class);
            $platforms = $platformModel
                ->where(\Weline\Geo\Model\Platform::schema_fields_IS_ENABLED, 1)
                ->select()
                ->fetchArray();

            foreach ($platforms as $platformData) {
                $platform = $platformModel->load($platformData['id']);
                $pushService->pushFeed($feed, $platform, null, \Weline\Geo\Model\PushLog::TYPE_AUTO);
            }
        } catch (\Exception $e) {
