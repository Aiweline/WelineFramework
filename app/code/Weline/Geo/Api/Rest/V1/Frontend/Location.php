 ...
 * 
 * 提供IP定位和地址解析API接口
 * 路由: /geo/rest/v1/frontend/location/*
 */
class Location extends FrontendRestController
{
     ...
     * 根据经纬度获取地址信息（反向地理编码）
     * 
     * POST /geo/rest/v1/frontend/location/address
    ...
    public function getAddress(): array
    {
        try {
            ...
            $eventsManager->dispatch('Weline_Geo::location-to-address', $eventData);
            
            // 获取事件返回的地址数据
            $event = $eventsManager->getEventData('Weline_Geo::location-to-address');
            if ($event) {
            ...
            $eventsManager->dispatch('Weline_Geo::address-updated', $updateEventData);
