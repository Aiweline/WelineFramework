...
namespace Weline\Help\Controller\Backend;
...
use Weline\Help\Service\HelpCmsEditorService;

#[Acl('Weline_Help::page', '帮助中心页面', 'help-circle', '管理帮助中心 CMS 页面', 'Weline_Backend::cms_group')]
final class Page extends BackendController
{
    public function __construct(
        private readonly HelpCmsEditorService $editor,
    ) {
    }

    #[Acl('Weline_Help::page_listing', '帮助页面列表', 'list', '查看帮助中心页面')]
    public function getListing(): string
    {
        ...
        foreach ($items as &$item) {
            ...
            $item['edit_url'] = $this->getUrl('help/backend/page/edit', ['page_id' => $pageId]);
            $item['cms_edit_url'] = $this->getUrl('cms/backend/page/edit', ['page_id' => $pageId]);
            $item['public_path'] = HelpNamespace::articlePublicPath($slug);
        }
        unset($item);
        ...
        $this->assign('new_url', $this->getUrl('help/backend/page/new'));
        $this->assign('listing_url', $this->getUrl('help/backend/page/listing'));

        return $this->fetch('Weline_Help::templates/Backend/Page/listing.phtml');
    }

    #[Acl('Weline_Help::page_new', '新建帮助页面', 'plus', '创建帮助中心草稿页')]
    public function getNew(): string
    {
        try {
            ...
            if ($page === null) {
                ...
                return $this->redirect($this->getUrl('help/backend/page/listing'));
            }
            $this->getMessageManager()->addSuccess((string)__('帮助草稿已创建，可打开可视化编辑器完善内容。'));

            return $this->redirect($this->getUrl('help/backend/page/edit', [
    ...
    #[Acl('Weline_Help::page_edit', '编辑帮助页面', 'edit', '编辑帮助中心页面')]
    public function getEdit(): string
    {
        ...
        if (strtolower(trim((string)($page['path_group'] ?? ''))) !== HelpNamespace::PREFIX) {
        ...
        $this->assign('public_path', HelpNamespace::articlePublicPath((string)($page['slug'] ?? '')));
        ...
        return $this->fetch('Weline_Help::templates/Backend/Page/edit.phtml');
    }
    ...
    private function groupPagesByWebsite(array $pages, array $websites, string $websiteIdFilter): array
    {
        ...
        foreach ($websites as $website) {
            ...
            $groups[$key] = [
                ...
                'new_url' => $this->getUrl('help/backend/page/new', [
                    'website_id' => $websiteId,
                    'website_code' => $code,
                ]),
                'pages' => [],
