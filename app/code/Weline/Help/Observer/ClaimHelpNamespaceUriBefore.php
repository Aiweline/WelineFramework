...
namespace Weline\Help\Observer;
...
/** Claims /help and /faq before CMS ProcessCmsPageUriBefore. */
final class ClaimHelpNamespaceUriBefore implements ObserverInterface
{
    ...
    public function execute(Event &$event): void
    {
        ...
        if (!HelpNamespace::isHelpIdentifier($identifier)) {
            return;
        }

        $this->request->setData('cms_uri_skip', HelpNamespace::skipPayload());
        $this->request->setData('help_namespace_owner', 'Weline_Help');
    }
    ...
    private function normalizeIdentifier(string $path): string
    {
        ...
        foreach ($candidates as $candidate) {
            ...
            if (HelpNamespace::isHelpIdentifier($candidate)) {
                return $candidate;
            }
        }
