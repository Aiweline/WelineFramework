...
/** Durable Website-scoped B2B rollout shared by candidate and later cutover. */
final class B2BRolloutGate implements CommerceRolloutGateInterface
{
    public const CAPABILITY = B2BPriceEngine::CAPABILITY;
    public const CONFIG_ROOT = 'commerce.rollout.b2b';
    ...
    public function setMode(
        ...
        if ($capability !== self::CAPABILITY) {
            throw new \InvalidArgumentException('b2b_rollout_capability_invalid');
        }
        ...
        if ($this->envConfiguration() !== null) {
            throw new \RuntimeException('b2b_rollout_env_locked');
        }

        // Fail closed on a partial write: disable first unless enabling a scoped mode.
        ...
        $expected = $this->normalizeConfiguration(['mode' => $mode, 'allowlist' => $rows]);
        if ($actual['mode'] !== $expected['mode']
            || array_keys($actual['allowlist']) !== array_keys($expected['allowlist'])
        ) {
            throw new \RuntimeException('b2b_rollout_readback_mismatch');
        }
    }
    ...
    {
        if ($this->testingConfiguration !== null) {
            return $this->normalizeConfiguration($this->testingConfiguration) + ['env_locked' => false];
        }
        ...
        try {
            ...
            throw new \RuntimeException('b2b_rollout_config_unavailable', 0, $exception);
        }
    }
    ...
    private function envConfiguration(): ?array
    {
        ...
        if (!is_array($value) || array_is_list($value)) {
            throw new \RuntimeException('b2b_rollout_config_invalid:env_root');
        }
        return $value;
    }
    ...
    private function normalizeConfiguration(array $configuration): array
    {
        ...
        if ($keys !== ['allowlist', 'mode']) {
            throw new \RuntimeException('b2b_rollout_config_invalid:fields');
        }
        ...
        if (!is_string($mode)
            ...
            throw new \RuntimeException('b2b_rollout_config_invalid:mode');
        }
        $rawRows = $configuration['allowlist'];
        if (!is_array($rawRows) || !array_is_list($rawRows)) {
            throw new \RuntimeException('b2b_rollout_config_invalid:allowlist');
        }

        $allowlist = [];
        $rows = [];
        foreach ($rawRows as $row) {
            if (!is_array($row) || array_keys($row) !== ['website_id']) {
                throw new \RuntimeException('b2b_rollout_config_invalid:allowlist_entry');
            }
            $websiteId = $row['website_id'];
            if (!is_int($websiteId) || $websiteId < 0) {
                throw new \RuntimeException('b2b_rollout_config_invalid:allowlist_values');
            }
            $key = self::scopeKey($websiteId);
            if (isset($allowlist[$key])) {
                throw new \RuntimeException('b2b_rollout_config_invalid:allowlist_duplicate');
            }
            $allowlist[$key] = true;
    ...
    private function normalizeSubjects(array $subjects): array
    {
        ...
        foreach ($subjects as $subject) {
            ...
            if (preg_match('/^website:(0|[1-9][0-9]*)$/D', $subject, $match) !== 1) {
                throw new \InvalidArgumentException('b2b_rollout_subject_invalid:' . $subject);
            }
            $key = self::scopeKey((int)$match[1]);
            if (isset($rows[$key])) {
                throw new \InvalidArgumentException('b2b_rollout_subject_duplicate:' . $subject);
            }
    ...
    {
        if ($websiteId < 0) {
            throw new \InvalidArgumentException('b2b_rollout_website_invalid');
        }
    ...
    private function write(string $key, mixed $value): void
    {
        if (!$this->configStore()->setScopedConfig(
            ...
            throw new \RuntimeException('b2b_rollout_config_write_failed:' . $key);
        }
    }
