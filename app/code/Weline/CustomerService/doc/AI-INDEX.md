...
# Weline_CustomerService AI 开发入口
...
4. 本文件：`app/code/Weline/CustomerService/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/CustomerService/doc/README.md`
...
- 模块代码：`Weline_CustomerService`
- 目录：`app/code/Weline/CustomerService`
- Vendor：`Weline`
...
入口/配置文件：
- `app/code/Weline/CustomerService/etc/module.xml`
- `app/code/Weline/CustomerService/etc/backend/menu.xml`
- `app/code/Weline/CustomerService/composer.json`
...
- `i18n`：国际化资源。用户可见文案使用中文 source/key，en_US/zh_Hans_CN 对齐。 文件数：13
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/CustomerServiceQueryProvider.php；前端/跨模块读数据先查 query 帮助。

## doc 目录

- `app/code/Weline/CustomerService/doc/README.md`
- `app/code/Weline/CustomerService/doc/event/翻译事件.md`
- `app/code/Weline/CustomerService/doc/功能现状.md`
- `app/code/Weline/CustomerService/doc/开发日志.md`
- `app/code/Weline/CustomerService/doc/需求.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_CustomerService> [operation]` 或对应 `w_query` 帮助。
