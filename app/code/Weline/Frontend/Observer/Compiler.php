...
class Compiler implements \Weline\Framework\Event\ObserverInterface
{
        ...
        if (self::area === $area) {
            switch ($type):
                case self::modules_require_js_type:
                    ...
                    // 注意：resources 已经按 area 分组，这里只包含 frontend 的资源
    ...
    /**
     * 合并所有模块配置（前端专用，去重）
     * 注意：resources 已经按 area 分组，这里只包含 frontend 的资源，无需再过滤后端资源
     * @param string $resources 前端模块的 weline.modules.js 文件内容（已连接）
    ...
    protected function processModuleConfig(string $moduleConfig, string $moduleName): string
    {
        ...
        if (preg_match_all('/["\']([^"\']+)["\']/', $pathsStr, $pathMatches)) {
            foreach ($pathMatches[1] as $path) {
                ...
                // 硬：paths 必须保留 Module:: 逻辑名，由浏览器端 resolveStaticPath 按运行环境解析。
                // 禁止按本机 DEV 烤死绝对 URL——本机 compile 产物会随 core:update 上生产，
                // 烤成 /Weline/.../view/statics/... 后生产 404（货币/i18n/wishlist 等切换失效）。
                if (strpos($path, '::') !== false) {
                    ...
                    // 已是绝对/相对 URL：保留；运行时仍可由 resolveStaticPath 纠偏 DEV 形路径
                    $pathsArray[] = $path;
                    $originPaths[] = null;
                }
            }
