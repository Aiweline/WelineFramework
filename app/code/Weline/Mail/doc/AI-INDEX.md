...
4. 本文件：`app/code/Weline/Mail/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Mail/doc/README.md`
...
入口/配置文件：
- `app/code/Weline/Mail/etc/backend/menu.xml`
- `app/code/Weline/Mail/composer.json`
...
- `Service`：模块内业务编排层。跨模块读取数据优先发布/使用 w_query。真实邮件读取由 `StalwartEngineAdapter` 通过 loopback JMAP 完成；前台权限以 `MailAccount.customer_id + active` 为唯一绑定依据。 文件数：6
...
- 识别到 QueryProvider 相关 PHP 文件：extends/module/Weline_Framework/Query/MailQueryProvider.php；前端/跨模块读数据先查 query 帮助。

## doc 目录

- `app/code/Weline/Mail/doc/README.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Mail> [operation]` 或对应 `w_query` 帮助。
