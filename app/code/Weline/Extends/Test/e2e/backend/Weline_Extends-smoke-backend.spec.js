...
const CANDIDATE_ROUTES = ["extendsController","extendscontroller","extends_controller","extends","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Extends 后台流程', () => {
  moduleCase(
    test,
    { module: MODULE, id: 'EXTENDS-SMOKE-001' },
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'EXTENDS-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
