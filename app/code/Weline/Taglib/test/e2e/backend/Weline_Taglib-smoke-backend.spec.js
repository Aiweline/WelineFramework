...
const CANDIDATE_ROUTES = ["scope","taglib/listing","taglib","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Taglib 后台流程', () => {
  moduleCase(
    test,
    { module: MODULE, id: 'TAGLIB-SMOKE-001' },
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'TAGLIB-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
