...
- Marketplace endpoint policy: local development is locked to `https://app.test.weline.com:9523` from `E:\WelineFramework\Framework-Official\App`; deployed/production environments are locked to `https://app.aiweline.com`. `www.test.weline.com:9518` is the local official website project and must not be used as the App Store marketplace endpoint.
- Runtime marketplace clients use `AppStorePlatformUrlResolver`: if `app/etc/env.php` explicitly sets `deploy=dev/local`, the resolver accepts `appstore.platform_url` or `WELINE_APPSTORE_PLATFORM_URL` only when the normalized root is exactly `https://app.test.weline.com:9523`; otherwise it falls back to that local App Store root. Non-local runtime paths only accept production `var/deploy/current.json` when `appstore_platform_url=https://app.aiweline.com` and `appstore_platform_url_source=production_default`. Missing or drifted production deployment metadata falls back to `https://app.aiweline.com` without reading local env/config residue, and release gates reject the drift before live marketplace evidence is accepted. Known official website hosts using `www.*`, custom roots, or full API URLs are not accepted as marketplace roots.
