...
# Weline_ElFinderFileManager AI 开发入口
...
4. 本文件：`app/code/Weline/ElFinderFileManager/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/ElFinderFileManager/doc/README.md`
...
- Vendor：`Weline`
- Module：`ElFinderFileManager`

## 代码面清单

入口/配置文件：
- `app/code/Weline/ElFinderFileManager/composer.json`
...
- `view/statics`：静态资源源文件。浏览器业务请求必须走 Weline.Api.*。 文件数：193
...
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_ElFinderFileManager> [operation]` 或对应 `w_query` 帮助。
