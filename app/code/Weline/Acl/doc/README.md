...
保存时以勾选共享集合为事实：jstree three_state 只把叶子标为 selected、父节点常不出现在 POST，
服务端不得把「父菜单未提交」当成整棵子树取消，从而丢掉仍勾选的 CMS/接口叶子（`REQ-ACL-0001`）。
仍勾选的叶子必须写回 `role_access`，再向上补齐菜单祖先；只有提交集合中完全没有子孙的菜单
才视为已取消。权限树顶层分组必须让展开箭头、复选框、图标和标题处于同一行并垂直居中。jstree 节点本身
