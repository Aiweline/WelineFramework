...
namespace Weline\Acl\Extends\Module\Weline_Framework\Query;

use Weline\Acl\Service\IpWhitelistManagementService;
use Weline\Framework\Manager\ObjectManager;
use Weline\Framework\Service\Query\Provider\QueryProviderInterface;

class AclAdminQueryProvider implements QueryProviderInterface
{
    ...
    public function execute(string $operation, array $params = []): mixed
    {
        return match ($operation) {
            'createIpWhitelist' => $this->createIpWhitelist($params),
    ...
    public function getDescriptor(): array
    {
        return [
            'provider' => 'acl',
            'name' => 'Weline_Acl admin bridge',
            'module' => 'Weline_Acl',
            'operations' => [
                [
                    'name' => 'createIpWhitelist',
                    'description' => 'Create an IP whitelist entry',
                    'frontend' => true,
                    'auth' => 'backend',
                    'backend' => true,
                    'backend_acl' => [
                        'kind' => 'source',
                        'source_id' => 'Weline_Acl::ip_whitelist_add',
                    ],
                    'mode' => 'write',
                    'graph' => false,
                    'params' => [
                        ['name' => 'ip', 'type' => 'string', 'required' => true, 'max_length' => 45],
    ...
    private function createIpWhitelist(array $params): array
    {
        /** @var IpWhitelistManagementService $service */
        $service = ObjectManager::getInstance(IpWhitelistManagementService::class);
