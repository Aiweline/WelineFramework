    ...
    public function execute(): string
    {
        try {
            ...
            $accounts = $accountModel->reset()
                ->where(\Weline\Seo\Model\SeoAccount::schema_fields_IS_ACTIVE, \Weline\Seo\Model\SeoAccount::STATUS_ACTIVE)
                ->where(\Weline\Seo\Model\SeoAccount::schema_fields_ENABLE_CRON_PUSH_URLS, 1)
                ->select()
                ->fetchArray();
            ...
            foreach ($accounts as $account) {
                $provider = (string)($account[\Weline\Seo\Model\SeoAccount::schema_fields_PROVIDER] ?? '');
                $accountId = (int)($account[\Weline\Seo\Model\SeoAccount::schema_fields_ACCOUNT_ID] ?? 0);
                $scope = (string)($account[\Weline\Seo\Model\SeoAccount::schema_fields_SCOPE] ?? '');
                ...
                $existingTask = $taskModel->reset()
                    ...
                    ->where(SeoTask::schema_fields_SCOPE, $scope)
                    ->where(SeoTask::schema_fields_PAYLOAD, '%"account_id":' . $accountId . '%', 'LIKE')
                    ->find()
                    ->fetch();
                ...
                $subjectQuery = $subjectModel->reset()
                    ...
                    ->where(SeoSubject::schema_fields_URL, '', '!=')
                    ->where(SeoSubject::schema_fields_UPDATED_AT, date('Y-m-d H:i:s', strtotime('-7 days')), '>=');

                if ($scope !== '') {
                    $subjectQuery->where(SeoSubject::schema_fields_SCOPE, $scope);
                }
