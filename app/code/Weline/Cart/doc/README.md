...
- Public contract (V2)：见 [`cart.md`](cart.md)（`OfferIdentity` + Scope + server selection hash）。
- Provider path V1：`extends/module/Weline_Cart/CartItemSnapshotProvider/{ProviderName}.php`。
- Provider path V2：`extends/module/Weline_Cart/CartItemSnapshotProvider/{ProviderName}.php`。
...
Cart storefront ownership is server-authoritative. Browser requests may carry
