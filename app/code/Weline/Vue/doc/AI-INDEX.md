...
4. 本文件：`app/code/Weline/Vue/doc/AI-INDEX.md`
5. 模块说明：`app/code/Weline/Vue/doc/README.md`
...
入口/配置文件：
- `app/code/Weline/Vue/composer.json`
...
- `app/code/Weline/Vue/doc/README.md`
- `app/code/Weline/Vue/doc/功能现状.md`
- `app/code/Weline/Vue/doc/开发日志.md`
- `app/code/Weline/Vue/doc/需求.md`

## 开发前门禁

- 先声明本次任务命中的模块、代码面和应读文档；没有命中文档时先补读源码，不要按通用经验猜。
- 涉及浏览器前后端业务请求时，只能使用 `Weline.Api.resource()`、`Weline.Api.graph()` 或 `Weline.Api.stream()`。
- 涉及跨模块读数据时，先查 `php bin/w query:help <provider|Weline_Vue> [operation]` 或对应 `w_query` 帮助。
