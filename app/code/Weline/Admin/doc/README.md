...
左侧栏收起态（`html[data-backend-sidebar-collapsed="true"]` / `.w-backend-shell[data-sidebar-collapsed="true"]`）只把顶层收成 icon 栏（含搜索 icon），不重写菜单树。**点击顶层可展开项**会通过 `backend-sidebar-collapse.expandOverlay` 临时展开整栏，并保持对应 `details` 为 `open`（抑制随后原生 `summary` click 的二次 toggle，避免在点击处立刻收回）；展开后的分组 `summary` 以 sticky 钉在侧栏滚动区顶部，便于浏览子菜单。**打开就是打开**：已展开的侧栏或已打开的菜单分组不会因点击主内容区等其他地方而自动收回；仅用户点击收起按钮，或搜索临时展开且无打开菜单时点外部/失焦，才会收回 icon 栏。localStorage 仍只记录用户主动切换的收起偏好。禁止对侧栏滚动容器使用会裁切展开层的 `contain: layout`。相关样式与交互以 `Weline_Theme/view/ui/css/backend.css`、`Weline_Theme/view/ui/js/weline-ui.js`（`nav-filter` / `backend-sidebar-collapse`）与 `view/templates/common/left-sidebar.phtml` 为准。
