...
namespace Weline\Admin\Controller\Api;
...
class User extends \Weline\Framework\App\Controller\BackendController
{
    ...
    public function getAdd()
    {
        ...
        $this->assign('action', $this->request->getUrlBuilder()->getBackendUrl('*/api/user/add'));
    ...
    public function getEdit()
    {
        ...
        $this->assign('action', $this->request->getUrlBuilder()->getBackendUrl('*/api/user/edit', ['id' => $userId]));
        return $this->fetch('form');
    }
    ...
    public function postEdit()
    {
        try {
            ...
            $this->redirect('*/api/user/edit', ['id' => $userId]);
        } catch (\Exception $exception) {
            $this->getMessageManager()->addWarning(__('修改失败：%{1}', [$exception->getMessage()]));
            if (DEV) {
                $this->getMessageManager()->addException($exception);
            }
            $this->redirect('*/api/user/edit', ['id' => $userId ?? 0]);
        }
    }
    ...
    public function getAssignPermissions()
    {
        ...
        $this->assign('action', $this->request->getUrlBuilder()->getBackendUrl('*/api/user/assign-permissions', ['id' => $userId]));
    ...
    public function postAssignPermissions()
    {
        ...
        $this->redirect('*/api/user/assign-permissions', ['id' => $userId ?? 0]);
    }
}
